...
const AutoUpdater = {
 ...
 checkInterval: 5 * 60 * 1000, // 5 minutes instead of 5 seconds to avoid GitHub rate limiting
        ...
        modalContent = `
          <div style="padding: 20px;">
            <div style="text-align: center; margin-bottom: 20px;">
              ...
              <div style="color: #00ff88; font-size: 20px; font-weight: bold;">Nova Atualização Disponível no GitHub!</div>
            </div>
            <div style="background: rgba(0,0,0,0.3); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
              ...
              <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                <span style="color: #888;">Nova versão (GitHub):</span>
            ...
            <div style="display: flex; gap: 12px;">
              ...
              <button type="button" onclick="window.open('${data.downloadUrl}', '_blank')" class="btn-neon" style="flex: 1;">⬇ Baixar no GitHub</button>
            </div>
          </div>`;
        break;
  ...
  async fetchLatestRelease() {
    const response = await fetch('/api/check-update', { cache: 'no-cache' });
    const data = await response.json();
    if (!data.ok) throw new Error(data.error || 'Não foi possível conectar ao GitHub');
    console.log('[AutoUpdater] Release recebida:', data.release.tag_name, data.release.name);
    return data.release;
  },
  ...
  async checkForUpdatesWithModal() {
    ...
    try {
      ...
      // Tenta extrair versão do tag_name primeiro, depois do nome da release
      let tagVersion = release.tag_name?.replace(/^v/i, '') || '';
      let nameVersion = release.name?.match(/(\d+\.\d+\.\d+)/)?.[1] || '';
      this.latestVersion = tagVersion || nameVersion || release.name;
      const updateNotes = release.body || 'Sem notas de atualização.';
      const downloadUrl = release.html_url;
      
      console.log('[AutoUpdater] Versão atual:', this.currentVersion);
      console.log('[AutoUpdater] Versão latest:', this.latestVersion);
      console.log('[AutoUpdater] Tag name:', release.tag_name);
      console.log('[AutoUpdater] Release name:', release.name);
      
      this.updateAvailable = this.compareVersions(this.latestVersion, this.currentVersion) > 0;
      console.log('[AutoUpdater] Update available:', this.updateAvailable);
  ...
  async checkForUpdates(silent = true) {
    try {
      ...
      // Only log error for automatic checks, don't show modal
      if (silent) {
        console.log('[AutoUpdater] Não foi possível verificar atualizações:', e.message);
      } else {
        throw e; // Re-throw for manual checks to show error modal
      }
      return false;
    }
  },
  ...
  showUpdateNotification(release) {
    ...
    banner.innerHTML = `
      <div class="update-banner-content">
        ...
        <div class="update-info">
          <div class="update-title">NOVA VERSÃO NO GITHUB!</div>
          <div class="update-version">Versão ${this.latestVersion} • Você está na ${this.currentVersion}</div>
        </div>
        <button class="update-btn" onclick="AutoUpdater.openDownloadPage()">BAIXAR NO GITHUB</button>
    ...
    if (typeof showToast === 'function') {
      showToast(`🔄 Nova versão ${this.latestVersion} disponível no GitHub!`);
    }
  },
  ...
  async init() {
    ...
    // Initial check after 10 seconds, silent mode
    setTimeout(() => this.checkForUpdates(true), 10000);
    // Periodic checks every 5 minutes, silent mode
    setInterval(() => this.checkForUpdates(true), this.checkInterval);
