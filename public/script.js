...
// Em desenvolvimento/Electron/localhost, usa o servidor local
// Em produção (acesso via URL público), usa o servidor do Render
...
const socketUrl = (isFileProtocol || isLocalhost) ? 'http://localhost:3002' : 'https://sagile-xenon.onrender.com';

// Guard: verificar se io está disponível
if (typeof io === 'undefined') {
  console.error('[SOCKET] Socket.IO não carregado. Verifique se /socket.io/socket.io.js foi incluído.');
  window.socket = null;
} else {
  ...
  console.log(`[SOCKET] Protocolo: ${window.location.protocol}`);
  console.log(`[SOCKET] isElectron: ${isElectron}`);
  console.log(`[SOCKET] isLocalhost: ${isLocalhost}`);
  console.log(`[SOCKET] isFileProtocol: ${isFileProtocol}`);

  window.socket?.on('connect', () => {
    console.log('[SOCKET] CONNECT', window.socket.id);
    // Re-emit login se o usuário já estiver identificado.
    // Usa a variável de módulo 'username' diretamente (closure) como fonte primária,
    // pois window.username não é a mesma variável — evita enviar nome inválido na reconexão.
    const u = username || window.currentUsername || sessionStorage.getItem('username') || localStorage.getItem('zx_username');
    if (u && u !== 'Usuário' && u.trim() !== '') {
      window.socket?.emit('user:login', { username: u });
    }
    
    // Solicitar dados do servidor ao conectar
    setTimeout(() => {
      window.socket?.emit('shorts:request');
      window.socket?.emit('community:get-suggested');
      console.log('[SOCKET] Dados solicitados: shorts e comunidades');
    }, 500);
  });
  ...
  window.socket?.on('connect_error', (err) => {
    ...
    console.error('[SOCKET] ERROR DETAILS', err.message, err.description, err.context, err.type);
  });
  ...
  setInterval(() => {
    console.log('[SOCKET STATUS]', window.socket.connected, 'ID:', window.socket.id);
  }, 5000);
}
...
// ── Cache global único de avatares ──
window.friendAvatarCache = JSON.parse(localStorage.getItem('zx_friend_avatars') || '{}');
const avatarRequestsPending = new Set();

function setFriendAvatar(username, avatarData) {
  if (!username || !avatarData) return;
  const key = username.toLowerCase();
  window.friendAvatarCache[key] = avatarData;
  localStorage.setItem('zx_friend_avatars', JSON.stringify(window.friendAvatarCache));
}

function getFriendAvatar(username) {
  if (!username) return null;
  return window.friendAvatarCache[username.toLowerCase()] || null;
}

function resolveAvatarUrl(username, avatarUrl) {
  if (avatarUrl !== undefined && avatarUrl !== null) return avatarUrl;
  const selfName = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
  if (username && username.toLowerCase() === selfName) {
    return profileAvatarUrl || localStorage.getItem('zx_avatar') || '';
  }
  return getFriendAvatar(username);
}

function renderAvatar(el, username, avatarUrl) {
  if (!el) return;
  const url = resolveAvatarUrl(username, avatarUrl);
...
function applyAvatarToElement(el, username, avatarUrl) {
  renderAvatar(el, username, avatarUrl);
}

function requestUserAvatar(username) {
  if (!username || !socket?.connected) return;
  const key = username.toLowerCase();
  if (getFriendAvatar(username) || avatarRequestsPending.has(key)) return;
  avatarRequestsPending.add(key);
  if (window.socket) window.socket?.emit('user:avatar:get', { username });
}

function refreshInicialAvatars() {
  document.querySelectorAll('#inicial-tab-content .inicial-friend-row').forEach(row => {
    const nameEl = row.querySelector('.inicial-friend-name');
    const avEl = row.querySelector('.inicial-friend-avatar');
    if (nameEl && avEl) renderAvatar(avEl, nameEl.textContent.trim());
  });
}

function refreshAllAvatarViews() {
  if (typeof renderOnlineSidebar === 'function') renderOnlineSidebar();
  refreshInicialAvatars();
  document.querySelectorAll('.msg-avatar[data-username]').forEach(el => {
    renderAvatar(el, el.dataset.username);
  });
  document.querySelectorAll('.dm-conv-item[data-username]').forEach(item => {
    const uname = item.dataset.username;
    if (!uname) return;
    const avEl = item.querySelector('.profile-avatar, .dm-conversation-avatar, .dm-avatar');
    if (avEl) renderAvatar(avEl, uname);
  });
  if (typeof currentDmUser !== 'undefined' && currentDmUser) {
    const headerAv = document.querySelector('#dm-chat-area .dm-avatar');
    if (headerAv) renderAvatar(headerAv, currentDmUser);
  }
}

window.renderAvatar = renderAvatar;
window.getFriendAvatar = getFriendAvatar;
window.setFriendAvatar = setFriendAvatar;
window.requestUserAvatar = requestUserAvatar;
window.refreshAllAvatarViews = refreshAllAvatarViews;
...
function applyAvatarToEl(el, url, fallbackLetter) {
  renderAvatar(el, window.username || localStorage.getItem('zx_username') || 'Usuário', url);
}

function applyProfileMedia() {
  const currentUsername = window.username || localStorage.getItem('zx_username') || 'Usuário';
  const letter = currentUsername ? currentUsername[0].toUpperCase() : '?';
  [
    userAvatar,
...
function doLogin() {
  ...
  showDiscoverView();
  if (window.socket) {
    window.socket?.emit('feed:join');
    window.socket?.emit('user:login', { username, email: (function(){ try { return JSON.parse(localStorage.getItem('zx_user_data') || '{}').email || ''; } catch(e){ return ''; } })() });
    // Sincronizar avatar do usuário com o servidor no login
    (function() {
      const savedAvatar = localStorage.getItem('zx_avatar');
      if (savedAvatar && savedAvatar.startsWith('data:image')) {
        window.socket?.emit('user:avatar:save', { avatar: savedAvatar });
      }
    })();
    ...
    (function() {
      try {
        ...
        window.socket?.emit('user:servers:report', { servers: myServers });
      } catch(e) {}
    })();
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // Salvar username no sessionStorage para o sistema de permissões funcionar
    sessionStorage.setItem('username', username);
    localStorage.setItem('zx_username', username);
  ...
  try { applyProfileMedia(); } catch(e) { console.error('[ZX] applyProfileMedia falhou:', e); }
  ...
  class FriendsSystem {
    ...
    init() {
      // CORREÇÃO BUG #3: Remover handlers window.socket?.on duplicados desta classe.
    ...
    sendFriendRequest(username) {
      ...
      if (typeof window.sendFriendRequest === 'function') {
        ...
        if (!clean) return;
        window.socket?.emit('friend:request', { to: clean });
      }
    }

    acceptFriendRequest(username) {
      window.socket?.emit('friend:accept', { to: username });
    ...
    rejectFriendRequest(username) {
      window.socket?.emit('friend:reject', { to: username });
      this.requests = this.requests.filter(r => r !== username);
      this.renderFriendsList();
    }

    removeFriend(username) {
      window.socket?.emit('friend:remove', { to: username });
    ...
    isOnline(username) {
      ...
      try {
        if (typeof onlineSet !== 'undefined' && onlineSet instanceof Set) {
          if (!onlineSet.has(key)) return false;
          const st = (typeof userStatuses !== 'undefined') ? userStatuses[key] : undefined;
    ...
    renderFriendsList() {
      ...
      container.innerHTML = this.friends.map(friend => {
        const friendAv = getFriendAvatar(friend);
        if (!friendAv) requestUserAvatar(friend);
        const avatarStyle = friendAv
          ? `background-image:url(${friendAv});background-size:cover;background-position:center;color:transparent;`
          : '';
        const avatarContent = friendAv ? '' : friend.charAt(0).toUpperCase();
        return `
        <div class="dm-friend-item">
          <div class="dm-friend-avatar" style="${avatarStyle}">${avatarContent}</div>
    ...
    renderConversationsList() {
      ...
      container.innerHTML = this.friends.map(friend => {
        ...
        const avatarContent = friendAv ? '' : friend.charAt(0).toUpperCase();
        return `
        <div class="dm-conversation-item dm-conv-item" data-username="${friend}" style="cursor:pointer;">
          <div class="dm-conversation-avatar" style="${avatarStyle}">${avatarContent}</div>
          <div class="dm-conversation-name">${friend}</div>
          <div class="dm-conversation-status ${this.isOnline(friend) ? 'online' : ''}"></div>
        </div>`;
      }).join('');
      
      // FIX: Add event delegation for both dm-conversation-item and dm-conv-item classes
      // This ensures clicks work regardless of which function renders the list
      if (!container._convClickDelegate) {
        container._convClickDelegate = true;
        container.addEventListener('click', (e) => {
          const item = e.target.closest('.dm-conversation-item') || e.target.closest('.dm-conv-item');
          if (item && item.dataset.username) {
            console.log('[Conversations List Click] Opening conversation for:', item.dataset.username);
            this.openConversation(item.dataset.username);
          }
        });
        console.log('[renderConversationsList] Event delegation attached to container');
      }
    }
...
window.currentDmUser = currentDmUser; // Tornar global para acesso pelo HTML
let dmMessages = {};
let myUserId = null; // [BUG2 FIX] ID único do usuário logado
let friendIds = {};  // [BUG2 FIX] username.toLowerCase() -> userId

// [DM CALL FIX] Tornar friendIds e myUserId globais para dm-call-system.js acessar
window.friendIds = friendIds;
window.myUserId = myUserId;

function renderDmList() {
  const dmList = document.getElementById('dm-conversations-list');
  if (!dmList) {
    console.log('[renderDmList] dm-conversations-list element not found');
    return;
  }

  const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
  const storedFriends = JSON.parse(localStorage.getItem('zx_friends') || '[]');

  // Deduplicar usando sempre username.toLowerCase() como chave — evita duplicatas
  // que surgiam quando f.id era usado como chave no loop de amigos e uname como
  // chave no loop de dmMessages, resultando no mesmo usuário aparecendo duas vezes.
  const uniqueMap = new Map(); // key: uname.toLowerCase() -> display name (preserva capitalização original)

  storedFriends.forEach(f => {
    const uname = typeof f === 'string' ? f : (f.username || f.nick || '');
    if (!uname || uname.toLowerCase() === currentUser) return;
    uniqueMap.set(uname.toLowerCase(), uname);
  });

  // Incluir usuários com conversa ativa (dmMessages é indexado por lowercase)
  Object.keys(dmMessages).forEach(uLow => {
    if (!dmMessages[uLow] || dmMessages[uLow].length === 0) return;
    if (uLow === currentUser) return;
    if (!uniqueMap.has(uLow)) {
      const displayName = dmMessages[uLow][0]?.from || dmMessages[uLow][0]?.to || uLow;
      uniqueMap.set(uLow, displayName);
    }
  });
  ...
  dmList.innerHTML = dmUsers.map(uname => {
    ...
    const isOnline = onlineSet.has(statusKey);
    const st = userStatuses[statusKey] || (isOnline ? 'online' : 'offline');
    const isUserOnline = st !== 'offline' && st !== 'invisible';
    ...
    const friendAv = getFriendAvatar(uname);
    if (!friendAv) requestUserAvatar(uname);
    const avatarBg = friendAv
      ? `background-image:url(${friendAv});background-size:cover;background-position:center;background-color:transparent;`
      : 'background:linear-gradient(135deg,#8b00ff,#ff00ff);';
    const avatarText = friendAv ? '' : initial;
    
    return `
      <div class="dm-conv-item dm-conversation-item" data-username="${escHtml(uname)}" style="
        display:flex;align-items:center;gap:10px;padding:8px 8px;border-radius:8px;cursor:pointer;
        transition:background 0.15s;margin-bottom:2px;position:relative;pointer-events:auto;
      " onmouseenter="this.style.background='rgba(255,255,255,0.06)'" onmouseleave="this.style.background='transparent'">
        <div style="position:relative;flex-shrink:0;">
          <div class="profile-avatar av-${initial}" style="width:36px;height:36px;font-size:14px;border:none;box-shadow:none;${avatarBg}">${avatarText}</div>
  ...
  // FIX: Use document-level event delegation for robustness
  // This won't be affected by DOM re-rendering
  if (!document._dmListClickHandler) {
    document._dmListClickHandler = (e) => {
      const item = e.target.closest('.dm-conv-item') || e.target.closest('.dm-conversation-item');
      if (!item || !item.dataset.username) return;
      
      const username = item.dataset.username;
      console.log('[DM List Click] Opening chat for:', username);
      
      // Always open chat immediately - this is the primary action
      openDmChat(username);
      
      // If profile avatar was clicked, also show profile (secondary action)
      const avatar = e.target.closest('.profile-avatar');
      if (avatar && typeof showUserProfile === 'function') {
        setTimeout(() => showUserProfile(username), 100);
      }
    };
    document.addEventListener('click', document._dmListClickHandler);
    console.log('[renderDmList] Document-level click handler attached');
  }
  
  console.log('[renderDmList] Rendered', dmUsers.length, 'DM conversations');
...
function openDmChat(username) {
  ...
  // FIX: Request presence update to get accurate status when opening chat
  if (socket && socket.connected) {
  ...
  const status = userStatuses[key] || (isOnline ? 'online' : 'offline');
  const isUserOnline = status !== 'offline' && status !== 'invisible';
  const initial = username[0].toUpperCase();

  // Mostra area de chat, oculta estado vazio e inicial view
  const chatArea = document.getElementById('dm-chat-area');
  const emptyState = document.getElementById('dm-empty-state');
  const inicialView = document.getElementById('dm-inicial-view');
  if (chatArea) {
    chatArea.classList.remove('hidden');
    chatArea.style.display = 'flex';
    // FIX: Define dataset.activeChat para compatibilidade com patches que dependem deste atributo
    chatArea.dataset.activeChat = username;
  }
  if (emptyState) { emptyState.classList.add('hidden'); emptyState.style.display = 'none'; }
  if (inicialView) { inicialView.style.display = 'none'; inicialView.classList.add('hidden'); }
  ...
  if (avatarEl) {
    renderAvatar(avatarEl, username);
    if (!getFriendAvatar(username)) {
      requestUserAvatar(username);
      if (window.socket && window.socket.connected) {
        window.socket?.emit('dm:user:profile', { username });
      }
    }
  }
  if (usernameEl) usernameEl.textContent = username;
  if (statusEl) {
    statusEl.textContent = isUserOnline ? '● Online' : '● Offline';
    statusEl.style.color = isUserOnline ? '#23d18b' : '#747f8d';
    statusEl.className = `dm-status ${isUserOnline ? 'online' : 'offline'}`;
  }

  const activityEl = document.getElementById('dm-activity');
  if (activityEl) activityEl.textContent = isUserOnline ? 'Ativo agora' : '';
...
function renderDmMessages() {
  ...
  container.innerHTML = messages.map(msg => {
    ...
    // Usar avatar do cache global via getFriendAvatar
    const _senderKey = (msg.from || '').toLowerCase();
    const avatarUrl = isSelf
      ? resolveAvatarUrl(username)
      : (msg.avatar || getFriendAvatar(msg.from || '') || '');
    if (!isSelf && !avatarUrl) requestUserAvatar(msg.from || '');
    const avatarBg = avatarUrl
      ? `style="background-image:url(${avatarUrl});background-size:cover;background-position:center;background-repeat:no-repeat;"`
      : '';
    const avatarContent = avatarUrl ? '' : initial;
    
    // Detectar se eh GIF ou midia
    const isGif = /\.(gif)(\?.*)?$/i.test(msg.text) || msg.text.includes('tenor.com') || msg.text.includes('giphy.com');
    
    return `
      <div class="message ${isSelf ? 'self' : ''}">
        <div class="msg-avatar av-${initial}${avatarUrl ? ' has-image' : ''}" ${avatarBg} data-username="${escHtml(msg.from || '')}">${avatarContent}</div>
        <div class="msg-body">
          <div class="msg-meta">
          ...
          <div class="msg-text">
            ${isGif
              ? `<img src="${escHtml(msg.text)}" style="max-width:300px;max-height:200px;border-radius:8px;" />`
              : escHtml(msg.text)
            }
          </div>
        </div>
...
function sendDmMessage() {
  ...
  renderDmMessages();
  // FIX: Don't call renderDmList() here - message sending doesn't require full re-render
...
// CORREÇÃO: usar função nomeada permite window.socket?.off(name) preciso e re-registro no connect.
...
function _onDmMessage(msg) {
  ...
  // Mostrar badge de notificação se não estiver na tela de DM
  const isDmScreen = document.body.classList.contains('dm-view-active') || 
                     document.body.classList.contains('dm-active');
  const isCurrentDmUser = (currentDmUser || '').toLowerCase() === (msg.from || '').toLowerCase();
  
  console.log('[DM BADGE] isDmScreen:', isDmScreen, 'isCurrentDmUser:', isCurrentDmUser, 'currentDmUser:', currentDmUser, 'msg.from:', msg.from);
  
  if (!isDmScreen || !isCurrentDmUser) {
    const badge = document.getElementById('dm-notification-badge');
    const sidebarBadge = document.getElementById('dm-notification-badge-sidebar');
    console.log('[DM BADGE] badge encontrado:', !!badge, 'sidebarBadge encontrado:', !!sidebarBadge);
    
    if (badge) {
      const currentCount = parseInt(badge.textContent) || 0;
      badge.textContent = currentCount + 1;
      badge.style.display = 'block';
      console.log('[DM BADGE] Badge atualizado para:', badge.textContent);
    }
    if (sidebarBadge) {
      ...
      console.log('[DM BADGE] Sidebar badge atualizado para:', sidebarBadge.textContent);
    }
  } else {
    console.log('[DM BADGE] Badge não atualizado - usuário está na tela de DM ou conversa atual');
  }
  ...
  // FIX: Don't call renderDmList() here - message receiving doesn't require full re-render
  // if (typeof renderDmList === 'function') renderDmList();
...
// Registrar o listener agora (primeira carga) — window.socket?.off garante listener único
window.socket?.off('dm:message', _onDmMessage);
window.socket?.on('dm:message', _onDmMessage);

// Confirmação de mensagem enviada pelo servidor (com timestamp canônico)
// [BUG2 FIX] Garantir listener único
window.socket?.off('dm:message:sent');
window.socket?.on('dm:message:sent', (msg) => {
...
window.socket?.on('dm:history', (data) => {
  ...
  // FIX: Don't call renderDmList() here - history loading doesn't require full re-render
  // if (typeof renderDmList === 'function') renderDmList();
});
...
btnDmList.addEventListener('click', () => {
  ...
  // Limpar badge de notificação ao abrir mensagens privadas
  const badge = document.getElementById('dm-notification-badge');
  const sidebarBadge = document.getElementById('dm-notification-badge-sidebar');
  if (badge) {
  ...
  // FIX: Show the inicial view when entering DM view (online/todos/solicitações)
  // This is the body layout that should appear when clicking "mensagens privadas"
  showInicialView();
  
  // FIX: Always use renderDmList for DM view to ensure clickability
  // renderDmList has proper event delegation that works reliably
  renderDmList();
...
function openCommunityPostsView() {
  ...
  document.getElementById('btn-discover-post')?.addEventListener('click', () => {
    ...
    window.socket?.emit('feed:post', { title, body, subreddit: sub, username });
...
function renderFandomTabs(server) {
  ...
  const fandoms = server.fandomTabs || [];
  const VISIBLE_FANDOMS_LIMIT = 5;
  
  if (fandoms.length > VISIBLE_FANDOMS_LIMIT) {
    // Mostra apenas os primeiros fandoms e cria um grupo para o resto
    const visibleFandoms = fandoms.slice(0, VISIBLE_FANDOMS_LIMIT);
    const groupedFandoms = fandoms.slice(VISIBLE_FANDOMS_LIMIT);
    
    // Renderiza fandoms visíveis
    visibleFandoms.forEach(fandom => {
    ...
    // Cria aba de grupo para os fandoms restantes
    const groupTab = document.createElement('div');
    groupTab.className = 'fandom-tab';
    groupTab.textContent = `+${groupedFandoms.length}`;
    groupTab.dataset.groupId = 'fandom-group';
    groupTab.title = `${groupedFandoms.length} fandoms agrupados`;
    groupTab.style.background = 'linear-gradient(135deg, #ff00ff, #00ffff)';
    
    groupTab.addEventListener('click', () => {
      // Expande o grupo mostrando todos os fandoms
      expandFandomGroup(groupedFandoms, server);
    });
    
    tabsContainer.appendChild(groupTab);
    
    // Salva fandoms agrupados para expansão
    window.groupedFandoms = groupedFandoms;
    window.currentServerForFandoms = server;
  } else {
    // Renderiza todos os fandoms normalmente
...
// Função para expandir o grupo de fandoms
function expandFandomGroup(groupedFandoms, server) {
  const tabsContainer = document.getElementById('fandom-tabs-container');
  if (!tabsContainer) return;
  
  // Remove a aba de grupo
  const groupTab = tabsContainer.querySelector('.fandom-tab[data-group-id="fandom-group"]');
  if (groupTab) {
    groupTab.remove();
  }
  
  // Renderiza todos os fandoms do grupo
  groupedFandoms.forEach(fandom => {
    const tab = document.createElement('div');
    tab.className = 'fandom-tab';
    tab.textContent = `⭐ ${fandom.name}`;
    tab.dataset.fandomId = fandom.id;
  ...
  // Limpa a referência aos fandoms agrupados
  window.groupedFandoms = null;
  window.currentServerForFandoms = null;
}
...
function renderSidebarChannels(server) {
  ...
  for (const [cat, channels] of Object.entries(categories)) {
    ...
    channels.forEach(ch => {
      ...
      li.draggable = true;           // ✅ ATIVADO para permitir arrastar canais
      li.style.cursor = 'grab';      // ✅ Cursor de arrastar
...
function renderServersRail() {
  ...
  serversRail.querySelectorAll('.server-rail-icon[data-group-id]').forEach(e => e.remove());
  ...
  const filteredServers = servers.filter(server => !String(server.id || '').startsWith('comm_'));
  
  // Limite de servidores visíveis antes de agrupar
  const VISIBLE_SERVERS_LIMIT = 5;
  
  if (filteredServers.length > VISIBLE_SERVERS_LIMIT) {
    // Mostra apenas os primeiros servidores e cria um grupo para o resto
    const visibleServers = filteredServers.slice(0, VISIBLE_SERVERS_LIMIT);
    const groupedServers = filteredServers.slice(VISIBLE_SERVERS_LIMIT);
    
    // Renderiza servidores visíveis
    ...
    // Cria ícone de grupo para os servidores restantes
    const groupIcon = document.createElement('div');
    groupIcon.className = 'server-rail-icon';
    groupIcon.dataset.groupId = 'server-group';
    groupIcon.textContent = `+${groupedServers.length}`;
    groupIcon.title = `${groupedServers.length} servidores agrupados`;
    groupIcon.style.background = 'linear-gradient(135deg, #ff00ff, #00ffff)';
    
    groupIcon.addEventListener('click', () => {
      // Expande o grupo mostrando todos os servidores
      expandServerGroup(groupedServers);
    });
    
    serversRail.insertBefore(groupIcon, addBtn);
    
    // Salva servidores agrupados para expansão
    window.groupedServers = groupedServers;
  } else {
    // Renderiza todos os servidores normalmente
...
// Função para expandir o grupo de servidores
function expandServerGroup(groupedServers) {
  if (!serversRail) return;
  
  // Remove o ícone de grupo
  const groupIcon = serversRail.querySelector('.server-rail-icon[data-group-id="server-group"]');
  if (groupIcon) {
    groupIcon.remove();
  }
  ...
  // Renderiza todos os servidores do grupo
  ...
  // Limpa a referência aos servidores agrupados
  window.groupedServers = null;
}
...
if (btnCommunityCreate) {
  btnCommunityCreate.addEventListener('click', () => {
    ...
    const newServer = {
      ...
      name: name,
      template: template,
      ownerId: username, // Define o criador como dono do servidor
      members: [{ username: username, role: 'OWNER', joinedAt: Date.now() }],
  ...
  btn.addEventListener('click', () => {
    ...
    if (modalEl) {
      ...
      if (btn.dataset.modal === 'friends-modal') {
        ...
        if (socket && socket.connected) window.socket?.emit('presence:request');
        // FIX: Debounce renderFriendsModal to prevent flickering
        if (typeof renderFriendsModal === 'function') {
          clearTimeout(window._friendsModalRenderTimeout);
          window._friendsModalRenderTimeout = setTimeout(renderFriendsModal, 100);
        }
      }
    }
  });
...
inputAvatar?.addEventListener('change', (e) => {
  ...
  readImageFile(file, (dataUrl) => {
    ...
    // Persistir no Neon via REST API (fonte de verdade)
    AccountAPI.updateProfile({ avatar: dataUrl }).catch(() => {});
    // Broadcast em tempo real para amigos online via socket
    if (window.socket && window.socket.connected) {
      window.socket?.emit('user:avatar:save', { avatar: dataUrl });
    }
  });
  e.target.value = '';
});
...
if (statusSelect) {
  ...
  statusSelect.addEventListener('change', () => {
    ...
    window.socket?.emit('user:status', { status: userStatus, username });
...
function openModal(modalEl) {
  ...
  if (modalEl && modalEl.id === 'friends-modal') {
    if (typeof socket !== 'undefined' && socket.connected) window.socket?.emit('presence:request');
    // FIX: Debounce renderFriendsModal to prevent flickering
    if (typeof renderFriendsModal === 'function') {
      clearTimeout(window._friendsModalRenderTimeout);
      window._friendsModalRenderTimeout = setTimeout(renderFriendsModal, 100);
...
async function joinVoiceChannel(ch) {
  ...
  window.socket?.emit('voice:join', { channelId: ch.id, communityId: currentServerId, username });
}
...
function leaveVoiceChannel() {
  ...
  if (voiceChannelId) {
    window.socket?.emit('voice:leave', { channelId: voiceChannelId, communityId: currentServerId });
  }
...
function createPeerConnection(remoteSocketId) {
  ...
  pc.onicecandidate = ({ candidate }) => {
    if (candidate) window.socket?.emit('voice:ice', { to: remoteSocketId, candidate });
  };
...
window.socket?.on('voice:peers', async ({ peers }) => {
  for (const peer of peers) {
    const pc = createPeerConnection(peer.socketId);
    const offer = await pc.createOffer();
    await pc.setLocalDescription(offer);
    window.socket?.emit('voice:offer', { to: peer.socketId, offer });
  }
  renderVoiceParticipants();
});

window.socket?.on('voice:user-joined', ({ socketId, username: uname }) => {
  renderVoiceParticipants();
});

window.socket?.on('voice:offer', async ({ from, offer, username: uname }) => {
  const pc = createPeerConnection(from);
  await pc.setRemoteDescription(new RTCSessionDescription(offer));
  const answer = await pc.createAnswer();
  await pc.setLocalDescription(answer);
  window.socket?.emit('voice:answer', { to: from, answer });
  renderVoiceParticipants();
});

window.socket?.on('voice:answer', async ({ from, answer }) => {
  const pc = peerConnections[from];
  if (pc) await pc.setRemoteDescription(new RTCSessionDescription(answer));
});

window.socket?.on('voice:ice', async ({ from, candidate }) => {
  const pc = peerConnections[from];
  if (pc) await pc.addIceCandidate(new RTCIceCandidate(candidate)).catch(() => {});
});

window.socket?.on('voice:user-left', ({ socketId }) => {
...
window.socket?.on('voice:room-users', ({ users }) => {
    ...
    item.addEventListener('click', () => {
      ...
      switch(item.dataset.action) {
        case 'mute':
          ...
          window.socket?.emit('voice:moderate', { action: 'mute', targetSocketId: socketId });
          break;
          
        case 'deafen':
          showToast(`🔇 Áudio desativado para ${uname}`);
          window.socket?.emit('voice:moderate', { action: 'deafen', targetSocketId: socketId });
          break;
        ...
        case 'move':
          ...
          if (targetChannel?.trim()) {
            ...
            window.socket?.emit('voice:moderate', { action: 'move', targetSocketId: socketId, channel: targetChannel.trim() });
          }
          break;
        ...
        case 'kick':
          if (confirm(`Desconectar ${uname} da chamada?`)) {
            ...
            window.socket?.emit('voice:moderate', { action: 'kick', targetSocketId: socketId });
          }
          break;
      }
    });
...
function renderDiscoverFeed() {
  ...
  discoverFeed.querySelectorAll('.discover-vote-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      window.socket?.emit('feed:vote', { postId, vote: next });
    });
  });
  ...
  discoverFeed.querySelectorAll('[data-comment-submit]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const text = input?.value.trim();
      if (!text) return;
      window.socket?.emit('feed:comment', { postId, text, username });
      if (input) input.value = '';
    });
  });
    ...
    if (!feedLoaded) {
      ...
      discoverFeed.innerHTML = `
        <div class="discover-empty">
          ...
          <button class="btn-ms" onclick="renderDiscoverFeed(); window.socket?.emit('feed:join');">🔄 Tentar novamente</button>
        </div>
      `;
    }
  }, 8000);
...
window.socket?.on('feed:history', (posts) => {
...
window.socket?.on('connect', () => {
  ...
  if (username && username !== 'Usuário') {
    window.socket?.emit('user:login', { username, email: _email });
  } else {
...
window.socket?.on('feed:posts', (posts) => {
...
window.socket?.on('feed:new', (post) => {
  feedPostsLocal.unshift(post);
  renderDiscoverFeed();
});

window.socket?.on('feed:updated', ({ id, score }) => {
...
window.socket?.on('feed:commented', ({ postId, comment }) => {
...
function renderServerMessage(msg) {
  ...
  div.innerHTML = `
    <div class="msg-avatar av-${inicial}">${inicial}</div>
    <div class="msg-body">
      <div class="msg-meta">
        <span class="msg-username">${escapeHtml(msg.username)}</span>
        <span class="msg-time">${hora}</span>
      </div>
      <div class="msg-text">${escapeHtml(msg.text || msg.content || '')}</div>
    </div>
  `;
...
// Expondo para window para uso em outros scripts
window.suggestedCommunities = suggestedCommunities;
...
// Quando uma comunidade é atualizada
window.socket?.on('suggested:updated', (community) => {
  const index = suggestedCommunities.findIndex(c => c.id === community.id);
  if (index !== -1) {
    suggestedCommunities[index] = { ...suggestedCommunities[index], ...community };
    window.suggestedCommunities = suggestedCommunities;
    renderSuggestedCommunities();
    console.log('✅ Comunidade sugerida atualizada:', community.name);
  }
});

// Quando uma comunidade é removida
window.socket?.on('suggested:removed', ({ communityId }) => {
...
window.socket?.on('suggested:exists', ({ community }) => {
  showToast(`⚠ "${community.name}" já está na lista de sugeridas!`);
});

// ✅ Feedback: sem permissão
window.socket?.on('suggested:error', ({ message }) => {
  showToast(`❌ ${message}`);
});
        ...
        btn.addEventListener('click', (e) => {
          ...
          if (action === 'remove') {
            if (confirm(`Remover "${community.name}" da lista de comunidades sugeridas?`)) {
              // Envia evento para o servidor remover
              window.socket?.emit('community:unsuggest', { communityId });
...
function bindServerSettingsEvents(sectionId, server) {
  ...
  if (sectionId === 'membros') {
    ...
    if (list) {
      // Usar sistema de presença global para mostrar status online/offline
      const serverMembers = server.members || [];
      const voiceUsers = window._voiceRoomUsers || [];
      
      if (serverMembers.length === 0) {
        list.innerHTML = '<p class="pf-hint">Nenhum membro neste servidor.</p>';
        return;
      }
      
      list.innerHTML = serverMembers.map(member => {
        const memberName = member.username || '';
        const isOnline = onlineSet.has(memberName.toLowerCase());
        const status = userStatuses[memberName.toLowerCase()] || 'offline';
        const inVoice = voiceUsers.some(u => (u.username || '').toLowerCase() === memberName.toLowerCase());
        ...
        const statusLabel = isOnline ? (status === 'online' ? 'Online' : status.charAt(0).toUpperCase() + status.slice(1)) : 'Offline';
        
        return `<div class="srv-member-row">
          <span class="srv-member-dot" style="background: ${statusColor}"></span>
          <span>${escHtml(memberName)}</span>
          ${inVoice ? '<span class="srv-voice-icon">🎤</span>' : ''}
          <span class="pf-hint">${statusLabel}</span>
        </div>`;
      }).join('');
    }
  }
...
function renderMessage(msg) {
  try {
    ...
    div.innerHTML = `
      <div class="msg-avatar av-${initial}${avatarUrl ? ' has-image' : ''}"${avatarStyle} data-username="${escHtml(safeUsername)}">${avatarUrl ? '' : initial}</div>
      <div class="msg-body">
        <div class="msg-meta">
...
window.socket?.on('friends:data', (data) => {
  ...
  if (data.userId) {
    ...
    window.myUserId = myUserId; // [DM CALL FIX] Atualizar global também
  ...
  frs.forEach(f => {
    if (f && typeof f === 'object' && f.id) {
      ...
      if (key) {
        friendIds[key] = String(f.id);
        window.friendIds[key] = String(f.id); // [DM CALL FIX] Atualizar global também
      }
    }
    // Cachear avatar do amigo se disponível
    if (f && typeof f === 'object' && f.avatar) {
      const uname = f.username || f.nick || '';
      if (uname) setFriendAvatar(uname, f.avatar);
    }
  });
  console.log('[DM CALL FIX] friendIds atualizado:', friendIds);
  ...
  renderFriendsModal();
  // Atualizar avatares na lista DM e cabeçalho após cachear via setFriendAvatar acima.
  refreshAllAvatarViews();
...
// Listener global: resposta de user:avatar:get
window.socket?.on('user:avatar:data', (data) => {
  if (!data?.username) return;
  const key = data.username.toLowerCase();
  avatarRequestsPending.delete(key);
  if (data.avatar) setFriendAvatar(data.username, data.avatar);
  refreshAllAvatarViews();
});

// Listener para receber perfil do amigo (avatar) do servidor
window.socket?.on('dm:user:profile', (data) => {
  if (!data || !data.username) return;
  if (data.avatar) {
    setFriendAvatar(data.username, data.avatar);
    refreshAllAvatarViews();
  }
});

window.socket?.on('friends:presence', ({ online, statuses, avatars }) => {
  ...
  // Popula cache de avatar com o mapa enviado pelo servidor
  if (avatars) {
    Object.keys(avatars).forEach(uname => {
      const av = avatars[uname];
      if (av) setFriendAvatar(uname, av);
    });
  }
  console.log('[presence] onlineSet atualizado:', [...onlineSet]);
  console.log('[presence] userStatuses:', userStatuses);
  // FIX: Don't call renderFriendsModal() here - friends-fix-v6.js handles surgical updates
  // renderFriendsModal();
  refreshAllAvatarViews();
  updateFriendsBadge();
  // FIX: Update DM chat header status if the current chat user's status changed
  if (currentDmUser) {
    const key = (currentDmUser || '').toLowerCase();
    const isOnline = onlineSet.has(key);
    const status = userStatuses[key] || (isOnline ? 'online' : 'offline');
    const statusEl = document.querySelector('#dm-chat-area .dm-status');
    const activityEl = document.getElementById('dm-activity');
    ...
    if (activityEl) {
      activityEl.textContent = isUserOnline ? 'Ativo agora' : '';
    }
  }
  // Atualizar lista de membros do servidor se estiver visível
  const membersList = document.getElementById('srv-members-list');
  if (membersList && document.body.classList.contains('server-body')) {
    renderServerSettingsSection('membros');
  }
});

window.socket?.on('friend:request:sent', ({ to, offline }) => {
  // CORREÇÃO BUG #3: toast único, vindo da confirmação do servidor (não mais duplicado)
  // FIX: Clear the pending request flag since the server confirmed receipt
  const requestKey = 'pending_request_' + to.toLowerCase();
...
window.socket?.on('friend:request:error', ({ message, to }) => {
  // FIX: Clear the pending request flag on error
  const requestKey = 'pending_request_' + to.toLowerCase();
  sessionStorage.removeItem(requestKey);
...
window.socket?.on('friend:request', ({ from, avatar }) => {
  if (avatar && from) setFriendAvatar(from, avatar);
...
window.socket?.on('friend:accepted', ({ by, avatar }) => {
  if (avatar && by) setFriendAvatar(by, avatar);
  ...
  renderFriendsModal();
  // FIX: Update DM list when friend is added - this is necessary
  if (typeof renderDmList === 'function') renderDmList();
  // FIX: Request presence update to get the friend's online status immediately
  if (socket && socket.connected) {
    window.socket?.emit('presence:request');
  }
});

window.socket?.on('friend:rejected', ({ by }) => {
  sentRequests = sentRequests.filter(u => u.toLowerCase() !== by.toLowerCase());
  saveSentRequests();
  showToast(`${by} recusou sua solicitação.`);
});

window.socket?.on('friend:removed', ({ by }) => {
    friends = friends.filter(f => (f.username || f).toLowerCase() !== by.toLowerCase());
    saveFriends();
    renderFriendsModal();
  });

  window.socket?.on('friend:status', ({ username: uname, status }) => {
    ...
    // Status changes are handled by surgical updates in friends-fix-v6.js
    // if (typeof renderDmList === 'function') renderDmList();
    // FIX: Update DM chat header status if this user's chat is currently open
    if (currentDmUser && (currentDmUser || '').toLowerCase() === key) {
      const statusEl = document.querySelector('#dm-chat-area .dm-status');
      const activityEl = document.getElementById('dm-activity');
      if (statusEl) {
      ...
      if (activityEl) {
        activityEl.textContent = isOnline ? 'Ativo agora' : '';
      }
    }
  });

  window.socket?.on('friend:cancel:ok', ({ to }) => {
    sentRequests = sentRequests.filter(u => u.toLowerCase() !== to.toLowerCase());
    saveSentRequests();
  ...
  window.socket?.on('friend:request:cancelled', ({ by }) => {
...
function sendFriendRequest(toUsername) {
  ...
  // FIX: Prevent duplicate requests by checking if already in the process of sending
  const requestKey = 'pending_request_' + target.toLowerCase();
  if (sessionStorage.getItem(requestKey)) {
    console.log('Solicitação já em processo:', target);
    return;
  }
  
  sentRequests.push(target);
  saveSentRequests(); // persistir no localStorage para sobreviver a F5
  sessionStorage.setItem(requestKey, 'true'); // Mark as pending
  console.log('Solicitação enviada:', { to: target });
  window.socket?.emit('friend:request', { to: target });
  // FIX: Don't call renderFriendsModal() here - friends-fix-v6.js handles surgical updates
  // renderFriendsModal();
  
  // Clear the pending flag after 5 seconds (timeout for server response)
  setTimeout(() => {
    sessionStorage.removeItem(requestKey);
  }, 5000);
}
...
function acceptFriendRequest(from) {
  ...
  window.socket?.emit('friend:accept', { to: from });
  updateFriendsBadge();
  // FIX: Don't call renderFriendsModal() here - friends-fix-v6.js handles surgical updates
  // renderFriendsModal();
}
...
function rejectFriendRequest(from) {
  friendRequests = friendRequests.filter(r => r.from !== from);
  saveRequests();
  window.socket?.emit('friend:reject', { to: from });
  updateFriendsBadge();
  // FIX: Don't call renderFriendsModal() here - friends-fix-v6.js handles surgical updates
  // renderFriendsModal();
}
...
function cancelFriendRequest(toUsername) {
    ...
    window.socket?.emit('friend:cancel', { to: target });
  }

  function removeFriend(uname) {
    friends = friends.filter(f => (f.username || f).toLowerCase() !== uname.toLowerCase());
    saveFriends();
    window.socket?.emit('friend:remove', { to: uname });
    // FIX: Don't call renderFriendsModal() here - friends-fix-v6.js handles surgical updates
    // renderFriendsModal();
  }
...
function renderOnlineFriends() {
    ...
    const online = friends.filter(f => {
      ...
      // Considera ativo se: está no onlineSet (conectado) E não tem status explícito de offline/invisible
...
function renderAllFriends() {
    ...
    const _isActive = (key) => {
      ...
      return onlineSet.has(key) && (!st || _ACTIVE.has(st));
    };
    ...
    function renderSubTab(sub) {
      ...
      if (sub === 'received') {
        ...
        content.innerHTML = validRequests.map(r => {
          ...
          const avatarUrl = getFriendAvatar(from);
          ...
          return `
            <div class="friend-card" data-friend-card="${escHtml(from)}">
              <div class="friend-avatar-wrap">
                <div class="friend-avatar av-${initial}" style="${avatarStyle}">${avatarContent}</div>
              </div>
        ...
        content.innerHTML = sentRequests.map(to => {
          ...
          const avatarUrl = getFriendAvatar(to);
          const avatarStyle = avatarUrl ? `background-image:url(${avatarUrl});background-size:cover;background-position:center;color:transparent;` : '';
          const avatarContent = avatarUrl ? '' : initial;
          return `
...
function friendCard(uname, isOnline) {
    ...
    // Verificar cache global (dm-avatar-call-fix) e cache de amigos
    const _ck = (uname || '').toLowerCase();
    const avatarUrl = getFriendAvatar(uname)
                   || (uname.toLowerCase() === (window.username || '').toLowerCase() ? (localStorage.getItem('zx_avatar') || '') : '');
    if (!avatarUrl) requestUserAvatar(uname);
...
// ================================================
// SISTEMA MÁQUINA DE ESCREVER - Apenas abrir view
// ================================================
...
// ============================================
// FIX DM - CORRIGE receiverId NULL
// ============================================
(function fixDmEmit() {
  if (!window.socket) {
    console.warn('[DM-FIX] Socket não encontrado, aguardando...');
    setTimeout(fixDmEmit, 500);
    return;
  }
  
  const originalEmit = window.socket.emit;
  window.socket.emit = function(event, data) {
    if (event === 'dm:message' && data && data.receiverId === null) {
      data.receiverId = data.to;
      console.log('[DM-FIX] receiverId corrigido para:', data.receiverId);
    }
    return originalEmit.call(this, event, data);
  };
  
  console.log('[DM-FIX] Aplicado com sucesso!');
})();

// ============================================
// FIX: Enter envia mensagem na DM
// ============================================
(function fixDmEnter() {
  // Usa event delegation para capturar Enter no input da DM
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && !e.shiftKey) {
      const input = e.target;
      // Verifica se é o input da DM (pode ser #dm-message-input ou #private-chat-input)
      if (input.id === 'dm-message-input' || input.id === 'private-chat-input') {
        e.preventDefault();
        // Procura o botão de enviar associado
        const sendBtn = input.closest('.message-input-area')?.querySelector('#dm-send-btn, #private-chat-send-btn') ||
                        document.getElementById('dm-send-btn') ||
                        document.getElementById('private-chat-send-btn');
        if (sendBtn) {
          sendBtn.click();
        }
      }
    }
  }, true); // useCapture para capturar antes de outros handlers

  console.log('[DM-FIX] Enter habilitado para envio de DM');
})();

// ============================================
// FIX: Atualiza o chat em tempo real na DM
// ============================================
(function fixDmRealtime() {
  if (!window.socket) {
    console.warn('[DM-RT] Socket não encontrado');
    return;
  }

  // FIX: window.socket?.off('dm:message') REMOVIDO — não deve matar o listener oficial _onDmMessage.
  // A renderização das mensagens é feita pelo listener oficial em script.js.
  // Este bloco é redundante com fixDmAutoUpdate abaixo; apenas loga.

  console.log('[DM-RT] Listener auxiliar registrado (sem duplicar renderização).');
})();

// ============================================
// FIX: Atualiza o chat automaticamente ao receber DM
// ============================================
(function fixDmAutoUpdate() {
  if (!window.socket) return;

  // FIX: window.socket?.off('dm:message') REMOVIDO — não deve matar o listener oficial _onDmMessage.
  // Este listener é auxiliar: apenas atualiza a lista lateral e o badge.
  // A renderização das mensagens é feita pelo listener oficial em script.js.
  window.socket?.on('dm:message', function(msg) {
    console.log('[DM-AutoUpdate] Mensagem recebida (apenas lista/badge):', msg);

    const currentUser = window.username || '';
    const isForMe = (msg.to || '').toLowerCase() === currentUser.toLowerCase();
    const isFromMe = (msg.from || '').toLowerCase() === currentUser.toLowerCase();

    if (!isForMe && !isFromMe) return;

    const targetUser = isForMe ? msg.from : msg.to;

    // 1. Atualiza a lista de conversas (sidebar) - com deduplicação
    updateDmList(targetUser);

    // 2. FIX: Não adiciona mensagem ao DOM aqui — o listener oficial já faz isso.
    // Apenas atualiza o badge se o chat não estiver aberto.
    const activeChat = document.querySelector('#dm-chat-area')?.dataset.activeChat;

    // 3. Atualiza o badge de não lidos se o chat não estiver aberto
    if (!activeChat || activeChat.toLowerCase() !== targetUser.toLowerCase()) {
      incrementBadge(targetUser);
    }
  });

  // Função para atualizar a lista de conversas (sidebar) sem duplicar
  function updateDmList(username) {
    if (typeof window.renderDmList === 'function') {
      // Se já existe função de render, usa ela
      window.renderDmList();
    } else {
      // Fallback: adiciona na lista manualmente
      const list = document.getElementById('dm-conversations-list');
      if (!list) return;
      
      // Remove duplicatas existentes
      const existing = list.querySelectorAll('[data-username]');
      ...
      const _friendAvatar = getFriendAvatar(username);
      const _avatarInner = _friendAvatar
        ? `<div class="dm-avatar" style="background-image:url(${_friendAvatar});background-size:cover;background-position:center;"></div>`
        : `<div class="dm-avatar">${username[0].toUpperCase()}</div>`;
      item.innerHTML = `
        ${_avatarInner}
        <div class="dm-conv-name">${username}</div>
        <div class="dm-conv-badge">●</div>
      `;
      item.style.cssText = `
        display: flex;
        align-items: center;
      ...
      `;
      item.onclick = () => openDmChat(username);
      list.prepend(item);
    }
  }

  function incrementBadge(username) {
    // Adiciona um badge de não lido na conversa
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;
    const items = list.querySelectorAll('[data-username]');
    for (let el of items) {
      if (el.dataset.username.toLowerCase() === username.toLowerCase()) {
        let badge = el.querySelector('.dm-conv-badge');
        if (!badge) {
          badge = document.createElement('span');
          badge.className = 'dm-conv-badge';
          badge.style.cssText = 'background: #ff00ff; color: white; border-radius: 50%; padding: 2px 6px; font-size: 10px; margin-left: auto;';
          el.appendChild(badge);
        }
  ...
  console.log('[DM-FIX] Auto-update ativado!');
})();


// ============================================================
// FIX DEFINITIVO: Avatar na barra superior da DM (CORRIGIDO)
// ============================================================
(function fixDmAvatarFinal() {
  function applyAvatarToDm() {
    const avatarEl = document.getElementById('user-avatar-dm');
    if (!avatarEl) {
      console.log('[DM-AVATAR] Elemento #user-avatar-dm ainda não existe');
      return false;
    }

    const saved = localStorage.getItem('zx_avatar');
    if (saved && saved.startsWith('data:image')) {
      // ✅ CORREÇÃO: NÃO use "background: none" – isso apaga a imagem!
      avatarEl.style.backgroundImage = `url(${saved})`;
      avatarEl.style.backgroundSize = 'cover';
      avatarEl.style.backgroundPosition = 'center';
      avatarEl.style.backgroundRepeat = 'no-repeat';
      // Remove o texto (iniciais)
      avatarEl.textContent = '';
      avatarEl.classList.add('has-image');
      console.log('[DM-AVATAR] ✅ Avatar aplicado com sucesso!');
      return true;
    } else {
      console.warn('[DM-AVATAR] Nenhum avatar salvo em zx_avatar');
      // Fallback: usa as iniciais
      const name = window.username || localStorage.getItem('zx_username') || 'Usuário';
      avatarEl.style.backgroundImage = 'none';
      avatarEl.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
  ...
  // Observer para detectar quando o elemento aparecer
  ...
  // Intercepta a abertura da view DM
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
      if (layoutId === 'dm-view') {
        applyAvatarToDm();
        setTimeout(applyAvatarToDm, 100);
        setTimeout(applyAvatarToDm, 300);
        setTimeout(applyAvatarToDm, 600);
      }
    };
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(applyAvatarToDm, 50);
      setTimeout(applyAvatarToDm, 200);
      setTimeout(applyAvatarToDm, 500);
    }
  });

  // Se a DM já estiver visível ao carregar
  ...
  console.log('[DM-AVATAR] Correção definitiva (sem background:none) ativada');
})();

// ============================================================
// FIX: Mensagens em tempo real no Direct (sem clicar no perfil)
// ============================================================
(function fixDmRealtime() {
  if (!window.socket) {
    console.warn('[DM-RT] Socket não disponível');
    return;
  }

  // FIX: Não remover o listener oficial _onDmMessage que já cuida da renderização.
  // Este bloco agora apenas cuida da reordenação da lista lateral e do badge.
  // window.socket?.off('dm:message') REMOVIDO para não matar o listener oficial.

  // Listener auxiliar para reordenar lista e badge (sem duplicar renderização)
  window.socket?.on('dm:message', function(msg) {
    console.log('[DM-RT] Mensagem recebida:', msg);

    const currentUser = (window.username || '').toLowerCase();
    const isForMe = msg.to.toLowerCase() === currentUser;
    const isFromMe = msg.from.toLowerCase() === currentUser;

    if (!isForMe && !isFromMe) return;

    // Quem é o interlocutor?
    const sender = isForMe ? msg.from : msg.to;

    // 1. Atualiza a lista lateral (reordena e coloca no topo)
    updateDmListAndReorder(sender);

    // 2. FIX: A renderização da mensagem no chat é feita pelo _onDmMessage oficial.
    // Este listener cuida apenas da reordenação da lista lateral.
    // Adicionar mensagem aqui causaria duplicação.
    console.log('[DM-RT] Lista lateral reordenada para:', sender);

    // 3. Atualiza o badge de não lidos (opcional)
    updateUnreadBadge(sender);
  });

  // Função para reordenar a lista lateral (coloca a conversa no topo)
  function updateDmListAndReorder(username) {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;

    // Procura o item da conversa
    const items = list.querySelectorAll('.dm-conv-item');
    let targetItem = null;
    for (const item of items) {
      if (item.dataset.username.toLowerCase() === username.toLowerCase()) {
        targetItem = item;
        break;
      }
    }
    ...
    if (targetItem) {
      ...
      // Dá um destaque visual
      targetItem.style.background = 'rgba(255,0,255,0.1)';
      setTimeout(() => { targetItem.style.background = ''; }, 500);
    } else {
      // Se não existe, cria um novo item (fallback)
      const item = document.createElement('div');
      item.className = 'dm-conv-item';
      item.dataset.username = username;
      item.style.cssText = `
      ...
      item.innerHTML = `
        ...
        <div style="flex:1;color:#eee;font-size:14px;">${username}</div>
      `;
      item.onclick = () => {
        if (typeof openDmChat === 'function') openDmChat(username);
      };
      list.prepend(item);
    }
  }

  // Função para incrementar badge de não lidos (opcional)
  function updateUnreadBadge(username) {
    // Você pode implementar se quiser um contador de não lidos
  }

  // Força uma atualização inicial da lista
  if (typeof renderDmList === 'function') {
    renderDmList();
  }

  console.log('[DM-RT] Correção aplicada: mensagens em tempo real no Direct');
})();

// ============================================================
// POLLING INSTANTÂNEO: Chat DM atualiza a cada 1 segundo
// ============================================================
(function instantDmPolling() {
  if (!window.socket) {
    console.warn('[POLLING] Socket não disponível');
    return;
  }
  ...
  // Função que pergunta ao servidor por novas mensagens
  function pollDmMessages() {
    const chatArea = document.getElementById('dm-chat-area');
    if (!chatArea || chatArea.classList.contains('hidden')) {
      // Se o chat foi fechado, para o polling
      if (pollingInterval) {
    ...
    // Se mudou de chat, reinicia o polling
    if (currentChatUser !== username) {
      ...
      // Cria um novo intervalo para este chat (1 segundo)
      pollingInterval = setInterval(() => {
        if (window.socket && window.socket.connected) {
          window.socket?.emit('dm:history', { with: username });
        }
      }, 8000);
      console.log('[POLLING] Monitorando chat com', username, '(a cada 8s)');
    }

    // Faz uma requisição imediata ao abrir ou mudar de chat
    if (window.socket && window.socket.connected) {
      window.socket?.emit('dm:history', { with: username });
    }
  }

  // FIX: Listener window.socket?.on('dm:history') REMOVIDO deste bloco.
  // O listener oficial em script.js (linha ~1156) já faz merge em dmMessages[]
  // e chama renderDmMessages(). Manter um segundo listener aqui causava o loop
  // "[DM HISTÓRICO] parceiro: X | mensagens: N" infinito a cada polling.

  // Quando o usuário enviar uma mensagem, força o polling imediato
  window.socket?.on('dm:message:sent', function(data) {
    // Aguarda 500ms para o servidor salvar e depois faz polling
    setTimeout(() => {
      if (currentChatUser) {
        window.socket?.emit('dm:history', { with: currentChatUser });
      }
    }, 500);
  });

  // Intercepta a abertura do chat (via click ou função)
  document.addEventListener('click', function(e) {
    const target = e.target.closest('.dm-conv-item, [data-open-chat]');
    if (target) {
      setTimeout(pollDmMessages, 100);
    }
  });

  // Se a view DM já estiver aberta ao carregar, inicia o polling
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(pollDmMessages, 200);
    }
  });

  // Observa a mudança de layout para iniciar/parar o polling
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  console.log('[POLLING] Instantâneo ativado (verifica a cada 1s e ao enviar)');
})();

// ============================================================
// FIX DEFINITIVO: Para o piscar no chat DM (sem recriar o DOM)
// ============================================================
(function stopDmFlicker() {
  // 1. Sobrescreve a função que renderiza mensagens para nunca usar innerHTML
  if (window.renderPrivateMessages) {
    const originalRender = window.renderPrivateMessages;
    window.renderPrivateMessages = function(username) {
      // Se a mensagem já existe, não faz nada (apenas adiciona novas)
      const messagesArea = document.getElementById('dm-messages-area');
      if (!messagesArea) return;

      const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
      const messages = privateMessages[username] || [];

      // Conta quantas mensagens já estão no DOM
      const existingCount = messagesArea.children.length;
      ...
      console.log('[FIX] Adicionadas', newMessages.length, 'mensagens sem recriar o DOM');
    };
  }

  // 2. CORRIGIDO: Bloqueio de innerHTML removido — impedia renderDmMessages() de funcionar.
  // O flicker é controlado pela lógica de deduplicacao em _onDmMessage e renderDmMessages.

  // 3. CORRIGIDO: clearInterval em massa removido — destruía timers de outros sistemas.
  // O polling de dm:history é controlado pelo instantDmPolling() que já tem sua própria lógica.

  console.log('[FIX] stopDmFlicker: patches de innerHTML e clearInterval em massa desativados (corrigido).');
})();

// ============================================================
// ADICIONA AVATAR E NOME NAS MENSAGENS DO CHAT DM
// ============================================================
(function addAvatarsToDmMessages() {
  function createDmMessage(msg, isMe) {
    const container = document.createElement('div');
    container.className = 'dm-message-wrapper';
    ...
    avatarDiv.className = 'dm-message-avatar';
    ...
    const username = isMe ? (window.username || 'Você') : msg.from || msg.username || 'Usuário';
    if (!isMe && !getFriendAvatar(username)) requestUserAvatar(username);
    renderAvatar(avatarDiv, username);
    ...
    nameDiv.className = 'dm-message-username';
    ...
    bubbleDiv.className = 'dm-message-bubble';
  ...
  // Sobrescreve a função que adiciona mensagens ao DOM
  const originalAddMessage = window.addDmMessageToDom || function() {};

  window.addDmMessageToDom = function(msg, isMe) {
    const messagesArea = document.getElementById('dm-messages-area');
    if (!messagesArea) return;
    ...
    if (lastMsg) {
      const lastText = lastMsg.querySelector('.dm-message-bubble')?.textContent;
      if (lastText === msg.text && lastMsg.dataset.sender === (isMe ? 'me' : 'other')) {
        return;
      }
    }

    const msgElement = createDmMessage(msg, isMe);
    msgElement.dataset.sender = isMe ? 'me' : 'other';
    messagesArea.appendChild(msgElement);
    messagesArea.scrollTop = messagesArea.scrollHeight;
  };

  // FIX: Listener dm:history removido deste bloco para evitar duplicação.
  // O listener oficial em script.js (linha ~1075) já faz merge e chama renderDmMessages().
  // addDmMessageToDom() permanece disponível para uso externo (ex: envio local).

  // Intercepta o envio local (quando o usuário envia)
  const originalSend = window.sendDmMessage;
  if (originalSend) {
    window.sendDmMessage = function(text, toUsername) {
      // Chama a função original para enviar pelo socket
      originalSend.call(this, text, toUsername);
      ...
      setTimeout(() => {
        window.addDmMessageToDom(msg, true);
      }, 100);
    };
  }

  // Força a recriação das mensagens existentes ao abrir o chat
  const originalOpenDm = window.openDmChat;
  if (originalOpenDm) {
    window.openDmChat = function(username) {
      originalOpenDm.call(this, username);
      // Aguarda o DOM carregar e recria as mensagens com o novo estilo
      setTimeout(() => {
        const messagesArea = document.getElementById('dm-messages-area');
        if (!messagesArea) return;

        // Salva as mensagens atuais
        const currentMessages = Array.from(messagesArea.children).map(child => {
          const bubble = child.querySelector('.dm-message-bubble');
          return {
            text: bubble ? bubble.textContent : '',
            sender: child.dataset.sender || 'other'
          };
        });

        // Limpa e recria com o novo estilo
        messagesArea.innerHTML = '';
        const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
        const msgs = privateMessages[username] || [];
        msgs.forEach(msg => {
          const isMe = msg.sender === 'me' || msg.from === window.username;
          window.addDmMessageToDom(msg, isMe);
        });
      }, 200);
    };
  }

  console.log('[AVATAR] Mensagens agora mostram avatar e nome');
})();

// ============================================================
// BARRA DM + PICKERS (TUDO DO ZERO)
// ============================================================
(function dmBarFromScratch() {
  console.log('[DM-SCRATCH] Iniciando construção...');

  // ── 1. CRIA OS PICKERS (dentro do body) ──
  function createPickers() {
    // Emoji picker
    let emojiPicker = document.getElementById('dm-emoji-picker');
    if (!emojiPicker) {
      emojiPicker = document.createElement('div');
      emojiPicker.id = 'dm-emoji-picker';
      emojiPicker.className = 'dm-picker';
    ...
    emojiPicker.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;max-height:350px;overflow-y:auto;justify-content:center;padding:4px;">
        ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟'].map(e => `<span class="dm-emoji-item" style="font-size:28px;cursor:pointer;padding:4px;border-radius:6px;display:inline-block;">${e}</span>`).join('')}
      </div>
    `;

    // GIF picker
    let gifPicker = document.getElementById('dm-gif-picker');
    if (!gifPicker) {
      gifPicker = document.createElement('div');
      gifPicker.id = 'dm-gif-picker';
      gifPicker.className = 'dm-picker';
      gifPicker.style.cssText = emojiPicker.style.cssText;
      document.body.appendChild(gifPicker);
    }
    gifPicker.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;max-height:350px;overflow-y:auto;padding:4px;">
        ${['https://media.giphy.com/media/3o7aTskHEUdgCQAXde/giphy.gif','https://media.giphy.com/media/26tn33aiTi1jkl6H6/giphy.gif','https://media.giphy.com/media/3o6Zt481isNVuQI1l6/giphy.gif','https://media.giphy.com/media/l0HlNQ03J5JxX6lva/giphy.gif','https://media.giphy.com/media/3o7abKhOpu0N9H8s/giphy.gif','https://media.giphy.com/media/l0HlOBPvWqHj1D7J2/giphy.gif'].map(url => `<div class="dm-gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;aspect-ratio:16/9;"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
      </div>
    `;

    // Sticker picker
    let stickerPicker = document.getElementById('dm-sticker-picker');
    if (!stickerPicker) {
      stickerPicker = document.createElement('div');
      stickerPicker.id = 'dm-sticker-picker';
      stickerPicker.className = 'dm-picker';
      stickerPicker.style.cssText = emojiPicker.style.cssText;
      document.body.appendChild(stickerPicker);
    }
    stickerPicker.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;max-height:350px;overflow-y:auto;padding:4px;">
        ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️'].map(s => `<span class="dm-sticker-item" style="font-size:36px;cursor:pointer;padding:6px;border-radius:8px;display:inline-block;">${s}</span>`).join('')}
      </div>
    `;

    // Modal "Mais opções"
    let moreModal = document.getElementById('dm-more-modal');
    if (!moreModal) {
      moreModal = document.createElement('div');
      moreModal.id = 'dm-more-modal';
      moreModal.className = 'dm-picker';
      moreModal.style.cssText = `
        position: fixed !important;
        bottom: 80px !important;
        left: 50% !important;
        transform: translateX(-50%) !important;
      ...
      moreModal.innerHTML = `
        <div style="display:flex;flex-direction:column;gap:8px;">
          <button id="dm-poll-btn" style="padding:12px;background:rgba(255,0,255,0.1);border:1px solid #ff00ff;border-radius:8px;color:#fff;cursor:pointer;font-size:16px;">📊 Criar Enquete</button>
          <button id="dm-topic-btn" style="padding:12px;background:rgba(0,255,255,0.1);border:1px solid #00ffff;border-radius:8px;color:#fff;cursor:pointer;font-size:16px;">💬 Criar Tópico</button>
        </div>
      `;
  ...
  // ── 2. FUNÇÃO PARA ABRIR/FECHAR PICKER ──
  function togglePicker(pickerId) {
    const picker = document.getElementById(pickerId);
    if (!picker) {
      console.warn(`[DM] Picker ${pickerId} não encontrado`);
      return;
    }
    // Fecha todos os outros
    document.querySelectorAll('.dm-picker.active').forEach(p => {
      if (p !== picker) p.classList.remove('active');
    });
    const isActive = picker.classList.toggle('active');
    picker.style.display = isActive ? 'flex' : 'none';
    console.log(`[DM] ${pickerId} ${isActive ? 'aberto' : 'fechado'}`);
    if (isActive) {
  ...
  // ── 3. CRIA A BARRA ──
  function createBar() {
    let target = document.getElementById('dm-input-area');
    if (!target) {
      const dmView = document.getElementById('dm-view');
      if (!dmView) return;
      target = document.createElement('div');
      target.id = 'dm-input-area';
      target.style.cssText = 'padding: 12px 16px; border-top: 1px solid rgba(255,0,255,0.1);';
      const messages = dmView.querySelector('#dm-messages-area, .messages-area');
      if (messages) {
        messages.parentNode.insertBefore(target, messages.nextSibling);
      } else {
        dmView.appendChild(target);
      }
    ...
    `;
      ...
      wrapper.innerHTML = `
              <button type="button" id="dm-btn-plus" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">➕</button>
              <button type="button" id="dm-btn-emoji" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">😀</button>
              <button type="button" id="dm-btn-gif" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎬</button>
              <button type="button" id="dm-btn-stickers" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎭</button>
              <button type="button" id="dm-btn-attach" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">📎</button>
              <div style="flex:1; position:relative; display:flex; align-items:center;">
                <textarea id="dm-message-input" placeholder="Digite sua mensagem..." maxlength="4000" autocomplete="off" style="flex:1;padding:10px 14px;background:rgba(255,255,255,0.06);border:none;border-radius:12px;color:#fff;outline:none;font-size:14px;min-width:50px;resize:none;height:40px;max-height:150px;overflow-y:auto;"></textarea>
                <div id="dm-char-counter" style="position:absolute; right:10px; bottom:-14px; font-size:9px; color:#888;">0/4000</div>
              </div>
              <button type="button" id="dm-btn-voice" class="dm-btn" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎤</button>
              <button id="dm-send-btn" type="button" style="background:linear-gradient(135deg,#8b00ff,#ff00ff);border:none;border-radius:50%;width:40px;height:40px;color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;">➤</button>
            `;

    target.innerHTML = '';
    target.appendChild(wrapper);
    console.log('[DM] Barra criada');

    // ── 4. CONECTA EVENTOS DIRETAMENTE (não por delegation) ──
    const btnEmoji = document.getElementById('dm-btn-emoji');
    const btnGif = document.getElementById('dm-btn-gif');
    const btnSticker = document.getElementById('dm-btn-stickers');
    const btnPlus = document.getElementById('dm-btn-plus');
    const btnAttach = document.getElementById('dm-btn-attach');
    const btnSend = document.getElementById('dm-send-btn');
    const input = document.getElementById('dm-message-input');

    // Conecta botões de picker
    function connectButton(btn, pickerId) {
      if (!btn) return;
      ...
      newBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log(`[DM] Clique em ${this.id}`);
        togglePicker(pickerId);
      });
    }

    connectButton(btnEmoji, 'dm-emoji-picker');
    connectButton(btnGif, 'dm-gif-picker');
    connectButton(btnSticker, 'dm-sticker-picker');
    connectButton(btnPlus, 'dm-more-modal');

    // Botão anexar
    if (btnAttach) {
      const newAttach = btnAttach.cloneNode(true);
      btnAttach.parentNode.replaceChild(newAttach, btnAttach);
      newAttach.addEventListener('click', function(e) {
        e.preventDefault();
        const fileInput = document.getElementById('dm-file-upload');
    ...
    // ── 5. INSERE ITENS DOS PICKERS NO INPUT ──
    document.addEventListener('click', function(e) {
      const target = e.target.closest('.dm-emoji-item, .dm-gif-item, .dm-sticker-item');
      if (!target) return;
      const picker = target.closest('.dm-picker');
      if (!picker) return;
      if (!input) return;

      let value = '';
      if (target.classList.contains('dm-gif-item')) {
      ...
      if (value) {
        ...
        console.log(`[DM] Inserido: ${value}`);
      }
    });

    // ── 6. FUNÇÃO DE ENVIO ──
    function sendDm(inputElement) {
      ...
      if (typeof window.sendDmMessage === 'function') {
        // A função sendDmMessage no script.js não aceita parâmetros, ela pega do input
        // Mas o patch do final do arquivo tenta usar parâmetros.
        // Vamos garantir que o input global tenha o valor para as funções que o usam.
        const globalInput = document.getElementById('dm-message-input');
        if (globalInput && globalInput !== inputElement) globalInput.value = text;
        
        window.sendDmMessage(); 
      } else if (typeof window.sendPrivateMessage === 'function') {
        window.sendPrivateMessage(activeUser);
      }
      inputElement.value = '';
    ...
    // ── 7. ANEXAR ARQUIVO ──
    let fileInput = document.getElementById('dm-file-upload');
    if (!fileInput) {
      fileInput = document.createElement('input');
      fileInput.type = 'file';
      fileInput.id = 'dm-file-upload';
      fileInput.className = 'hidden';
      fileInput.multiple = true;
      document.body.appendChild(fileInput);
    }
    fileInput.addEventListener('change', function() {
      console.log('[DM] Arquivo(s) selecionado(s):', this.files);
      this.value = '';
    });

    // ── 8. AÇÕES DO MODAL "MAIS OPÇÕES" ──
    document.addEventListener('click', function(e) {
      if (e.target.id === 'dm-poll-btn') {
        alert('📊 Criar enquete (em desenvolvimento)');
        togglePicker('dm-more-modal');
      } else if (e.target.id === 'dm-topic-btn') {
        alert('💬 Criar tópico (em desenvolvimento)');
        togglePicker('dm-more-modal');
      }
    });

    console.log('[DM] Todos os eventos conectados');
  }
  ...
  function waitForDM() {
    const dmView = document.getElementById('dm-view');
    if (!dmView) {
      setTimeout(waitForDM, 300);
      return;
    }

    const observer = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        console.log('[DM] DM aberta, construindo...');
        setTimeout(() => {
          createPickers();
          createBar();
        }, 100);
    ...
    observer.observe(dmView, { attributes: true, attributeFilter: ['class'] });
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    ...
    document.addEventListener('DOMContentLoaded', waitForDM);
  }
  ...
  console.log('[DM] Script do zero carregado');
})();

// ── FECHAR PICKERS AO CLICAR FORA ──
document.addEventListener('click', function(e) {
  const pickers = document.querySelectorAll('.dm-picker.active');
  if (pickers.length === 0) return;
  
  // Verifica se o clique foi dentro de algum picker ou em algum botão que abre picker
  const isInsidePicker = e.target.closest('.dm-picker');
  const isButton = e.target.closest('#dm-btn-emoji, #dm-btn-gif, #dm-btn-stickers, #dm-btn-plus');
  ...
  if (!isInsidePicker && !isButton) {
    ...
    console.log('[DM] Pickers fechados por clique fora');
  }
});

// ============================================================
// CORREÇÃO DEFINITIVA DE ENVIO NO DM
// ============================================================
(function fixDmSendFinal() {
  console.log('[DM-SEND] Aplicando correção definitiva...');

  // ── 0. FIX: Salva a função original ANTES de sobrescrever (ordem corrigida) ──
  if (typeof window.sendDmMessage === 'function' && !window._originalSendDm) {
    window._originalSendDm = window.sendDmMessage;
  }

  // ── 1. GARANTE QUE O data-activeChat ESTEJA DEFINIDO ──
  function ensureActiveChat() {
    const chatArea = document.getElementById('dm-chat-area');
    if (!chatArea) return;
    if (chatArea.dataset.activeChat) return;
    ...
    if (header) {
      ...
      if (username) {
        chatArea.dataset.activeChat = username;
        console.log('[DM-SEND] activeChat definido como:', username);
      }
    }
  }

  // ── 2. FUNÇÃO DE ENVIO (GLOBAL) ──
  window.sendDmMessage = function(text, toUsername) {
    if (!text || !toUsername) return;
    console.log(`[DM-SEND] Enviando para ${toUsername}: ${text}`);

    // Tenta usar a função original se existir
    if (typeof window._originalSendDm === 'function') {
      window._originalSendDm(text, toUsername);
      return;
    }

    // Fallback: emite via socket diretamente
    if (window.socket && window.socket.connected) {
      const msg = {
      ...
      window.socket?.emit('dm:message', msg);
      console.log('[DM-SEND] Mensagem enviada via socket fallback');
    } else {
      console.warn('[DM-SEND] Socket não conectado');
    }
  };
  ...
  function sendCurrentDm() {
    ...
    // Garante que o activeChat esteja definido
    ensureActiveChat();
  ...
  // ── 4. EVENT DELEGATION PARA CAPTURAR CLIQUE NO BOTÃO ──
  document.addEventListener('click', function(e) {
    const sendBtn = e.target.closest('#dm-send-btn, [id$="-send-btn"]');
    if (sendBtn) {
      e.preventDefault();
      e.stopPropagation();
      console.log('[DM-SEND] Clique no botão enviar');
      sendCurrentDm();
    }
  });

  // ── 5. EVENT DELEGATION PARA CAPTURAR ENTER ──
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && !e.shiftKey) {
      const input = e.target.closest('#dm-message-input');
      if (input) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[DM-SEND] Enter pressionado');
        sendCurrentDm();
      }
    }
  });

  // ── 6. TAMBÉM OUVIR KEYPRESS (FALLBACK) ──
  document.addEventListener('keypress', function(e) {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      if (input) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[DM-SEND] Enter (keypress)');
        sendCurrentDm();
      }
    }
  });

  // ── 7. OBSERVA A ABERTURA DO CHAT PARA DEFINIR activeChat ──
  ...
  // ── 8. EXECUTA IMEDIATAMENTE PARA CASO O CHAT JÁ ESTEJA ABERTO ──
  setTimeout(ensureActiveChat, 500);

  // FIX: Save da função original movido para o início do bloco (passo 0).

  console.log('[DM-SEND] Correção definitiva aplicada');
})();

// Código removido por ser órfão e causar erros de referência (btn is not defined)
/*
...
window.postarReel = async function() {
  const videoInput = document.getElementById("video-input");
  if (!videoInput) return;
  const videoUrl = videoInput.value;
  if (typeof supabase === 'undefined') return console.error("Supabase não inicializado");
  const { data, error } = await supabase.from('Xenon').insert([{ video_url: videoUrl }]);
  if (error) console.error("ERRO:", error);
  else console.log("SALVO:", data);
};

window.addSuggested = async function(serverId) {
  try {
...
// 🔹 FUNÇÃO NOVA - Obtém o parceiro da DM de forma confiável (adicione isso no script.js)
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (callBtn) {
    // 🔹 SOBRESCREVE o onclick ANTIGO (sem remover o atributo do HTML)
