...
(function () {
  ...
  function setupSocketRoleListener() {
    const trySocket = setInterval(() => {
      ...
      sock.on('user:role', function (data) {
        ...
        // ❌ updateDropdownVisibility() desativado - permissions-system.js gerencia a visibilidade
      });
  ...
  // ──────────────────────────────────────────────
  // ❌ DESATIVADO - O sistema de permissões (permissions-system.js) já gerencia isso corretamente
  // Esta função estava interferindo com o sistema de permissões por servidor
  /*
  ...
  */
  ...
  function setupServerDropdown() {
    ...
    // ❌ DESATIVADO - Não adicionar classe staff-item pois o permissions-system.js gerencia a visibilidade
    /*
  ...
  function showProfilePopup(username, anchorEl) {
    ...
    // ── Aplicar avatar real do cache (ou solicitar ao servidor) ──────────
    (function applyPopupAvatar() {
      var avKey = username.toLowerCase();
      var avEl  = popup.querySelector('.zxp-avatar');
      if (!avEl) return;

      function _setImg(url) {
        if (!url || !avEl) return;
        // Remove texto da inicial
        avEl.childNodes.forEach(function(n) { if (n.nodeType === 3) n.textContent = ''; });
        // Insere ou reutiliza o <img>
        var img = avEl.querySelector('img');
        if (!img) { img = document.createElement('img'); avEl.insertBefore(img, avEl.firstChild); }
        img.src = url;
      ...
      var cached = (window.friendAvatarCache || {})[avKey];
      if (cached) { _setImg(cached); return; }

      // Ainda não está em cache — solicitar ao servidor
      var sock = window.socket || window.io_socket;
      if (!sock || !sock.connected) return;
      if (typeof window.requestUserAvatar === 'function') {
        window.requestUserAvatar(username);
      } else {
        sock.emit('user:avatar:get', { username: username });
      }

      // Ouvir a resposta uma única vez para este popup
      function _onData(data) {
        if (!data || (data.username || '').toLowerCase() !== avKey) return;
        sock.off('user:avatar:data', _onData);
        if (data.avatar) {
          if (typeof window.setFriendAvatar === 'function') window.setFriendAvatar(data.username, data.avatar);
          if (popup.classList.contains('active')) _setImg(data.avatar);
        }
      }
      sock.on('user:avatar:data', _onData);
      // Limpar listener se o popup fechar antes
      setTimeout(function() { sock.off('user:avatar:data', _onData); }, 8000);
    })();
      ...
      .then(function(data) {
        ...
        if (userShorts.length > 0) {
          ...
          userShorts.slice(0, 6).forEach(function(s) {
            ...
            if (isImg) {
              html += '<img src="' + escHtml(s.fileUrl) + '" loading="lazy" onerror="this.style.display=\'none\';this.parentElement.innerHTML=\'<div style=\\\'color:#888;font-size:10px;text-align:center;padding:10px;\\\'>Imagem não disponível</div>\'">';
            } else {
              html += '<video src="' + escHtml(s.fileUrl) + '" muted playsinline preload="metadata" onerror="this.style.display=\'none\';this.parentElement.innerHTML=\'<div style=\\\'color:#888;font-size:10px;text-align:center;padding:10px;\\\'>Vídeo não disponível</div>\'"></video>';
            }
            html += '</div>';
          });
          html += '</div>';
