...
// ═══════════════════════════════════════════════════════════════════════════
// PERSISTÊNCIA DE SOLICITAÇÕES DE AMIZADE NO NEON (PostgreSQL)
// ═══════════════════════════════════════════════════════════════════════════
...
async function initFriendRequestsTable() {
  try {
    ...
    `);
    console.log('✅ [DB] Tabela friend_requests verificada/criada no Neon');
  } catch (err) {
    console.error('❌ [DB] Erro ao criar tabela friend_requests:', err.message);
  }
}
...
async function dbSaveFriendRequest(fromUser, toUser) {
  try {
    ...
    console.error(`❌ [DB] Erro ao salvar solicitação ${fromUser} → ${toUser}:`, err.message);
  }
}
...
async function dbLoadReceivedRequests(toUser) {
  try {
    ...
    console.log(`📥 [DB] Solicitações recebidas por "${toUser}":`, res.rows.map(r => r.from_user));
    return res.rows.map(r => r.from_user);
  } catch (err) {
...
async function dbLoadSentRequests(fromUser) {
  try {
    ...
    console.log(`📤 [DB] Solicitações enviadas por "${fromUser}":`, res.rows.map(r => r.to_user));
    return res.rows.map(r => r.to_user);
  } catch (err) {
...
io.on('connection', (socket) => {
  ...
  socket.on('user:login', async ({ username, email }) => {
    ...
    console.log(`\n🔐 [LOGIN] ───────────────────────────────────────`);
    console.log(`🔐 [LOGIN] Usuário: "${username}" | socket.id: ${socket.id}`);
    
    socket.username = username;
    socket.email = (email || '').trim().toLowerCase();
    addOnlineUser(username, socket.id); // suporta múltiplas abas e reconexões
    broadcastPresence();
    
    // ─── Carregar solicitações de amizade do Neon (fonte primária) ───
    // Fallback para o arquivo JSON se o DB falhar
    let receivedReqs = await dbLoadReceivedRequests(username);
    let sentReqs;
    if (receivedReqs !== null) {
      // Sincronizar memória com DB (fonte da verdade)
      friendRequests[username] = receivedReqs;
      sentReqs = await dbLoadSentRequests(username);
      if (sentReqs === null) {
        sentReqs = Object.keys(friendRequests).filter(u => (friendRequests[u] || []).includes(username));
      }
    } else {
      // Fallback: usar dados do arquivo JSON em memória
      receivedReqs = friendRequests[username] || [];
      sentReqs = Object.keys(friendRequests).filter(u => (friendRequests[u] || []).includes(username));
    }

    console.log(`📥 [LOGIN] Solicitações RECEBIDAS por "${username}":`, receivedReqs);
    console.log(`📤 [LOGIN] Solicitações ENVIADAS por "${username}":`, sentReqs);
    console.log(`👥 [LOGIN] Amigos de "${username}":`, friends[username] || []);
    console.log(`🔐 [LOGIN] ─────────────────────────────────────────\n`);
    ...
    const isStaff = staffUsers.includes(username) || DEV_EMAILS.includes((email || '').trim().toLowerCase());
    ...
    if (bannedUsers.has(username)) {
      socket.emit('member:banned', { target: username, reason: 'Você foi banido deste servidor.' });
  ...
  console.log(`📨 [FRIEND:REQUEST] DESTINATÁRIO LIMPO: "${cleanTo}"`);

  // após localizar o usuário
  console.log(`📨 [FRIEND:REQUEST] DESTINATÁRIO ENCONTRADO:`, realTo);

  console.log(`📨 [FRIEND:REQUEST] USUÁRIOS ONLINE:`);
  console.log(onlineUsers);

  console.log(`📨 [FRIEND:REQUEST] FRIEND REQUESTS ANTES:`);
  console.log(friendRequests);

  // depois do push/save
  console.log(`📨 [FRIEND:REQUEST] FRIEND REQUESTS DEPOIS:`);
  console.log(friendRequests);
  ...
  console.log(`📨 [FRIEND:REQUEST] ─────────────────────────────────\n`);
    ...
    if (!cleanTo || cleanTo === socket.username) {
      console.warn(`⚠ [FRIEND:REQUEST] Bloqueado: destinatário inválido ("${cleanTo}")`);
      return;
    }
    ...
    console.log(`📨 [FRIEND:REQUEST] Estado atual de friendRequests["${realTo}"]:`, friendRequests[realTo]);
    console.log(`📨 [FRIEND:REQUEST] ─────────────────────────────────────\n`);
    ...
    if (!delivered) {
      console.log(`📨 [FRIEND:REQUEST] "${realTo}" está offline — solicitação salva para o próximo login`);
      socket.emit('friend:request:sent', { to: realTo, offline: true });
    } else {
      console.log(`📨 [FRIEND:REQUEST] "${realTo}" está online — solicitação entregue em tempo real`);
...
server.listen(PORT, () => {
  ...
  // Criar tabela friend_requests no Neon se ainda não existir
  initFriendRequestsTable().catch((err) => {
    console.error('Erro ao inicializar tabela friend_requests:', err.message);
  });
});
