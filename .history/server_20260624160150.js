...
// ============================================================
// Socket.IO deve ser anexado ANTES do Express
// ============================================================
const io = new Server(server, {
  ...
  maxHttpBufferSize: 10e6 // Aumentado para 10MB para suportar dados base64
});

// ============================================================
// Anexa o Express como listener (será chamado DEPOIS do Socket.IO)
// ============================================================
server.on('request', app);
console.log('🔧 Socket.IO é o primeiro listener, Express é o segundo');
...
supabase.storage.listBuckets().then(({ data, error }) => {
  if (error) {
    ...
    console.log('✅ Conectado ao Supabase Storage. Buckets:', data.map(b => b.name));
  }
}).catch(err => {
  console.error('❌ Erro ao testar Supabase:', err.message);
});

// ✅ MULTER - Upload de arquivos para Shorts/Reels (memory storage for Supabase)
...
// ✅ ENDPOINT DE UPLOAD DE SHORT (Supabase Storage)
app.post('/api/upload-short', shortUpload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Nenhum arquivo enviado' });

  try {
    console.log('📤 Iniciando upload para Supabase:', req.file.mimetype, req.file.size);

    // Gerar nome único para o arquivo
    const ext = require('path').extname(req.file.originalname) || (req.file.mimetype.startsWith('video/') ? '.mp4' : '.jpg');
    const fileName = 'short_' + Date.now() + '_' + Math.random().toString(36).slice(2,7) + ext;
    ...
    if (uploadError) {
      console.error('❌ Erro ao upload para Supabase:', uploadError);
    ...
    res.json({ ok: true, fileUrl: publicUrl, fileType: req.file.mimetype });
...
app.get('/api/check-update', async (req, res) => {
  ...
  // Desabilita cache para garantir que pegue sempre a última release
  ...
  try {
    ...
    console.log('[AutoUpdater] Release obtida do GitHub:', data.tag_name, data.name);
    res.json({ ok: true, release: data });
  } catch (e) {
    console.error('[AutoUpdater] Erro ao verificar atualizações:', e.message);
...
const shorts = []; // ✅ Lista global de Shorts / Reels - carregada do Neon PostgreSQL
const suggestedCommunities = []; // ✅ Comunidades sugeridas GLOBAIS - carregadas do Neon PostgreSQL
...
const userAvatars   = {}; // username -> avatar dataURL (in-memory cache, populated on login/avatar-update)
const userServers = {}; // username -> [{ id, name }] — em memória, não persistido

// ─── Normalização de nomes de rooms DM ─────────────────────────────────────────
// Padroniza nomes de usuários para rooms DM: lowercase, sem espaços
function normalizeDmRoom(name) {
  return String(name || '')
    .toLowerCase()
    .replace(/\s+/g, '');
}
...
// DESABILITADO TEMPORARIAMENTE para debug de conexão Socket.IO
/*
setInterval(() => {
  const now = Date.now();
...
function broadcastPresence() {
  ...
  io.emit('friends:presence', { online: list, statuses: userStatuses, avatars: userAvatars });
}
...
io.on('connection', (socket) => {
  ...
  socket.on('user:login', async ({ username, email }) => {
    ...
    socket.join('dm:user:' + normalizeDmRoom(username)); // room imediata
    ...
    try {
      const nickRes = await database.query(
        'SELECT id, nick, avatar FROM accounts WHERE LOWER(nick) = LOWER($1)',
        [username]
      );
      if (nickRes.rows.length > 0) {
        ...
        socket.avatar  = nickRes.rows[0].avatar || null; // avatar do usuário para broadcast
        userAvatars[canonicalUsername] = socket.avatar;  // mantém mapa em memória atualizado
        console.log(`[LOGIN] [USER ID] Resolved userId=${socket.userId} for username="${canonicalUsername}"`);
        if (canonicalUsername !== username) {
          console.log(`🔐 [LOGIN] Nick normalizado: "${username}" → "${canonicalUsername}" | userId: ${socket.userId}`);
        }
      } else {
        console.warn(`⚠ [LOGIN] Nick "${username}" não encontrado na tabela accounts — usando como está (sem userId)`);
        socket.userId = null; // Explicitly set to null if not found
      }
    } catch (normErr) {
      console.warn(`⚠ [LOGIN] Falha ao normalizar nick (${normErr.message}) — usando "${username}" como está (sem userId)`);
      socket.userId = null; // Explicitly set to null on error
    }
    
    socket.username = canonicalUsername;
    const receiverRoom = 'dm:user:' + normalizeDmRoom(canonicalUsername);
    console.log('[LOGIN] [ROOM JOIN]', receiverRoom);
    ...
    console.log(`[LOGIN] [ONLINE USER] Added username="${canonicalUsername}" with socketId=${socket.id}`);
    ...
    if (socket.userId) {
      ...
      console.log(`[LOGIN] [ONLINE USER ID] Registered userId=${socket.userId} → socketId=${socket.id}`);
    } else {
      console.warn(`[LOGIN] [ONLINE USER ID] Skipped - userId is null for username="${canonicalUsername}"`);
    }
    console.log(`[LOGIN] [ONLINE USER ID] onlineUserIds size=${onlineUserIds.size}`);
    console.log('[LOGIN] [ONLINE USER IDS]', [...onlineUserIds.entries()]);
    ...
    socket.join('dm:user:' + normalizeDmRoom(canonicalUsername));
    ...
    console.log(`📤 [LOGIN] Enviados ${shorts.length} Shorts para ${canonicalUsername}`);

    // Enviar Comunidades Sugeridas (movido do connection para user:login)
    // Recarregar do banco de dados para garantir dados atualizados
    await loadSuggestedCommunitiesFromNeon();
    socket.emit('suggested:communities', suggestedCommunities);
    console.log(`📤 [LOGIN] Enviadas ${suggestedCommunities.length} Comunidades Sugeridas para ${canonicalUsername}`);
  ...
  socket.on('friend:request', async ({ to }) => {
    ...
    const delivered = emitToUser(realTo, 'friend:request', { from: socket.username, avatar: socket.avatar || null });
    if (!delivered) {
  ...
  socket.on('friend:accept', async ({ to }) => {
    ...
    emitToUser(to, 'friend:accepted', { by: socket.username, avatar: socket.avatar || null });
  ...
  socket.on('presence:request', async () => {
    ...
    socket.emit('friends:presence', { online: Object.keys(onlineUsers), statuses: mergedStatuses, avatars: userAvatars });
  ...
  const room = 'dm:user:' + normalizeDmRoom(receiver);
  console.log('[DM MESSAGE] Sending to room:', room);
...
socket.on('connection', (socket) => {
  ...
  socket.on('user:login', (data) => {
    ...
    if (username) {
      socket.join('dm:user:' + normalizeDmRoom(username));
      // ...
    }
  });
...
socket.on('dm:get-user-id', async ({ username }) => {
  ...
  socket.on('dm:list', async () => {
    ...
    try {
      ...
      // Enriquecer cada parceiro com avatar vindo do banco (ou do cache em memória)
      const partners = result.rows;
      if (partners.length > 0) {
        const partnerNames = partners.map(r => r.partner.toLowerCase());
        const avatarRes = await database.query(
          `SELECT nick, avatar FROM accounts WHERE LOWER(nick) = ANY($1::text[])`,
          [partnerNames]
        ).catch(() => ({ rows: [] }));
        const avatarMap = {};
        avatarRes.rows.forEach(r => { avatarMap[r.nick.toLowerCase()] = r.avatar || null; });
        partners.forEach(r => {
          r.avatar = avatarMap[r.partner.toLowerCase()] || userAvatars[r.partner] || null;
        });
      }
  ...
  socket.on('user:avatar:save', async ({ avatar }) => {
    ...
    try {
      ...
      // Atualizar cache em memória do próprio socket e do mapa global
      socket.avatar = avatar;
      userAvatars[socket.username] = avatar;
  ...
  // ── Chamadas de voz/DM: delivery por UUID (primário) + username (fallback) + room (último recurso) ──
  ...
  function _voiceEmit(toId, toUsername, event, payload) {
    console.log(`[VOICE] _voiceEmit called: toId=${toId}, toUsername=${toUsername}, event=${event}`);
    
    // 1. Tentar UUID primeiro (mais preciso) - Entrega para TODOS os sockets do usuário
    if (toId) {
      const targetSockets = [];
      io.sockets.sockets.forEach(s => {
        if (String(s.userId) === String(toId)) targetSockets.push(s.id);
      });

      if (targetSockets.length > 0) {
        targetSockets.forEach(sid => io.to(sid).emit(event, payload));
        console.log(`[VOICE] ✅ ${event} entregue por UUID ${toId} → ${targetSockets.length} sockets`);
        return;
      }
      console.log(`[VOICE] ⚠ UUID ${toId} não encontrado em nenhum socket ativo`);
    }
    
    // 2. Tentar username (fallback)
    if (toUsername) {
      const _fb = emitToUser(toUsername, event, payload);
      if (_fb) {
        console.log(`[VOICE] ✅ ${event} entregue por username "${toUsername}"`);
        return;
      }
      console.log(`[VOICE] ⚠ Username "${toUsername}" não encontrado em onlineUsers`);
    }
    
    // 3. Tentar room como último recurso
    if (toUsername) {
      const room = 'dm:user:' + normalizeDmRoom(toUsername);
      const roomMembers = io.sockets.adapter.rooms.get(room);
      if (roomMembers && roomMembers.size > 0) {
        io.to(room).emit(event, payload);
        console.log(`[VOICE] ✅ ${event} entregue por room "${room}" (${roomMembers.size} membros)`);
        return;
      }
      console.log(`[VOICE] ⚠ Room "${room}" não encontrada ou vazia`);
    }
    
    console.log(`[VOICE] ❌ ${event} não entregue - destinatário offline: toId=${toId}, toUsername=${toUsername}`);
  }
  
console.log('[DM CALL]');
console.log('to:', data.to);
console.log('socket alvo:', users[data.to]);
  // Iniciar chamada
socket.on('dm:call:start', (data) => {
  console.log('[SERVER] dm:call:start recebido', data);
console.log('\n==============================');
console.log('[DM CALL RECEBIDA]');
console.log('socket.username:', socket.username);
console.log('data:', data);
console.log('==============================\n');

  const targetId = data.toId || data.to;

  if (!socket.username || !targetId) {
    console.warn('[DM:CALL:START] Dados inválidos:', data);
    return;
  }

  io.to(targetId).emit('dm:call:incoming', {
    fromId: data.fromId,
    from: data.from || socket.username
  });

  console.log('[SERVER] dm:call:incoming enviado para', targetId);
    const from = socket.username;
    const to   = data.to;
    console.log(`[DM:CALL:START] ${from} → ${to} (toId=${data.toId})`);
    console.log(`[DM:CALL:START] Caller socket.userId=${socket.userId}`);
    console.log(`[DM:CALL:START] onlineUserIds entries:`, [...onlineUserIds.entries()]);
    console.log(`[DM:CALL:START] onlineUsers keys:`, Object.keys(onlineUsers));
    
    // Verificar se o destinatário está online
    const targetSocketId = data.toId ? onlineUserIds.get(String(data.toId)) : null;
    console.log(`[DM:CALL:START] targetSocketId by toId:`, targetSocketId);
    
    const targetByUsername = onlineUsers[to] || onlineUsers[to.toLowerCase()];
    console.log(`[DM:CALL:START] targetByUsername:`, targetByUsername);
    
    const room = 'dm:user:' + normalizeDmRoom(to);
    const roomMembers = io.sockets.adapter.rooms.get(room);
    console.log(`[DM:CALL:START] room:`, room, 'members:', roomMembers ? [...roomMembers] : 'none');
  ...
  // ==============================================
  // NOTA: suggested:communities é enviado no user:login para garantir que o cliente esteja pronto

  // Quando alguém adicionar uma comunidade às sugeridas
  socket.on('community:add-suggested', async (community) => {
    console.log('🔍 [ADD-SUGGESTED] Evento recebido (antes do try-catch)');
    try {
      console.log('🔍 [ADD-SUGGESTED] Recebido pedido para adicionar comunidade sugerida:', community.id, community.name);
      console.log('🔍 [ADD-SUGGESTED] Socket username:', socket.username);
      console.log('🔍 [ADD-SUGGESTED] Socket ID:', socket.id);
      console.log('🔍 [ADD-SUGGESTED] Payload completo:', JSON.stringify(community));

      // Qualquer usuário logado pode sugerir suas próprias comunidades
      if (!socket.username) {
        console.warn('⚠️ [ADD-SUGGESTED] Usuário não logado tentando adicionar comunidade sugerida');
    ...
    if (exists) {
      console.log('⚠️ [ADD-SUGGESTED] Comunidade já existe na lista:', community.id);
      // Se já existe, atualiza os dados (ícone, banner, etc.)
      ...
      if (index !== -1) {
        ...
        suggestedCommunities[index].addedBy = socket.username;
        suggestedCommunities[index].addedAt = Date.now();
      }
      
      // Atualizar no Neon
      try {
        console.log('🔍 [NEON] Atualizando comunidade sugerida no banco:', community.id, community.name);
        await database.query(
          `INSERT INTO communities (id, name, description, icon, banner, owner_id, is_suggested, members_count, created_at)
          ...
          [community.id, community.name, community.description, community.icon, community.banner,
           community.ownerId || 0, true, community.members || 0, new Date().toISOString()]
        );
        console.log('✅ [NEON] Comunidade sugerida atualizada no Neon:', community.id);
      } catch (err) {
        console.error('❌ [NEON] Erro ao atualizar comunidade sugerida no Neon:', err.message);
        console.error('❌ [NEON] Stack trace:', err.stack);
      }

      // Enviar atualização para TODOS os usuários conectados
      io.emit('suggested:updated', community);
      socket.emit('suggested:updated', community);
      return;
    }

    // Se não existe, adiciona na lista global
    community.addedBy = socket.username;
    community.addedAt = Date.now();
    // Garante que todos os campos tenham valores padrão
    community.icon = community.icon || community.iconData || 'Community-icon.png';
    community.banner = community.banner || community.bannerData || 'Community-banner.png';
    community.description = community.description || community.desc || 'Sem descrição';
    community.members = community.members || 0;
    suggestedCommunities.push(community);
    console.log('✅ [ADD-SUGGESTED] Comunidade adicionada à lista em memória:', community.id, community.name);

    // Salvar no Neon
    try {
      console.log('🔍 [NEON] Salvando comunidade sugerida no banco:', community.id, community.name);
      console.log('🔍 [NEON] Payload completo:', JSON.stringify(community));
      // Usar INSERT com ON CONFLICT para atualizar se já existir
      await database.query(
        `INSERT INTO communities (id, name, description, icon, banner, owner_id, is_suggested, members_count, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         ON CONFLICT (id) DO UPDATE SET
        ...
        [community.id, community.name, community.description || '', community.icon || '', community.banner || '',
         community.ownerId || socket.username || 'unknown', true, community.members || 0, new Date().toISOString()]
      );
      console.log('✅ [NEON] Comunidade sugerida salva no Neon:', community.id);
      
      // Verificar se foi salva corretamente
      const verifyResult = await database.query(
        `SELECT id, name, is_suggested FROM communities WHERE id = $1 AND is_suggested = TRUE`,
        [community.id]
      );
      if (verifyResult.rows.length > 0) {
        console.log('✅ [NEON] Verificação confirmada: comunidade está marcada como sugerida');
      } else {
        console.error('❌ [NEON] Verificação falhou: comunidade não está marcada como sugerida');
      }
    } catch (err) {
      console.error('❌ [NEON] Erro ao salvar comunidade sugerida no Neon:', err.message);
      console.error('❌ [NEON] Stack trace:', err.stack);
    }

    // Enviar para TODOS os usuários conectados
    console.log('📤 [SUGGESTED] Enviando nova comunidade sugerida para todos os clientes:', community.name);
    console.log('📤 [SUGGESTED] Total de clientes conectados:', io.sockets.sockets.size);
    io.emit('suggested:new', community);
    console.log('✅ [SUGGESTED] Evento enviado para todos os clientes');
    } catch (err) {
      console.error('❌ [ADD-SUGGESTED] Erro ao processar comunidade sugerida:', err.message);
      console.error('❌ [ADD-SUGGESTED] Stack trace:', err.stack);
      socket.emit('suggested:error', { message: 'Erro ao processar comunidade sugerida.' });
    }
  });

  // Buscar comunidade por ID (para adição manual nas sugeridas)
  socket.on('community:get-by-id', async ({ id }) => {
    console.log('🔍 [GET-BY-ID] Buscando comunidade pelo ID:', id);
    try {
      const result = await database.query(
        `SELECT id, name, description, icon, banner, owner_id, members_count, created_at
         FROM communities WHERE id = $1`,
        [id]
      );
      if (result.rows.length > 0) {
        const community = result.rows[0];
        socket.emit('community:by-id-response', {
          id: community.id,
          name: community.name,
        ...
        console.log('✅ [GET-BY-ID] Comunidade encontrada:', community.name);
      } else {
        socket.emit('community:by-id-response', null);
        console.log('⚠️ [GET-BY-ID] Comunidade não encontrada:', id);
      }
    } catch (err) {
      console.error('❌ [GET-BY-ID] Erro ao buscar comunidade:', err.message);
      socket.emit('community:by-id-response', null);
    }
  });
  ...
  socket.on('community:unsuggest', async ({ communityId }) => {
    console.log('🔍 [UNSUGGEST] Recebido pedido para remover comunidade:', communityId);
    console.log('🔍 [UNSUGGEST] Lista atual de comunidades sugeridas:', suggestedCommunities.map(c => c.id));
    
    const index = suggestedCommunities.findIndex(c => c.id === communityId);
    console.log('🔍 [UNSUGGEST] Índice encontrado:', index);
    
    if (index === -1) {
      console.log('⚠️ [UNSUGGEST] Comunidade não encontrada na lista');
      return;
    }

    suggestedCommunities.splice(index, 1);
    console.log('✅ [UNSUGGEST] Comunidade removida da lista em memória');

    // Atualizar no Neon (mudar is_suggested para FALSE)
    try {
      const result = await database.query('UPDATE communities SET is_suggested = FALSE WHERE id = $1 RETURNING id, name, is_suggested', [communityId]);
      console.log('✅ [UNSUGGEST] Comunidade removida das sugeridas no Neon:', communityId);
      console.log('🔍 [UNSUGGEST] Resultado da query:', result.rows);
    } catch (err) {
      console.error('❌ [UNSUGGEST] Erro ao remover comunidade sugerida do Neon:', err.message);
    }

    // Avisar todos para remover
    io.emit('suggested:removed', { communityId });
    console.log('✅ [UNSUGGEST] Evento suggested:removed emitido para todos');
  });
  ...
  socket.on('community:approve', async ({ communityId }) => {
    ...
    try {
      await database.query(
        `INSERT INTO suggested_communities (id, name, description, icon, banner, owner_id, added_by, added_at, is_suggested)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         ON CONFLICT (id) DO UPDATE SET name = $2, description = $3, icon = $4, banner = $5`,
        [community.id, community.name, community.description, community.icon, community.banner, 
         community.ownerId, community.submittedBy, community.approvedAt, true]
      );
      console.log('✅ Comunidade aprovada salva no Neon:', community.id);
    } catch (err) {
      console.error('❌ Erro ao salvar comunidade aprovada no Neon:', err.message);
    }
  ...
  socket.on('disconnect', () => {
    console.log(`[DISCONNECT] ${socket.username || 'unknown'} (${socket.id}) disconnected`);
    if (socket.voiceRoom) {
      const key = socket.voiceRoom;
      if (voiceRooms[key]) {
        voiceRooms[key] = voiceRooms[key].filter(u => u.socketId !== socket.id);
    ...
    if (socket.username) {
      ...
      console.log(`[DISCONNECT] Removed socket ${socket.id} for username=${socket.username}`);
      // Remove userId do mapa ao desconectar APENAS se não houver outros sockets ativos
      if (socket.userId) {
        let stillOnline = false;
        io.sockets.sockets.forEach(s => {
          if (s.id !== socket.id && String(s.userId) === String(socket.userId)) stillOnline = true;
        });
        if (!stillOnline) {
          onlineUserIds.delete(String(socket.userId));
          console.log(`[DISCONNECT] userId=${socket.userId} removido de onlineUserIds (último socket encerrado)`);
        } else {
          console.log(`[DISCONNECT] userId=${socket.userId} permanece em onlineUserIds (outros sockets ativos)`);
        }
        console.log('[ONLINE USERS]', [...onlineUserIds.entries()]);
      }
...
// NOTA: /socket.io/ é processado pelo Socket.IO antes deste middleware
...
(async () => {
  try {
    ...
    await initSuggestedCommunitiesTable();
    console.log('✅ [INIT] suggested_communities OK');
    await migrations.runMigration();
    console.log('✅ [INIT] Migrations concluídas.');
    
    // Alter table reels para suportar base64
    try { await database.query('ALTER TABLE reels ALTER COLUMN file_url TYPE TEXT'); } catch(_e) {}
    
    // Carregar dados do Neon para memória
    await loadShortsFromNeon();
    await loadSuggestedCommunitiesFromNeon();
    console.log('✅ [INIT] Dados carregados do Neon para memória.');
  } catch (err) {
...
async function initSuggestedCommunitiesTable() {
  try {
    await database.query(`
    ...
    `);
    console.log('✅ [DB] Tabela suggested_communities pronta');
  } catch (err) {
    console.error('❌ [DB] Erro ao criar tabela suggested_communities:', err.message);
  }
}

async function loadShortsFromNeon() {
  try {
    const result = await database.query(
      `SELECT id, title, description, tags, file_type, file_url, username, time, timestamp
       FROM reels ORDER BY created_at DESC LIMIT 100`
    );
    if (result.rows.length > 0) {
      shorts.length = 0; // Limpar array atual
      result.rows.forEach(row => {
        // Verificar se URL é local (antiga) e marcar para aviso
        const isLocalPath = row.file_url && (row.file_url.startsWith('/uploads/') || row.file_url.startsWith('file://'));
        if (isLocalPath) {
          console.log('⚠️ Short com URL local antiga (não funcionará para outros):', row.id, row.file_url);
        }
      ...
      console.log(`✅ [NEON] ${shorts.length} Shorts carregados do banco de dados`);
    }
  } catch (err) {
    console.error('❌ [NEON] Erro ao carregar Shorts:', err.message);
  }
}

async function loadSuggestedCommunitiesFromNeon() {
  try {
    console.log('🔍 [NEON] Carregando comunidades sugeridas do banco de dados...');
    // Carregar da tabela communities onde is_suggested = TRUE
    const result = await database.query(
      `SELECT id, name, description, icon, banner, owner_id, is_suggested, members_count
       FROM communities WHERE is_suggested = TRUE ORDER BY created_at DESC`
    );
    console.log(`🔍 [NEON] Query executada, ${result.rows.length} linhas retornadas`);
    if (result.rows.length > 0) {
      suggestedCommunities.length = 0; // Limpar array atual
      result.rows.forEach(row => {
        suggestedCommunities.push({
          id: row.id.toString(), // Converter ID para string
          name: row.name,
          description: row.description,
          icon: row.icon,
          banner: row.banner,
      ...
      console.log(`✅ [NEON] ${suggestedCommunities.length} Comunidades Sugeridas carregadas da tabela communities`);
      console.log('🔍 [NEON] IDs das comunidades carregadas:', suggestedCommunities.map(c => c.id));
    } else {
      console.log(`ℹ️ [NEON] Nenhuma comunidade com is_suggested = TRUE encontrada na tabela communities`);
    }
  } catch (err) {
    console.error('❌ [NEON] Erro ao carregar Comunidades Sugeridas:', err.message);
    console.error('❌ [NEON] Stack trace:', err.stack);
  }
}
...
app.post('/api/suggested', (req, res) => {
  const { serverId } = req.body

  suggested.push(serverId)
