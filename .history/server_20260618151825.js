...
console.log('[ROOM MEMBERS]', [...(io.sockets.adapter.rooms.get(receiverRoom) || [])]);
  ...
  // Solicita o avatar do novo amigo
  if (socket.connected) {
    socket.emit('dm:user:profile', { username: by });
  }
...
function getFriendAvatar(username) {
  ...
  if (window.friendAvatarCache && window.friendAvatarCache[username.toLowerCase()]) {
    return window.friendAvatarCache[username.toLowerCase()];
  }

  // Fallback: tenta carregar do localStorage
  try {
    const cache = JSON.parse(localStorage.getItem('zx_friend_avatars') || '{}');
    if (cache[username.toLowerCase()]) {
      // Atualiza o cache em memória
      if (!window.friendAvatarCache) window.friendAvatarCache = {};
      window.friendAvatarCache[username.toLowerCase()] = cache[username.toLowerCase()];
...
socket.on('friends:data', (data) => {
  // ... código existente que popula o cache ...
