...
const DEV_USERS = ['MDK']; // Apenas devs podem gerenciar staffs
...
function loadStaffList() {
  ...
  if (Array.isArray(saved) && saved.length > 0) {
    ...
    staffUsers.length = 0;
    const merged = new Set([...DEV_USERS, ...saved]);
...
io.on('connection', (socket) => {
  ...
  if (DEV_USERS.includes(socket.username)) {
    socket.emit('staff:list', staffUsers.filter(u => !DEV_USERS.includes(u)));
  }
    ...
    try {
      ...
      if (res.rows.length === 0) {
        ...
        socket.emit('staff:search-result', {
          ...
          isDev: DEV_USERS.includes(user.nick)
        });
      }
    } catch(e) {
      socket.emit('staff:search-result', null);
  ...
  socket.on('staff:get-list', () => {
    if (!DEV_USERS.includes(socket.username)) return;
    socket.emit('staff:list', staffUsers.filter(u => !DEV_USERS.includes(u)));
  });
