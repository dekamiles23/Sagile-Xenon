...
io.on('connection', (socket) => {
  ...
  socket.on('friend:request', ({ to }) => {
    // ✅ Remover @ do início do nick se o usuário digitou @nick
    const cleanTo = String(to || '').trim().replace(/^@/, '');
    if (!cleanTo || cleanTo === socket.username) return;

    // Salvar solicitação no servidor
    if (!friendRequests[cleanTo]) friendRequests[cleanTo] = [];
    if (!friendRequests[cleanTo].includes(socket.username)) {
      friendRequests[cleanTo].push(socket.username);
      saveData();
    }
    
    const targetId = onlineUsers[cleanTo];
    if (targetId) {
      io.to(targetId).emit('friend:request', { from: socket.username });
    } else {
      // Usuário offline — solicitação salva, será entregue no próximo login
      socket.emit('friend:request:sent', { to: cleanTo, offline: true });
    }
  });
  ...
  socket.on('community:add-suggested', (community) => {
    // ✅ Verificar se o usuário tem permissão (dev ou staff)
    const isAuthorized = DEV_EMAILS.includes(socket.email || '') || staffUsers.includes(socket.username);
    if (!isAuthorized) {
      socket.emit('suggested:error', { message: 'Sem permissão para adicionar comunidades sugeridas.' });
      return;
    }
  ...
  socket.on('member:ban', ({ target, reason }) => {
    if (!socket.username) return;
    ...
    // Desconectar o alvo se estiver online
    const targetSocketId = onlineUsers[target];
    if (targetSocketId) {
      io.to(targetSocketId).emit('member:banned', { target, reason: reason || 'Sem motivo informado' });
      const targetSocket = io.sockets.sockets.get(targetSocketId);
      if (targetSocket) setTimeout(() => targetSocket.disconnect(true), 1500);
    }
  ...
  socket.on('member:kick', ({ target }) => {
    ...
    const targetSocketId = onlineUsers[target];
    if (targetSocketId) {
      io.to(targetSocketId).emit('member:kicked', { target });
      const targetSocket = io.sockets.sockets.get(targetSocketId);
      if (targetSocket) setTimeout(() => targetSocket.disconnect(true), 1500);
    }
  ...
  socket.on('member:punish', ({ target, minutes }) => {
    if (!socket.username) return;
    const isAuth = staffUsers.includes(socket.username) || DEV_EMAILS.includes(socket.email || '');
    ...
    const targetSocketId = onlineUsers[target];
    if (targetSocketId) {
      io.to(targetSocketId).emit('member:punished', { target, minutes: mins, by: socket.username });
    }
  ...
  socket.on('member:set-nickname', ({ target, nickname }) => {
    ...
    const targetSocketId = onlineUsers[target];
    if (targetSocketId) {
      io.to(targetSocketId).emit('member:nickname-changed', { nickname });
    }
  });
