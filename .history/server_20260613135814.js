...
app.post('/api/auth/register', (req, res) => {
  try {
    const result = accounts.registerAccount(req.body || {});
...
app.post('/api/auth/login', (req, res) => {
  try {
    const result = accounts.loginAccount(req.body || {});
    res.json(result);
  } catch (err) {
...
app.patch('/api/account', accounts.authMiddleware, (req, res) => {
  try {
    const oldNick = req.authAccount.nick;
    const result = accounts.updateAccount(req.authToken, req.body || {});
...
app.post('/api/account/logout', accounts.authMiddleware, (req, res) => {
  accounts.logoutAccount(req.authToken);
  res.json({ ok: true });
});

app.delete('/api/account', accounts.authMiddleware, (req, res) => {
  try {
    const deleted = accounts.deleteAccount(req.authToken, req.body?.password);
...
const staffUsers = ['Developer', 'Admin', 'Staff', 'demid']; // ✅ Lista de staffs
...
io.on('connection', (socket) => {
  ...
  socket.on('community:submit', (community) => {
    ...
    // ✅ ENVIAR NOTIFICAÇÃO PARA TODOS OS STAFFS ONLINE
    staffUsers.forEach(staffUsername => {
      ...
      if (staffSocketId) {
        ...
        io.to(staffSocketId).emit('notification:new', {
          id: `notif_${Date.now()}`,
