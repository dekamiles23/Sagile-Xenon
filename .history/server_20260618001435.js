...
socket.on('get-avatar', ({ username }) => {
  // Busca o avatar no banco (ex: coluna avatar)
  db.get('SELECT avatar FROM accounts WHERE LOWER(nick) = LOWER(?)', [username], (err, row) => {
    if (row && row.avatar) {
      socket.emit('avatar:data', { username, avatar: row.avatar });
    } else {
      // Se não tiver, não envia nada (ou envia null)
    }
  });
});