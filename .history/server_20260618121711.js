...
// Recebe avatar via Socket.IO
socket.on('upload-avatar', ({ username, avatar }) => {
  if (!username || !avatar) return;
  // Atualiza a coluna avatar na tabela users
  db.run('UPDATE users SET avatar = ? WHERE LOWER(username) = LOWER(?)', [avatar, username], (err) => {
    ...
    // Opcional: emite para todos os usuários que têm esse amigo
    // io.emit('avatar:data', { username, avatar });
    console.log(`Avatar salvo para ${username}`);
  });
});

// Quando um cliente pedir o avatar de alguém
socket.on('get-avatar', ({ username }) => {
  db.get('SELECT avatar FROM users WHERE LOWER(username) = LOWER(?)', [username], (err, row) => {
    if (row && row.avatar) {
