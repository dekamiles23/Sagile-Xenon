...
socket.on('dm:get-user-id', async ({ username }) => {
    ...
    try {
      await database.query(
        ...
        [community.id, community.name, community.description, community.icon, community.banner, 
         community.ownerId, community.addedBy, community.addedAt, true]
      );
      console.log('✅ Comunidade sugerida salva no Neon:', community.id);
    } catch (err) {
      console.error('❌ Erro ao salvar comunidade sugerida no Neon:', err.message);
    }
  ...
  socket.on('community:unsuggest', async ({ communityId }) => {
    ...
    // Remover do Neon
    try {
      await database.query('DELETE FROM suggested_communities WHERE id = $1', [communityId]);
      console.log('✅ Comunidade sugerida removida do Neon:', communityId);
    } catch (err) {
      console.error('❌ Erro ao remover comunidade sugerida do Neon:', err.message);
    }
...
if (server.listeners('request').length > 0) {
  // Remove todos os listeners 'request' (incluindo o Express)
  server.removeAllListeners('request');
  // Adiciona o Express de volta, mas agora será chamado DEPOIS do Socket.IO
  server.on('request', app);
  console.log('🔧 Patch: Socket.IO será o primeiro listener');
}

// ... depois de tudo configurado

// Reordena os listeners para dar prioridade ao Socket.IO
const listeners = server.listeners('request');
if (listeners.length > 0 && listeners[0] === app) {
  server.removeListener('request', app);
  server.on('request', app);
  console.log('🔧 Socket.IO agora é o primeiro listener');
}
...
async function loadSuggestedCommunitiesFromNeon() {
  try {
    const result = await database.query(
      `SELECT id, name, description, icon, banner, owner_id, added_by, added_at, is_suggested
       FROM suggested_communities ORDER BY added_at DESC`
    );
    if (result.rows.length > 0) {
      ...
      console.log(`✅ [NEON] ${suggestedCommunities.length} Comunidades Sugeridas carregadas do banco de dados`);
    }
  } catch (err) {
...
// ============================================================
// server.js – CORRIGIDO (Socket.IO antes do Express)
// ============================================================
...
// 1️⃣ Cria o servidor HTTP SEM anexar o Express ainda
const server = http.createServer();

// 2️⃣ Anexa o Socket.IO PRIMEIRO (ele será o primeiro listener)
const io = new Server(server, {
  cors: {
    origin: '*',
    methods: ['GET', 'POST'],
...
// 3️⃣ Agora anexa o Express como listener (será chamado DEPOIS do Socket.IO)
server.on('request', app);
...
// ── OUTRAS ROTAS (version, update, etc.) ──
app.get('/version', (req, res) => {
  // ...
});

// ── FALLBACK CATCH-ALL: IGNORA /socket.io/ ──
app.use((req, res, next) => {
  if (req.path.startsWith('/socket.io/')) {
    // Se chegar aqui, o Socket.IO não processou → erro de configuração
    return res.status(404).send('Socket.IO endpoint not found');
  }
  // Fallback para SPA: envia auth.html ou index.html
  res.sendFile(path.join(__dirname, 'public', 'auth.html'));
});

// ── INICIALIZAÇÃO DO BANCO E LISTENERS DO SOCKET.IO ──
// (coloque todo o código do io.on('connection', ...) aqui)

io.on('connection', (socket) => {
  console.log(`Usuário conectado: ${socket.id}`);
  // ... todo o seu código de eventos do socket (user:login, message, dm:message, etc.)
});
...
// ── TRATAMENTO DE ERROS ──
process.on('unhandledRejection', (reason, promise) => {
  console.error('[SERVIDOR] Promise rejeitada não tratada:', reason);
});
process.on('uncaughtException', (err) => {
  console.error('[SERVIDOR] Exceção não capturada:', err.message);
});