...
socket.on('user:login', (user) => {
    ...
    onlineUserIds.set(String(user.id), socket.id);

    console.log('[LOGIN]', user.username, user.id, socket.id);
});
    ...
    addOnlineUserId(socket.userId, socket.id); // [BUG2 FIX] registra por ID para DM routing
    ...
    try {
      ...
      if (rawList.length > 0) {
        const idsRes = await database.query(
          `SELECT nick, id::text AS id FROM accounts WHERE LOWER(nick) = ANY($1::text[])`,
          [rawList.map(f => (typeof f === 'string' ? f : f.username || '').toLowerCase())]
        );
        const idMap = {};
        idsRes.rows.forEach(r => { idMap[r.nick.toLowerCase()] = r.id; });
        friendsPayload = rawList.map(f => {
          const uname = typeof f === 'string' ? f : (f.username || '');
          return { username: uname, id: idMap[uname.toLowerCase()] || null };
        });
      }
    ...
    async function _emitFriendsData(targetSocket, targetUsername) {
      ...
      try {
        if (fList.length > 0) {
          ...
          const m = {}; r.rows.forEach(row => { m[row.nick.toLowerCase()] = row.id; });
          fList = fList.map(f => { const u = typeof f === 'string' ? f : (f.username || ''); return { username: u, id: m[u.toLowerCase()] || null }; });
        }
      } catch (_e) {}
      const payload = {
  ...
  socket.on('dm:message', async (msg) => {
    ...
    socket.on('dm:message', async (msg) => {
  console.log('DM RECEBIDA NO SERVIDOR:', msg);

  if (!socket.username) {
    console.warn('[DM] Mensagem ignorada: socket.username não definido');
    return;
  }
  ...
  console.log('receiverId:', msg.receiverId);
    ...
    // ── Logs de diagnóstico (exato como solicitado) ──────────────────────────
    console.log('[ONLINE USERS]', [...onlineUserIds.entries()].map(([id, sid]) => ({ id, sid })));
    console.log('[RECEIVER ID]', receiverId);
    const _receiverSocketId = receiverId ? onlineUserIds.get(receiverId) : undefined;
    console.log('[RECEIVER SOCKET]', _receiverSocketId);

    if (!_receiverSocketId) {
      console.warn('[DM-RT] receiverSocketId undefined — destinatário não registrado em onlineUserIds (offline ou não logou ainda)');
    }

    // ── Entrega 1 (PRIMÁRIA): por UUID do Neon — mais confiável ──────────────
    let _delivered = false;
    if (receiverId && _receiverSocketId) {
      // Não enviar para o próprio remetente (self-DM)
      if (_receiverSocketId !== socket.id) {
        io.to(_receiverSocketId).emit('dm:message', fullMsg);
        console.log(`[DM-RT] ✅ Entregue por UUID receiverId=${receiverId} → socket=${_receiverSocketId}`);
        _delivered = true;
      }
    }

    // ── Entrega 2 (PARALELA): pela room Socket.IO — garante multi-aba ─────────
    const receiverRoom = 'dm:user:' + canonicalTo.toLowerCase();
    socket.to(receiverRoom).emit('dm:message', fullMsg);
    console.log(`[DM-RT] Room "${receiverRoom}" notificada (socket.to exclui sender)`);

    // ── Entrega 3 (FALLBACK): por username caso UUID não resolveu ─────────────
    if (!_delivered) {
      const fbDelivered = emitToUser(canonicalTo, 'dm:message', fullMsg);
      if (fbDelivered) {
        console.warn(`[DM-RT] ⚠ Fallback username "${canonicalTo}" usado (receiverId indisponível)`);
      } else {
        console.warn(`[DM-RT] ✖ "${canonicalTo}" parece OFFLINE — mensagem persistida no Neon para carregamento futuro`);
      }
    }
  ...
  socket.on('dm:user:profile', async ({ username: targetUser }) => {
    ...
    try {
      const result = await database.query(
        'SELECT nick, visual_profile FROM accounts WHERE LOWER(nick) = LOWER($1)',
        [targetUser]
      );
      ...
      socket.emit('dm:user:profile', { username: targetUser, visualProfile: {} });
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (socket.username) {
      ...
      // [BUG2 FIX] Remove ID do mapa de IDs ao desconectar
      if (socket.userId) removeOnlineUserId(socket.userId);
