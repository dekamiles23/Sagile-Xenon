...
socket.on('dm:get-user-id', ({ username }) => {
  // Busca no banco pelo nick (case-insensitive)
  db.get('SELECT id FROM accounts WHERE LOWER(nick) = LOWER(?)', [username], (err, row) => {
    if (row) {
      socket.emit('dm:user-id', { username, userId: row.id });
    } else {
...
// ============================================
// FIX: sendDmMessage com fallback para receiverId
// ============================================

// Garantir que essas variáveis existam no escopo global
if (typeof friendIds === 'undefined') window.friendIds = new Map();
if (typeof dmQueue === 'undefined') window.dmQueue = [];

window.sendDmMessage = function(text, toUsername) {
  // ... (coloque todo o código da nova função aqui)
};

socket.on('get-avatar', ({ username }) => {
  db.get('SELECT avatar FROM users WHERE LOWER(nick) = LOWER(?)', [username], (err, row) => {
    if (row && row.avatar) {
      socket.emit('avatar:data', { username, avatar: row.avatar });
    }
  });
});

socket.on('user:login', ({ username }) => {
  db.get('SELECT id FROM users WHERE LOWER(nick) = LOWER(?)', [username], (err, user) => {
    if (!user) return;
    db.all(`
    ...
    `, [user.id], (err, friends) => {
      socket.emit('friends:data', {
        userId: user.id,
        friends: friends.map(f => ({ username: f.nick, avatar: f.avatar }))
      });
    });
  });
});
...
app.post('/api/upload-avatar', upload.single('avatar'), (req, res) => {
  ...
  db.run('UPDATE users SET avatar = ? WHERE id = ?', [base64, userId], (err) => {
...
app.post('/api/upload-avatar', upload.single('avatar'), (req, res) => {
  // Identifica o usuário (ex: via sessão, JWT, ou passando o username)
  const username = req.body.username; // ou req.user.username (se autenticado)
  if (!username) {
  ...
  // Atualiza o banco
  db.run('UPDATE users SET avatar = ? WHERE LOWER(username) = LOWER(?)', [base64, username], (err) => {
    if (err) {
      console.error('Erro ao salvar avatar:', err);
      return res.status(500).json({ error: 'Erro ao salvar avatar' });
    }
    // Emite para o cliente que fez o upload (opcional)
    req.io.emit('avatar:data', { username, avatar: base64 });
    res.json({ success: true, avatar: base64 });
  });
});
...
socket.on('user:login', ({ username }) => {
  ...
  `, [username], (err, friends) => {
    socket.emit('friends:data', {
      userId: user.id,
      friends: friends.map(f => ({ username: f.username, avatar: f.avatar }))
    });
  });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('upload-avatar', ({ username, avatar }) => {
    db.run('UPDATE users SET avatar = ? WHERE LOWER(username) = LOWER(?)', [avatar, username], (err) => {
      if (err) return console.error(err);
      socket.emit('avatar:data', { username, avatar });
    });
  });
});
