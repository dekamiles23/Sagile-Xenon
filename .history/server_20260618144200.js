...
// ========== 1. PRIMEIRO: ROTAS DA API ==========
app.get('/api/avatar/:username', async (req, res) => {
  const { username } = req.params;
  try {
    const result = await pool.query('SELECT avatar FROM usuarios WHERE username = $1', [username]);
...
// ========== 2. DEPOIS: ARQUIVOS ESTÁTICOS ==========
app.use(express.static(__dirname));

// ========== 3. POR ÚLTIMO: FALLBACK PARA SPA ==========
app.get('*', (req, res) => {
