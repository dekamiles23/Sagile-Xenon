...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => cb(null, `sticker_${Date.now()}_${Math.random().toString(36).substring(2,8)}${path.extname(file.originalname)}`)
});
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowed = ['image/png', 'image/jpeg', 'image/gif', 'image/apng'];
