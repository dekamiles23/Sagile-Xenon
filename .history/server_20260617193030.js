...
console.log('onlineUserIds:', [...onlineUserIds.entries()]);
console.log('================');
  ...
  socket.on('dm:message', async (msg) => {
    ...
    if (!receiverSocketId) {
      console.warn('[DM-RT] receiverSocketId undefined — destinatário offline ou userId não resolvido em onlineUserIds');
    }

    // ── Entrega em tempo real: direto ao socket do destinatário ──────────────
    if (receiverSocketId && receiverSocketId !== socket.id) {
      io.to(receiverSocketId).emit('dm:message', fullMsg);
      console.log(`[DM-RT] ✅ Entregue: receiverId=${receiverId} → socket=${receiverSocketId}`);
    }
