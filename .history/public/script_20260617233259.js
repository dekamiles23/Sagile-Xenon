...
// ============================================================
// AVATAR NAS MENSAGENS DO CHAT DM
// ============================================================
(function dmMessagesAvatar() {
  console.log('[DM-AVATAR] Iniciando...');

  // Mapa para armazenar avatares de usuários (além do próprio)
  // Chave: username.toLowerCase(), valor: URL da imagem (data:image ou URL)
  window.userAvatars = window.userAvatars || {};
  ...
  function getUserAvatar(username) {
    ...
    // 1. Tenta obter do mapa global
    if (window.userAvatars && window.userAvatars[key]) {
      return window.userAvatars[key];
    }

    // 2. Se for o próprio usuário, usa zx_avatar
    const currentUser = (window.username || '').toLowerCase();
    if (key === currentUser) {
      const saved = localStorage.getItem('zx_avatar');
      if (saved && saved.startsWith('data:image')) {
        window.userAvatars[key] = saved;
        return saved;
      }
    }

    // 3. Tenta buscar de localStorage (outros usuários)
    const stored = localStorage.getItem(`avatar_${key}`);
    if (stored && stored.startsWith('data:image')) {
      window.userAvatars[key] = stored;
      return stored;
    }

    // 4. Fallback: iniciais (retorna null para que o código use as iniciais)
    return null;
  }

  // Salva avatar de um usuário (ex: quando recebe via socket)
  function setUserAvatar(username, avatarData) {
  ...
  // ── Modifica a criação de mensagens para usar avatar ──
  // Intercepta a função que adiciona mensagens ao DOM (se existir)
  const originalAddMessage = window.addDmMessageToDom || window.addMessageToDm || function() {};
  ...
  window.addDmMessageToDom = function(msg, isMe) {
    ...
    const avatarData = getUserAvatar(sender);
    if (avatarData) {
      avatarDiv.style.backgroundImage = `url(${avatarData})`;
      avatarDiv.style.backgroundSize = 'cover';
      avatarDiv.style.backgroundPosition = 'center';
    ...
    // Evita duplicatas (verifica a última mensagem)
    const lastMsg = messagesArea.lastElementChild;
    if (lastMsg && lastMsg.textContent?.includes(msg.text) && lastMsg.textContent?.includes(sender)) {
      return;
    }
  ...
  // ── Atualiza o avatar quando receber via socket (se o servidor enviar) ──
  if (window.socket) {
    socket.on('user:avatar', function(data) {
      if (data && data.username && data.avatar) {
        setUserAvatar(data.username, data.avatar);
        // Re-renderiza as mensagens para atualizar os avatares (opcional)
        // Aqui você pode chamar uma função para recarregar as mensagens
        console.log('[DM-AVATAR] Avatar recebido para', data.username);
      }
    });
  }

  // ── Força a recriação das mensagens existentes com avatares ──
  function refreshDmMessages() {
    const messagesArea = document.getElementById('dm-messages-area');
    if (!messagesArea) return;
    // Não recria tudo – apenas aplica avatares nos elementos existentes
    // Se quiser recriar todas as mensagens, você pode limpar e recarregar do localStorage
    // Mas isso pode causar flicker. Vou apenas atualizar os avatares existentes:
    ...
    messages.forEach(msg => {
      ...
      const sender = isMe ? (window.username || 'Você') : 'amigo'; // Não sabemos quem é, então usamos fallback
      // Na prática, precisamos saber o remetente. Vamos pular por enquanto.
    });
  }
  ...
  console.log('[DM-AVATAR] Sistema de avatares ativado. Use getUserAvatar(username) para obter.');
})();