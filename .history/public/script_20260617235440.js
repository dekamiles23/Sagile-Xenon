...
// ============================================================
// AVATAR COMPLETO: MENSAGENS + LISTA LATERAL DE DMs
// ============================================================
(function fullAvatarSystem() {
  console.log('[FULL-AVATAR] Iniciando...');

  // ── 1. Cache e funções de avatar ──
  const avatarCache = {};
  ...
  // ── 2. Atualiza avatares da lista lateral de DMs ──
  function updateDmListAvatars() {
    const items = document.querySelectorAll('#dm-conversations-list .dm-conv-item');
    items.forEach(item => {
      const avatarEl = item.querySelector('.dm-avatar, .avatar, .dm-conv-avatar');
      if (!avatarEl) return;
      const nameEl = item.querySelector('.dm-conv-name, .name');
      if (!nameEl) return;
      const username = nameEl.textContent.trim();
      if (username) {
        applyAvatarToElement(avatarEl, username);
      }
    });
    console.log('[FULL-AVATAR] Avatares da lista atualizados:', items.length);
  }

  // ── 3. Atualiza avatares das mensagens ──
  ...
  // ── 4. Atualiza todos os avatares ──
  function refreshAllAvatars() {
    updateDmListAvatars();
    updateMessageAvatars();
    console.log('[FULL-AVATAR] Todos os avatares atualizados');
  }

  // ── 5. Cria uma mensagem com avatar ──
  function createMessageWithAvatar(msg, isMe) {
    const sender = isMe ? (window.username || 'Você') : (msg.from || msg.username || 'Usuário');
    const container = document.createElement('div');
    ...
    `;
    applyAvatarToElement(avatarDiv, sender);

    const bodyDiv = document.createElement('div');
    bodyDiv.className = 'msg-body';
    bodyDiv.style.cssText = 'flex:1;min-width:0;display:flex;flex-direction:column;gap:4px;';

    const nameDiv = document.createElement('div');
    nameDiv.className = 'msg-meta';
    nameDiv.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:13px;';
    const nameSpan = document.createElement('span');
    nameSpan.className = 'msg-username';
    nameSpan.style.cssText = `font-weight:700;color:${isMe ? '#ff00ff' : '#00ffff'};font-size:15px;`;
    nameSpan.textContent = sender;
    nameDiv.appendChild(nameSpan);

    const textDiv = document.createElement('div');
    textDiv.className = 'msg-text';
    textDiv.style.cssText = 'color:#eee;font-size:14px;line-height:1.4;word-break:break-word;';
  ...
  // ── 6. Substitui funções de renderização ──
  if (typeof window.addDmMessageToDom === 'function') {
    const original = window.addDmMessageToDom;
    window.addDmMessageToDom = function(msg, isMe) {
      const messagesArea = document.getElementById('dm-messages-area');
      if (!messagesArea) return;
      const lastMsg = messagesArea.lastElementChild;
      if (lastMsg && lastMsg.textContent?.includes(msg.text) && lastMsg.textContent?.includes(isMe ? 'Você' : msg.from)) return;
      const el = createMessageWithAvatar(msg, isMe);
      messagesArea.appendChild(el);
      messagesArea.scrollTop = messagesArea.scrollHeight;
      setTimeout(updateDmListAvatars, 50);
    };
  }

  if (typeof window.renderPrivateMessages === 'function') {
    const original = window.renderPrivateMessages;
    window.renderPrivateMessages = function(username) {
      const messagesArea = document.getElementById('private-chat-messages');
      if (!messagesArea) return;
      const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
      const messages = privateMessages[username] || [];
      messagesArea.innerHTML = '';
  ...
  // ── 7. Intercepta renderDmList para aplicar avatares ──
  if (typeof window.renderDmList === 'function') {
    const originalRenderDmList = window.renderDmList;
    window.renderDmList = function() {
      originalRenderDmList.call(this);
      setTimeout(updateDmListAvatars, 50);
    };
  }

  // ── 8. Listener de avatares do servidor ──
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        ...
        console.log('[FULL-AVATAR] Avatar recebido para', data.username);
        refreshAllAvatars();
      }
    });
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        ...
        console.log('[FULL-AVATAR] Avatares carregados de friends:data');
        refreshAllAvatars();
      }
    });
  }

  // ── 9. Observa a lista de DMs para novos itens ──
  function observeDmList() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;
    const observer = new MutationObserver(() => {
      setTimeout(updateDmListAvatars, 50);
    ...
    console.log('[FULL-AVATAR] Observador da lista de DMs ativado');
  }

  // ── 10. Quando o chat é aberto, solicita avatar do amigo ──
  function onChatOpen(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (!avatarCache[key] && !localStorage.getItem('avatar_' + key)) {
      if (window.socket && window.socket.connected) {
        console.log('[FULL-AVATAR] Solicitando avatar de', username);
  ...
  // ── 11. Hooks de abertura ──
  const origOpen = window.openPrivateChat || window.openDmChat;
  if (origOpen) {
    window.openPrivateChat = function(username) {
      origOpen.call(this, username);
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Aplica avatares na lista inicial se já estiver renderizada
    setTimeout(updateDmListAvatars, 500);
  });

  // ── 13. Expor funções ──
  window.refreshAvatars = refreshAllAvatars;
  window.setFriendAvatar = function(username, base64) {
    if (!username || !base64) return;
    const key = username.toLowerCase();
    avatarCache[key] = base64;
    localStorage.setItem('avatar_' + key, base64);
    refreshAllAvatars();
    console.log('[FULL-AVATAR] Avatar de', username, 'definido manualmente');
  };

  console.log('[FULL-AVATAR] Sistema completo ativo. Use refreshAvatars() para forçar.');
})();