...
function renderFandomTabs() {
  ...
  // ✅ BOTÃO ENVIAR COMUNIDADE PARA APROVAÇÃO
  const isStaff = localStorage.getItem('user_role') === 'staff' || localStorage.getItem('user_role') === 'dev';
  ...
  if (!isStaff) {
    ...
    sendBtn.style.cssText = `
      ...
      transition: all 0.2s;
    `;
    sendBtn.textContent = '📤 Enviar Comunidade';
    
    sendBtn.addEventListener('click', () => {
      if (confirm('Deseja enviar esta comunidade para análise da equipe?')) {
        // Salvar solicitação
        ...
        const notificationCount = parseInt(localStorage.getItem('notification_count') || '0') + 1;
        ...
        showToast('✅ Comunidade enviada para análise!');
    ...
    const requests = JSON.parse(localStorage.getItem('community_requests') || '[]');
    const alreadySent = requests.some(r => r.communityId === communityId);
    ...
    serverNameDisplay.parentNode.appendChild(sendBtn);
  }
...
window.openPublicChat = function openPublicChat() {
