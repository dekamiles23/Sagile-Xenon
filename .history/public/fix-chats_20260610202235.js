...
(() => {
  ...
  // ✅ PEGAR NICK REAL DO USUÁRIO LOGADO
  const currentUsername = localStorage.getItem('zx_username') 
                       || localStorage.getItem('username') 
                       || sessionStorage.getItem('username') 
                       || 'Usuário';
  
  // ✅ CORREÇÃO: Adicionar exemplos APENAS NA PRIMEIRA VEZ
  const alreadyInitialized = localStorage.getItem(`community_${communityId}_chats_initialized`);
  
  if (chats.length === 0 && !alreadyInitialized) {
    chats = [
    ...
    // ✅ Marcar como inicializado para NUNCA MAIS adicionar automaticamente
    localStorage.setItem(`community_${communityId}_chats_initialized`, 'true');
    saveChats();
  }
      ...
      item.innerHTML = `
        ...
        ">
          ...
          <div style="flex: 1; min-width: 0;">
            ...
            ">${chat.nome || chat.name}</div>
            
            <div style="
              color: #888;
              font-size: 12px;
            ">Criado por ${chat.criadoPor || chat.creator}</div>
          </div>
      `;
    ...
    modal.innerHTML = `
      ...
      ">
        ...
        ">
          ...
          <div style="font-size: 12px; margin-top: 4px;">Seja o primeiro a enviar uma mensagem!</div>
        </div>
      </div>
      ...
      <!-- ✅ BOTÃO REGRAS DO CHAT -->
      <div style="
        padding: 8px 16px;
        background: rgba(0,0,0,0.3);
        border-top: 1px solid rgba(255,0,255,0.1);
      ">
        <button onclick="showChatRules('${chat.id}')" style="
          width: 100%;
          padding: 10px;
  ...
  // ✅ MOSTRAR REGRAS DO CHAT
  window.showChatRules = function(chatId) {
    const chat = chats.find(c => c.id == chatId);
    if (!chat) return;

    // Fechar modal existente
    document.getElementById('chat-rules-modal')?.remove();

    const modal = document.createElement('div');
    modal.id = 'chat-rules-modal';
    modal.style.cssText = `
      position: fixed;
      top: 0;
      left: 0;
    ...
    modal.innerHTML = `
      ...
      ">
        <div style="
          ...
          margin-bottom: 16px;
        ">
          <h3 style="color: white; margin: 0;">📜 Regras do Chat</h3>
          <button onclick="closeChatRulesModal()" style="
            background: transparent;
            border: none;
            color: #aaa;
            font-size: 20px;
        ...
        ">${chat.regras || 'Nenhuma regra definida para este chat.'}</div>
      </div>
    `;
    ...
    modal.addEventListener('click', (e) => {
      if (e.target === modal) closeChatRulesModal();
    });
  }

  window.closeChatRulesModal = function() {
    document.getElementById('chat-rules-modal')?.remove();
  }
  ...
  // ✅ MODAL VER TODOS OS CHATS
  window.openAllChatsModal = function() {
    // Remover modal existente
    document.getElementById('all-chats-modal')?.remove();
    
    const modal = document.createElement('div');
    modal.id = 'all-chats-modal';
    modal.className = 'modal-overlay';
    modal.style.display = 'flex';
    
    modal.innerHTML = `
      <div class="modal-modern" style="width: 600px; max-width: 95vw; max-height: 80vh;">
        <div class="mm-header">
          <span class="mm-title">💬 Todos os Chats da Comunidade</span>
          <button class="mm-close" onclick="closeAllChatsModal()">✕</button>
        </div>
        <div class="mm-body" style="padding: 16px; overflow-y: scroll; max-height: 60vh; scrollbar-width: thin; scrollbar-color: #ff00ff rgba(0,0,0,0.3);">
          ${chats.length === 0 ? `
            <div class="empty-state">
              <div class="empty-state-icon">💬</div>
              <div class="empty-state-title">Nenhum chat criado ainda</div>
              <div class="empty-state-text">Clique em "+ Criar Chat" para criar o primeiro chat!</div>
            </div>
          ` : chats.map(chat => `
            <div class="chat-item" onclick="closeAllChatsModal(); openChat(${JSON.stringify(chat).replace(/"/g, '"')})" style="cursor: pointer; margin-bottom: 8px;">
              <div style="
                width: 48px;
                height: 48px;
                border-radius: 10px;
              ...
              <div class="chat-info" style="flex: 1;">
                <div class="chat-name">${chat.nome || chat.name}</div>
                <div class="chat-members">Criado por ${chat.criadoPor || chat.creator} · ${chat.members || 1} membros</div>
              </div>
    ...
    // Fechar ao clicar fora
    modal.addEventListener('click', (e) => {
      if (e.target === modal) closeAllChatsModal();
    });
  }
  
  window.closeAllChatsModal = function() {
    document.getElementById('all-chats-modal')?.remove();
  }
