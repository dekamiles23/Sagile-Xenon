...
// ============================================================
// RECRIA A BARRA DO DM EXATAMENTE IGUAL À DO SERVIDOR
// ============================================================
(function recreateDmBar() {
  function buildDmBar() {
    ...
    // 1. Obtém a estrutura da barra do servidor como modelo
    const serverWrapper = document.querySelector('#chat-view .message-input-area .input-wrapper');
    if (!serverWrapper) {
      console.warn('[DM-BAR] Barra do servidor não encontrada');
      setTimeout(buildDmBar, 300);
      return;
    }

    // 2. Cria um novo wrapper (não clona para evitar referências)
    const wrapper = document.createElement('div');
    wrapper.className = 'input-wrapper';
    wrapper.style.cssText = 'display:flex;align-items:center;gap:6px;position:relative;';

    // 3. Obtém os botões e input do servidor (modelo)
    const serverButtons = serverWrapper.querySelectorAll('button');
    const serverInput = serverWrapper.querySelector('#message-input');
    ...
    // 5. Cria os botões com os mesmos textos/ícones do servidor
    const buttonConfigs = [
      { id: 'btn-plus', label: '+' },
      { id: 'btn-emoji', label: '😀' },
      { id: 'btn-gif', label: '🎬' },
      { id: 'btn-stickers', label: '🎭' },
      { id: 'btn-attach-file', label: '📎' },
      { id: 'btn-voice', label: '🎤' },
    ];
    ...
    buttonConfigs.forEach(cfg => {
      ...
      btn.style.cssText = 'background:transparent;border:none;color:#fff;font-size:18px;cursor:pointer;padding:8px;';
      wrapper.appendChild(btn);
    });

    // 6. Cria o input
    const input = document.createElement('input');
    input.id = 'dm-message-input';
    input.type = 'text';
    input.placeholder = 'Conversar em #geral'; // ou a mesma do servidor
    input.maxLength = 500;
    input.autocomplete = 'off';
    input.style.cssText = 'flex:1;padding:10px 14px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,0,255,0.3);border-radius:24px;color:#fff;outline:none;font-size:14px;';
    wrapper.appendChild(input);

    // 7. Cria o botão enviar (com ícone ➤)
    const sendBtn = document.createElement('button');
    sendBtn.id = 'dm-send-btn';
    sendBtn.type = 'button';
    sendBtn.textContent = '➤';
    sendBtn.style.cssText = 'background:linear-gradient(135deg,#8b00ff,#ff00ff);border:none;border-radius:50%;width:44px;height:44px;color:#fff;font-size:18px;cursor:pointer;';
    wrapper.appendChild(sendBtn);

    // 8. Substitui a área do DM pela nova barra
    ...
    // 10. Conecta pickers (usando os mesmos pickers globais do servidor)
    ...
    connectPicker('dm-btn-emoji', 'emoji-picker', '.emoji-item', function(text) {
      if (dmInput) {
        const start = dmInput.selectionStart;
        const end = dmInput.selectionEnd;
        const val = dmInput.value;
        dmInput.value = val.substring(0, start) + text + val.substring(end);
        dmInput.selectionStart = dmInput.selectionEnd = start + text.length;
      }
    });

    // GIF
    connectPicker('dm-btn-gif', 'gif-picker', '.gif-item img', function(url) {
      if (dmInput) dmInput.value += `[GIF] ${url} `;
    });
    ...
    if (attachBtn) {
      ...
      fileInput.addEventListener('change', function() {
        ...
        console.log('[DM] Anexar arquivo:', this.files);
        this.value = '';
      });
    }

    // Gravar áudio (simples)
    const voiceBtn = document.getElementById('dm-btn-voice');
    if (voiceBtn) {
      voiceBtn.addEventListener('click', function() {
        console.log('[DM] Gravar áudio');
      });
    }
    ...
    if (plusBtn) {
      plusBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        console.log('[DM] Mais opções');
      });
    }

    console.log('[DM-BAR] Barra recriada com sucesso');
  }
