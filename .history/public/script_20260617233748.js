...
// ============================================================
// CORREÇÃO DEFINITIVA DE AVATARES NO DM
// ============================================================
(function fixAvatarsFinal() {
  console.log('[AVATAR-FIX] Iniciando...');

  // ── 1. CARREGA O AVATAR DO PRÓPRIO USUÁRIO ──
  function loadOwnAvatar() {
    const saved = localStorage.getItem('zx_avatar');
    if (saved && saved.startsWith('data:image')) {
      window.ownAvatar = saved;
      console.log('[AVATAR-FIX] Avatar próprio carregado');
      return saved;
    }
    console.warn('[AVATAR-FIX] Nenhum avatar próprio encontrado em zx_avatar');
    return null;
  }

  // ── 2. FUNÇÃO PARA OBTER AVATAR DE QUALQUER USUÁRIO ──
  window.getAvatarForUser = function(username) {
    ...
    // Próprio usuário
    if (key === currentUser) {
      if (window.ownAvatar) return window.ownAvatar;
    ...
    // Amigos: tenta do mapa global, depois do localStorage
  ...
  // ── 3. ATUALIZA OS AVATARES NAS MENSAGENS EXISTENTES ──
  function updateAllMessageAvatars() {
    const messagesArea = document.getElementById('dm-messages-area') ||
                         document.getElementById('private-chat-messages');
    if (!messagesArea) return;

    const messages = messagesArea.querySelectorAll('.message, .private-message');
    messages.forEach(msg => {
      const avatarDiv = msg.querySelector('.msg-avatar, .avatar');
      if (!avatarDiv) return;

      // Descobre o remetente
      let sender = null;
      const nameEl = msg.querySelector('.msg-username, .username');
      if (nameEl) {
      ...
      const avatarData = window.getAvatarForUser(sender);
      if (avatarData) {
        avatarDiv.style.backgroundImage = `url(${avatarData})`;
        avatarDiv.style.backgroundSize = 'cover';
        avatarDiv.style.backgroundPosition = 'center';
    ...
    console.log('[AVATAR-FIX] Avatares atualizados em', messages.length, 'mensagens');
  }

  // ── 4. OBSERVA NOVAS MENSAGENS PARA APLICAR AVATAR ──
  function observeNewMessages() {
    ...
    const observer = new MutationObserver(() => {
      // Quando novas mensagens são adicionadas, atualiza os avatares
      updateAllMessageAvatars();
    });
    observer.observe(messagesArea, { childList: true, subtree: true });
    console.log('[AVATAR-FIX] Observador de novas mensagens ativado');
  }

  // ── 5. SOBRESCREVE A FUNÇÃO QUE CRIA MENSAGENS (para garantir) ──
  function patchMessageCreation() {
    // Se existir addDmMessageToDom, patcheia
    if (typeof window.addDmMessageToDom === 'function') {
      const original = window.addDmMessageToDom;
      window.addDmMessageToDom = function(msg, isMe) {
        original.call(this, msg, isMe);
        // Após adicionar, atualiza os avatares
        setTimeout(updateAllMessageAvatars, 10);
      };
      console.log('[AVATAR-FIX] addDmMessageToDom patched');
    }

    // Se existir renderPrivateMessages, patcheia
    if (typeof window.renderPrivateMessages === 'function') {
      const original = window.renderPrivateMessages;
      window.renderPrivateMessages = function(username) {
        original.call(this, username);
        setTimeout(updateAllMessageAvatars, 50);
      };
      console.log('[AVATAR-FIX] renderPrivateMessages patched');
    }
  }

  // ── 6. EXECUTA QUANDO A DM É ABERTA ──
  function onDmOpen() {
    loadOwnAvatar();
    setTimeout(updateAllMessageAvatars, 100);
    setTimeout(updateAllMessageAvatars, 300);
    setTimeout(updateAllMessageAvatars, 600);
  }

  // ── 7. SETUP ──
  loadOwnAvatar();

  // Observa abertura da view DM
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  // Inicialização imediata se a DM já estiver aberta
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      onDmOpen();
    ...
    observeNewMessages();
    patchMessageCreation();
  });

  // ── EXPÕE FUNÇÕES PARA DEBUG ──
  window.refreshAvatars = updateAllMessageAvatars;
  window.setFriendAvatar = function(username, avatarData) {
    if (!username || !avatarData) return;
    const key = username.toLowerCase();
    if (!window.userAvatars) window.userAvatars = {};
    window.userAvatars[key] = avatarData;
    localStorage.setItem(`avatar_${key}`, avatarData);
    updateAllMessageAvatars();
    console.log('[AVATAR-FIX] Avatar de', username, 'definido manualmente');
  };

  console.log('[AVATAR-FIX] Sistema pronto. Use refreshAvatars() para forçar atualização.');
})();