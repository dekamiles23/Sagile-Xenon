...
// ============================================================
// COPIA BARRA DO SERVIDOR PARA O DM (COMPLETO)
// ============================================================
(function cloneServerBarToDM() {
  function applyBar() {
    // 1. Encontra a barra do servidor
    const serverWrapper = document.querySelector('#chat-view .message-input-area .input-wrapper');
    if (!serverWrapper) {
      console.warn('[DM-BAR] Barra do servidor não encontrada. Tentando novamente...');
      setTimeout(applyBar, 500);
      return;
    }

    // 2. Encontra a área de input do DM
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      console.warn('[DM-BAR] dm-input-area não encontrada. Tentando novamente...');
      setTimeout(applyBar, 500);
      return;
    }

    // 3. Clona a barra do servidor (deep clone)
    const clone = serverWrapper.cloneNode(true);

    // 4. Remove os pickers internos (já existem globais)
    clone.querySelectorAll('.chat-picker').forEach(p => p.remove());

    // 5. Substitui os IDs para evitar conflitos
    const idMap = {
    ...
    // 6. Remove o conteúdo atual e insere o clone
    dmInputArea.innerHTML = '';
    dmInputArea.appendChild(clone);

    // 7. Reconecta os eventos
    const input = document.getElementById('dm-message-input');
    ...
    // 8. Conecta os pickers globais
    function connectGlobalPicker(btnId, pickerId, itemSelector, insertFn) {
    ...
    // Emoji
    connectGlobalPicker('dm-btn-emoji', 'emoji-picker', '.emoji-item', function(text) {
      if (input) {
    ...
    // GIF
    connectGlobalPicker('dm-btn-gif', 'gif-picker', '.gif-item img', function(url) {
      if (input) input.value += `[GIF] ${url} `;
    });

    // Sticker
    connectGlobalPicker('dm-btn-stickers', 'sticker-picker', '.sticker-item', function(text) {
      if (input) input.value += text + ' ';
    });

    // 9. Anexar arquivo (copiado do servidor, mas pode precisar de adaptação)
    const attachBtn = document.getElementById('dm-btn-attach');
    if (attachBtn) {
      // Remove o file input antigo se existir
      const oldFileInput = attachBtn.parentNode.querySelector('#dm-file-upload');
  ...
  // ── Observa quando a view DM fica visível ──
  function waitForDM() {
    const dmView = document.getElementById('dm-view');
    if (!dmView) return;
    ...
    // Se a DM já estiver visível ao carregar, aplica
    if (!dmView.classList.contains('hidden')) {
      applyBar();
    }
  }
