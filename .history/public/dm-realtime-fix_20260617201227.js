...
function sendDmMessage(text, toUsername) {
  ...
  if (!receiverId) {
    socket.emit('dm:get-user-id', { username: toUsername });
    dmQueue.push({ text: text, to: toUsername, pending: true });
    console.log('[DM] ID não encontrado, enfileirado para', toUsername);
    return;
  }
