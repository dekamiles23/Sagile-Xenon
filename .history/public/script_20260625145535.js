...
// No Electron ou file://, sempre conectar ao localhost
const socketUrl = (isElectron || isFileProtocol || isLocalhost) ? 'http://localhost:3002' : 'https://sagile-xenon.onrender.com';
...
socket.on('connect', () => {
  ...
  if (u && u !== 'Usuário' && u.trim() !== '') {
    socket.emit('user:login', { username: u });
  }
});
...
function requestUserAvatar(username) {
  ...
  socket.emit('user:avatar:get', { username });
}
...
function doLogin() {
  ...
  (function() {
    ...
    if (savedAvatar && savedAvatar.startsWith('data:image')) {
      socket.emit('user:avatar:save', { avatar: savedAvatar });
    }
  })();
...
socket.on('suggested:updated', (community) => {
        ...
        btn.addEventListener('click', (e) => {
          ...
          if (action === 'remove') {
            if (confirm(`Remover "${community.name}" da lista de comunidades sugeridas?`)) {
              ...
              socket.emit('community:unsuggest', { communityId });
...
socket.on('user:avatar:data', (data) => {
...
socket.on('dm:user:profile', (data) => {
...
socket.on('friends:presence', ({ online, statuses, avatars }) => {
...
socket.on('friend:request', ({ from, avatar }) => {
...
socket.on('friend:accepted', ({ by, avatar }) => {
...
(function fixDmRealtime() {
  ...
  // FIX: socket.off('dm:message') REMOVIDO — não deve matar o listener oficial _onDmMessage.
...
(function fixDmRealtime() {
  ...
  // socket.off('dm:message') REMOVIDO para não matar o listener oficial.
...
(function instantDmPolling() {
  ...
  // FIX: Listener socket.on('dm:history') REMOVIDO deste bloco.
  ...
  socket.on('dm:message:sent', function(data) {
    // Aguarda 500ms para o servidor salvar e depois faz polling
    setTimeout(() => {
      if (currentChatUser) {
        socket.emit('dm:history', { with: currentChatUser });
      }
    }, 500);
  });
