...
// ============================================================
// AVATAR NAS MENSAGENS DO DM - VERSÃO SIMPLES E FUNCIONAL
// ============================================================
(function simpleDmAvatar() {
  console.log('[SIMPLE-AVATAR] Iniciando...');

  // Lê o avatar do localStorage (próprio ou amigo)
  ...
  // Escaneia e corrige todos os avatares existentes
  function fixAllAvatars() {
    // Procura elementos de avatar nas mensagens
    const avatars = document.querySelectorAll('.msg-avatar, .message .avatar, .private-message .avatar, .dm-message .avatar');
    avatars.forEach(el => {
      // Descobre o nome do usuário
      let username = null;
      const parent = el.closest('.message, .private-message, .dm-message, .chat-message');
      if (parent) {
        const nameEl = parent.querySelector('.msg-username, .username, .message-username');
      ...
      if (!username) {
        // Se ainda não, usa o conteúdo (se for uma letra, pode ser a inicial)
        const text = el.textContent.trim();
        if (text && text.length === 1) {
          // Não temos o nome completo, então usamos uma busca: tenta encontrar o nome no parent
          const parentText = parent ? parent.textContent : '';
          const match = parentText.match(/^([A-Za-zÀ-ÖØ-öø-ÿ]+)/);
    ...
    console.log('[SIMPLE-AVATAR] Avatares atualizados:', avatars.length);
  }

  // ── Observa novas mensagens ──
  function observeMessages() {
    const target = document.getElementById('dm-messages-area') || document.getElementById('private-chat-messages') || document.body;
    const obs = new MutationObserver(() => {
      // Quando novas mensagens são adicionadas, aplica avatares
      fixAllAvatars();
    });
    obs.observe(target, { childList: true, subtree: true });
    console.log('[SIMPLE-AVATAR] Observador ativado');
  }

  // ── Executa ao abrir a view DM ──
  function onDmOpen() {
    setTimeout(fixAllAvatars, 100);
    setTimeout(fixAllAvatars, 300);
    setTimeout(fixAllAvatars, 600);
  }
  ...
  window.fixAvatars = fixAllAvatars;
  window.setFriendAvatar = function(username, base64) {
    ...
    fixAllAvatars();
    console.log('[SIMPLE-AVATAR] Avatar de', username, 'definido');
  };

  console.log('[SIMPLE-AVATAR] Sistema ativo. Use fixAvatars() para forçar.');
})();