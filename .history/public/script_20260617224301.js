...
// ============================================================
// BARRA DO DM + PICKERS (COM CSS DEDICADO)
// ============================================================
(function dmBarWithCSS() {
  // ── 1. INJETA O CSS ──
  const styleId = 'dm-bar-styles';
  if (!document.getElementById(styleId)) {
    ...
    style.textContent = `
      ...
      .dm-input-wrapper .dm-btn:hover {
        background: rgba(255,255,255,0.08) !important;
      }

      /* Campo de texto */
      .dm-input-wrapper .dm-input {
        flex: 1 !important;
      ...
      .dm-input-wrapper .dm-input::placeholder {
        color: #666 !important;
      }

      /* Botão enviar */
      .dm-input-wrapper .dm-send-btn {
      ...
      .dm-input-wrapper .dm-send-btn:hover {
      ...
      .dm-picker-header button:hover,
      .dm-picker-header button.active {
        color: #fff !important;
      ...
      .dm-picker-grid.emoji-grid {
        grid-template-columns: repeat(8, 1fr) !important;
      }

      .dm-picker-grid.gif-grid {
        grid-template-columns: repeat(2, 1fr) !important;
      }

      .dm-picker-grid.sticker-grid {
      ...
      .dm-gif-search input:focus {
        border-color: #ff00ff !important;
        box-shadow: 0 0 12px rgba(255,0,255,0.2) !important;
      }

      /* Scroll */
      .dm-picker-grid::-webkit-scrollbar {
        width: 4px !important;
      }
      .dm-picker-grid::-webkit-scrollbar-track {
        background: rgba(0,0,0,0.2) !important;
        border-radius: 4px !important;
      }
      .dm-picker-grid::-webkit-scrollbar-thumb {
    ...
    `;
    document.head.appendChild(style);
    console.log('[DM-BAR] CSS injetado com sucesso');
  }

  // ── 2. FUNÇÃO PARA CRIAR A BARRA E OS PICKERS ──
  function buildDmBar() {
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      setTimeout(buildDmBar, 300);
      return;
    }

    // Remove qualquer wrapper antigo
    const oldWrapper = dmInputArea.querySelector('.dm-input-wrapper');
    if (oldWrapper) oldWrapper.remove();

    // ── Cria a barra ──
    const wrapper = document.createElement('div');
    wrapper.className = 'dm-input-wrapper';

    wrapper.innerHTML = `
      <button type="button" class="dm-btn" id="dm-btn-plus" title="Mais opções">➕</button>
      <button type="button" class="dm-btn" id="dm-btn-emoji" title="Emojis">😀</button>
      <button type="button" class="dm-btn" id="dm-btn-gif" title="GIFs">🎬</button>
      <button type="button" class="dm-btn" id="dm-btn-stickers" title="Figurinhas">🎭</button>
      <button type="button" class="dm-btn" id="dm-btn-attach" title="Anexar arquivo">📎</button>
      <input type="text" class="dm-input" id="dm-message-input" placeholder="Digite sua mensagem..." maxlength="500" autocomplete="off">
      <button type="button" class="dm-btn" id="dm-btn-voice" title="Gravar áudio">🎤</button>
      <button type="button" class="dm-send-btn" id="dm-send-btn">➤</button>
    `;
    ...
    ensurePicker('dm-emoji-picker', `
      ...
      <div class="dm-picker-grid emoji-grid">
        ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟']}
      </div>
    `);
    ...
    ensurePicker('dm-sticker-picker', `
      <div class="dm-picker-header">
        <button class="active">Padrão</button>
        <button>Reações</button>
      </div>
      <div class="dm-picker-grid sticker-grid">
        ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️']}
      </div>
    `);
    ...
    function connectPicker(btnId, pickerId, itemSelector, insertFn) {
      ...
      picker.addEventListener('click', function(e) {
        ...
        if (!target) return;
        let value = '';
        if (itemSelector === '.dm-gif-item img') {
          value = target.src || '';
        } else {
    ...
    connectPicker('dm-btn-emoji', 'dm-emoji-picker', '.dm-emoji-item', function(text) {
      if (input) {
    ...
    connectPicker('dm-btn-gif', 'dm-gif-picker', '.dm-gif-item img', function(url) {
      if (input) input.value += `[GIF] ${url} `;
    });

    connectPicker('dm-btn-stickers', 'dm-sticker-picker', '.dm-sticker-item', function(text) {
    ...
    if (attachBtn) {
      ...
      fileInput.addEventListener('change', function() {
        // ... implementação de anexo (mantida)
        this.value = '';
      });
    }
    ...
    if (voiceBtn) {
      // ... implementação de áudio (mantida)
    }
    ...
    console.log('[DM-BAR] Barra construída com CSS dedicado');
  }
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    ...
    document.addEventListener('DOMContentLoaded', buildDmBar);
  }
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      ...
      if (layoutId === 'dm-view') {
        setTimeout(buildDmBar, 100);
        setTimeout(buildDmBar, 300);
        setTimeout(buildDmBar, 600);
      }
    };
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(buildDmBar, 200);
      setTimeout(buildDmBar, 500);
    }
  });

  console.log('[DM-BAR] Script com CSS dedicado carregado');
})();

// ============================================================
// CORRIGE PICKERS: GIFs reais e emojis renderizados
// ============================================================
(function fixPickersContent() {
  // Lista de GIFs reais (substitua por URLs funcionais)
  const gifUrls = [
  ...
  // Função para atualizar o picker de GIF
  function updateGifPicker() {
    const gifPicker = document.getElementById('dm-gif-picker');
    if (!gifPicker) return;

    const grid = gifPicker.querySelector('.dm-picker-grid.gif-grid');
    if (grid) {
      grid.innerHTML = gifUrls.map(url => `
        <div class="dm-gif-item">
  ...
  // Função para garantir que os emojis usem fonte adequada
  function fixEmojiFont() {
    const pickers = ['dm-emoji-picker', 'dm-sticker-picker'];
    pickers.forEach(id => {
      const picker = document.getElementById(id);
      if (picker) {
        picker.style.fontFamily = "'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji', 'EmojiOne Color', 'Twemoji Mozilla', sans-serif";
        // Garante que os itens de emoji tenham fonte de emoji
        picker.querySelectorAll('.dm-emoji-item, .dm-sticker-item').forEach(el => {
  ...
  function applyFixes() {
    updateGifPicker();
    fixEmojiFont();
    console.log('[DM-FIX] Pickers corrigidos: GIFs reais e emojis com fonte apropriada');
  }
  ...
  console.log('[DM-FIX] Script de correção de conteúdo carregado');
})();