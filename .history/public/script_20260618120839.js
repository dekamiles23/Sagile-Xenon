...
// ============================================================
// BOTÃO 📷 PERMANENTE NA LISTA DE DMs (COM SOCKET.IO)
// ============================================================
(function permanentUploadButton() {
  console.log('[PERM] Iniciando botão permanente...');

  // 1. Cria input de arquivo oculto (uma única vez)
    ...
    fileInput.addEventListener('change', function(e) {
      ...
      reader.onload = function(ev) {
        ...
        if (window.socket && window.socket.connected) {
          ...
          alert('📤 Enviando avatar de ' + targetUser + ' para o servidor...');
        } else {
  ...
  // 2. Função que adiciona o botão em cada item da lista
  function addButtons() {
    const items = document.querySelectorAll('#dm-conversations-list .dm-conv-item');
    if (items.length === 0) {
      console.warn('[PERM] Nenhum item encontrado');
      return;
    }
    items.forEach(function(item) {
      ...
      console.log('[PERM] Botão adicionado para', friendName);
    });
  }

  // 3. Observa a lista para novos itens (quando a lista for recriada)
  const list = document.getElementById('dm-conversations-list');
  if (list) {
    ...
    console.log('[PERM] Observador ativado');
  }

  // 4. Executa imediatamente e com delays (garante que a lista já esteja pronta)
  addButtons();
  setTimeout(addButtons, 300);
  setTimeout(addButtons, 600);
  setTimeout(addButtons, 1000);

  console.log('[PERM] Sistema pronto. Passe o mouse sobre o amigo para ver o 📷.');
})();