...
function setFriendAvatar(username, avatarData) {
  ...
  window.friendAvatarCache[username.toLowerCase()] = avatarData;
...
function renderDmList() {
  ...
  const friends = JSON.parse(localStorage.getItem('zx_friends') || '[]');
  const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');

  // Usa um Map para garantir unicidade (case-insensitive)
  const unique = new Map();

  // 1. Amigos (exceto o próprio)
  friends.forEach(f => {
    const name = typeof f === 'string' ? f : f.username;
    if (name && name.toLowerCase() !== currentUser) {
      unique.set(name.toLowerCase(), name);
    }
  });

  // 2. Contatos de mensagens (exceto o próprio)
  Object.keys(privateMessages).forEach(name => {
    if (name.toLowerCase() !== currentUser) {
      unique.set(name.toLowerCase(), name);
    }
  });

  // 3. (Opcional) Contatos que enviaram mensagens via socket recentemente
  //    você pode adicionar aqui se houver um array de contatos ativos
  ...
  unique.forEach((name) => {
    ...
    item.innerHTML = `
      ...
      <div class="dm-conv-name" style="color: #eee; font-size: 14px;">${name}</div>
      <div class="dm-conv-badge" style="margin-left: auto; color: #888; font-size: 12px;">●</div>
    `;
    item.onclick = () => {
      if (typeof openDmChat === 'function') openDmChat(name);
    };
    dmList.appendChild(item);
  });

  console.log('[DM] Lista renderizada com', unique.size, 'conversas (excluindo o próprio)');
...
function openDmChat(username) {
  ...
  if (avatarEl) {
    const friendAv = getFriendAvatar(username);
    if (friendAv) {
      avatarEl.style.backgroundImage = `url(${friendAv})`;
      ...
      // Solicitar perfil do amigo para obter avatar
...
socket.on('dm:user:profile', (data) => {
  ...
  if (data.avatar) {
    ...
    // Atualizar avatar no cabeçalho do chat DM se estiver aberto para este usuário
    if (typeof currentDmUser !== 'undefined' && (currentDmUser || '').toLowerCase() === data.username.toLowerCase()) {
      const avatarEl = document.querySelector('#dm-chat-area .dm-avatar');
      if (avatarEl) {
    ...
    // Re-renderizar lista DM para mostrar o avatar atualizado
...
function friendCard(uname, isOnline) {
    ...
    const avatarUrl = (window.userAvatarCache || {})[_ck]
                   || getFriendAvatar(uname)
                   || (uname.toLowerCase() === (window.username || '').toLowerCase() ? (localStorage.getItem('zx_avatar') || '') : '');
    // Se não há avatar em cache, solicitar ao servidor (dm-avatar-call-fix irá responder)
    if (!avatarUrl && window.socket && window.socket.connected) {
      window.socket.emit('user:avatar:get', { username: uname });
    }
...
(function fixDmRealtime() {
  ...
  window.socket.off('dm:message');

  // Escuta novas mensagens DM
  window.socket.on('dm:message', function(msg) {
    console.log('[DM-RT] Mensagem recebida:', msg);

    // Verifica se a mensagem é para o usuário atual OU enviada por ele
    ...
    // Se o chat com esse usuário está aberto, adiciona a mensagem
    ...
    if (activeChat && activeChat.toLowerCase() === targetUser.toLowerCase()) {
      // Adiciona a mensagem na área de mensagens
      ...
      // Se o chat não está aberto, incrementa um badge de não lido
      // (opcional – você pode implementar depois)
      console.log('[DM-RT] Mensagem de', msg.from, 'aguardando em background');
    }
  });

  console.log('[DM-RT] Listener dm:message registrado com sucesso!');
})();
...
(function fixDmAutoUpdate() {
  ...
  // Sobrescreve o listener dm:message para atualizar a UI
  ...
  socket.on('dm:message', function(msg) {
    ...
    // 2. Se o chat com esse usuário está aberto, atualiza a área de mensagens
    const activeChat = document.querySelector('#dm-chat-area')?.dataset.activeChat;
    if (activeChat && activeChat.toLowerCase() === targetUser.toLowerCase()) {
      // Verifica se já existe essa mensagem para evitar duplicação
      const messagesArea = document.getElementById('dm-messages-area');
      const existingMessages = messagesArea?.querySelectorAll('.message') || [];
      let alreadyExists = false;
      // Verifica as últimas 5 mensagens para evitar duplicatas
      const lastMessages = Array.from(existingMessages).slice(-5);
      for (let el of lastMessages) {
        if (el.textContent?.includes(msg.text) && el.textContent?.includes(msg.from)) {
          alreadyExists = true;
          break;
        }
      }
...
// ============================================
// FIX: Remove duplicatas da lista de DMs
// ============================================
(function fixDmDuplicates() {
  // Sobrescreve a função original de renderização (se existir)
  const originalRender = window.renderDmList || function() {};
  ...
  window.renderDmList = function() {
    ...
    // Chama a função original (se houver) ou faz a renderização manual
    if (typeof originalRender === 'function') {
      originalRender();
    } else {
      // Fallback: renderiza a partir dos amigos
      const friends = JSON.parse(localStorage.getItem('zx_friends') || '[]');
      const unique = new Map();
      friends.forEach(f => {
        const name = typeof f === 'string' ? f : f.username;
      ...
      unique.forEach((name) => {
        ...
        item.className = 'dm-conv-item';
        item.dataset.username = name;
        const _av = getFriendAvatar(name);
        const _avHtml = _av
          ? `<div class="dm-avatar" style="background-image:url(${_av});background-size:cover;background-position:center;"></div>`
          : `<div class="dm-avatar">${name[0].toUpperCase()}</div>`;
        item.innerHTML = `
          ${_avHtml}
          <div class="dm-conv-name">${name}</div>
        `;
        item.onclick = () => openDmChat(name);
        list.appendChild(item);
      });
    }
  };

  console.log('[DM-FIX] Renderização com deduplicação ativada!');
})();

// ============================================================
// PATCH: renderDmList corrigida (sem remover código existente)
// ============================================================
(function patchRenderDmList() {
  ...
  window.renderDmList = function() {
    ...
    const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
    if (!currentUser) {
      console.warn('[DM-PATCH] Usuário atual não definido!');
      return;
    }
    ...
    // Adiciona amigos (exceto o próprio)
    friends.forEach(f => {
      const name = typeof f === 'string' ? f : f.username;
      if (name && name.toLowerCase() !== currentUser) {
        unique.set(name.toLowerCase(), name);
    ...
    // Adiciona contatos de mensagens (exceto o próprio)
    Object.keys(privateMessages).forEach(name => {
      if (name.toLowerCase() !== currentUser) {
        unique.set(name.toLowerCase(), name);
      }
    ...
    if (unique.size === 0) {
      dmList.innerHTML = `<div style="padding: 20px; text-align: center; color: #666;">Nenhuma conversa ainda</div>`;
      return;
    }
    ...
    console.log('[DM-PATCH] Lista renderizada com', unique.size, 'conversas (excluindo o próprio)');
  };
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    ...
    document.addEventListener('DOMContentLoaded', window.renderDmList);
  }

  // Corrige o listener dm:message para atualizar a lista e o chat
  if (window.socket) {
    socket.off('dm:message');
    socket.on('dm:message', function(msg) {
      console.log('[DM-PATCH] Mensagem recebida:', msg);
      ...
      // Atualiza a lista de conversas (sem duplicar)
      window.renderDmList();

      // Se o chat com esse usuário está aberto, adiciona a mensagem na área de mensagens
      const activeChat = document.querySelector('#dm-chat-area')?.dataset.activeChat;
      if (activeChat && activeChat.toLowerCase() === targetUser.toLowerCase()) {
        const messagesArea = document.getElementById('dm-messages-area');
        if (messagesArea) {
          // Evita duplicação verificando a última mensagem
          const lastMsg = messagesArea.lastElementChild;
          if (lastMsg && lastMsg.textContent?.includes(msg.text)) {
            return;
          }
  ...
  console.log('[DM-PATCH] Aplicado com sucesso!');
})();

// ============================================================
// PATCH: Avatar na barra superior de Mensagens Privadas
// ============================================================
(function fixDmAvatar() {
  // Função que atualiza o avatar da DM
  function updateDmAvatar() {
    const dmAvatar = document.getElementById('user-avatar-dm');
    const mainAvatar = document.getElementById('user-avatar');
    if (!dmAvatar) return;

    // Se o avatar principal tem imagem, copia
    if (mainAvatar && mainAvatar.style.backgroundImage && mainAvatar.style.backgroundImage !== 'none') {
      dmAvatar.style.backgroundImage = mainAvatar.style.backgroundImage;
      dmAvatar.style.backgroundSize = 'cover';
      dmAvatar.style.backgroundPosition = 'center';
      dmAvatar.textContent = '';
      dmAvatar.classList.add('has-image');
      return;
    }
    ...
    if (saved && saved.startsWith('data:image')) {
      dmAvatar.style.backgroundImage = `url(${saved})`;
      dmAvatar.style.backgroundSize = 'cover';
      dmAvatar.style.backgroundPosition = 'center';
      dmAvatar.textContent = '';
      dmAvatar.classList.add('has-image');
    ...
    // Se nada funcionar, usa as iniciais (fallback roxo)
    const name = window.username || localStorage.getItem('zx_username') || 'Usuário';
    const initial = name.charAt(0).toUpperCase();
    dmAvatar.style.backgroundImage = 'none';
    dmAvatar.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
    dmAvatar.textContent = initial;
    dmAvatar.classList.remove('has-image');
  }

  // Intercepta a exibição da view DM
  const originalShowLayout = window.showLayout;
  if (originalShowLayout) {
    window.showLayout = function(layoutId) {
      // Chama a função original
      originalShowLayout.call(this, layoutId);
      // Se for a view DM, atualiza o avatar
      if (layoutId === 'dm-view') {
        setTimeout(updateDmAvatar, 50);
      }
    };
  }

  // Também escuta cliques no botão de DM (fallback)
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm');
    if (btn) {
      setTimeout(updateDmAvatar, 100);
    }
  });

  // Força atualização quando a página carregar
  document.addEventListener('DOMContentLoaded', function() {
    // Se a view DM já estiver visível, atualiza
  ...
  console.log('[PATCH] Avatar da DM será carregado corretamente');
})();

// ============================================================
// FIX: Avatar do usuário na barra superior de Mensagens Privadas
// ============================================================
(function fixDmAvatar() {
  // Função que carrega o avatar do localStorage e aplica no elemento da DM
  function loadDmAvatar() {
    const dmAvatar = document.getElementById('user-avatar-dm');
    if (!dmAvatar) {
      console.warn('[DM-AVATAR] Elemento user-avatar-dm não encontrado');
      return;
    }
    ...
    if (saved && saved.startsWith('data:image')) {
      ...
      dmAvatar.style.backgroundSize = 'cover';
      dmAvatar.style.backgroundPosition = 'center';
      dmAvatar.style.background = 'none'; // remove fallback
      dmAvatar.textContent = ''; // remove as iniciais
      dmAvatar.classList.add('has-image');
      console.log('[DM-AVATAR] Avatar carregado do localStorage');
      return;
    }

    // Se não tem imagem salva, usa as iniciais (fallback)
    const name = window.username || localStorage.getItem('zx_username') || 'Usuário';
    const initial = name.charAt(0).toUpperCase();
    dmAvatar.style.backgroundImage = 'none';
    dmAvatar.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
    dmAvatar.textContent = initial;
    dmAvatar.classList.remove('has-image');
    console.log('[DM-AVATAR] Fallback: usando iniciais');
  }
  ...
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm');
    if (btn) {
      setTimeout(loadDmAvatar, 150);
    }
  });

  // Se a página já estiver na view DM ao carregar, atualiza
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(loadDmAvatar, 200);
    }
  });

  console.log('[DM-AVATAR] Patch aplicado: avatar será carregado na view DM');
})();
...
(function fixDmRealtime() {
  ...
  socket.on('dm:message', function(msg) {
    ...
    // 2. Verifica se a conversa com este usuário está aberta
    const chatArea = document.getElementById('dm-chat-area');
    const activeChat = chatArea?.dataset.activeChat;

    if (activeChat && activeChat.toLowerCase() === sender.toLowerCase()) {
      // A conversa está aberta → adiciona a mensagem na área de mensagens
      const messagesArea = document.getElementById('dm-messages-area');
      if (messagesArea) {
        // Evita duplicação (verifica a última mensagem)
        const lastMsg = messagesArea.lastElementChild;
        if (lastMsg && lastMsg.textContent?.includes(msg.text) && lastMsg.textContent?.includes(msg.from)) {
          console.log('[DM-RT] Mensagem já exibida, ignorando');
          return;
        }
        ...
        messagesArea.appendChild(msgDiv);
        messagesArea.scrollTop = messagesArea.scrollHeight;
        console.log('[DM-RT] Mensagem adicionada ao chat');
      }
    } else {
      console.log('[DM-RT] Chat com', sender, 'não está aberto. Apenas lista atualizada.');
    }
...
(function instantDmPolling() {
  ...
  function pollDmMessages() {
    ...
    if (currentChatUser !== username) {
      ...
      console.log('[POLLING] Monitorando chat com', username, '(a cada 1s)');
    }
  ...
  // Escuta a resposta do servidor com o histórico
  socket.on('dm:history', function(data) {
    if (!data || !data.messages) return;

    const username = data.with || data.username;
    if (!username) return;

    // Verifica se o chat com este usuário está aberto
    const chatArea = document.getElementById('dm-chat-area');
    ...
    const header = document.querySelector('.chat-header .dm-username') ||
                   ...
                   document.querySelector('.dm-username');
    if (!header) return;
    const openUser = header.textContent.trim();
    if (openUser.toLowerCase() !== username.toLowerCase()) return;

    // Salva no localStorage
    const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
    const existing = privateMessages[username] || [];

    // Adiciona apenas mensagens novas (comparação por texto + timestamp)
    const newMessages = data.messages.filter(msg => {
      return !existing.some(ex => ex.text === msg.text && ex.timestamp === msg.timestamp);
    });

    if (newMessages.length === 0) return;

    // Atualiza o localStorage
    privateMessages[username] = [...existing, ...newMessages];
    ...
    console.log('[POLLING] ✅ Chat atualizado com', newMessages.length, 'nova(s) mensagem(ns)');
  });
...
(function stopDmFlicker() {
  ...
  // 2. Bloqueia qualquer innerHTML na área de mensagens
  const messagesArea = document.getElementById('dm-messages-area');
  if (messagesArea) {
    const originalInnerHTML = Object.getOwnPropertyDescriptor(Element.prototype, 'innerHTML');
    Object.defineProperty(messagesArea, 'innerHTML', {
      set: function(value) {
        // Permite apenas se estiver vazio (para inicialização)
        if (this.children.length === 0) {
          originalInnerHTML.set.call(this, value);
        } else {
          console.warn('[FIX] Tentativa de sobrescrever mensagens bloqueada');
        }
      },
      get: function() {
  ...
  // 3. Desativa o polling (opcional – se você quiser que as mensagens cheguem apenas via socket)
  // Comente esta parte se quiser manter o polling, mas com o patch acima ele não vai piscar.
  const clearPolling = setInterval(() => {
    // Procura por intervalos que chamam dm:history e os limpa
    const intervals = [];
    const id = setInterval(() => {}, 1000);
    for (let i = 1; i < id; i++) {
      clearInterval(i);
    }
    console.log('[FIX] Polling desativado (se existia)');
  }, 100);

  console.log('[FIX] Chat DM não vai mais piscar');
})();
...
(function addAvatarsToDmMessages() {
  ...
  function getUserAvatar(username) {
    const currentUser = (window.username || '').toLowerCase();
    const isMe = username.toLowerCase() === currentUser;

    if (isMe) {
      // Tenta carregar o avatar do próprio usuário
      const saved = localStorage.getItem('zx_avatar');
      if (saved && saved.startsWith('data:image')) {
        return saved;
      }
    }
    // Tenta buscar do cache de avatares de amigos
    const cached = getFriendAvatar(username);
    if (cached) return cached;

    return null;
  }

  // Função para criar uma mensagem com avatar e nome
  function createDmMessage(msg, isMe) {
    ...
    // Verifica se há avatar para este usuário
    const avatarData = getUserAvatar(username);
    if (avatarData) {
  ...
  // Intercepta o polling para usar a nova função
  if (window.socket) {
    socket.on('dm:history', function(data) {
      if (!data || !data.messages) return;
      const username = data.with || data.username;
...
(function dmBarFromScratch() {
  ...
  function createBar() {
    ...
    wrapper.innerHTML = `
      ...
      <input id="dm-message-input" type="text" placeholder="Digite sua mensagem..." maxlength="500" autocomplete="off" style="flex:1;padding:10px 14px;background:rgba(255,255,255,0.06);border:none;border-radius:24px;color:#fff;outline:none;font-size:14px;min-width:50px;">
...
(function fixDmSendFinal() {
  ...
  // ── 9. SALVA A FUNÇÃO ORIGINAL (SE EXISTIR) ──
...
btn.addEventListener('click', function(e) {
  e.stopPropagation();
  // Cria um input de arquivo dinâmico
  const input = document.createElement('input');
  input.type = 'file';
  input.accept = 'image/*';
  input.onchange = function() {
    ...
    formData.append('username', username); // envia o nome do amigo
    ...
    .then(data => {
      if (data.success) {
        // Salva localmente e atualiza a UI
        localStorage.setItem('avatar_' + username.toLowerCase(), data.avatar);
        alert('✅ Avatar de ' + username + ' salvo no banco!');
        if (typeof window.renderDmList === 'function') {
          window.renderDmList();
        } else {
          location.reload();
...
async function carregarAvatar(username) {
  try {
    const response = await fetch(`/api/avatar/${encodeURIComponent(username)}`);
    const data = await response.json();
    if (data.avatar) {
      // Aplica a imagem no elemento
      const avatarElement = document.querySelector(`[data-username="${username}"] .avatar`);
      if (avatarElement) {
        avatarElement.style.backgroundImage = `url(${data.avatar})`;
        avatarElement.style.backgroundSize = 'cover';
        avatarElement.textContent = ''; // remove iniciais
      }
    } else {
      // Fallback: iniciais
      mostrarIniciais(username);
    }
  } catch (err) {
...
// ============================================================
// SISTEMA DE AVATAR CENTRALIZADO NO NEON
// ============================================================

// 1. Função para buscar o avatar do Neon
async function buscarAvatarNeon(username) {
  try {
    const response = await fetch(`/api/avatar/${encodeURIComponent(username)}`);
    if (!response.ok) throw new Error('Avatar não encontrado');
    const data = await response.json();
    return data.avatar || null;
  } catch (error) {
    console.warn(`[Avatar] Erro ao buscar avatar de ${username}:`, error);
    return null;
  }
}

// 2. Função para aplicar o avatar em um elemento HTML
function aplicarAvatarEmElemento(elemento, avatarUrl, username) {
  if (!elemento) return;
  if (avatarUrl) {
    ...
    elemento.dataset.hasAvatar = 'true';
  } else {
    // Fallback: iniciais
    const iniciais = (username || '?').charAt(0).toUpperCase();
    elemento.style.backgroundImage = 'none';
    elemento.style.backgroundColor = '#2a0040';
    elemento.textContent = iniciais;
    elemento.dataset.hasAvatar = 'false';
  }
}

// 3. Função para atualizar TODOS os avatares da página
async function atualizarTodosAvatares() {
  // Lista de seletores onde os avatares aparecem
  const seletores = [
    '.msg-avatar',
    '.dm-avatar',
    '.profile-avatar',
  ...
  // Para cada elemento, tenta extrair o nome do usuário
  for (const seletor of seletores) {
    document.querySelectorAll(seletor).forEach(async (el) => {
      // Tenta obter o username de várias formas
      let username = el.dataset.username || 
                     el.getAttribute('data-user') || 
                     el.getAttribute('data-username') ||
                     el.closest('[data-username]')?.dataset?.username ||
                     el.closest('.message')?.querySelector('.msg-username')?.textContent?.trim() ||
                     el.closest('.dm-item')?.querySelector('.dm-username')?.textContent?.trim() ||
                     el.closest('.friend-item')?.querySelector('.friend-name')?.textContent?.trim();
      
      if (!username) return; // se não achar, pula

      // Busca a foto no Neon
      const avatarUrl = await buscarAvatarNeon(username);
      aplicarAvatarEmElemento(el, avatarUrl, username);
    });
  }
}

// 4. Função para fazer upload da foto e atualizar o banco
async function uploadAvatar(username, file) {
  ...
  try {
    ...
    // Após upload bem-sucedido, atualiza todos os avatares
    await atualizarTodosAvatares();
    
    // Se tiver WebSocket, emite evento para outros clientes
    if (window.socket && socket.connected) {
      socket.emit('avatar:updated', { username, avatar: data.avatar });
    }
...
// 5. Configurar o input de arquivo para upload
function configurarUploadAvatar(inputElementId, username) {
  ...
  input.addEventListener('change', async (e) => {
    ...
    if (avatarUrl) {
      // Atualiza o avatar no perfil imediatamente
      const avatarElement = document.querySelector('.profile-avatar-big, .profile-avatar');
      if (avatarElement) {
        aplicarAvatarEmElemento(avatarElement, avatarUrl, username);
      }
    }
...
// 6. Inicialização: ao carregar a página, busca todos os avatares
document.addEventListener('DOMContentLoaded', () => {
  // Atualiza avatares imediatamente
  atualizarTodosAvatares();
  
  // E a cada 10 segundos (para pegar novos elementos que surgirem)
  setInterval(atualizarTodosAvatares, 10000);
  
  // Se tiver WebSocket, escuta atualizações de outros usuários
  if (window.socket) {
    socket.on('avatar:updated', (data) => {
      // Atualiza apenas o avatar do usuário que mudou
      const { username, avatar } = data;
      document.querySelectorAll('[data-username="' + username + '"]').forEach(el => {
        aplicarAvatarEmElemento(el, avatar, username);
      });
    });
  }
});

// 7. Expor funções globalmente (para usar no HTML)
window.buscarAvatarNeon = buscarAvatarNeon;
window.atualizarTodosAvatares = atualizarTodosAvatares;
window.uploadAvatar = uploadAvatar;
window.configurarUploadAvatar = configurarUploadAvatar;
window.aplicarAvatarEmElemento = aplicarAvatarEmElemento;