  ...
  mainArea.innerHTML = `
    ...
    <div style="width:100%;max-width:900px;margin:0 auto;padding:0 20px;">
      <div style="margin-bottom:32px;">
        ...
        <div id="suggested-communities-container" style="display:flex;gap:16px;overflow-x:auto;padding-bottom:12px;scrollbar-width:thin;scrollbar-color:#00ffff rgba(0,0,0,0.2);justify-content:center;align-items:center;">
...
// ââ Configurações: conteúdo das seçőes ââ
const SETTINGS_SECTIONS = {
  ...
  cla: () => {
    const clanItems = userClans.length
      ...
      : '<p class="pf-hint">Você ainda não faz parte de nenhum clă.</p>';
    return `
    ...
    btnCameraTest?.addEventListener('click', async () => {
      if (!cameraActive) {
        try {
          ...
          showToast('Não foi possível acessar a câmera. Verifique permissőes.');
        }
      } else {
    ...
    btnMicTest?.addEventListener('click', async () => {
      if (!micTestActive) {
        try {
          ...
          showToast('Não foi possível acessar o microfone. Verifique permissőes.');
        }
      } else {
        micTestActive = false;
...
document.getElementById('btn-toggle-cam')?.addEventListener('click', async () => {
  if (!isCameraActive) {
    try {
      ...
      showToast('? Não foi possível acessar a webcam. Verifique permissőes.');
      console.error(err);
    }
  } else {
    if (cameraStream) {
...
// ? CORREÇĂO FINAL: Não solicita feed APENAS quando estiver na view do discover
...
function renderSuggestedCommunities() {
  ...
  suggestedCommunities.forEach(community => {
    ...
    card.style.cssText = 'min-width:220px;height:280px;border-radius:16px;overflow:hidden;border:2px solid rgba(255,0,255,0.3);cursor:pointer;position:relative;flex-shrink:0';
    card.innerHTML = community.banner
      ? `<img src="${community.banner}" style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0" /><div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.9));padding:16px"><div style="color:#fff;font-weight:700">${escHtml(community.name)}</div></div>`
      : `<div style="width:100%;height:100%;background:linear-gradient(180deg,rgba(128,0,255,0.2),rgba(0,0,0,0.8));display:flex;flex-direction:column;align-items:center;justify-content:flex-end;padding:20px;gap:8px"><div style="font-size:48px">🌐</div><div style="color:#fff;font-weight:700">${escHtml(community.name)}</div></div>`;
    card.addEventListener('click', () => openServer(community.id));
...
const SERVER_SETTINGS_SECTIONS = {
  ...
  eventos: (server) => {
    const evs = (server.events || []).map((ev, i) =>
      `<div class="srv-channel-row"><span>đ ${escHtml(ev.name)} — ${escHtml(ev.date)}</span>
  ...
  integracoes: (server) => {
    ...
    <div class="ms-block">
      ...
      <p class="pf-hint" style="margin-top:0.75rem;font-size:0.8rem">
        💡 Para criar um webhook no Discord: Configurações do Servidor → Integraçőes → Webhooks → Novo Webhook
      </p>
    </div>
...
function bindServerSettingsEvents(sectionId, server) {
  ...
  if (sectionId === 'integracoes') {
    ...
    document.getElementById('btn-save-discord-integration')?.addEventListener('click', () => {
      ...
      if (webhookUrl) {
        showToast('? Integração com Discord salva! Mensagens serăo sincronizadas.');
      } else {
  ...
  pane.innerHTML = friendRequests.map(r => `
    <div class="friend-card">
      ...
      <div class="friend-actions">
        <button class="friend-btn friend-btn-accept" data-accept="${escHtml(r.from)}">✔</button>
...
function friendCard(uname, isOnline) {
  ...
  return `
    <div class="friend-card">
      ...
      <div class="friend-actions">
        <button class="friend-btn friend-btn-remove" data-remove="${escHtml(uname)}" title="Remover amigo">🗑️</button>
      </div>
    </div>`;
}
