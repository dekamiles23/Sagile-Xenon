...
// ============================================================
// BARRA DE INPUT DO DM COM PICKERS CRIADOS DO ZERO
// ============================================================
(function buildDmInputWithPickers() {
  function buildBar() {
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) return;

    // ── Cria a barra de input ──
    const wrapper = document.createElement('div');
    wrapper.className = 'input-wrapper';
    wrapper.style.cssText = 'display:flex;align-items:center;gap:6px;position:relative;';

    wrapper.innerHTML = `
      <button type="button" class="input-action-btn" id="dm-btn-plus" title="Mais opções">+</button>
      <button type="button" class="input-action-btn" id="dm-btn-emoji" title="Emojis">😀</button>
      <button type="button" class="input-action-btn" id="dm-btn-gif" title="GIFs">🎬</button>
      <button type="button" class="input-action-btn" id="dm-btn-stickers" title="Figurinhas">🎭</button>
      <button type="button" class="input-action-btn" id="dm-btn-attach" title="Anexar arquivo">📎</button>
      <input id="dm-message-input" type="text" placeholder="Digite sua mensagem..." maxlength="500" autocomplete="off" style="flex:1;padding:10px 14px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,0,255,0.3);border-radius:24px;color:#fff;outline:none;font-size:14px;">
      <button type="button" class="input-action-btn" id="dm-btn-voice" title="Gravar áudio">🎤</button>
      <button id="dm-send-btn" type="button" style="background:linear-gradient(135deg,#8b00ff,#ff00ff);border:none;border-radius:50%;width:44px;height:44px;color:#fff;font-size:18px;cursor:pointer;">➤</button>
    `;
    ...
    function createPicker(pickerId, content) {
    ...
    // ── Emoji picker ──
    const emojiPicker = createPicker('dm-emoji-picker', `
      <div class="emoji-categories">
        ...
        <button class="emoji-category-btn" data-category="objects">❤️</button>
    ...
    // ── GIF picker (simples) ──
    const gifPicker = createPicker('dm-gif-picker', `
    ...
    const stickerPicker = createPicker('dm-sticker-picker', `
      <div class="sticker-tabs" style="display:flex;border-bottom:1px solid rgba(255,0,255,0.2);background:rgba(0,0,0,0.3);overflow-x:auto;">
        <button class="sticker-pack-btn active" data-pack="default">Padrão</button>
        <button class="sticker-pack-btn" data-pack="reactions">Reações</button>
      </div>
    ...
    function connectPicker(btnId, pickerId, itemSelector, insertFn) {
      ...
      btn.addEventListener('click', function(e) {
        ...
        picker.style.display = picker.classList.contains('active') ? 'flex' : 'none';
    ...
    console.log('[DM] Barra com pickers próprios criada');
  }

  // Executa ao carregar e ao abrir DM
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    buildBar();
  } else {
