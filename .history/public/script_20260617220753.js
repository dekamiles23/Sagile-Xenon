...
// ============================================================
// DESATIVA O PISCAR: remove a classe de animação
// ============================================================
(function stopFlashing() {
  // 1. Remove a classe de animação de todas as mensagens existentes
  document.querySelectorAll('.message-new').forEach(el => {
    el.classList.remove('message-new');
  });

  // 2. Sobrescreve a adição de novas mensagens para não aplicar animação
  const originalAppendChild = Node.prototype.appendChild;
  Node.prototype.appendChild = function(child) {
    if (child.classList && child.classList.contains('message-new')) {
      child.classList.remove('message-new');
    }
    return originalAppendChild.call(this, child);
  };

  // 3. Desativa qualquer polling ou intervalo que esteja causando o piscar
  // Isso interrompe a atualização automática (opcional)
  const intervals = [];
  const originalSetInterval = window.setInterval;
  window.setInterval = function(fn, delay) {
    // Se for o polling do DM (delay 2000), substitui por um intervalo mais longo ou para
    if (delay === 2000 && fn.toString().includes('dm:history')) {
      console.log('[STOP] Polling do DM desativado');
      return null;
    }
    return originalSetInterval.call(this, fn, delay);
  };

  console.log('[STOP] Animação de piscada desativada');
})();