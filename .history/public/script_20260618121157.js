...
// ============================================================
// BOTÃO 📷 INTEGRADO NA RENDERIZAÇÃO DA LISTA DE DMs
// ============================================================
(function persistentUploadButton() {
  console.log('[PERSIST] Iniciando...');
  ...
  function addButtonToItem(item) {
    ...
    console.log('[PERSIST] Botão adicionado a', friendName);
  }

  // 3. Sobrescreve renderDmList (se existir) para incluir o botão
  if (typeof window.renderDmList === 'function') {
    const originalRender = window.renderDmList;
    window.renderDmList = function() {
      originalRender();
      // Após renderizar, adiciona botões em todos os itens
      setTimeout(() => {
    ...
    console.log('[PERSIST] renderDmList sobrescrita');
  }

  // 4. Adiciona botões em itens existentes (caso a lista já esteja pronta)
  ...
  // 5. Observa a lista para novos itens (MutationObserver)
  const list = document.getElementById('dm-conversations-list');
  if (list) {
    const observer = new MutationObserver(() => {
      addButtonsToAll();
    ...
    console.log('[PERSIST] Observador ativado');
  }

  // 6. Executa imediatamente e com delays (garantia)
  addButtonsToAll();
  setTimeout(addButtonsToAll, 300);
  setTimeout(addButtonsToAll, 600);
  setTimeout(addButtonsToAll, 1000);

  // 7. Também reage quando a view DM for aberta
  ...
  console.log('[PERSIST] Sistema pronto. Os botões 📷 serão mantidos.');
})();