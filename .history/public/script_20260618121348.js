...
// ============================================================
// FORÇA BOTÃO 📷 EM QUALQUER LISTA DE DMs (AGRESSIVO)
// ============================================================
(function forceButtonEverywhere() {
  console.log('[FORCE-BTN] Iniciando...');
  ...
  // 2. Função que adiciona o botão em um item
  function addButtonToItem(item) {
    if (item.querySelector('.dm-upload-btn')) return;
    const nameEl = item.querySelector('.dm-conv-name, .username, .name, .friend-name');
    if (!nameEl) return;
    ...
    console.log('[FORCE-BTN] Botão adicionado a', friendName);
  }

  // 3. Adiciona em todos os itens existentes
  function scanAndAdd() {
    const items = document.querySelectorAll('#dm-conversations-list .dm-conv-item, .dm-conv-item, .conv-item, [class*="conv-item"]');
    items.forEach(item => addButtonToItem(item));
  }

  // 4. Executa a cada 1 segundo (polling agressivo)
  setInterval(scanAndAdd, 1000);

  // 5. Também observa mudanças no DOM
  ...
  console.log('[FORCE-BTN] Sistema ativo. Botão será adicionado a cada 1 segundo.');
})();