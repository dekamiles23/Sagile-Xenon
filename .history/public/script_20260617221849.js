...
// ============================================================
// COPIA BARRA DO SERVIDOR PARA DM (COM TODOS OS BOTÕES FUNCIONAIS)
// ============================================================
(function copyInputBarFull() {
  function copyBar() {
    ...
    if (!serverInput) {
      console.warn('[COPY] Barra do servidor não encontrada');
      return;
    }
    ...
    // ── Pickers (emoji, GIF, stickers) ──
    function setupPicker(btnId, pickerId) {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn || !picker) return;
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        document.querySelectorAll('.chat-picker.active').forEach(p => p.classList.remove('active'));
        picker.classList.toggle('active');
        if (input) input.focus();
      });
    }
    setupPicker('dm-btn-emoji', 'emoji-picker');
    setupPicker('dm-btn-gif', 'gif-picker');
    setupPicker('dm-btn-stickers', 'sticker-picker');
        ...
        files.forEach(file => {
          ...
          reader.onload = function(e) {
            ...
            if (typeof window.addDmMessageToDom === 'function') {
              window.addDmMessageToDom({ text: msg.text, from: window.username }, true);
            } else {
              const messagesArea = document.getElementById('dm-messages-area');
              if (messagesArea) {
                const div = document.createElement('div');
        ...
        try {
          ...
          mediaRecorder.onstop = () => {
            ...
            reader.onload = function(e) {
              ...
              if (typeof window.addDmMessageToDom === 'function') {
                window.addDmMessageToDom({ text: '🎤 Áudio enviado', from: window.username }, true);
              } else {
                const messagesArea = document.getElementById('dm-messages-area');
                if (messagesArea) {
                  const div = document.createElement('div');
    ...
    if (plusBtn) {
      plusBtn.addEventListener('click', function(e) {
        ...
        `;
        menu.innerHTML = `
          <div class="plus-item" data-action="clear" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;">🗑️ Limpar histórico</div>
          <div class="plus-item" data-action="pin" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;">📌 Fixar conversa</div>
          <div class="plus-item" data-action="mute" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;">🔇 Silenciar notificações</div>
        `;
        menu.querySelectorAll('.plus-item').forEach(item => {
          ...
          item.addEventListener('click', function() {
            ...
            if (action === 'clear') {
              ...
              alert('📌 Conversa fixada (simulado)');
            } else if (action === 'mute') {
              alert('🔇 Notificações silenciadas (simulado)');
            }
            menu.remove();
          });
        });
    ...
    console.log('[COPY] Barra completa copiada e funcional');
  }
