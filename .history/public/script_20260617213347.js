...
(function fixDmAvatar() {
  // Função que carrega o avatar do localStorage e aplica no elemento da DM
  function loadDmAvatar() {
    // Tenta encontrar o elemento de avatar na DM
    let dmAvatar = document.getElementById('user-avatar-dm');
    
    // Se não encontrar, tenta buscar dentro da view DM
    if (!dmAvatar) {
      const dmView = document.getElementById('dm-view');
      if (dmView) {
        dmAvatar = dmView.querySelector('#user-avatar-dm');
      }
    }
    ...
    // Tenta carregar do localStorage (prioridade)
    const saved = localStorage.getItem('user_avatar');
    if (saved && saved.startsWith('data:image')) {
      dmAvatar.style.backgroundImage = `url(${saved})`;
      dmAvatar.style.backgroundSize = 'cover';
  ...
  // Intercepta a abertura da view DM (via showLayout)
  const originalShowLayout = window.showLayout;
  if (originalShowLayout) {
    window.showLayout = function(layoutId) {
      originalShowLayout.call(this, layoutId);
      if (layoutId === 'dm-view') {
        // Aguarda o DOM ser renderizado e carrega o avatar
        setTimeout(loadDmAvatar, 200);
        // Tenta novamente após um tempo maior (caso o elemento seja criado dinamicamente)
        setTimeout(loadDmAvatar, 500);
      }
    };
  }
  ...
  // Força o carregamento sempre que a visibilidade da página mudar (ex: voltar de outra aba)
