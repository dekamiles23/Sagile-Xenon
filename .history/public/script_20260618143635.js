...
// ============================================================
// SISTEMA DE AVATAR CENTRALIZADO NO NEON (CORRIGIDO)
// ============================================================

// Cache para evitar requisições repetidas
const avatarCache = new Map();
let atualizandoAvatares = false;
let intervalId = null;

// 1. Função para buscar avatar com cache
async function buscarAvatarNeon(username) {
  ...
  try {
    ...
    avatarCache.set(username, null); // cache do erro
    return null;
  }
}

// 2. Aplicar avatar no elemento
...
// 3. Atualizar TODOS os avatares (com controle de execução)
async function atualizarTodosAvatares() {
  // Evita execução simultânea
  if (atualizandoAvatares) {
    console.log('[Avatar] Já está atualizando, ignorando...');
    return;
  }
  
  atualizandoAvatares = true;
  ...
  try {
    ...
    // Coleta todos os elementos únicos com seu username
    const elementosPorUsuario = new Map();
    ...
    for (const seletor of seletores) {
      document.querySelectorAll(seletor).forEach(el => {
        ...
        if (!elementosPorUsuario.has(username)) {
          elementosPorUsuario.set(username, []);
        }
        elementosPorUsuario.get(username).push(el);
      });
    }

    // Busca cada usuário uma única vez
    const promises = [];
    for (const [username, elements] of elementosPorUsuario) {
      promises.push(
        buscarAvatarNeon(username).then(avatarUrl => {
          elements.forEach(el => aplicarAvatarEmElemento(el, avatarUrl, username));
        })
      );
    }
    
    await Promise.all(promises);
    console.log('[Avatar] Atualização concluída.');
  } catch (error) {
    console.error('[Avatar] Erro na atualização:', error);
  } finally {
    atualizandoAvatares = false;
  }
}
...
async function uploadAvatar(username, file) {
  ...
  try {
    ...
    // Atualiza apenas os avatares desse usuário
    document.querySelectorAll(`[data-username="${username}"]`).forEach(el => {
      aplicarAvatarEmElemento(el, data.avatar, username);
    });
...
// 6. Inicialização (apenas uma vez)
...
function initAvatarSystem() {
  ...
  // Atualiza avatares ao carregar
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(atualizarTodosAvatares, 500);
  });
  
  // Atualiza avatares quando a página mudar (ex: navegação SPA)
  // Usa MutationObserver para detectar novos elementos
  const observer = new MutationObserver(() => {
    if (!atualizandoAvatares) {
      // Debounce: espera 500ms após a última mudança
      clearTimeout(window._avatarTimeout);
      window._avatarTimeout = setTimeout(atualizarTodosAvatares, 500);
    }
  });
  ...
  if (window.socket) {
    socket.on('avatar:updated', (data) => {
      ...
      if (avatar) avatarCache.set(username, avatar);
      document.querySelectorAll(`[data-username="${username}"]`).forEach(el => {
  ...
  console.log('[Avatar] Sistema inicializado.');
}
