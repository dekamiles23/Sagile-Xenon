...
(function autoAvatar() {
  ...
  // ── 2. Função para obter avatar (prioriza cache, depois localStorage, depois servidor) ──
  function getAvatar(username, callback) {
    ...
    // Cache em memória
    if (avatarCache[key]) {
      return callback(avatarCache[key]);
    }
    ...
    if (stored && stored.startsWith('data:image')) {
      avatarCache[key] = stored;
      return callback(stored);
    }

    // Se não tem em cache, pede ao servidor
    if (window.socket && window.socket.connected) {
      console.log('[AUTO-AVATAR] Solicitando avatar de', username);
      window.socket.emit('get-avatar', { username: username });
      // Aguarda resposta (o listener está abaixo)
      // Por enquanto, retorna null (callback será chamado quando chegar)
  ...
  function applyAvatarToElement(el, username) {
    ...
    const key = username.toLowerCase();
    const avatarData = avatarCache[key] || localStorage.getItem('avatar_' + key) || localStorage.getItem('zx_avatar');
  ...
  // ── 5. Atualiza todos os avatares na UI ──
  function refreshAvatarsInUI() {
    document.querySelectorAll('.msg-avatar, .message .avatar, .dm-message .avatar').forEach(el => {
      ...
      if (username) {
        applyAvatarToElement(el, username);
      }
    });
    console.log('[AUTO-AVATAR] UI atualizada');
  }

  // ── 6. Observa novas mensagens ──
  function observeNewMessages() {
    const target = document.getElementById('dm-messages-area') || document.getElementById('private-chat-messages');
    if (!target) return;
    const observer = new MutationObserver(() => {
      // Quando novas mensagens são adicionadas, tenta aplicar avatar
      setTimeout(refreshAvatarsInUI, 50);
    });
    observer.observe(target, { childList: true, subtree: true });
    console.log('[AUTO-AVATAR] Observador de mensagens ativado');
  }

  // ── 7. Quando o chat é aberto, solicita avatares dos participantes ──
  function onChatOpen(username) {
    if (!username) return;
    // Se não tiver avatar do amigo em cache, pede ao servidor
    const key = username.toLowerCase();
    if (!avatarCache[key] && !localStorage.getItem('avatar_' + key)) {
      if (window.socket && window.socket.connected) {
        window.socket.emit('get-avatar', { username: username });
      }
    }
    // Atualiza a UI
    setTimeout(refreshAvatarsInUI, 200);
    setTimeout(refreshAvatarsInUI, 500);
  }

  // ── 8. Hook para abertura de chat ──
  // Intercepta openPrivateChat ou openDmChat
  const origOpenChat = window.openPrivateChat || window.openDmChat;
  if (origOpenChat) {
    window.openPrivateChat = function(username) {
      origOpenChat.call(this, username);
      setTimeout(() => onChatOpen(username), 300);
    };
    window.openDmChat = function(username) {
      if (typeof window.openPrivateChat === 'function') {
        window.openPrivateChat(username);
  ...
  // ── 9. Quando a view DM é aberta ──
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        // Tenta obter o nome do amigo do cabeçalho
        setTimeout(() => {
          const nameEl = document.querySelector('.dm-username, .username, .chat-header .username');
          if (nameEl) {
            const username = nameEl.textContent.trim();
            if (username) onChatOpen(username);
          }
        }, 300);
      }
    };
  ...
  document.addEventListener('DOMContentLoaded', function() {
    observeNewMessages();
    // Se a DM já estiver aberta, tenta carregar avatares
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(() => {
        const nameEl = document.querySelector('.dm-username, .username');
        if (nameEl) onChatOpen(nameEl.textContent.trim());
      }, 400);
    }
  });

  // ── 11. Fallback: clique na lista de DMs ──
  document.addEventListener('click', function(e) {
    const item = e.target.closest('.dm-conv-item');
    if (item) {
      const username = item.dataset.username || item.textContent.trim();
      setTimeout(() => onChatOpen(username), 300);
    }
  });

  // ── Expõe funções para debug ──
  window.refreshAvatarsUI = refreshAvatarsInUI;
...
// ============================================================
// CORREÇÃO DE CLIQUE NA LISTA DE DMs (FORÇADO)
// ============================================================
(function fixDmListClick() {
  console.log('[DM-CLICK] Aplicando correção de clique...');

  // Função que abre o chat com um usuário
  function openChatWith(username) {
    if (!username) return;
    // Evita abrir consigo mesmo
    const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
    if (username.toLowerCase() === currentUser) {
      console.log('[DM-CLICK] Não é possível abrir chat consigo mesmo');
      return;
    }

    console.log('[DM-CLICK] Abrindo chat com:', username);

    // Tenta diferentes funções de abertura
    if (typeof window.openDmChat === 'function') {
      window.openDmChat(username);
    } else if (typeof window.openPrivateChat === 'function') {
      window.openPrivateChat(username);
    } else {
      // Fallback: simula clique no botão de DM e depois tenta abrir
  ...
  // ── Event delegation com capture: true para garantir prioridade ──
  document.addEventListener('click', function(e) {
    // Procura o item da lista de DMs mais próximo
    const item = e.target.closest('.dm-conv-item');
    if (!item) return;

    // Obtém o nome do usuário (de dataset ou texto)
    ...
    // Previne que outros handlers processem o mesmo clique
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
  ...
  }, true); // capture: true para ser chamado antes de outros

  // ── Também adiciona um listener no próprio item (redundante, mas seguro) ──
  // Isso garante que mesmo se o delegation falhar, o clique direto funcione
  function attachDirectListeners() {
    document.querySelectorAll('.dm-conv-item').forEach(item => {
      if (item.dataset.clickFixed) return;
      ...
      item.addEventListener('click', function(e) {
        ...
        if (!username) {
          const nameEl = this.querySelector('.dm-conv-name');
          if (nameEl) username = nameEl.textContent.trim();
          else username = this.textContent.trim();
        }
  ...
  // Observa a lista de DMs para anexar listeners a novos itens
  ...
  function startObserver() {
    ...
    if (list) {
      ...
      attachDirectListeners();
      console.log('[DM-CLICK] Observador ativado na lista de DMs');
    } else {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    startObserver();
    // Se a DM já estiver aberta, também aplica
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(attachDirectListeners, 200);
    }
  });

  // Fallback: quando a view DM for aberta
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  console.log('[DM-CLICK] Correção aplicada. Clique nos itens da lista para abrir o chat.');
})();