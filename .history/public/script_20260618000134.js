...
// ============================================================
// CORREÇÃO: AVATAR COM DEBOUNCE E SEM PISCAR
// ============================================================
(function fixAvatarBlink() {
  console.log('[AVATAR-BLINK] Aplicando correção...');

  let updateTimeout = null;

  // Função que aplica avatar com debounce
  function updateAvatarsSafely() {
    if (updateTimeout) {
      clearTimeout(updateTimeout);
      updateTimeout = null;
    }
    updateTimeout = setTimeout(() => {
      console.log('[AVATAR-BLINK] Aplicando avatares (debounced)');
      applyAvatarsNow();
      updateTimeout = null;
    }, 100);
  }

  // Função real de aplicação (chamada uma vez)
  function applyAvatarsNow() {
    // Lista de DMs
    document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
      ...
      if (avatarEl && nameEl) {
        ...
        if (username) applyAvatarSafe(avatarEl, username);
      }
    });
    // Mensagens
    document.querySelectorAll('.msg-avatar').forEach(el => {
      ...
      if (nameEl) {
        const username = nameEl.textContent.trim();
        if (username) applyAvatarSafe(el, username);
      }
    });
  }

  // Aplica avatar com fallback e sem piscar
  function applyAvatarSafe(el, username) {
    if (!el || !username) return;
    ...
    const currentUser = (window.username || '').toLowerCase();
    if (key === currentUser) {
      img = localStorage.getItem('zx_avatar');
    } else {
      img = localStorage.getItem('avatar_' + key);
      if (!img && window._avatarCache) img = window._avatarCache[key];
      if (!img && window._friendAvatars) img = window._friendAvatars[key];
    }

    if (img && img.startsWith('data:image')) {
      // Só aplica se a imagem for diferente da atual (evita re-render)
  ...
  // ── Sobrescreve a função de atualização global ──
  window.updateAllAvatars = updateAvatarsSafely;

  // ── Intercepta renderDmList ──
  if (typeof window.renderDmList === 'function') {
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      ...
      if (layoutId === 'dm-view') {
        setTimeout(updateAvatarsSafely, 100);
        setTimeout(updateAvatarsSafely, 300);
      }
    };
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(updateAvatarsSafely, 200);
    }
    // Aplica imediatamente após carregar
    setTimeout(updateAvatarsSafely, 500);
  });

  // ── Fallback: clique na lista ──
  document.addEventListener('click', function(e) {
    if (e.target.closest('.dm-conv-item')) {
      setTimeout(updateAvatarsSafely, 150);
    }
  });

  // ── Expõe função forçada ──
  window.forceAvatarUpdate = updateAvatarsSafely;

  console.log('[AVATAR-BLINK] Correção aplicada. O piscar deve parar.');
})();