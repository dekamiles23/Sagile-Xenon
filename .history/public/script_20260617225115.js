...
// ============================================================
// MOVE A BARRA DO SERVIDOR PARA O DM (COM VERIFICAÇÕES)
// ============================================================
(function moveServerBarToDM() {
  let barraElement = null;
  let containerOriginal = null;
  let dmViewObserver = null;

  function moverBarraParaDM() {
    ...
    // Se a barra já está no DM, não faz nada
    if (dmInputArea.querySelector('.input-wrapper')) {
      console.log('[DM-BAR] Barra já está no DM');
      return true;
    }

    // Procura a barra no servidor
    let serverWrapper = document.querySelector('#chat-view .message-input-area .input-wrapper');
    if (!serverWrapper) {
      // Tenta novamente após 500ms
      setTimeout(() => moverBarraParaDM(), 500);
      return false;
    }

    // Guarda o container original
    containerOriginal = serverWrapper.parentElement;
    barraElement = serverWrapper;

    // Move a barra para o DM
    dmInputArea.innerHTML = '';
    dmInputArea.appendChild(serverWrapper);
    console.log('[DM-BAR] ✅ Barra movida para o DM');

    // Garante que os pickers estejam no body
    ['emoji-picker', 'gif-picker', 'sticker-picker'].forEach(id => {
      const picker = document.getElementById(id);
      if (picker && picker.parentNode !== document.body) {
        document.body.appendChild(picker);
        console.log(`[DM-BAR] Pickers ${id} movido para o body`);
      }
    });
  ...
  function devolverBarraParaServidor() {
    if (!barraElement || !containerOriginal) {
      console.log('[DM-BAR] Nada para devolver');
      return;
    }

    const dmInputArea = document.getElementById('dm-input-area');
    if (dmInputArea && dmInputArea.contains(barraElement)) {
      dmInputArea.innerHTML = ''; // limpa o DM
    }

    // Devolve a barra ao lugar original
    containerOriginal.appendChild(barraElement);
    console.log('[DM-BAR] ✅ Barra devolvida ao servidor');
  ...
  function setupObserver() {
    ...
    if (dmViewObserver) dmViewObserver.disconnect();

    dmViewObserver = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        // DM aberto
        console.log('[DM-BAR] DM aberto, movendo barra...');
        setTimeout(() => moverBarraParaDM(), 100);
      } else {
        // DM fechado
        console.log('[DM-BAR] DM fechado, devolvendo barra...');
        setTimeout(() => devolverBarraParaServidor(), 100);
      }
    });

    dmViewObserver.observe(dmView, { attributes: true, attributeFilter: ['class'] });

    // Se a DM já estiver visível, move a barra imediatamente
    if (!dmView.classList.contains('hidden')) {
      setTimeout(() => moverBarraParaDM(), 200);
    }

    console.log('[DM-BAR] Observador ativado');
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(() => moverBarraParaDM(), 150);
    }
  });

  // ── Força a movimentação se a DM estiver visível ao carregar ──
  window.addEventListener('load', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(() => moverBarraParaDM(), 300);
    }
  });

  console.log('[DM-BAR] Script de movimentação carregado');
})();