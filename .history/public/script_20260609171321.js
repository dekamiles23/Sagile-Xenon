...
function readImageFile(file, callback) {
  ...
  if (file.size > MAX_IMAGE_SIZE) {
    showToast('Imagem muito grande. Máximo 2 MB.');
    return;
  }
...
function renderSidebarChannels(server) {
  ...
  for (const [cat, channels] of Object.entries(categories)) {
    ...
    catHeader.innerHTML = `<span>${cat}</span><button class="ch-cat-add" data-cat="${cat}" title="Criar canal nesta categoria">+</button>`;
...
document.getElementById('dd-create-category').addEventListener('click', () => {
  serverDropdown.classList.add('hidden');
  openServerSettingsModal('categorias');
});
...
const SETTINGS_SECTIONS = {
  conta: () => `
    ...
    <p class="ms-section-desc">Gerencie suas informações pessoais e segurança.</p>
    <div class="ms-block">
      <div class="ms-block-title">Informações do usuário</div>
      <div class="ms-field"><label>Nome de usuário</label><input type="text" id="set-username" value="${escHtml(username)}" maxlength="32" /></div>
      <div class="ms-field"><label>E-mail</label><input type="email" placeholder="seu@email.com" /></div>
      <div class="ms-field"><label>Telefone</label><input type="tel" placeholder="+55 (00) 00000-0000" /></div>
      <div class="ms-field"><label>Classificação</label><select><option>Membro</option><option>Nitro (em breve)</option><option>Moderador</option></select></div>
    </div>
    <div class="ms-block">
      <div class="ms-block-title">Segurança</div>
      <div class="ms-field"><label>Senha</label><input type="password" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" /></div>
      <label class="toggle-row"><span>Autenticação em duas etapas</span><input type="checkbox" /></label>
  ...
  privacidade: () => `
    ...
    <div class="ms-block">
      <label class="toggle-row"><span><span>Usar dados do programa</span><small>Permite melhorar recomendações e experiência</small></span><input type="checkbox" checked /></label>
      <div class="ms-actions"><button type="button" class="btn-ms">Solicitar meus dados</button></div>
    </div>`,
  ligamentos: () => `
    <h2 class="ms-section-title">Ligamentos</h2>
    <p class="ms-section-desc">Conecte suas redes sociais ao perfil.</p>
    <div class="social-grid">
      <button type="button" class="social-btn" data-social="steam">🎮 Steam</button>
      <button type="button" class="social-btn" data-social="twitch">ðŸ“º Twitch</button>
      <button type="button" class="social-btn" data-social="youtube">â–¶ YouTube</button>
      <button type="button" class="social-btn" data-social="twitter">ðŸ¦ X / Twitter</button>
      <button type="button" class="social-btn" data-social="spotify">🎵 Spotify</button>
      <button type="button" class="social-btn" data-social="github">ðŸ’» GitHub</button>
    </div>`,
  voz: () => `
    <h2 class="ms-section-title">Voz e vídeo</h2>
    <p class="ms-section-desc">Configure áudio e câmera para chamadas.</p>
    <div class="ms-block">
      <div class="ms-block-title">Áudio</div>
      <div class="ms-field"><label>Microfone</label><select><option>Microfone padrão</option><option>Headset</option></select></div>
      <div class="ms-field"><label>Alto-falante</label><select><option>Alto-falante padrão</option><option>Headset</option></select></div>
      <div class="ms-field"><label>Volume do microfone</label><input type="range" min="0" max="100" value="80" id="mic-volume-slider" /></div>
      <div class="ms-field"><label>Volume do alto-falante</label><input type="range" min="0" max="100" value="70" /></div>
    ...
    <div class="ms-block">
      <div class="ms-block-title">Noise & câmera</div>
      <label class="toggle-row"><span>Supressão de ruído no microfone</span><input type="checkbox" checked /></label>
      <div class="ms-field"><label>Webcam</label><select><option>Desativada</option><option>Câmera padrão</option></select></div>
      <div class="ms-field"><label>Layout na transmissão (em call)</label><select><option>Tela cheia</option><option>Picture-in-picture</option><option>Lateral</option></select></div>
    </div>`,
  aparencia: () => {
  ...
  const custom = localStorage.getItem('zx_wallpaper_custom') || '';
    ...
    <div class="ms-block">
      ...
      <label class="toggle-row"><span>Mostrar mídia incorporada</span><input type="checkbox" id="toggle-embed-media" checked /></label>
      <label class="toggle-row"><span>Borrão de spoiler em imagens</span><input type="checkbox" id="toggle-spoiler-blur" checked /></label>
      <label class="toggle-row"><span>Prévia de links</span><input type="checkbox" id="toggle-link-preview" checked /></label>
  ...
  acessibilidade: () => `
    ...
    <div class="ms-block">
      ...
      <div class="ms-field"><label>Espaço entre grupos de mensagens</label><input type="range" min="0" max="24" value="4" /></div>
      <div class="ms-field"><label>Zoom da interface</label><input type="range" min="80" max="125" value="100" /></div>
      <div class="ms-field"><label>Modo daltônico</label><select><option>Desativado</option><option>Protanopia</option><option>Deuteranopia</option><option>Tritanopia</option></select></div>
      <label class="toggle-row"><span>Sons do Discord para guiar (cegueira)</span><input type="checkbox" /></label>
      <label class="toggle-row"><span>Cores de cargos acessíveis</span><input type="checkbox" checked /></label>
      <label class="toggle-row"><span>Alto contraste</span><input type="checkbox" /></label>
    </div>`,
  idioma: () => `
    <h2 class="ms-section-title">Idioma</h2>
    <p class="ms-section-desc">Selecione o idioma do programa.</p>
    <div class="lang-options">
      <label class="lang-opt"><input type="radio" name="app-lang" value="en" /> English</label>
      <label class="lang-opt"><input type="radio" name="app-lang" value="pt-BR" checked /> Português (Brasil)</label>
      <label class="lang-opt"><input type="radio" name="app-lang" value="pt-PT" /> Português (Portugal)</label>
      <label class="lang-opt"><input type="radio" name="app-lang" value="es" /> Español</label>
    </div>`,
  atalhos: () => `
  ...
  sobreposicao: () => `
    ...
    <div class="ms-block">
      <label class="toggle-row"><span>Ativar sobreposição em jogos</span><input type="checkbox" id="overlay-toggle" checked /></label>
      <label class="toggle-row"><span>Mostrar avatares em call</span><input type="checkbox" checked /></label>
      <label class="toggle-row"><span>Mostrar nomes dos participantes</span><input type="checkbox" checked /></label>
      <div class="ms-field"><label>Posição na tela</label><select><option>Canto superior direito</option><option>Canto inferior direito</option><option>Centro</option></select></div>
    </div>`,
  atividades: () => `
    <h2 class="ms-section-title">Config. de atividades</h2>
    <p class="ms-section-desc">Controle se jogos aparecem no seu perfil.</p>
    <div class="ms-block">
      <label class="toggle-row"><span>Exibir jogo atual no perfil</span><input type="checkbox" checked /></label>
      <label class="toggle-row"><span>Compartilhar atividade com amigos</span><input type="checkbox" checked /></label>
      <label class="toggle-row"><span>Detectar jogos automaticamente</span><input type="checkbox" checked /></label>
    </div>`,
};
    ...
    btnMicTest?.addEventListener('click', async () => {
      if (!micTestActive) {
        try {
          ...
          micTestActive = true;
          btnMicTest.textContent = 'â¹ Parar teste';
