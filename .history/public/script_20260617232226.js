...
// ============================================================
// STATUS EM TEMPO REAL NA BARRA DO CHAT DM
// ============================================================
(function dmStatusRealtime() {
  // Mapeamento de status para cor e texto
  const STATUS_MAP = {
  ...
  function updateDmStatus() {
    ...
    console.log('[DM-STATUS] Atualizado para', username, '->', status);
  }

  // Atualiza ao receber evento de status individual
  if (window.socket) {
    socket.on('friend:status', function(data) {
      if (data && data.username) {
        // Verifica se o chat DM está aberto com este usuário
        const usernameEl = document.querySelector('.dm-username');
    ...
    socket.on('friends:presence', function() {
      // Atualiza o status do chat atual se houver
      const chatArea = document.getElementById('dm-chat-area');
      if (chatArea && !chatArea.classList.contains('hidden')) {
        updateDmStatus();
      }
  ...
  // Quando a view DM é aberta, atualiza o status
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  // Quando o chat é aberto via clique, atualiza
  document.addEventListener('click', function(e) {
    if (e.target.closest('.dm-conv-item') || e.target.closest('#btn-dm-list') || e.target.closest('#btn-dm-list-sidebar') || e.target.closest('#btn-dm-list-dm')) {
      setTimeout(updateDmStatus, 300);
      setTimeout(updateDmStatus, 600);
    }
  });

  // Também escuta a abertura do modal de chat privado (se for usado)
  const origOpenPrivateChat = window.openPrivateChat;
  if (origOpenPrivateChat) {
  ...
  // Inicializa se a DM já estiver aberta
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(updateDmStatus, 400);
    }
  });

  console.log('[DM-STATUS] Sistema de status em tempo real ativado');
})();