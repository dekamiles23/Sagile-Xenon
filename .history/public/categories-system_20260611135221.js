...
document.addEventListener('click', (e) => {
  ...
  if (e.target.dataset.action === 'suggest' || e.target.closest('[data-action="suggest"]')) {
    ...
    if (serverId) {
      ...
      if (server) {
        // ✅ USAR A FUNÇÃO QUE JÁ EXISTE E FUNCIONA NO INDEX.HTML
        if (window.addToSuggestedCommunities) {
          window.addToSuggestedCommunities(server);
          console.log('✅ Chamada função addToSuggestedCommunities com sucesso!');
        }
        
        // ✅ RENDERIZAR A LISTA DE SUGERIDAS IMEDIATAMENTE
        if (window.renderSuggestedCommunities) {
          window.renderSuggestedCommunities();
          console.log('✅ Lista de comunidades sugeridas atualizada!');
        }
