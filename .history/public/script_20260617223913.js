...
// ============================================================
// BARRA DE INPUT DO DM + PICKERS (TUDO EM UM)
// ============================================================
(function fullDmInputAndPickers() {
  function buildEverything() {
    console.log('[DM] Iniciando construção da barra e pickers...');

    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      console.warn('[DM] dm-input-area não encontrado. Tentando novamente em 500ms...');
      setTimeout(buildEverything, 500);
      return;
    }

    // ── 1. CRIA A BARRA DE INPUT ──
    ...
    console.log('[DM] Barra criada com sucesso');
    ...
    // ── 3. CRIA OS PICKERS (se não existirem) ──
    function ensurePicker(id, html) {
      ...
      if (!picker) {
        ...
        console.log(`[DM] Pickers criado: ${id}`);
      } else {
        // Se já existe, apenas atualiza o HTML para garantir layout
        picker.innerHTML = html;
        picker.style.display = 'none';
        console.log(`[DM] Pickers atualizado: ${id}`);
      }
      return picker;
    }

    // ── Emoji picker ──
    ensurePicker('dm-emoji-picker', `
      <div style="display:flex;border-bottom:1px solid rgba(255,0,255,0.15);background:rgba(0,0,0,0.3);padding:4px;flex-shrink:0;">
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#fff;cursor:pointer;font-size:16px;border-radius:6px;background:rgba(255,0,255,0.15);">😀</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">👋</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">❤️</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🐶</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🍔</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🏳️</button>
      </div>
      <div style="display:grid;grid-template-columns:repeat(8,1fr);gap:4px;padding:12px;overflow-y:auto;flex:1;">
        ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟']}
      </div>
    `);

    // ── GIF picker ──
    ensurePicker('dm-gif-picker', `
      <div style="padding:10px 12px;border-bottom:1px solid rgba(255,0,255,0.15);flex-shrink:0;">
        <input type="text" placeholder="Pesquisar GIFs..." style="width:100%;padding:10px 14px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,0,255,0.3);border-radius:8px;color:#fff;outline:none;font-size:14px;box-sizing:border-box;">
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;padding:12px;overflow-y:auto;flex:1;">
        ${['https://media.tenor.com/1a1f1f1f1f1f1f1f1f1f1f1f1f1f1f1f/tenor.gif','https://media.tenor.com/2a2f2f2f2f2f2f2f2f2f2f2f2f2f2f2f/tenor.gif','https://media.tenor.com/3a3f3f3f3f3f3f3f3f3f3f3f3f3f3f3f/tenor.gif','https://media.tenor.com/4a4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f/tenor.gif'].map(url => `<div style="border-radius:8px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:all 0.2s;aspect-ratio:16/9;background:rgba(255,0,255,0.05);"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
      </div>
    `);

    // ── Sticker picker ──
    ensurePicker('dm-sticker-picker', `
      <div style="display:flex;border-bottom:1px solid rgba(255,0,255,0.15);background:rgba(0,0,0,0.3);padding:4px;flex-shrink:0;">
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#fff;cursor:pointer;font-size:13px;border-radius:6px;background:rgba(255,0,255,0.15);">Padrão</button>
        <button style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:13px;border-radius:6px;">Reações</button>
      </div>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;padding:12px;overflow-y:auto;flex:1;">
        ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️']}
      </div>
    `);

    // ── 4. CONECTA OS PICKERS AOS BOTÕES ──
    function connectPicker(btnId, pickerId, itemSelector, insertFn) {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn || !picker) {
        console.warn(`[DM] ${btnId} ou ${pickerId} não encontrado`);
        return;
      }
      ...
      picker.addEventListener('click', function(e) {
        ...
        if (!target) return;
        let value = '';
        if (itemSelector === '.gif-item img' || itemSelector === 'img') {
          value = target.src || '';
        } else {
    ...
    // ── GIF (seletor: div com img) ──
    connectPicker('dm-btn-gif', 'dm-gif-picker', 'div[style*="border-radius:8px"] img', function(url) {
      const input = document.getElementById('dm-message-input');
      if (input) {
        input.value += `[GIF] ${url} `;
      }
    ...
    console.log('[DM] Tudo construído com sucesso!');
  }
  ...
  function tryBuild() {
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
      ...
      document.addEventListener('DOMContentLoaded', buildEverything);
    }
  }
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      ...
      if (layoutId === 'dm-view') {
        setTimeout(buildEverything, 100);
        setTimeout(buildEverything, 300);
        setTimeout(buildEverything, 600);
      }
    };
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(buildEverything, 200);
      setTimeout(buildEverything, 500);
    }
  });

  console.log('[DM] Script de barra e pickers carregado. Aguardando abertura do DM...');
})();

// ============================================================
// CORRIGE LAYOUT DOS BOTÕES DA BARRA DO DM
// ============================================================
(function fixDmButtonLayout() {
  function applyFix() {
    console.log('[DM-FIX] Ajustando layout dos botões...');

    // Encontra a barra de input do DM
    const wrapper = document.querySelector('#dm-input-area .input-wrapper');
    if (!wrapper) {
      console.warn('[DM-FIX] Barra não encontrada. Tentando novamente em 500ms...');
      setTimeout(applyFix, 500);
      return;
    }

    // Aplica estilos robustos no container
    wrapper.style.cssText = `
      display: flex !important;
      align-items: center !important;
      gap: 4px !important;
    ...
    // Ajusta todos os botões dentro da barra
    ...
    // Ajusta o input
    const input = wrapper.querySelector('#dm-message-input');
    if (input) {
      input.style.cssText = `
        flex: 1 !important;
    ...
    // Ajusta o botão de enviar (destacado)
    const sendBtn = wrapper.querySelector('#dm-send-btn');
    if (sendBtn) {
      ...
      sendBtn.addEventListener('mouseenter', () => {
        ...
        sendBtn.style.boxShadow = '0 0 20px rgba(255,0,255,0.4)';
      });
    ...
    console.log('[DM-FIX] Layout corrigido com sucesso!');
  }

  // Executa imediatamente e com delays para garantir
  applyFix();
  setTimeout(applyFix, 300);
  setTimeout(applyFix, 600);

  // Reconstrói ao abrir a view DM
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
  ...
  console.log('[DM-FIX] Script de correção de layout carregado');
})();