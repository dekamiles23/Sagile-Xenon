...
// ✅ CORREÇÃO FINAL: Sistema de seleção visual DAS TAGS DE COMUNIDADE
let selectedCommunityTemplate = 'base';

// ✅ Função para atualizar visualização da seleção
function updateCommunitySelection(selectedBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    if (defaultBtn) {
      updateCommunitySelection(defaultBtn);
    }
  }, 100);
});
...
communityOptions.forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    updateCommunitySelection(btn);
    selectedCommunityTemplate = btn.dataset.template;
    
    console.log('✅ TAG SELECIONADA:', btn.dataset.template);
  });
});
