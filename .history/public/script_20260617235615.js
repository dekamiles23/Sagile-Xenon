...
// ============================================================
// PATCH FINAL: LISTA DE DMs COM CLIQUE E AVATAR
// ============================================================
(function finalDmListFix() {
  console.log('[DM-LIST-FIX] Iniciando...');

  // ── 1. Funções de avatar (cópia das anteriores) ──
  const avatarCache = {};
  ...
  // ── 2. Função para abrir o chat (com fallback) ──
  function openChatWith(username) {
    if (!username) return;
    const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
    if (username.toLowerCase() === currentUser) {
      console.log('[DM-LIST-FIX] Não é possível abrir chat consigo mesmo');
      return;
    }
    console.log('[DM-LIST-FIX] Abrindo chat com:', username);
    if (typeof window.openDmChat === 'function') {
      window.openDmChat(username);
    } else if (typeof window.openPrivateChat === 'function') {
      window.openPrivateChat(username);
    } else {
      // Fallback: tenta abrir a view DM e depois o chat
  ...
  // ── 3. Cria um item da lista com avatar e evento de clique ──
  function createDmItem(username) {
    ...
    `;
    applyAvatarToElement(avatarDiv, username);
  ...
  // ── 4. Sobrescreve a função renderDmList ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      // Chama a original (se necessário) ou faz a nossa própria renderização
      // Mas vamos fazer do zero para garantir o clique
      const list = document.getElementById('dm-conversations-list');
      if (!list) return;
      ...
      console.log('[DM-LIST-FIX] Lista renderizada com', unique.size, 'itens');
    };
  }

  // ── 5. Fallback: event delegation no container ──
  document.addEventListener('click', function(e) {
    // Procura o item mais próximo
    const item = e.target.closest('.dm-conv-item');
    if (!item) return;

    // Se o item já tiver um listener direto (criado por createDmItem), ele será executado primeiro.
    // Mas como fallback, também tratamos aqui.
    // Verifica se o item não tem um listener próprio (indicado por dataset.clickFixed)
    if (item.dataset.clickFixed) return; // já tem listener direto

    // Se chegou aqui, é porque o item foi criado por outro método (ex: original renderDmList)
    // Então extraímos o nome e abrimos
  ...
  }, true); // capture para prioridade

  // ── 6. Atualiza avatares na lista periodicamente (fallback) ──
  ...
  // ── 7. Observa mudanças na lista para aplicar avatares ──
  function observeList() {
    ...
    console.log('[DM-LIST-FIX] Observador da lista ativado');
  }

  // ── 8. Inicialização ──
  document.addEventListener('DOMContentLoaded', function() {
    observeList();
    // Se já houver uma lista, renderiza novamente com nosso método
    const list = document.getElementById('dm-conversations-list');
    if (list && list.children.length > 0) {
      // Se já tiver itens, aplica avatares e garante eventos
      updateListAvatars();
      // Também adiciona listeners diretos aos itens existentes (caso não tenham)
      list.querySelectorAll('.dm-conv-item').forEach(item => {
        if (!item.dataset.clickFixed) {
    ...
    // Se a view DM estiver aberta, renderiza a lista
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(() => {
        if (typeof window.renderDmList === 'function') {
  ...
  // ── 9. Quando a view DM for aberta, renderiza a lista ──
  ...
  // ── 10. Expõe funções para debug ──
  window.forceDmRender = function() {
    if (typeof window.renderDmList === 'function') {
      window.renderDmList();
    }
  };

  console.log('[DM-LIST-FIX] Patch aplicado. Use forceDmRender() para recarregar a lista.');
})();