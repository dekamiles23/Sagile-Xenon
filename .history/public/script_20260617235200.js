...
// ============================================================
// AVATAR AUTOMÁTICO - COM LOGS E FORÇA DE ATUALIZAÇÃO
// ============================================================
(function autoAvatarFinal() {
  console.log('[AVATAR-FINAL] Iniciando...');

  // ── 1. Cache de avatares ──
  const avatarCache = {};

  // ── 2. Função para obter avatar (prioriza cache, localStorage) ──
  ...
  function applyAvatarToElement(el, username) {
    ...
    if (avatarData) {
      ...
      el.textContent = '';
      console.log('[AVATAR-FINAL] Avatar aplicado a', username);
      return true;
    } else {
      el.style.backgroundImage = 'none';
      el.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
      el.textContent = username.charAt(0).toUpperCase();
      console.log('[AVATAR-FINAL] Fallback (iniciais) para', username);
      return false;
    }
  }

  // ── 4. Atualiza TODOS os avatares existentes ──
  function refreshAllAvatars() {
    console.log('[AVATAR-FINAL] Atualizando todos os avatares...');
    const avatars = document.querySelectorAll('.msg-avatar');
    avatars.forEach(el => {
      const parent = el.closest('.message, .dm-message, .private-message, .chat-message');
      if (!parent) return;
    ...
    console.log('[AVATAR-FINAL] Atualizados', avatars.length, 'avatares');
  }
  ...
  if (typeof window.addDmMessageToDom === 'function') {
    ...
    window.addDmMessageToDom = function(msg, isMe) {
      ...
      if (!messagesArea) return;
      const lastMsg = messagesArea.lastElementChild;
      if (lastMsg && lastMsg.textContent?.includes(msg.text) && lastMsg.textContent?.includes(isMe ? 'Você' : msg.from)) {
        return;
      }
  ...
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        ...
        console.log('[AVATAR-FINAL] Avatar recebido para', data.username);
        // Atualiza todos os avatares na UI
        refreshAllAvatars();
      }
    });
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        ...
        console.log('[AVATAR-FINAL] Avatares carregados de friends:data');
        refreshAllAvatars();
      }
    });
  }

  // ── 8. Observa novas mensagens ──
  function observeMessages() {
    ...
    console.log('[AVATAR-FINAL] Observador de mensagens ativado');
  }

  // ── 9. Quando o chat for aberto, solicita avatar do amigo ──
  function onChatOpen(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (!avatarCache[key] && !localStorage.getItem('avatar_' + key)) {
      if (window.socket && window.socket.connected) {
        console.log('[AVATAR-FINAL] Solicitando avatar de', username);
  ...
  window.setFriendAvatar = function(username, base64) {
    ...
    refreshAllAvatars();
    console.log('[AVATAR-FINAL] Avatar de', username, 'definido manualmente');
  };

  console.log('[AVATAR-FINAL] Sistema ativo. Use refreshAvatars() para forçar atualização.');
})();