...
// ============================================================
// POLLING: Força atualização do chat DM a cada 3 segundos
// ============================================================
(function dmPolling() {
  let lastChecked = {};
  ...
  function pollDmMessages() {
    ...
    // Evita múltiplas requisições para o mesmo usuário
    const now = Date.now();
    if (lastChecked[username] && (now - lastChecked[username] < 3000)) return;
    lastChecked[username] = now;
  ...
  if (window.socket) {
    socket.on('dm:history', function(data) {
      ...
      // Adiciona apenas mensagens que não existem (comparação por texto + timestamp)
  ...
  // Executa o polling a cada 3 segundos
  setInterval(pollDmMessages, 3000);
  console.log('[POLLING] Ativado (verifica a cada 3s)');
})();