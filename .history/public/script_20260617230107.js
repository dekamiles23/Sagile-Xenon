...
// ============================================================
// BARRA DM + PICKERS GARANTIDOS (COM LOGS)
// ============================================================
(function dmBarFinal() {
  console.log('[DM] Iniciando construção...');

  // ── 1. CRIA OS PICKERS (se não existirem) ──
  function createPickers() {
    const pickers = [
      {
        id: 'emoji-picker',
        html: `
          <div style="display:flex;flex-wrap:wrap;gap:4px;padding:12px;max-height:300px;overflow-y:auto;">
            ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟'].map(e => `<span class="emoji-item" style="font-size:28px;cursor:pointer;padding:4px;border-radius:6px;display:inline-block;">${e}</span>`).join('')}
          </div>
        `
      },
      {
        id: 'gif-picker',
        html: `
          <div style="padding:12px;display:grid;grid-template-columns:repeat(2,1fr);gap:8px;max-height:300px;overflow-y:auto;">
            ${['https://media.giphy.com/media/3o7aTskHEUdgCQAXde/giphy.gif','https://media.giphy.com/media/26tn33aiTi1jkl6H6/giphy.gif','https://media.giphy.com/media/3o6Zt481isNVuQI1l6/giphy.gif','https://media.giphy.com/media/l0HlNQ03J5JxX6lva/giphy.gif','https://media.giphy.com/media/3o7abKhOpu0N9H8s/giphy.gif','https://media.giphy.com/media/l0HlOBPvWqHj1D7J2/giphy.gif'].map(url => `<div class="gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;aspect-ratio:16/9;"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
          </div>
        `
      },
      {
        id: 'sticker-picker',
        html: `
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;padding:12px;max-height:300px;overflow-y:auto;">
            ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️'].map(s => `<span class="sticker-item" style="font-size:36px;cursor:pointer;padding:6px;border-radius:8px;text-align:center;">${s}</span>`).join('')}
          </div>
        `
      }
    ];

    pickers.forEach(({ id, html }) => {
      ...
      if (!picker) {
        ...
        // Se já existe, apenas atualiza o conteúdo e garante que está no body
  ...
  function createBar() {
    ...
    if (!target) {
      ...
      if (dmView) {
        ...
        const messagesArea = dmView.querySelector('#dm-messages-area, .messages-area');
        if (messagesArea) {
          messagesArea.parentNode.insertBefore(target, messagesArea.nextSibling);
        } else {
          dmView.appendChild(target);
        }
        console.log('[DM] dm-input-area criado');
      } else {
        console.warn('[DM] View DM não encontrada');
        return;
      }
    }
    ...
    wrapper.innerHTML = `
      <button type="button" id="dm-btn-plus" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">➕</button>
      <button type="button" id="dm-btn-emoji" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">😀</button>
      <button type="button" id="dm-btn-gif" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎬</button>
      <button type="button" id="dm-btn-stickers" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎭</button>
      <button type="button" id="dm-btn-attach" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">📎</button>
      ...
      <button type="button" id="dm-btn-voice" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎤</button>
    ...
    const emojiBtn = document.getElementById('dm-btn-emoji');
    const gifBtn = document.getElementById('dm-btn-gif');
    const stickerBtn = document.getElementById('dm-btn-stickers');

    function togglePicker(pickerId) {
      const picker = document.getElementById(pickerId);
      if (!picker) {
        console.warn(`[DM] Pickers ${pickerId} não encontrado`);
        return;
      }
      // Fecha outros
      ...
      console.log(`[DM] Pickers ${pickerId} ${isActive ? 'aberto' : 'fechado'}`);
      const input = document.getElementById('dm-message-input');
    ...
    if (emojiBtn) {
      emojiBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('[DM] Clique em 😀');
        togglePicker('emoji-picker');
      });
    }
    ...
    if (stickerBtn) {
      stickerBtn.addEventListener('click', function(e) {
        ...
        e.stopPropagation();
        console.log('[DM] Clique em 🎭');
        togglePicker('sticker-picker');
      });
    }

    // ── INSERIR ITENS DOS PICKERS ──
    ['emoji-picker', 'gif-picker', 'sticker-picker'].forEach(pickerId => {
      const picker = document.getElementById(pickerId);
      if (!picker) return;
      picker.addEventListener('click', function(e) {
        const target = e.target.closest('.emoji-item, .gif-item, .sticker-item');
        if (!target) return;
        let value = '';
        if (target.classList.contains('emoji-item') || target.classList.contains('sticker-item')) {
          value = target.textContent;
        } else if (target.classList.contains('gif-item')) {
        ...
        if (value) {
          const input = document.getElementById('dm-message-input');
          if (input) {
            if (target.classList.contains('gif-item')) {
    ...
    if (attachBtn) {
      ...
      fileInput.addEventListener('change', function() {
        console.log('[DM] Arquivos:', this.files);
        this.value = '';
      });
    }
  ...
  console.log('[DM] Script final carregado');
})();

// ============================================================
// ABRE PICKERS POR EVENT DELEGATION (NÃO DEPENDE DA CRIAÇÃO DA BARRA)
// ============================================================
(function openPickersByDelegation() {
  console.log('[DM-PICKER] Iniciando event delegation...');

  // Mapeia botão → picker
  const pickerMap = {
    'dm-btn-emoji': 'emoji-picker',
    'dm-btn-gif': 'gif-picker',
    'dm-btn-stickers': 'sticker-picker'
  };

  // Função que garante que os pickers existam no body
  function ensurePickers() {
    Object.values(pickerMap).forEach(id => {
      let picker = document.getElementById(id);
      if (!picker) {
        picker = document.createElement('div');
        picker.id = id;
        ...
        `;
        picker.innerHTML = `<div style="font-size:40px;">📦</div><div>Picker ${id} (placeholder)</div><div style="font-size:12px;color:#888;">Clique em um item para inserir</div>`;
        document.body.appendChild(picker);
        console.log(`[DM-PICKER] Criado placeholder para ${id}`);
      } else if (picker.parentNode !== document.body) {
        document.body.appendChild(picker);
        console.log(`[DM-PICKER] Movido ${id} para o body`);
      }
    });
  }

  // Função que abre/fecha um picker
  function togglePicker(pickerId) {
    const picker = document.getElementById(pickerId);
    if (!picker) {
      console.warn(`[DM-PICKER] Picker ${pickerId} não encontrado`);
      return;
    }
    ...
    const isActive = picker.classList.toggle('active');
    picker.style.display = isActive ? 'flex' : 'none';
    console.log(`[DM-PICKER] ${pickerId} ${isActive ? 'aberto' : 'fechado'}`);
    // Foca no input se estiver aberto
    if (isActive) {
      const input = document.getElementById('dm-message-input');
      if (input) input.focus();
    }
  ...
  // Listener global para cliques em botões da DM
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('[id^="dm-btn-"]');
    if (!btn) return;
    const pickerId = pickerMap[btn.id];
    if (pickerId) {
      e.preventDefault();
      e.stopPropagation();
      console.log(`[DM-PICKER] Clique em ${btn.id} → abrir ${pickerId}`);
      togglePicker(pickerId);
    }
  });

  // Insere itens nos pickers (para os placeholders, adiciona itens de exemplo)
  function populatePickers() {
    // Emoji picker
    const emojiPicker = document.getElementById('emoji-picker');
    if (emojiPicker && !emojiPicker.querySelector('.emoji-item')) {
      emojiPicker.innerHTML = `
        <div style="display:flex;flex-wrap:wrap;gap:6px;padding:12px;max-height:300px;overflow-y:auto;">
          ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟'].map(e => `<span class="emoji-item" style="font-size:28px;cursor:pointer;padding:4px;border-radius:6px;display:inline-block;">${e}</span>`).join('')}
        </div>
      `;
    }
    // GIF picker
    const gifPicker = document.getElementById('gif-picker');
    if (gifPicker && !gifPicker.querySelector('.gif-item')) {
      gifPicker.innerHTML = `
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;padding:12px;max-height:300px;overflow-y:auto;">
          ${['https://media.giphy.com/media/3o7aTskHEUdgCQAXde/giphy.gif','https://media.giphy.com/media/26tn33aiTi1jkl6H6/giphy.gif','https://media.giphy.com/media/3o6Zt481isNVuQI1l6/giphy.gif','https://media.giphy.com/media/l0HlNQ03J5JxX6lva/giphy.gif','https://media.giphy.com/media/3o7abKhOpu0N9H8s/giphy.gif','https://media.giphy.com/media/l0HlOBPvWqHj1D7J2/giphy.gif'].map(url => `<div class="gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;aspect-ratio:16/9;"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
        </div>
      `;
    }
    // Sticker picker
    const stickerPicker = document.getElementById('sticker-picker');
    if (stickerPicker && !stickerPicker.querySelector('.sticker-item')) {
  ...
  // Insere item no input ao clicar no picker
  document.addEventListener('click', function(e) {
    const target = e.target.closest('.emoji-item, .gif-item, .sticker-item');
    if (!target) return;
    ...
    if (value) {
      ...
      picker.classList.remove('active');
      picker.style.display = 'none';
      console.log(`[DM-PICKER] Inserido: ${value}`);
    }
  });
  ...
  // Reaplica quando a view DM for aberta
  ...
  console.log('[DM-PICKER] Script de event delegation carregado');
})();