...
// ============================================================
// BARRA DO DM COMPLETA (com modais de emoji, gif, sticker, anexo e mais opções)
// ============================================================
(function dmBarFull() {
  console.log('[DM-BAR] Iniciando construção...');

  // ── 1. CRIA OS MODAIS (PICKERS) ──
  function createPickers() {
    ...
    if (!emojiPicker) {
      ...
      emojiPicker.className = 'chat-picker';
    ...
    emojiPicker.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;max-height:350px;overflow-y:auto;justify-content:center;">
    ...
    // GIF picker
    let gifPicker = document.getElementById('gif-picker');
    if (!gifPicker) {
      gifPicker = document.createElement('div');
      gifPicker.id = 'gif-picker';
      gifPicker.className = 'chat-picker';
    ...
    // Sticker picker
    let stickerPicker = document.getElementById('sticker-picker');
    if (!stickerPicker) {
      stickerPicker = document.createElement('div');
      stickerPicker.id = 'sticker-picker';
      stickerPicker.className = 'chat-picker';
    ...
    stickerPicker.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;max-height:350px;overflow-y:auto;padding:4px;">
        ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️'].map(s => `<span class="sticker-item" style="font-size:36px;cursor:pointer;padding:6px;border-radius:8px;display:inline-block;">${s}</span>`).join('')}
      </div>
    `;

    // Modal "Mais opções" (enquete e tópico)
    let moreModal = document.getElementById('dm-more-modal');
    if (!moreModal) {
      moreModal = document.createElement('div');
      moreModal.id = 'dm-more-modal';
      moreModal.className = 'chat-picker';
      moreModal.style.cssText = emojiPicker.style.cssText + 'width:300px;';
      moreModal.innerHTML = `
  ...
  // ── 2. FUNÇÃO PARA ABRIR/FECHAR MODAIS ──
  ...
  function createBar() {
    ...
    wrapper.innerHTML = `
      <button type="button" id="dm-btn-plus" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">➕</button>
      <button type="button" id="dm-btn-emoji" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">😀</button>
      <button type="button" id="dm-btn-gif" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎬</button>
      <button type="button" id="dm-btn-stickers" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎭</button>
      <button type="button" id="dm-btn-attach" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">📎</button>
      ...
      <button type="button" id="dm-btn-voice" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎤</button>
    ...
    console.log('[DM-BAR] Barra criada');

    // ── Conecta eventos dos botões usando event delegation ──
    document.addEventListener('click', function(e) {
      ...
      } else if (id === 'dm-btn-gif') {
        e.preventDefault();
        togglePicker('gif-picker');
      } else if (id === 'dm-btn-stickers') {
        e.preventDefault();
        togglePicker('sticker-picker');
      } else if (id === 'dm-btn-plus') {
        e.preventDefault();
        togglePicker('dm-more-modal');
      } else if (id === 'dm-btn-attach') {
        e.preventDefault();
    ...
    // ── Inserir itens dos pickers no input ──
    document.addEventListener('click', function(e) {
    ...
    fileInput.addEventListener('change', function() {
      console.log('[DM-BAR] Arquivo(s) selecionado(s):', this.files);
      // Aqui você pode implementar o envio dos arquivos
      this.value = '';
    });
    ...
    if (voiceBtn) {
      voiceBtn.addEventListener('click', function() {
        alert('🎤 Gravação de áudio em desenvolvimento.');
      });
    }

    // ── Ações do modal "Mais opções" ──
    document.addEventListener('click', function(e) {
    ...
    console.log('[DM-BAR] Todos os eventos conectados');
  }
  ...
  function waitForDM() {
    ...
    const observer = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        console.log('[DM-BAR] DM aberta, construindo...');
        setTimeout(() => {
          createPickers();
          createBar();
        }, 100);
  ...
  console.log('[DM-BAR] Script completo carregado');
})();