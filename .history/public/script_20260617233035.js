...
// ============================================================
// STATUS EM TEMPO REAL NO DM (COM CRIAÇÃO DE ELEMENTO)
// ============================================================
(function dmStatusFinal() {
  ...
  // Obtém o status de um usuário (case-insensitive)
  function getStatus(username) {
    if (!username) return 'offline';
    const key = username.toLowerCase();
    if (window.userStatuses && window.userStatuses[key] !== undefined) {
  ...
  function ensureStatusElement() {
    // Procura o cabeçalho do chat DM
    const header = document.querySelector('.chat-header') ||
                   document.querySelector('#dm-chat-area .chat-header') ||
                   document.querySelector('.dm-chat-header');
    if (!header) return null;

    // Procura o elemento de status existente
    let statusEl = header.querySelector('.dm-status, .status-text, .user-status');
    if (!statusEl) {
      // Cria o elemento de status
      statusEl = document.createElement('span');
      statusEl.className = 'dm-status';
      statusEl.style.cssText = 'font-size: 12px; margin-left: 8px;';
      // Insere após o nome do usuário
      const nameEl = header.querySelector('.dm-username, .username, .pcb-title');
      if (nameEl) {
        nameEl.after(statusEl);
      } else {
        // Se não houver nome, insere no final do header
        header.appendChild(statusEl);
      }
      console.log('[DM-STATUS] Elemento de status criado');
    }
    return statusEl;
  }
  ...
  function updateStatus() {
    const dmView = document.getElementById('dm-view');
    if (!dmView || dmView.classList.contains('hidden')) {
      // Se a DM não está visível, não faz nada
      return;
    }

    // Garante que o elemento de status exista
    const statusEl = ensureStatusElement();
    if (!statusEl) {
      console.warn('[DM-STATUS] Não foi possível criar elemento de status');
      return;
    }

    // Obtém o nome do usuário do cabeçalho
    const header = statusEl.closest('.chat-header') ||
                   statusEl.closest('#dm-chat-area .chat-header') ||
                   statusEl.closest('.dm-chat-header');
    let nameEl = header ? header.querySelector('.dm-username, .username, .pcb-title') : null;
    if (!nameEl) {
      // Fallback: procura em qualquer lugar
      nameEl = document.querySelector('.dm-username, .username, .pcb-title');
    }
    if (!nameEl) {
      console.warn('[DM-STATUS] Nome do usuário não encontrado');
      return;
    }
  ...
  function startPolling() {
    ...
    pollingInterval = setInterval(() => {
      ...
      if (dmView && !dmView.classList.contains('hidden')) {
        ...
        // Para o polling se a DM for fechada
        clearInterval(pollingInterval);
        pollingInterval = null;
        console.log('[DM-STATUS] Polling parado (DM fechada)');
      }
    }, 1000);
    console.log('[DM-STATUS] Polling iniciado (a cada 1s)');
  }
  ...
  function stopPolling() {
    if (pollingInterval) {
      clearInterval(pollingInterval);
      pollingInterval = null;
      console.log('[DM-STATUS] Polling parado');
    }
  }

  // ── Observer para detectar quando a DM é aberta/fechada ──
  function setupDMObserver() {
    const dmView = document.getElementById('dm-view');
    if (!dmView) {
      setTimeout(setupDMObserver, 300);
      return;
    }

    const observer = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        console.log('[DM-STATUS] DM aberta, iniciando polling');
        // Atualiza imediatamente e inicia polling
        setTimeout(updateStatus, 100);
        setTimeout(updateStatus, 300);
        startPolling();
      } else {
        console.log('[DM-STATUS] DM fechada, parando polling');
        stopPolling();
      }
    });
    observer.observe(dmView, { attributes: true, attributeFilter: ['class'] });

    // Se a DM já estiver aberta, inicia polling
    if (!dmView.classList.contains('hidden')) {
      setTimeout(updateStatus, 100);
      setTimeout(updateStatus, 300);
      startPolling();
  ...
  if (window.socket) {
    socket.on('friend:status', function(data) {
      if (data && data.username) {
        console.log('[DM-STATUS] Evento friend:status recebido:', data);
        const dmView = document.getElementById('dm-view');
        if (dmView && !dmView.classList.contains('hidden')) {
          // Verifica se é o usuário atual do chat
          const nameEl = document.querySelector('.dm-username, .username, .pcb-title');
    ...
    socket.on('friends:presence', function() {
      console.log('[DM-STATUS] Evento friends:presence recebido');
      const dmView = document.getElementById('dm-view');
      if (dmView && !dmView.classList.contains('hidden')) {
        updateStatus();
      }
  ...
  // ── Quando a view DM for aberta via showLayout ──
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  document.addEventListener('DOMContentLoaded', function() {
    setupDMObserver();
    // Se a DM já estiver visível, inicia polling
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(updateStatus, 200);
      setTimeout(updateStatus, 500);
  ...
  window.updateDmStatus = updateStatus;

  console.log('[DM-STATUS] Sistema ativo. Use updateDmStatus() para forçar.');
})();