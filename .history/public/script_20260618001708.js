...
// ============================================================
// SISTEMA CENTRALIZADO DE AVATARES (SEM PISCAR)
// ============================================================
(function unifiedAvatarSystem() {
  console.log('[UNIFIED-AVATAR] Iniciando...');
  ...
  // ── 4. Atualiza TODOS os avatares (com debounce) ──
    ...
    updateTimeout = setTimeout(() => {
      ...
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        ...
        if (avatarEl && nameEl) {
          ...
          if (username) applyAvatar(avatarEl, username);
        }
      });
      // Mensagens
  ...
  // ── 5. Solicita avatar ao servidor (se necessário) ──
  function requestAvatar(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (requested.has(key)) return;
    if (getAvatar(username)) return;

    if (window.socket && window.socket.connected) {
      requested.add(key);
      console.log('[UNIFIED-AVATAR] Solicitando avatar de', username);
      window.socket.emit('get-avatar', { username: username });
    }
  }

  // ── 6. Processa avatar recebido do servidor ──
  function receiveAvatar(username, base64) {
    ...
    console.log('[UNIFIED-AVATAR] Avatar recebido para', username);
    refreshAllAvatars();
  }

  // ── 7. Listener do socket ──
  if (window.socket) {
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        data.friends.forEach(friend => {
          if (friend.avatar) {
            receiveAvatar(friend.username, friend.avatar);
          }
        });
      }
    });
  ...
  // ── 8. Integra com renderDmList ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      original.call(this);
      setTimeout(() => {
        // Solicita avatares para todos os amigos da lista
        document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
          const nameEl = item.querySelector('.dm-conv-name');
          if (nameEl) {
            const username = nameEl.textContent.trim();
  ...
  function observeList() {
    ...
    if (!list) return;
    const observer = new MutationObserver(() => {
      // Solicita avatares para novos itens
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        const nameEl = item.querySelector('.dm-conv-name');
        if (nameEl) {
          const username = nameEl.textContent.trim();
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Aplica avatares imediatamente após o DOM carregar
  ...
  window.setFriendAvatar = function(username, base64) {
    receiveAvatar(username, base64);
  };

  console.log('[UNIFIED-AVATAR] Sistema ativo. Use refreshAvatars() para forçar.');
})();