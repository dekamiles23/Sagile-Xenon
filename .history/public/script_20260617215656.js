...
// ============================================================
// PATCH FINAL: Chat DM em tempo real (com logs)
// ============================================================
(function fixDmRealtimeFinal() {
  if (!window.socket) {
    console.warn('[DM-RT] Socket não disponível, aguardando...');
    setTimeout(fixDmRealtimeFinal, 500);
    return;
  }
  ...
  socket.on('dm:message', function(msg) {
    ...
    // Determina o interlocutor (quem enviou ou para quem foi enviada)
    const sender = isForMe ? msg.from : msg.to;
    console.log('[DM-RT] Interlocutor:', sender);
    ...
    // 2. Verifica se a conversa com este usuário está aberta
    const chatArea = document.getElementById('dm-chat-area');
    const isChatOpen = chatArea && !chatArea.classList.contains('hidden');
    const activeUser = chatArea?.dataset.activeChat;

    // Se o chat estiver aberto e for com o remetente certo
    if (isChatOpen && activeUser && activeUser.toLowerCase() === sender.toLowerCase()) {
      console.log('[DM-RT] Chat com', sender, 'está aberto, adicionando mensagem');
      const messagesArea = document.getElementById('dm-messages-area');
      if (messagesArea) {
        // Evita duplicação (verifica a última mensagem)
        const lastMsg = messagesArea.lastElementChild;
      ...
      console.log('[DM-RT] Chat com', sender, 'não está aberto (activeUser:', activeUser, ')');
    }

    // 3. Marca como não lida (opcional)
    markUnread(sender);
  });
  ...
  function markUnread(username) {
    // Implementar badge de não lidos se quiser
  }
  ...
  console.log('[DM-RT] ✅ Patch final aplicado. Aguardando mensagens...');
})();