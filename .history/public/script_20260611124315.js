...
// ? FUNÇĂO SEGURA PARA EVITAR UNDEFINED
...
// Verificar autenticaçăo
document.addEventListener('DOMContentLoaded', () => {
  // Se năo estiver autenticado, redireciona para auth.html
  ...
  appEl.style.display = 'flex !important';
  appEl.style.visibility = 'visible !important';
  appEl.style.opacity = '1 !important';
  appEl.style.width = '100% !important';
  appEl.style.height = '100vh !important';
  appEl.style.position = 'relative !important';
  appEl.style.zIndex = '1 !important';
  ...
  discoverView.style.display = 'block !important';
  discoverView.style.visibility = 'visible !important';
  discoverView.style.opacity = '1 !important';
});
  ...
  class FriendsSystem {
      ...
      centerArea.innerHTML = `
        ...
        <div class="dm-input-area">
          <div class="input-wrapper">
            ...
            <button id="dm-send-btn" type="button">â¤</button>
          </div>
        </div>
      `;
    }
    ...
    openAddFriendModal() {
      ...
      if (username && username.trim()) {
        ...
        alert('Solicitaçăo de amizade enviada!');
      }
    }
  }
...
// Botăo Mensagens Privadas
btnDmList.addEventListener('click', () => {
  hideAllViews();
  
  // ? CORREÇĂO TOTAL: Reseta TODOS os estados visuais e limpa layout de servidor
  ...
  // ? CORREÇĂO: Remove classe active de todos os botőes da navbar
  ...
  powerModal.innerHTML = `
    <div class="modal modal-login" style="max-width: 520px; padding: 2rem;">
      <div style="text-align: center; margin-bottom: 1.5rem;">
        <div style="font-size: 4rem; margin-bottom: 1rem;">âĄ</div>
      ...
      <div style="display: grid; gap: 1rem; margin: 1.5rem 0;">
        <div style="background: rgba(168, 0, 255, 0.08); border: 1px solid rgba(168, 0, 255, 0.2); border-radius: 10px; padding: 1rem; display: flex; gap: 1rem; align-items: center;">
          ...
          <div>
            ...
            <p style="color: #aaa; margin: 0.25rem 0 0 0; font-size: 0.9rem;">Animaçőes e efeitos especiais no seu perfil</p>
          </div>
        </div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1.5rem;">
        ...
        <button class="btn-neon">âĄ ASSINAR POWER</button>
      </div>
    </div>
  `;
...
function showDiscoverView() {
  ...
  try {
    ...
    // Remove TODO estilo da sidebar para năo deixar espaço vazio na lateral
    serverSidebar.classList.add('hidden');
    serverSidebar.removeAttribute('style');
    
    document.body.classList.remove('server-body', 'dm-active', 'dm-view-active', 'dm-mode');
    
    // ? FORÇA RENDERIZAÇĂO DO RESET
    void document.body.offsetWidth;
    
    hideAllViews();
    
    // ✅ CORREÇÃO FINAL: FORÇAR APARECER O CONTEÚDO PRINCIPAL
    discoverView.classList.remove('hidden');
    discoverView.style.display = 'block !important';
    discoverView.style.visibility = 'visible !important';
    discoverView.style.opacity = '1 !important';
    discoverView.style.width = '100% !important';
    discoverView.style.height = '100% !important';
    discoverView.style.position = 'relative !important';
    discoverView.style.zIndex = '1 !important';
    
    // Forçar reflow 3x
    void discoverView.offsetWidth;
    void discoverView.offsetHeight;
    void discoverView.offsetLeft;
    currentServerId = null;
    currentChannel = null;
    currentDmUser = null;
    ...
    // ✅ CORREÇÃO FINAL: NÃO FICA CARREGANDO INFINITAMENTE
    // Mostra conteúdo padrão IMEDIATAMENTE, não espera servidor
    ...
    if (feedEl) {
      feedEl.innerHTML = `
        <div style="text-align: center; padding: 60px 20px;">
          ...
          <p style="color: #aaa; font-size: 18px; margin-bottom: 48px; max-width: 500px; margin-left: auto; margin-right: auto;">Conecte-se com pessoas que compartilham seus interesses, compartilhe ideias e participe de discussões.</p>
    ...
    console.log("? Conteúdo padrão carregado imediatamente");
    
    // Solicita dados ao servidor em background
    socket.emit('feed:join');
    
    console.log("? Renderizaçăo concluída");
...
function openServer(serverId) {
  ...
  // ? ?? RESET ABSOLUTO TOTAL - NĂO DEIXA NENHUM RASTRO DO LAYOUT DM
  ...
  // ? FORÇA RENDERIZAÇĂO DO RESET - TÉCNICA PROFISSIONAL
  // Isso força o navegador a aplicar TODAS as alteraçőes acima ANTES de continuar
  void document.body.offsetWidth;
  ...
  // ? Renderiza canais 2x para garantir atualizaçăo
...
function openChannel(ch) {
  ...
  if (!ch) {
    console.log("? [DEBUG 1] Canal é NULL - EXECUÇĂO PAROU AQUI");
    return;
  }
  ...
  if (ch.type === 'voice') {
    ...
    // ? NĂO ENTRA AUTOMATICAMENTE - ABRE MODAL DE CONFIRMAÇĂO
...
// ? Funçăo para renderizar ABAS DOS FANDOMS no topo
...
function renderSidebarChannels(server) {
  ...
  // ? ADICIONAR CATEGORIAS VAZIAS QUE NĂO TEM CANAIS AINDA
  ...
  for (const [cat, channels] of Object.entries(categories)) {
    ...
    catHeader.addEventListener('contextmenu', (e) => {
      ...
      ctxMenu.innerHTML = `
        ...
        <div class="ctx-item" data-action="rename-cat">âď¸ Renomear categoria</div>
        <div class="ctx-item" data-action="move-up">âŹď¸ Mover para cima</div>
        <div class="ctx-item" data-action="move-down">âŹď¸ Mover para baixo</div>
        <div class="ctx-sep"></div>
      ...
      ctxMenu.querySelectorAll('[data-action]').forEach(item => {
        item.addEventListener('click', () => {
          ...
          if (item.dataset.action === 'delete-cat') {
            if (confirm(`Excluir categoria ${cat}? Os canais serăo movidos para padrăo.`)) {
              // Remove categoria
              const idx = server.customCategories.indexOf(cat);
    ...
    channels.forEach(ch => {
      ...
      // Menu de contexto com botăo direito
                  ...
                  if (server.channels.length === 0) {
                    ...
                    msContent.innerHTML = `
                      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:16px;color:#888">
                        ...
                        <h2 style="margin:0;color:#ccc">Năo há canais por aqui</h2>
                        <p style="margin:0;max-width:400px;text-align:center">Este servidor ainda năo tem nenhum canal criado. Clique no botăo + acima para criar o primeiro canal!</p>
                      </div>
                    `;
...
function channelIcon(type) {
  // ? CORREÇĂO: Ícones com encoding correto para evitar caracteres estranhos
  if (type === 'text') return '#';
...
function renderServersRail() {
  ...
  servers.forEach(server => {
    ...
    // ? EVENTO DE BOTĂO DIREITO PARA ABRIR MENU DE CONTEXTO
...
function createServer(template) {
  ...
  const baseChannels = [
    ...
    { id: `${id}_forum`, name: 'forum', type: 'forum', desc: 'Discussőes estilo Reddit' },
  ];
...
// Evento botăo Criar Fandom na barra inferior
...
function createFandom() {
  ...
  const newFandom = {
    ...
    channels: [
      ...
      { id: `${fandomId}_discussao`, name: 'discussăo', type: 'text', desc: 'Discussőes gerais' },
    ],
    createdAt: Date.now()
  };
...
// ✅ CORREÇÃO FINAL ABSOLUTA DAS CATEGORIAS
// NÃO USA NENHUMA CLASSE - PEGA DIRETAMENTE OS ELEMENTOS QUE EXISTEM NA TELA
setInterval(() => {
  // Pegar TODOS os botões dentro da área de categorias
  document.querySelectorAll('#community-modal div[role="button"], #community-modal button, #community-modal div[onclick], #community-modal div > div > div').forEach(btn => {
    // Verificar se é um botão de categoria (tem ícone e texto dentro)
    if (btn.querySelector('svg, img, span') && btn.textContent.trim().length > 0 && btn.textContent.trim().length < 15) {
      
      // Adicionar evento APENAS se ainda não tiver
      if (!btn.hasAttribute('data-click-fixed')) {
        ...
        btn.addEventListener('click', (e) => {
          ...
          // ✅ REMOVER ESTILO DE TODOS OS OUTROS
          document.querySelectorAll('#community-modal div[role="button"], #community-modal button, #community-modal div[onclick], #community-modal div > div > div').forEach(b => {
            if (b.querySelector('svg, img, span') && b.textContent.trim().length > 0 && b.textContent.trim().length < 15) {
              b.style.background = 'transparent';
              b.style.border = '1px solid rgba(255, 0, 255, 0.3)';
              b.style.boxShadow = 'none';
              b.style.transform = 'scale(1)';
          ...
          // ✅ APLICAR ESTILO ROSA NO BOTÃO CLICADO
          btn.style.background = 'rgba(255, 0, 255, 0.25) !important';
          btn.style.border = '2px solid #ff00ff !important';
          btn.style.boxShadow = '0 0 12px rgba(255, 0, 255, 0.5) !important';
          btn.style.transform = 'scale(1.02) !important';
          btn.classList.add('selected');
          
          // ✅ Salvar categoria selecionada
          selectedCommunityCategory = btn.textContent.trim().toLowerCase();
          
          console.log('✅ CATEGORIA SELECIONADA COM SUCESSO:', btn.textContent.trim());
        });
      }
    }
  });
...
// Botão confirmar criação de comunidade
...
btnConfirmChannel.addEventListener('click', () => {
  ...
  // ? Renderiza 2x para garantir atualizaçăo
  renderSidebarChannels(server);
...
btnHomeBtn.addEventListener('click', () => {
  // Volta para a página inicial padrăo do Xenon Comunidade
  hideAllViews();
  discoverView.classList.remove('hidden');
  
  // ? CORREÇĂO: Remove classe server-body e oculta sidebar do servidor
  document.body.classList.remove('server-body');
  serverSidebar.classList.add('hidden');
  ...
  mainArea.innerHTML = `
    <div style="text-align: center;">
      <div style="font-size: 120px; margin-bottom: 24px; opacity: 0.7;">??</div>
      <h1 style="color: #fff; font-size: 32px; margin-bottom: 12px;">Bem-vindo ao Xenon Comunidade</h1>
      <p style="color: #aaa; font-size: 18px; margin-bottom: 48px; max-width: 500px;">Conecte-se com pessoas que compartilham seus interesses, compartilhe ideias e participe de discussőes.</p>
  ...
  // Reativa o evento do botăo Postagens de comunidade
  setTimeout(() => {
    document.getElementById('btn-open-community-posts')?.addEventListener('click', () => {
      ...
      mainArea.innerHTML = `
        <div class="discover-community-banner">
          <div class="community-banner-header">
            ...
            <div class="community-banner-info">
              ...
              <p>Seja vocę mesmo! Conecte-se com pessoas que compartilham seus interesses.</p>
            </div>
          </div>
        ...
        <div class="discover-compose">
          ...
          <textarea id="discover-post-body" placeholder="O que vocę quer compartilhar?" maxlength="2000" rows="2"></textarea>
...
// ? CORREÇĂO: Reseta todos os estados visuais ao voltar para home
...
// ? EVENTO BOTĂO DIREITO NO AVATAR DA BARRA SUPERIOR
...
// ? EVENTO BOTĂO DIREITO NO AVATAR DA PÁGINA DE MENSAGENS PRIVADAS
...
// Coleçăo de jogos no perfil
function renderProfileGames() {
  const list = document.getElementById('pf-game-list');
  if (!list) return;
  if (profileGames.length === 0) {
    list.innerHTML = '<div class="empty-state small">??<p>Nenhum jogo na coleçăo ainda.</p></div>';
    return;
  }
  list.innerHTML = profileGames.map((g, i) =>
    `<div class="pf-game-item"><span>?? ${escHtml(g)}</span><button type="button" data-i="${i}">â</button></div>`
  ).join('');
...
// ââ Configuraçőes: conteúdo das seçőes ââ
const SETTINGS_SECTIONS = {
  conta: () => {
    const savedUsername = localStorage.getItem('zx_username') || username;
    return `
    <h2 class="ms-section-title">Conta</h2>
    <p class="ms-section-desc">Gerencie suas informaçőes pessoais.</p>
    <div class="ms-block">
      <div class="ms-block-title">Informaçőes do usuário</div>
    ...
    <div class="ms-block">
      ...
      <div class="ms-field">
        ...
        <select id="mic-device-select">
          <option value="default" ${savedMic === 'default' ? 'selected' : ''}>Padrăo do sistema</option>
        </select>
      </div>
      ...
      <div class="ms-field">
        ...
        <select id="speaker-device-select">
          <option value="default" ${savedSpeaker === 'default' ? 'selected' : ''}>Padrăo do sistema</option>
        </select>
      </div>
    ...
    <div class="ms-block">
      ...
      <div class="ms-field">
        ...
        <select id="camera-device-select">
          <option value="default" ${savedCamera === 'default' ? 'selected' : ''}>Padrăo do sistema</option>
        </select>
      </div>
      ...
      <div class="ms-field">
        ...
        <select id="camera-bg-select">
          <option value="none" ${savedBg === 'none' ? 'selected' : ''}>Nenhum (padrăo)</option>
      ...
      <div style="margin-top:1rem; border-radius: 12px; overflow: hidden; background: #000;">
        ...
        <div id="camera-preview-placeholder" style="width:100%; height: 200px; display: flex; align-items: center; justify-content: center; color: #666; background: #111;">
          ?? Pré-visualizaçăo da câmera
        </div>
      </div>
      
      <button type="button" class="btn-ms btn-ms-primary" id="btn-camera-test" style="margin-top: 1rem">?? Ligar pré-visualizaçăo</button>
    </div>`;
  },
  ...
  aparencia: () => {
    ...
    return `
    <h2 class="ms-section-title">Aparęncia</h2>
    <p class="ms-section-desc">Personalize a aparęncia do programa.</p>
    <div class="ms-block">
      <div class="ms-field">
        <label>Plano de fundo</label>
        <div class="wallpaper-picker-row">
          <label class="wallpaper-opt"><input type="radio" name="wallpaper" value="default" ${wp === 'default' ? 'checked' : ''} /> Padrăo (Neon)</label>
      ...
      <label class="toggle-row"><span>Animaçőes de interface</span><input type="checkbox" id="toggle-animations" checked /></label>
      
      <div class="ms-field" style="margin-top:16px">
        <label>Resoluçăo da janela</label>
        <select id="window-resolution-select">
          <option value="100%">100% (Padrăo)</option>
  ...
  acessibilidade: () => `
    ...
    <p class="ms-section-desc">Ajustes para melhor leitura e navegaçăo.</p>
    <div class="ms-block">
  ...
  cla: () => {
    const clanItems = userClans.length
      ...
      : '<p class="pf-hint">Vocę ainda năo faz parte de nenhum clă.</p>';
    return `
    <h2 class="ms-section-title">Clă</h2>
    <p class="ms-section-desc">Crie, edite ou entre em clăs.</p>
    <div class="clan-list" id="clan-list">${clanItems}</div>
    <div class="ms-actions">
      <button type="button" class="btn-ms btn-ms-primary" id="btn-clan-create">Criar clă</button>
      <button type="button" class="btn-ms" id="btn-clan-join">Entrar em clă</button>
    </div>`;
  },
  ...
  privacidade: () => {
    ...
    <p class="ms-section-desc">Controle como seus dados săo usados no ZX Chat.</p>
    <div class="ms-block">
      <label class="toggle-row"><span><span>Usar dados do programa</span><small>Permite melhorar recomendaçőes e experięncia</small></span><input type="checkbox" id="toggle-use-data" ${useData ? 'checked' : ''} /></label>
      <label class="toggle-row"><span><span>Mostrar status online</span><small>Outros usuários verăo se vocę está online</small></span><input type="checkbox" id="toggle-show-online" ${showOnline ? 'checked' : ''} /></label>
      <label class="toggle-row"><span><span>Confirmaçăo de leitura</span><small>Mostra quando vocę leu uma mensagem</small></span><input type="checkbox" id="toggle-read-receipts" ${readReceipts ? 'checked' : ''} /></label>
      <label class="toggle-row"><span><span>Indicador de digitaçăo</span><small>Mostra quando vocę está digitando</small></span><input type="checkbox" id="toggle-typing-indicator" ${typingIndicator ? 'checked' : ''} /></label>
  ...
  idioma: () => {
    ...
    <div class="lang-options">
      ...
      <label class="lang-opt"><input type="radio" name="app-lang" value="pt-BR" ${lang === 'pt-BR' ? 'checked' : ''} /> Portuguęs (Brasil)</label>
      <label class="lang-opt"><input type="radio" name="app-lang" value="pt-PT" ${lang === 'pt-PT' ? 'checked' : ''} /> Portuguęs (Portugal)</label>
      <label class="lang-opt"><input type="radio" name="app-lang" value="es" ${lang === 'es' ? 'checked' : ''} /> Espańol</label>
    </div>`;
  },
  ...
  atalhos: () => {
    ...
    <div class="ms-block">
      <div class="ms-row"><span>Sobreposiçăo de tela<span><small>Abre o painel de overlay durante jogos</small></span></span><kbd style="color:var(--neon)">Shift + \`</kbd></div>
      ...
      <div class="ms-row"><span>Abrir configuraçőes</span><kbd style="color:var(--neon)">Ctrl + ,</kbd></div>
  ...
  sobreposicao: () => {
    ...
    return `
    <h2 class="ms-section-title">Sobreposiçăo</h2>
    <p class="ms-section-desc">Mostra quem está em call com vocę enquanto joga (estilo Steam).</p>
    <div class="ms-block">
      <label class="toggle-row"><span>Ativar sobreposiçăo em jogos</span><input type="checkbox" id="overlay-toggle" ${overlayEnabled ? 'checked' : ''} /></label>
      ...
      <div class="ms-field"><label>Posiçăo na tela</label><select id="overlay-position">
  ...
  seguranca: () => {
    ...
    <p class="ms-section-desc">Proteja sua conta contra acessos năo autorizados.</p>
    <div class="ms-block">
      <label class="toggle-row"><span>Autenticaçăo em duas etapas</span><input type="checkbox" id="toggle-2fa" ${twoFactor ? 'checked' : ''} /></label>
...
function bindSettingsSectionEvents(sectionId) {
  ...
  if (sectionId === 'voz') {
    ...
    async function loadDevices() {
      try {
        ...
        if (micDeviceSelect) {
          micDeviceSelect.innerHTML = '<option value="default">Padrăo do sistema</option>';
        ...
        if (speakerDeviceSelect) {
          speakerDeviceSelect.innerHTML = '<option value="default">Padrăo do sistema</option>';
        ...
        if (cameraDeviceSelect) {
          cameraDeviceSelect.innerHTML = '<option value="default">Padrăo do sistema</option>';
        ...
        console.log('Năo foi possível listar dispositivos:', err);
      }
    }
    
    loadDevices();
    
    // Eventos de seleçăo de dispositivos
    ...
    btnCameraTest?.addEventListener('click', async () => {
      if (!cameraActive) {
        try {
          ...
          showToast('Năo foi possível acessar a câmera. Verifique permissőes.');
        }
      } else {
        ...
        cameraActive = false;
        btnCameraTest.textContent = '?? Ligar pré-visualizaçăo';
    ...
    noiseSuppressionToggle?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Supressăo de ruído ativada' : 'Supressăo de ruído desativada');
    });
    ...
    btnMicTest?.addEventListener('click', async () => {
      if (!micTestActive) {
        try {
          ...
          showToast('Năo foi possível acessar o microfone. Verifique permissőes.');
        }
      } else {
        micTestActive = false;
    ...
    micLoopbackToggle?.addEventListener('change', (e) => {
      if (loopbackNode) {
        ...
        showToast(e.target.checked ? 'Loopback ativado: vocę ouvirá sua própria voz' : 'Loopback desativado');
      } else {
  ...
  if (sectionId === 'aparencia') {
    ...
    if (resolutionSelect) {
      ...
      resolutionSelect.addEventListener('change', (e) => {
        ...
        showToast(`Resoluçăo alterada para ${e.target.value}`);
      });
    }
  ...
  if (sectionId === 'cla') {
    ...
    document.getElementById('btn-clan-join')?.addEventListener('click', () => {
      const code = prompt('Código do clă:');
      if (code?.trim()) {
        userClans.push({ name: `Clă ${code.trim()}` });
        localStorage.setItem('zx_clans', JSON.stringify(userClans));
        renderSettingsSection('cla');
        activateSettingsNav('cla');
        showToast('Vocę entrou no clă!');
      }
    });
    ...
    msContent.querySelectorAll('[data-action="edit"]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const name = prompt('Novo nome do clă:', clan?.name);
        if (name?.trim() && clan) {
          clan.name = name.trim();
          localStorage.setItem('zx_clans', JSON.stringify(userClans));
          renderSettingsSection('cla');
  ...
  if (sectionId === 'privacidade') {
    ...
    document.getElementById('toggle-read-receipts')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Confirmaçőes de leitura ativadas' : 'Confirmaçőes de leitura desativadas');
    });
    document.getElementById('toggle-typing-indicator')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Indicador de digitaçăo ativado' : 'Indicador de digitaçăo desativado');
    });
  ...
  if (sectionId === 'sobreposicao') {
    document.getElementById('overlay-toggle')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Sobreposiçăo ativada' : 'Sobreposiçăo desativada');
    });
    document.getElementById('overlay-position')?.addEventListener('change', (e) => {
      ...
      showToast(`Posiçăo da sobreposiçăo alterada para ${e.target.value}`);
    });
  }
  ...
  if (sectionId === 'atividades') {
    document.getElementById('toggle-show-game')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Jogo atual será exibido no perfil' : 'Jogo atual năo será exibido');
    });
    document.getElementById('toggle-share-activity')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Atividade compartilhada com amigos' : 'Atividade năo compartilhada');
    });
    document.getElementById('toggle-auto-detect')?.addEventListener('change', (e) => {
      localStorage.setItem('zx_auto_detect', e.target.checked);
      showToast(e.target.checked ? 'Detecçăo automática de jogos ativada' : 'Detecçăo automática desativada');
    });
  }
  ...
  if (sectionId === 'seguranca') {
    document.getElementById('toggle-2fa')?.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 'Autenticaçăo em duas etapas ativada' : 'Autenticaçăo em duas etapas desativada');
    });
...
function showServerCtxMenu(x, y, serverId) {
  ...
  itemCreateServer.textContent = 'â Criar servidor';
  ...
  // Verifica se tem espaço suficiente abaixo, senăo abre para cima
  const menuHeight = serverCtxMenu.offsetHeight;
  const windowHeight = window.innerHeight;
  
  if (y + menuHeight > windowHeight) {
    // Năo tem espaço abaixo, posiciona acima do cursor
...
function sendMessage() {
  ...
  // Integraçăo Discord: envia mensagem também para o webhook configurado
...
function renderVoiceParticipants() {
  ...
  grid.innerHTML = allUsers.map(u => {
    ...
    <div class="voice-participant${isMuted && u.self ? ' muted' : ''}" id="vp-${u.socketId}">
      ${u.self ? '<span class="voice-participant-self-badge">Vocę</span>' : ''}
...
document.getElementById('btn-toggle-cam')?.addEventListener('click', async () => {
  if (!isCameraActive) {
    try {
      ...
      showToast('? Năo foi possível acessar a webcam. Verifique permissőes.');
      console.error(err);
    }
  } else {
    if (cameraStream) {
...
document.getElementById('btn-share-screen')?.addEventListener('click', async () => {
  if (!isSharingScreen) {
    try {
      ...
      showToast('? Năo foi possível compartilhar tela.');
      console.error(err);
    }
  } else {
    if (screenStream) {
...
// Botăo Compartilhar Áudio / Música
document.getElementById('btn-share-audio')?.addEventListener('click', async () => {
  if (!isSharingAudio) {
    try {
      ...
      showToast('? Năo foi possível compartilhar áudio.');
      console.error(err);
    }
  } else {
    if (audioShareStream) {
...
// Menu de contexto nos participantes para moderaçăo
...
function applyWallpaper(type) {
  ...
  if (type === 'dark') {
    ...
    // ? FORÇA COBERTURA TOTAL SEM DISTORÇĂO
    document.body.style.backgroundSize = 'cover';
    
    // ? CENTRALIZAÇĂO PERFEITA EM QUALQUER DIMENSĂO
    ...
    // ? FORÇA RENDERIZAÇĂO EM ALTA RESOLUÇĂO
...
function renderDiscoverFeed() {
  ...
  if (sorted.length === 0) {
    console.log('âšď¸ [FEED] Nenhuma postagem encontrada');
  ...
  discoverFeed.innerHTML = sorted.map(post => {
    ...
    const commentsHtml = (post.comments || []).slice(-3).map(c =>
      `<div class="discover-comment"><strong>${escHtml(c.username)}</strong> ˇ ${escHtml(c.text)}</div>`
    ).join('');
    return `
    <article class="discover-card" data-post-id="${post.id}">
      <div class="discover-votes">
        ...
        <button type="button" class="discover-vote-btn ${vote === -1 ? 'voted-down' : ''}" data-vote="-1" data-id="${post.id}">âź</button>
      </div>
...
// Botăo Atualizar Feed
document.addEventListener('click', (e) => {
  if (e.target && e.target.id === 'btn-discover-refresh') {
    console.log('?? Botăo Atualizar clicado - recarregando feed');
...
// ? CORREÇĂO: Sistema de timeout e tratamento de erros para o feed
let feedLoadTimeout = null;
let feedLoaded = false;

// Limpa loading automaticamente após 8 segundos se năo houver resposta
function startFeedLoadTimeout() {
  console.log('âąď¸ [FEED] Iniciando timeout de carregamento (8s)');
  feedLoadTimeout = setTimeout(() => {
    if (!feedLoaded) {
      console.warn('?? [FEED] Timeout! Servidor năo respondeu');
      discoverFeed.innerHTML = `
        <div class="discover-empty">
          <div style="font-size: 48px; margin-bottom: 16px;">âąď¸</div>
          <h3 style="margin: 0 0 8px 0;">Tempo de carregamento esgotado</h3>
          <p style="color: #888; margin: 0 0 16px 0;">O servidor năo respondeu. Tente recarregar a página.</p>
...
// ? CORREÇĂO FINAL: Năo solicita feed APENAS quando estiver na view do discover
...
socket.on('connect_error', (err) => {
  console.error('? [FEED] Erro de conexăo:', err.message);
  clearTimeout(feedLoadTimeout);
  discoverFeed.innerHTML = `
    <div class="discover-empty">
      <div style="font-size: 48px; margin-bottom: 16px;">?</div>
      <h3 style="margin: 0 0 8px 0;">Falha na conexăo</h3>
      <p style="color: #888; margin: 0 0 16px 0;">Năo foi possível carregar as postagens.</p>
      <button class="btn-ms" onclick="socket.connect(); renderDiscoverFeed();">?? Reconectar</button>
    </div>
  `;
});

// ? CORREÇĂO: Listener que estava FALTANDO para receber os posts do servidor
...
// ââ Configuraçőes do servidor ââ
const SERVER_SETTINGS_SECTIONS = {
    ...
    <div class="ms-block">
      ...
      <div class="srv-community-profile">
        <div class="srv-comm-banner" id="srv-comm-banner" style="${server.commBanner ? `background-image:url(${server.commBanner})` : ''}">
          ...
          <button type="button" class="banner-edit-btn" id="btn-srv-comm-banner">đź Alterar banner</button>
        </div>
        ...
        <div class="ms-field"><label>Descriçăo</label>
  ...
  eventos: (server) => {
    const evs = (server.events || []).map((ev, i) =>
      `<div class="srv-channel-row"><span>đ ${escHtml(ev.name)} â ${escHtml(ev.date)}</span>
  ...
  cargos: (server) => {
    ...
    const rows = roles.map((r, i) => `
      <div class="srv-role-row">
        ...
        <div class="clan-actions">
          <button type="button" data-edit-role="${i}">Permissőes</button>
          <button type="button" data-del-role="${i}">Remover</button>
        </div>
      </div>`).join('');
    return `
    <h2 class="ms-section-title">Cargos</h2>
    <p class="ms-section-desc">Crie e gerencie cargos com permissőes específicas.</p>
  ...
  modelo: (server) => {
    ...
    <p class="ms-section-desc">Compartilhe a configuraçăo completa do seu servidor para outras pessoas criarem um idęntico instantaneamente.</p>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div>
          ...
          <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.5rem">
            ...
            <li>? Cargos e permissőes</li>
            <li>? Emojis e figurinhas</li>
            <li>? Configuraçőes do servidor</li>
            <li>? Estrutura e organizaçăo</li>
          </ul>
        </div>
        <div>
          <h4 style="margin-bottom:0.75rem">? O que NĂO é copiado:</h4>
          <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.5rem">
            <li>? Mensagens e conteúdo</li>
            <li>? Membros e usuários</li>
            <li>? Ícone e banner personalizados</li>
            <li>? Webhooks e integraçőes</li>
    ...
    <div class="ms-block">
      ...
      <div class="ms-field"><label>Descriçăo do modelo</label>
  ...
  integracoes: (server) => {
    ...
    return `
    <h2 class="ms-section-title">Integraçőes</h2>
    ...
    <div class="ms-block">
      <div class="ms-block-title">?? Integraçăo Discord Webhook</div>
      ...
      <button type="button" class="btn-ms btn-ms-primary" id="btn-save-discord-integration" style="margin-top:0.5rem">?? Salvar integraçăo</button>
      <p class="pf-hint" style="margin-top:0.75rem;font-size:0.8rem">
        ?? Para criar um webhook no Discord: Configuraçőes do Servidor â Integraçőes â Webhooks â Novo Webhook
      </p>
    </div>
  ...
  seguranca: (server) => {
    ...
    return `
    <h2 class="ms-section-title">Segurança</h2>
    <p class="ms-section-desc">Proteçăo contra raids e configuraçőes de segurança.</p>
    <div class="ms-block">
      <div class="ms-block-title">?? Proteçăo contra Raids</div>
      ...
      <label class="toggle-row"><span>Verificaçăo de e-mail obrigatória</span><input type="checkbox" id="sec-email-verify" ${s.emailVerify ? 'checked' : ''} /></label>
      ...
      <div class="ms-field" style="margin-top:0.75rem"><label>Nível de verificaçăo</label>
      ...
      <button type="button" class="btn-ms btn-ms-primary" id="btn-save-security" style="margin-top:0.75rem">Salvar configuraçőes</button>
    </div>`;
  },
  ...
  auditoria: (server) => {
    ...
    <p class="ms-section-desc">Registro de açőes no servidor.</p>
    <div class="ms-block srv-audit-log">${rows || '<p class="pf-hint">Nenhuma açăo registrada.</p>'}</div>`;
  },

  notificacoes: (server) => {
    const s = server.settings || {};
    return `
    <h2 class="ms-section-title">Notificaçőes</h2>
    <p class="ms-section-desc">Controle alertas deste servidor.</p>
    <div class="ms-block">
      <label class="toggle-row"><span>Notificaçőes do servidor</span><input type="checkbox" id="srv-notif-all" ${s.notifications !== false ? 'checked' : ''} /></label>
  ...
  excluir: (server) => `
    ...
    <p class="ms-section-desc">Esta açăo é irreversível. Todos os canais e mensagens serăo perdidos.</p>
    <div class="ms-block">
...
function bindServerSettingsEvents(sectionId, server) {
  if (sectionId === 'geral') {
    ...
    document.getElementById('btn-srv-save-geral')?.addEventListener('click', () => {
      ...
      addAuditLog(server, `Editou informaçőes gerais`);
  ...
  if (sectionId === 'emojis') {
    // Botăo para fazer upload de imagem como emoji
    ...
    document.getElementById('srv-emoji-file')?.addEventListener('change', (e) => {
      ...
      // ? Validaçăo de tamanho máximo 2MB
      ...
      setTimeout(() => {
        if (typeof updateEmojiCategories === 'function') {
          ...
          if (emojiPicker.classList.contains('active')) {
            // ? FORÇA RENDERIZAÇĂO COMPLETA DA CATEGORIA DO SERVIDOR
  ...
  if (sectionId === 'membros') {
    ...
    if (list) {
      ...
      list.innerHTML = all.length
        ...
        : '<p class="pf-hint">Apenas vocę no momento.</p>';
    }
  }
  ...
  if (sectionId === 'cargos') {
    ...
    srvMsContent.querySelectorAll('[data-edit-role]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        srvMsContent.insertAdjacentHTML('beforeend', `<div id="role-perm-panel" class="ms-block" style="margin-top:1rem"><div class="ms-block-title">Permissőes: ${escHtml(r.name)}</div>${checked}<button type="button" class="btn-ms btn-ms-primary" id="btn-save-role-perms" style="margin-top:0.5rem">Salvar</button></div>`);
  ...
  if (sectionId === 'integracoes') {
    ...
    document.getElementById('btn-import-discord-model')?.addEventListener('click', async () => {
      ...
      try {
        ...
        // Se falhar, usa modo offline e cria estrutura padrăo baseada no código
        if (!guild) {
          guild = {
            id: inviteCode,
        ...
        // Cria estrutura completa de canais padrăo do Discord
        server.channels = [
        ...
        server.channels[0].category = 'INFORMAÇĂES';
        server.channels[1].category = 'INFORMAÇĂES';
        ...
        showToast('? Modelo padrăo do Discord criado!');
        console.error(err);
      }
    });

    // Integraçăo Discord Webhook
    document.getElementById('btn-save-discord-integration')?.addEventListener('click', () => {
      ...
      addAuditLog(server, 'Configurou integraçăo com Discord');
      
      if (webhookUrl) {
        showToast('? Integraçăo com Discord salva! Mensagens serăo sincronizadas.');
      } else {
        showToast('Integraçăo com Discord desativada.');
      }
  ...
  if (sectionId === 'seguranca') {
    document.getElementById('btn-save-security')?.addEventListener('click', () => {
      ...
      saveServers(); addAuditLog(server, 'Atualizou configuraçőes de segurança');
      showToast('Configuraçőes de segurança salvas!');
    });
  }
  ...
  if (sectionId === 'notificacoes') {
    document.getElementById('btn-srv-save-notif')?.addEventListener('click', () => {
      ...
      saveServers(); showToast('Notificaçőes salvas!');
    });
  }
...
socket.on('friend:accepted', ({ by }) => {
  ...
  showToast(`? ${by} aceitou sua solicitaçăo!`);
  renderFriendsModal();
});

socket.on('friend:rejected', ({ by }) => {
  showToast(`${by} recusou sua solicitaçăo.`);
});
...
function sendFriendRequest(toUsername) {
  ...
  if (friends.find(f => f.username === target)) { showToast('Já săo amigos!'); return; }
  socket.emit('friend:request', { to: target });
  showToast(`Solicitaçăo enviada para ${target}!`);
}
...
function renderFriendRequests() {
  ...
  if (friendRequests.length === 0) {
    pane.innerHTML = '<div class="empty-state">??<p>Nenhuma solicitaçăo pendente.</p></div>';
    return;
  }
  pane.innerHTML = friendRequests.map(r => `
    <div class="friend-card">
      ...
      <div class="friend-info">
        ...
        <span class="friend-status pending">Solicitaçăo de amizade</span>
      </div>
      <div class="friend-actions">
        <button class="friend-btn friend-btn-accept" data-accept="${escHtml(r.from)}">â</button>
        <button class="friend-btn friend-btn-reject" data-reject="${escHtml(r.from)}">â</button>
      </div>
    </div>`).join('');
  ...
  pane.innerHTML = `
    <div class="friend-add-box">
      ...
      <div class="friend-add-row">
        ...
        <button type="button" class="btn-neon" id="btn-send-friend-req">Enviar solicitaçăo</button>
      </div>
    </div>`;
...
btnTypewriter.addEventListener('click', () => {
  ...
  showToast('â¨ Máquina de Escrever');
});
...
btnTypewriterSave.addEventListener('click', () => {
  ...
  try {
    ...
    typeSound.play().catch(() => {
      // Fallback se o arquivo năo existir
      console.log('Som type.mp3 năo encontrado');
    });
  } catch {}
  ...
  if (typewriterSaves.length === 0) {
    typewriterSavesList.innerHTML = `
      <div style="padding: 2rem; text-align: center; color: #888;">
        Nenhum salvamento ainda. Clique no botăo acima!
      </div>
    `;
    return;
  }
...
// ================================================
// SISTEMA DE CONFIGURAÇĂES VISUAIS
// ================================================
...
// Atualiza pré-visualizaçăo em tempo real
...
vsResetBtn.addEventListener('click', () => {
  ...
  loadVisualSettings();
  showToast('Aparęncia resetada para padrăo');
});

vsSaveBtn.addEventListener('click', () => {
  // Valida legibilidade
  if (!validateContrast(vsBubbleColor.value, vsTextColor.value)) {
    showToast('?? Combinaçăo de cores com baixa legibilidade. Escolha cores com maior contraste.');
    return;
  }
  ...
  localStorage.setItem('zx_visual_profile', JSON.stringify(userVisualProfile));
  visualSettingsModal.classList.add('hidden');
  showToast('? Configuraçőes salvas com sucesso!');
});

// Eventos de alteraçăo para pré-visualizaçăo em tempo real
...
// Adiciona animaçőes CSS para toast
...
// ? REMOVER ESTA FUNÇĂO COMPLETAMENTE - DUPLICADA
// A FUNÇĂO OFICIAL ESTÁ NO chat-input.js
...
if (!btnEmoji || !gifPicker || !stickerPicker) {
  // Năo inicializa pickers se elementos năo existem (ex: server.html)
} else {
...
const stickerPacks = {
  default: { name: 'Padrăo', stickers: ['??','??','??','??','??','??','??','??','??','??','??','??','??','?','??','??'] },
  reactions: { name: 'Reaçőes', stickers: ['??','??','??','??','??','??','??','??','??','??','??','??','??','??','??','??'] },
...
function updateEmojiCategories() {
  ...
  if (server && server.emojis && server.emojis.length > 0) {
    // Adicionar botăo de categoria do servidor se năo existir
    ...
    // Remover botăo se năo houver emojis
    if (existingServerBtn) {
          ...
          <div class="modal-modern" style="width: 500px;">
            ...
            <div style="padding: 1.5rem; display: flex; flex-direction: column; gap: 16px;">
              ...
              <div class="ms-field">
                <label>Opçőes</label>
                <div id="poll-options">
                  <input type="text" class="poll-option-input" placeholder="Opçăo 1" maxlength="60" />
                  <input type="text" class="poll-option-input" placeholder="Opçăo 2" maxlength="60" />
                </div>
                <button type="button" class="btn-ghost-sm" onclick="
                  ...
                  input.placeholder = 'Opçăo ' + (container.children.length + 1);
                  input.maxLength = 60;
                  container.appendChild(input);
                ">+ Adicionar opçăo</button>
              </div>
              <div class="create-channel-actions" style="margin-top: 1rem;">
                ...
                <button type="button" class="btn-neon" onclick="
                  ...
                  if (!question || options.length < 2) {
                    showToast('Digite a pergunta e pelo menos 2 opçőes');
                    return;
                  }
...
fileUploadInput?.addEventListener('change', (e) => {
  ...
  for (const file of files) {
    if (file.size > 8 * 1024 * 1024) { showToast(`?? ${file.name} muito grande. Máximo 8 MB.`); continue; }
    const reader = new FileReader();
    reader.onload = () => {
