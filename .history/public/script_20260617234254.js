...
(function fixDmSendFinal() {
  ...
  function createMessageWithAvatar(msg, isMe) {
    ...
    if (avatarData) {
      console.log('[AVATAR] Aplicando imagem para', sender); // log para verificar
      avatarDiv.style.backgroundImage = `url(${avatarData})`;
      avatarDiv.style.backgroundSize = 'cover';
      avatarDiv.style.backgroundPosition = 'center';
      avatarDiv.style.background = 'none'; // remove o gradiente, mas mantém a imagem
      // Reforça o background-image com !important via setProperty
      avatarDiv.style.setProperty('background-image', `url(${avatarData})`, 'important');
      avatarDiv.style.setProperty('background-size', 'cover', 'important');
      avatarDiv.style.setProperty('background-position', 'center', 'important');
      avatarDiv.style.setProperty('background-color', 'transparent', 'important');
      avatarDiv.textContent = '';
    } else {
      avatarDiv.textContent = sender.charAt(0).toUpperCase();
    }

    // ... resto do código (corpo da mensagem)
