...
inputAvatar?.addEventListener('change', (e) => {
  ...
  readImageFile(file, (dataUrl) => {
    ...
    // Sincronizar avatar com o servidor para que amigos possam ver
    if (window.socket && window.socket.connected) {
...
function friendCard(uname, isOnline) {
    ...
    const avatarUrl = getFriendAvatar(uname) || (uname.toLowerCase() === (window.username || '').toLowerCase() ? (localStorage.getItem('zx_avatar') || '') : '');
...
// ============================================================
// BOTÃO DE UPLOAD DE AVATAR (VERSÃO ROBUSTA)
// ============================================================
(function robustUploadButton() {
  console.log('[ROBUST] Iniciando...');

  // Cria input de arquivo (uma vez)
  let fileInput = document.getElementById('friend-avatar-upload');
  if (!fileInput) {
    fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.accept = 'image/*';
    fileInput.id = 'friend-avatar-upload';
    ...
    fileInput.addEventListener('change', function(e) {
      ...
      reader.onload = function(ev) {
        localStorage.setItem('avatar_' + username.toLowerCase(), ev.target.result);
        alert('✅ Avatar de ' + username + ' salvo!');
        // Recarrega a lista
        if (typeof window.renderDmList === 'function') {
          window.renderDmList();
        } else {
  ...
  function addButtons() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) {
      console.warn('[ROBUST] Lista não encontrada');
      return;
    }

    const items = list.querySelectorAll('.dm-conv-item');
    if (items.length === 0) {
      console.warn('[ROBUST] Nenhum item na lista');
      return;
    }

    items.forEach(item => {
      // Evita duplicação
      if (item.querySelector('.dm-upload-btn')) return;

      const nameEl = item.querySelector('.dm-conv-name');
      if (!nameEl) {
        console.warn('[ROBUST] Nome não encontrado no item');
        return;
      }
      ...
      btn.className = 'dm-upload-btn';
      btn.textContent = '📷';
      btn.title = 'Definir avatar para ' + username;
      btn.style.cssText = 'background:rgba(255,255,255,0.1);border:none;color:#fff;cursor:pointer;font-size:14px;padding:2px 6px;border-radius:4px;margin-left:auto;display:none;';
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        fileInput.dataset.username = username;
        fileInput.click();
      });
      item.addEventListener('mouseenter', () => { btn.style.display = 'block'; });
      item.addEventListener('mouseleave', () => { btn.style.display = 'none'; });
      item.appendChild(btn);
      console.log('[ROBUST] Botão adicionado para', username);
    });
  }

  // Observa a lista para novos itens
  function observeList() {
    ...
    console.log('[ROBUST] Observador ativado');
  }

  // Executa imediatamente e periodicamente (fallback)
  function start() {
    observeList();
    addButtons();
    // Fallback: a cada 2 segundos, tenta adicionar botões
    setInterval(addButtons, 2000);
    console.log('[ROBUST] Sistema ativo');
  }
  ...
  // Também quando a view DM for aberta
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
  ...
  console.log('[ROBUST] Script carregado.');
})();

// ============================================================
// FORÇA ATUALIZAÇÃO DO AVATAR APÓS UPLOAD
// ============================================================
(function forceAvatarUpdate() {
  console.log('[FORCE-UPDATE] Iniciando...');

  // Função que aplica avatar em todos os lugares
  function applyAvatarToAll() {
    // Lista de DMs
    document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
      const avatar = item.querySelector('.dm-avatar');
      const name = item.querySelector('.dm-conv-name');
      if (avatar && name) {
        const username = name.textContent.trim();
        if (username) {
          const img = localStorage.getItem('avatar_' + username.toLowerCase());
          if (img && img.startsWith('data:image')) {
            ...
            avatar.textContent = '';
            console.log('[FORCE-UPDATE] Avatar aplicado a', username);
          } else {
            // Fallback iniciais
            avatar.style.backgroundImage = 'none';
            avatar.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
    ...
    document.querySelectorAll('.msg-avatar').forEach(el => {
      const parent = el.closest('.message, .dm-message');
      if (parent) {
        const nameEl = parent.querySelector('.msg-username, .username');
        if (nameEl) {
          const username = nameEl.textContent.trim();
          if (username) {
            const img = localStorage.getItem('avatar_' + username.toLowerCase());
            if (img && img.startsWith('data:image')) {
              ...
              el.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
  ...
  // Intercepta o evento de upload para forçar atualização
  const originalFileInput = document.getElementById('friend-avatar-upload');
  if (originalFileInput) {
    const originalChange = originalFileInput.onchange;
    originalFileInput.onchange = function(e) {
      // Chama o handler original (que salva no localStorage)
      if (typeof originalChange === 'function') {
        originalChange.call(this, e);
      }
      // Depois de salvar, força a atualização
      setTimeout(() => {
        // Tenta recriar a lista (se disponível)
        if (typeof window.renderDmList === 'function') {
          window.renderDmList();
        }
        // Aplica avatares manualmente
        setTimeout(applyAvatarToAll, 100);
        setTimeout(applyAvatarToAll, 300);
        console.log('[FORCE-UPDATE] UI atualizada após upload');
      }, 200);
    };
  }

  // Expõe função para forçar manualmente
  window.forceAvatarUI = applyAvatarToAll;

  console.log('[FORCE-UPDATE] Sistema ativo. Use forceAvatarUI() para forçar atualização.');
})();