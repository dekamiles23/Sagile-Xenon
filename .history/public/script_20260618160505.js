...
function setFriendAvatar(username, avatarData) {
  ...
  // Sync to dm-avatar-call-fix.js cache so both systems stay consistent
  if (window.userAvatarCache) window.userAvatarCache[key] = avatarData;
}
...
function renderDmMessages() {
  ...
  container.innerHTML = messages.map(msg => {
    ...
    // Usar avatar do cache: msg.avatar (campo enviado pelo servidor), userAvatarCache ou friendAvatarCache
    const _senderKey = (msg.from || '').toLowerCase();
    const avatarUrl = isSelf
      ? (profileAvatarUrl || localStorage.getItem('zx_avatar') || '')
      : (msg.avatar || (window.userAvatarCache || {})[_senderKey] || (window.friendAvatarCache || {})[_senderKey] || '');
...
socket.on('dm:user:profile', (data) => {
  ...
  if (data.avatar) {
    ...
    if (typeof currentDmUser !== 'undefined' && (currentDmUser || '').toLowerCase() === data.username.toLowerCase()) {
      ...
      // Re-renderizar mensagens DM para aplicar o avatar novo a todas as mensagens
      if (typeof renderDmMessages === 'function') renderDmMessages();
    }
    // Patch avatares de mensagens já renderizadas no server-chat (#messages-area)
    const _avKey = data.username.toLowerCase();
    document.querySelectorAll(`#messages-area .msg-avatar[data-username]`).forEach(el => {
      if ((el.dataset.username || '').toLowerCase() === _avKey) {
    ...
    // Re-renderizar lista DM e modal de amigos
    if (typeof renderDmList === 'function') renderDmList();
    if (typeof renderFriendsModal === 'function') renderFriendsModal();
  }
});
...
socket.on('friends:presence', ({ online, statuses, avatars }) => {
  ...
  // Popula caches de avatar com o mapa enviado pelo servidor
  if (avatars) {
    Object.keys(avatars).forEach(uname => {
      const av = avatars[uname];
      if (!av) return;
      const key = uname.toLowerCase();
      // Atualiza friendAvatarCache (usado por getFriendAvatar/renderDmList)
      if (window.friendAvatarCache) window.friendAvatarCache[key] = av;
      // Atualiza userAvatarCache (usado por dm-avatar-call-fix e server-chat)
      if (window.userAvatarCache)   window.userAvatarCache[key]   = av;
    });
  }
...
socket.on('friend:request', ({ from, avatar }) => {
  // Cache sender avatar when the request arrives
  if (avatar && from) {
    const key = from.toLowerCase();
    if (window.friendAvatarCache) window.friendAvatarCache[key] = avatar;
    if (window.userAvatarCache)   window.userAvatarCache[key]   = avatar;
  }
...
socket.on('friend:accepted', ({ by, avatar }) => {
  // Cache accepter avatar when notified
  if (avatar && by) {
...
// ============================================================
// GLOBAL AVATAR SYNC
// Atualiza todos os avatares existentes na tela
// ============================================================

(function () {
    if (window.__globalAvatarSyncLoaded) return;
    window.__globalAvatarSyncLoaded = true;

    function updateAllAvatars() {
        if (!window.friendAvatarCache) return;
        ...
        document.querySelectorAll("[data-username]").forEach(el => {
            ...
            const avatar = window.friendAvatarCache[username];
    ...
    setInterval(updateAllAvatars, 500);

    console.log("[GLOBAL AVATAR] Sincronizador iniciado");
})();