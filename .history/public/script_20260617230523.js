...
// ============================================================
// RECRIA A BARRA DO SERVIDOR PARA O DM (COM PICKERS FUNCIONAIS)
// ============================================================
(function recreateServerBarInDM() {
  console.log('[DM-BAR] Iniciando recriação da barra...');

  // ── 1. GARANTE QUE OS PICKERS EXISTAM NO BODY ──
  function ensurePickers() {
    ...
    pickerData.forEach(({ id, items, className, render }) => {
      ...
      if (!picker) {
        ...
        console.log(`[DM-BAR] Picker criado: ${id}`);
      }
      ...
      if (!picker.querySelector(`.${className}`)) {
        ...
        items.forEach(item => {
          ...
          // O elemento filho já tem a classe, mas precisamos que o clique seja capturado
          // Vamos usar event delegation no próprio picker para capturar cliques nos itens
          grid.appendChild(el.firstElementChild || el);
        });
        picker.appendChild(grid);
        console.log(`[DM-BAR] Picker populado: ${id}`);
      }
  ...
  // ── 2. CRIA A BARRA DE INPUT ──
  function createBar() {
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      console.warn('[DM-BAR] dm-input-area não encontrado, criando...');
      const dmView = document.getElementById('dm-view');
      if (!dmView) return;
      const newArea = document.createElement('div');
      newArea.id = 'dm-input-area';
      newArea.style.cssText = 'padding: 12px 16px; border-top: 1px solid rgba(255,0,255,0.1);';
      const messages = dmView.querySelector('#dm-messages-area, .messages-area');
      if (messages) {
        messages.parentNode.insertBefore(newArea, messages.nextSibling);
      } else {
        dmView.appendChild(newArea);
      }
      // Recursão para recriar a barra agora que o container existe
    ...
    // ── 3. CONECTA OS EVENTOS DOS BOTÕES ──
    function connectPicker(btnId, pickerId) {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn) {
        console.warn(`[DM-BAR] Botão ${btnId} não encontrado`);
        return;
      }
      if (!picker) {
        console.warn(`[DM-BAR] Picker ${pickerId} não encontrado`);
        return;
      }
      ...
      newBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log(`[DM-BAR] Clique em ${btnId} → abrir ${pickerId}`);
      ...
      console.log(`[DM-BAR] Botão ${btnId} conectado ao picker ${pickerId}`);
    }

    connectPicker('dm-btn-emoji', 'emoji-picker');
    connectPicker('dm-btn-gif', 'gif-picker');
    connectPicker('dm-btn-stickers', 'sticker-picker');

    // ── 4. INSERE ITENS DOS PICKERS NO INPUT ──
    document.addEventListener('click', function(e) {
      ...
      if (value) {
        ...
        console.log(`[DM-BAR] Inserido: ${value}`);
      }
    });
  ...
  // ── 9. OBSERVA A VIEW DM PARA CRIAR A BARRA QUANDO ABRIR ──
  function waitForDM() {
    const dmView = document.getElementById('dm-view');
    if (!dmView) {
  ...
  // Fallback: clique nos botões de DM
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(() => {
        ensurePickers();
  ...
  console.log('[DM-BAR] Script de recriação carregado');
})();