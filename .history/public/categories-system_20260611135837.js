  ...
  if (e.target.dataset.action === 'suggest' || e.target.closest('[data-action="suggest"]')) {
    ...
    if (serverId) {
      ...
      if (server) {
        ...
        // 1. Carregar lista existente
        let suggestedCommunities = JSON.parse(localStorage.getItem('suggestedCommunities') || '[]');
        
        // 2. Verificar se já existe para não duplicar
        const alreadyExists = suggestedCommunities.some(c => c.id === server.id);
        if (alreadyExists) {
          alert('⚠️ Esta comunidade já está nas sugeridas!');
        } else {
          // 3. Adicionar nova comunidade
          suggestedCommunities.unshift({
            id: server.id,
            name: server.name,
            icon: server.icon || '',
          ...
          // 4. Salvar no localStorage (persistente)
          localStorage.setItem('suggestedCommunities', JSON.stringify(suggestedCommunities));
          
          console.log('✅ Comunidade adicionada às sugeridas diretamente!');
