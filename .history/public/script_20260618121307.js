...
// ============================================================
// SUBSTITUI RENDERDMLIST PARA INCLUIR BOTÃO 📷
// ============================================================
(function replaceRenderDmList() {
  console.log('[RENDER-FIX] Iniciando...');

  // 1. Cria input de arquivo (uma vez)
  ...
  if (!fileInput) {
    ...
    fileInput.addEventListener('change', function(e) {
      ...
      reader.onload = function(ev) {
        ...
        if (window.socket && window.socket.connected) {
          window.socket.emit('upload-avatar', { username: targetUser, avatar: base64 });
          alert('📤 Enviando avatar de ' + targetUser);
        } else {
  ...
  // 2. Função que cria um item da lista com botão
  function createItemWithButton(username) {
    const item = document.createElement('div');
    item.className = 'dm-conv-item';
    item.dataset.username = username;
    item.style.cssText = 'display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;cursor:pointer;transition:background 0.15s;';

    // Avatar
    const avatarDiv = document.createElement('div');
    avatarDiv.className = 'dm-avatar';
    avatarDiv.style.cssText = 'width:32px;height:32px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:14px;color:white;background:linear-gradient(135deg,#8b00ff,#ff00ff);overflow:hidden;background-size:cover;background-position:center;background-repeat:no-repeat;';

    // Aplica avatar se existir
    ...
    // Nome
    const nameDiv = document.createElement('div');
    nameDiv.className = 'dm-conv-name';
    nameDiv.textContent = username;
    nameDiv.style.cssText = 'color:#eee;font-size:14px;flex:1;';
    ...
    btn.style.cssText = 'background:rgba(255,255,255,0.1);border:none;color:#fff;cursor:pointer;font-size:14px;padding:2px 6px;border-radius:4px;margin-left:auto;display:block;';
    btn.addEventListener('click', function(e) {
      e.stopPropagation();
    ...
    item.addEventListener('click', function(e) {
      if (e.target.closest('.dm-upload-btn')) return; // Ignora clique no botão
      const uname = this.dataset.username || this.querySelector('.dm-conv-name')?.textContent?.trim();
      if (uname && typeof window.openDmChat === 'function') {
  ...
  // 3. Substitui renderDmList
  window.renderDmList = function() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) {
      console.warn('[RENDER-FIX] Lista não encontrada');
      return;
    }
    ...
    if (unique.size === 0) {
      list.innerHTML = '<div style="padding:20px;text-align:center;color:#666;">Nenhuma conversa ainda</div>';
      return;
    }

    // Cria itens com botão
    unique.forEach(name => {
      const item = createItemWithButton(name);
      list.appendChild(item);
    });

    console.log('[RENDER-FIX] Lista renderizada com', unique.size, 'itens (com botões 📷)');
  };

  // 4. Força a renderização imediata
  setTimeout(() => {
    if (typeof window.renderDmList === 'function') {
      window.renderDmList();
    }
  ...
  // 5. Observa abertura da view DM
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(window.renderDmList, 100);
        setTimeout(window.renderDmList, 300);
      }
    };
  }

  console.log('[RENDER-FIX] renderDmList substituída com sucesso.');
})();