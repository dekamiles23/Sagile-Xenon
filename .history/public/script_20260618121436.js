...
// ============================================================
// BOTÃO 📷 BASEADO NO NOME (FUNCIONA EM QUALQUER LISTA)
// ============================================================
(function buttonByName() {
  console.log('[BTN-NAME] Iniciando...');
  ...
  // 2. Função que adiciona o botão ao lado do nome
  function addButtonToFriend(friendName) {
    // Procura qualquer elemento que contenha exatamente o nome do amigo
    const allElements = document.querySelectorAll('#dm-conversations-list *');
    let targetElement = null;
    allElements.forEach(el => {
      if (el.textContent.trim() === friendName) {
        targetElement = el;
      }
    });

    if (!targetElement) {
      console.warn('[BTN-NAME] Nome não encontrado:', friendName);
      return false;
    }

    // O item da lista é o pai mais próximo que é um div com display flex
    let item = targetElement.closest('div[style*="display:flex"], div[style*="display: flex"], .dm-conv-item, .conv-item');
    if (!item) {
      // Se não achar por estilo, usa o pai direto
      item = targetElement.parentElement;
      if (!item) return false;
    }

    // Evita duplicar botão
    if (item.querySelector('.dm-upload-btn')) return true;

    const btn = document.createElement('button');
    btn.className = 'dm-upload-btn';
    btn.textContent = '📷';
    btn.title = 'Definir avatar para ' + friendName;
    btn.style.cssText = 'background:rgba(255,255,255,0.1);border:none;color:#fff;cursor:pointer;font-size:14px;padding:2px 6px;border-radius:4px;margin-left:auto;display:block !important;';
    btn.addEventListener('click', function(e) {
      e.stopPropagation();
      fileInput.dataset.username = friendName;
      fileInput.click();
    });
    item.appendChild(btn);
    console.log('[BTN-NAME] Botão adicionado para', friendName);
    return true;
  }

  // 3. Função que escaneia a lista e adiciona botão para cada amigo
  function scanList() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;

    // Pega todos os nomes que parecem ser de amigos (dentro da lista)
    const nameElements = list.querySelectorAll('.dm-conv-name, .username, .name, [class*="name"]');
    nameElements.forEach(el => {
      const name = el.textContent.trim();
      if (name && name !== (window.username || '')) {
        addButtonToFriend(name);
      }
    });

    // Fallback: se não encontrou pelos seletores, tenta por qualquer texto
    if (nameElements.length === 0) {
      const allText = list.querySelectorAll('div, span, p');
      allText.forEach(el => {
        const text = el.textContent.trim();
        if (text && text.length > 1 && !text.includes('MENSAGENS') && !text.includes('DIRETAS')) {
          addButtonToFriend(text);
        }
      });
    }
  }

  // 4. Executa periodicamente (polling agressivo)
  setInterval(scanList, 1000);

  // 5. Observa mudanças no DOM
  const observer = new MutationObserver(() => scanList());
  observer.observe(document.body, { childList: true, subtree: true });

  // 6. Executa imediatamente
  scanList();
  setTimeout(scanList, 300);
  setTimeout(scanList, 600);
  setTimeout(scanList, 1000);

  console.log('[BTN-NAME] Sistema ativo. Procurando por "Miles Deka"...');
})();