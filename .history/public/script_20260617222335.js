...
// ============================================================
// BARRA DE INPUT DO DM COM TODOS OS BOTÕES FUNCIONAIS
// ============================================================
(function fullDmInputBar() {
  function copyBar() {
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) return;

    const serverInput = document.querySelector('#chat-view .message-input-area .input-wrapper');
    if (!serverInput) {
      console.warn('[DM-INPUT] Barra do servidor não encontrada');
      return;
    }

    // Clona a barra
    const clone = serverInput.cloneNode(true);

    // Mapeia IDs para evitar conflitos
    const idMap = {
    ...
    if (gifBtn && gifPicker) {
      gifBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        document.querySelectorAll('.chat-picker.active').forEach(p => {
          if (p !== gifPicker) p.classList.remove('active');
        });
    ...
    if (stickerBtn && stickerPicker) {
      stickerBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        document.querySelectorAll('.chat-picker.active').forEach(p => {
          if (p !== stickerPicker) p.classList.remove('active');
        });
    ...
    console.log('[DM-INPUT] Barra completa com todos os botões funcionais');
  }
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    ...
    document.addEventListener('DOMContentLoaded', copyBar);
  }
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      ...
      if (layoutId === 'dm-view') {
        setTimeout(copyBar, 100);
        setTimeout(copyBar, 300);
      }
    };
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(copyBar, 200);
    }
  });
})();

// ============================================================
// PATCH: Conecta botões de emoji, GIF e figurinhas no DM
// ============================================================
(function fixDmPickers() {
  function connectPickers() {
    // Mapeia botão → picker
    const pickers = [
      { btnId: 'dm-btn-emoji', pickerId: 'emoji-picker', itemSelector: '.emoji-item', insert: (text) => {
        const input = document.getElementById('dm-message-input');
        if (input) {
          const start = input.selectionStart;
          const end = input.selectionEnd;
      ...
      { btnId: 'dm-btn-gif', pickerId: 'gif-picker', itemSelector: '.gif-item img', insert: (text) => {
        const input = document.getElementById('dm-message-input');
        if (input) {
          input.value += `[GIF] ${text} `;
          input.focus();
        }
      }},
      { btnId: 'dm-btn-stickers', pickerId: 'sticker-picker', itemSelector: '.sticker-item', insert: (text) => {
        const input = document.getElementById('dm-message-input');
        if (input) {
          input.value += text + ' ';
          input.focus();
    ...
    pickers.forEach(({ btnId, pickerId, itemSelector, insert }) => {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn || !picker) {
        console.warn(`[DM-PICKER] ${btnId} ou ${pickerId} não encontrado`);
        return;
      }
      ...
      // Inserir item ao clicar dentro do picker
      picker.addEventListener('click', function(e) {
        ...
        if (itemSelector === '.gif-item img') {
          ...
          text = target.textContent || target.dataset.value || '';
        }
        if (text) {
          insert(text);
    ...
    console.log('[DM-PICKER] Pickers reconectados');
  }

  // Executa ao carregar e sempre que a view DM for aberta
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    connectPickers();
  } else {
    document.addEventListener('DOMContentLoaded', connectPickers);
  }
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(connectPickers, 150);
        setTimeout(connectPickers, 400);
      }
    };
  }
