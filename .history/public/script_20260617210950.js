...
// ============================================================
// STATUS EM TEMPO REAL NO CHAT PRIVADO (sem mexer na lista)
// ============================================================
(function fixPrivateChatStatus() {
  const STATUS_MAP = {
  ...
  function updateStatus(username) {
    ...
    // Procura o elemento de status no cabeçalho (pode ser um span com "● Online")
    const statusEl = modal.querySelector('.pcb-subtitle, [style*="font-size: 12px"]');
    if (!statusEl) return;
  ...
  if (window.socket) {
    socket.on('friend:status', (data) => {
      if (data && data.username) {
        const modal = document.getElementById('private-chat-modal');
        if (modal) {
          const title = modal.querySelector('.pcb-title');
          if (title && title.textContent.trim() === data.username) {
  ...
  console.log('[STATUS] Atualização em tempo real ativada para chat privado');
})();