...
// ============================================================
// STATUS EM TEMPO REAL NO CHAT DM (DEFINITIVO)
// ============================================================
(function dmStatusRealTime() {
  console.log('[DM-STATUS] Iniciando...');

  // Mapeamento de status
  const STATUS = {
    online:  { cor: '#23d18b', label: 'Online' },
    idle:    { cor: '#faa61a', label: 'Ausente' },
    dnd:     { cor: '#ed4245', label: 'Não perturbe' },
    offline: { cor: '#747f8d', label: 'Offline' }
  };
  ...
  function getStatus(username) {
    ...
    // Tenta obter do mapa global (mantido pelo friends-fix-v6.js)
    if (window.userStatuses && window.userStatuses[key]) {
      return window.userStatuses[key];
    }
    // Se estiver no conjunto de online, retorna online
  ...
  // Atualiza o status no cabeçalho do chat DM
  function updateStatus() {
    // Procura o elemento de status e o nome do usuário
    const statusEl = document.querySelector('.dm-status, .chat-header .dm-status');
    const nameEl = document.querySelector('.dm-username, .chat-header .dm-username');

    if (!statusEl) {
      // Se não encontrar, tenta novamente em 200ms (pode ser que o DOM ainda não esteja pronto)
    ...
    const info = STATUS[status] || STATUS.offline;

    // Atualiza o texto, a cor e a classe
  ...
  // ── Atualização via eventos do socket ──
  if (window.socket) {
    // Evento individual
    socket.on('friend:status', function(data) {
      if (data && data.username) {
        ...
        if (nameEl && nameEl.textContent.trim() === data.username) {
          console.log('[DM-STATUS] Atualizado via friend:status:', data.username, data.status);
          updateStatus();
        }
      }
    });

    // Evento de presença em massa (atualiza todos os amigos)
    socket.on('friends:presence', function() {
      const dmView = document.getElementById('dm-view');
      if (dmView && !dmView.classList.contains('hidden')) {
        console.log('[DM-STATUS] Atualizado via friends:presence');
        updateStatus();
      }
    });
  }
  ...
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
      if (layoutId === 'dm-view') {
        // Atualiza várias vezes para garantir (o DOM pode demorar a renderizar)
  ...
  setInterval(function() {
    ...
    if (dmView && !dmView.classList.contains('hidden')) {
      ...
      if (nameEl && nameEl.textContent.trim()) {
        // Atualiza sempre (se o status mudou, corrige)
        updateStatus();
      }
    }
  }, 3000);
  ...
  console.log('[DM-STATUS] Sistema ativo. Use updateDmStatus() para forçar atualização.');
})();