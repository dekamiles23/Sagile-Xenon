...
// ============================================================
// COPIA A BARRA DO SERVIDOR PARA O DM (FUNCIONAL)
// ============================================================
(function copyServerBarToDM() {
  console.log('[DM-BAR] Iniciando cópia da barra do servidor...');

  // ── 1. GARANTE QUE OS PICKERS GLOBAIS ESTEJAM NO BODY ──
  function ensurePickersInBody() {
    ['emoji-picker', 'gif-picker', 'sticker-picker'].forEach(id => {
      const picker = document.getElementById(id);
      if (picker && picker.parentNode !== document.body) {
        document.body.appendChild(picker);
        console.log(`[DM-BAR] Picker ${id} movido para o body`);
      }
    });
  }

  // ── 2. COPIA A BARRA DO SERVIDOR ──
  function copyBar() {
    ...
    if (!dmInputArea) {
      ...
      setTimeout(copyBar, 50);
      return;
    }

    // Procura a barra no servidor
    const serverWrapper = document.querySelector('#chat-view .message-input-area .input-wrapper');
    if (!serverWrapper) {
      console.warn('[DM-BAR] Barra do servidor não encontrada. Criando uma nova...');
      createFallbackBar();
      return;
    }

    // Clona a barra
    const clone = serverWrapper.cloneNode(true);

    // Remove pickers internos (já existem globais)
    clone.querySelectorAll('.chat-picker').forEach(p => p.remove());

    // Ajusta IDs para evitar conflitos
    const idMap = {
      'btn-plus': 'dm-btn-plus',
      'btn-emoji': 'dm-btn-emoji',
      'btn-gif': 'dm-btn-gif',
      'btn-stickers': 'dm-btn-stickers',
      'btn-attach-file': 'dm-btn-attach',
      'message-input': 'dm-message-input',
      'btn-voice': 'dm-btn-voice',
      'send-btn': 'dm-send-btn'
    };

    function replaceIds(el) {
      if (el.id && idMap[el.id]) el.id = idMap[el.id];
      for (let child of el.children) replaceIds(child);
    }
    replaceIds(clone);

    // Substitui a barra atual
    dmInputArea.innerHTML = '';
    dmInputArea.appendChild(clone);
    console.log('[DM-BAR] Barra copiada com sucesso');

    // Conecta os eventos via delegation
    connectEvents();
  }

  // ── 3. CRIA UMA BARRA FALLBACK (se a do servidor não existir) ──
  function createFallbackBar() {
    ...
    console.log('[DM-BAR] Barra fallback criada');

    connectEvents();
  }

  // ── 4. CONECTA EVENTOS (usando event delegation) ──
  function connectEvents() {
    ...
    // Event delegation para abrir pickers
    document.addEventListener('click', function(e) {
      const btn = e.target.closest('[id^="dm-btn-"]');
      if (!btn) return;
      const id = btn.id;
      const pickerId = pickerMap[id];
      if (pickerId) {
        e.preventDefault();
        e.stopPropagation();
        console.log(`[DM-BAR] Clique em ${id} → abrir ${pickerId}`);
    ...
    // Envio de mensagem (Enter e botão)
    document.addEventListener('click', function(e) {
    ...
    console.log('[DM-BAR] Eventos conectados');
  }

  // ── 5. FUNÇÃO PARA ABRIR/FECHAR PICKER ──
  function togglePicker(pickerId) {
    ...
    console.log(`[DM-BAR] ${pickerId} ${isActive ? 'aberto' : 'fechado'}`);
    if (isActive) {
      const input = document.getElementById('dm-message-input');
  ...
  function waitForDM() {
    ...
    const observer = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        console.log('[DM-BAR] DM aberta, copiando barra...');
        setTimeout(() => {
          ensurePickersInBody();
          copyBar();
        }, 100);
  ...
  // Fallback para cliques nos botões de DM
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(() => {
        ensurePickersInBody();
  ...
  console.log('[DM-BAR] Script de cópia carregado');
})();