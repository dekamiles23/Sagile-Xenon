...
// ============================================================
// BARRA DM COM EVENT DELEGATION (GARANTIDO)
// ============================================================
(function dmBarDelegate() {
  console.log('[DM] Iniciando...');

  // ── 1. CRIA OS PICKERS (MODAIS) ──
  function createPickers() {
    const pickerConfigs = [
      {
        ...
        render: (item) => `<span class="emoji-item" style="font-size:28px;cursor:pointer;padding:4px;border-radius:6px;display:inline-block;">${item}</span>`
      },
      {
        id: 'gif-picker',
        items: [
          ...
          'https://media.giphy.com/media/3o7abKhOpu0N9H8s/giphy.gif',
          'https://media.giphy.com/media/l0HlOBPvWqHj1D7J2/giphy.gif'
        ],
        className: 'gif-item',
        render: (item) => `<div class="gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;aspect-ratio:16/9;"><img src="${item}" style="width:100%;height:100%;object-fit:cover;"></div>`
      },
      {
        id: 'sticker-picker',
        items: ['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️'],
        className: 'sticker-item',
        render: (item) => `<span class="sticker-item" style="font-size:36px;cursor:pointer;padding:6px;border-radius:8px;display:inline-block;">${item}</span>`
      }
    ];

    pickerConfigs.forEach(({ id, items, className, render }) => {
      ...
      // Popula com itens se estiver vazio
      if (!picker.querySelector(`.${className}`)) {
        ...
        grid.style.cssText = `
          ...
          overflow-y: auto;
          justify-content: center;
          ${id === 'gif-picker' ? 'display:grid;grid-template-columns:repeat(2,1fr);gap:8px;' : ''}
        `;
        items.forEach(item => {
          const el = document.createElement('div');
          el.innerHTML = render(item);
          // Pega o primeiro filho (o elemento renderizado)
          const child = el.firstElementChild;
          if (child) grid.appendChild(child);
        });
        picker.appendChild(grid);
        console.log(`[DM] Picker populado: ${id}`);
      }

      // Garante que o picker esteja no body
    ...
    if (!moreModal) {
      ...
      console.log('[DM] Modal "Mais opções" criado');
    }
  }
  ...
  // ── 4. EVENT DELEGATION – CAPTURA CLIQUE EM QUALQUER BOTÃO DM ──
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('[id^="dm-btn-"]');
    if (!btn) return;

    const id = btn.id;
    console.log(`[DM] Clique capturado em: ${id}`);
    ...
    const map = {
      ...
      'dm-btn-plus': 'dm-more-modal'
    };

    const pickerId = map[id];
    if (pickerId) {
      e.preventDefault();
      e.stopPropagation();
  ...
  // ── 5. INSERE ITENS DOS PICKERS NO INPUT (via delegation) ──
  document.addEventListener('click', function(e) {
  ...
  console.log('[DM] Script com event delegation carregado');
})();