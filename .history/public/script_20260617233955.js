...
// ============================================================
// AVATAR NAS MENSAGENS DO DM (LEITURA DIRETA DO localStorage)
// ============================================================
(function dmAvatarDirect() {
  console.log('[DM-AVATAR-DIRECT] Iniciando...');

  // Função que retorna o avatar de um usuário (sempre lê do localStorage)
  function getAvatarDirect(username) {
    if (!username) return null;
    const key = username.toLowerCase();
    const currentUser = (window.username || '').toLowerCase();

    // 1. Próprio usuário → lê zx_avatar
    if (key === currentUser) {
      const saved = localStorage.getItem('zx_avatar');
      if (saved && saved.startsWith('data:image')) {
        return saved;
    ...
    // 2. Amigos → tenta avatar_username no localStorage
    const stored = localStorage.getItem(`avatar_${key}`);
    if (stored && stored.startsWith('data:image')) {
      return stored;
    }

    // 3. Tenta do mapa global (se existir)
    if (window.userAvatars && window.userAvatars[key]) {
      return window.userAvatars[key];
    }

    return null;
  }

  // ── Função para criar uma mensagem com avatar ──
  function createMessageWithAvatar(msg, isMe) {
    const sender = isMe ? (window.username || 'Você') : (msg.from || msg.username || 'Usuário');
    const avatarData = getAvatarDirect(sender);
  ...
  // ── Substitui a função addDmMessageToDom ──
  window.addDmMessageToDom = function(msg, isMe) {
    const messagesArea = document.getElementById('dm-messages-area');
    if (!messagesArea) return;
    ...
    const msgElement = createMessageWithAvatar(msg, isMe);
    messagesArea.appendChild(msgElement);
    messagesArea.scrollTop = messagesArea.scrollHeight;
  };

  // ── Substitui a função renderPrivateMessages (para o modal antigo) ──
  ...
  // ── Força a recarga das mensagens existentes ──
  function refreshMessages() {
    const chatArea = document.getElementById('dm-chat-area');
    if (chatArea && !chatArea.classList.contains('hidden')) {
      const username = chatArea.dataset.activeChat;
      if (username && typeof window.renderPrivateMessages === 'function') {
        window.renderPrivateMessages(username);
      }
    }
    // Também atualiza mensagens na área dm-messages-area
    const dmMessages = document.getElementById('dm-messages-area');
    if (dmMessages) {
      // Como não temos como recriar todas sem perder o contexto, apenas atualizamos os avatares existentes
      dmMessages.querySelectorAll('.message').forEach(msg => {
        const avatarDiv = msg.querySelector('.msg-avatar');
        if (!avatarDiv) return;
        const isMe = msg.style.flexDirection === 'row-reverse';
        const sender = isMe ? (window.username || 'Você') : 'amigo';
        // Não sabemos o nome exato, então pulamos.
        // Mas podemos usar o texto do nome que está no msg-meta
        const nameEl = msg.querySelector('.msg-username');
        if (nameEl) {
          const senderName = nameEl.textContent.trim();
          const avatarData = getAvatarDirect(senderName);
          if (avatarData) {
            avatarDiv.style.backgroundImage = `url(${avatarData})`;
            avatarDiv.style.backgroundSize = 'cover';
            avatarDiv.style.backgroundPosition = 'center';
            ...
            avatarDiv.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
            avatarDiv.textContent = senderName.charAt(0).toUpperCase();
          }
        }
      });
    }
    console.log('[DM-AVATAR-DIRECT] Mensagens recarregadas');
  }

  // ── Executa quando a DM é aberta ──
  function onDmOpen() {
  ...
  // ── Fallback: clique na lista de DMs ──
  document.addEventListener('click', function(e) {
    if (e.target.closest('.dm-conv-item')) {
      setTimeout(onDmOpen, 200);
    }
  });
  ...
  window.refreshDmAvatars = refreshMessages;
  window.setFriendAvatarDirect = function(username, base64) {
    if (!username || !base64) return;
    localStorage.setItem(`avatar_${username.toLowerCase()}`, base64);
    refreshMessages();
    console.log(`[DM-AVATAR-DIRECT] Avatar de ${username} definido`);
  };

  console.log('[DM-AVATAR-DIRECT] Ativo. Use refreshDmAvatars() para atualizar.');
})();