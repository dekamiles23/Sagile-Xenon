...
function renderUsersList() {
  ...
  // Atualiza contador de membros online EM TEMPO REAL
  const countEl = document.getElementById('online-count');
  if (countEl) {
    // Conta APENAS usuários válidos que realmente existem na lista
    const validUsers = onlineUsers.filter(u => u && u.id && u.name && u.name.trim() !== '');
    countEl.textContent = validUsers.length;
  }
