...
// ============================================================
// AVATAR AUTOMÁTICO COM POLLING (FORÇA CARGA DO SERVIDOR)
// ============================================================
(function autoAvatarWithPolling() {
  console.log('[AUTO-AVATAR] Iniciando...');

  let updateTimer = null;
  const requested = new Set();
  const fallbackInterval = 3000; // 3 segundos

  // ── 1. Obtém avatar do localStorage ──
  function getAvatar(username) {
    ...
    const stored = localStorage.getItem('avatar_' + key);
    if (stored && stored.startsWith('data:image')) return stored;
    return null;
  }

  // ── 2. Aplica avatar a um elemento ──
  function applyAvatar(el, username) {
    if (!el || !username) return false;
    const img = getAvatar(username);
    if (img) {
  ...
  // ── 3. Atualiza TODOS os avatares (debounced) ──
    ...
    updateTimer = setTimeout(() => {
      ...
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        ...
        if (avatar && name) {
          ...
          if (username) applyAvatar(avatar, username);
        }
      });
      // Mensagens
      document.querySelectorAll('.msg-avatar').forEach(el => {
        const parent = el.closest('.message, .dm-message, .private-message');
        if (parent) {
          const nameEl = parent.querySelector('.msg-username, .username');
          if (nameEl) {
            const username = nameEl.textContent.trim();
            if (username) applyAvatar(el, username);
          }
        }
      });
      // Cabeçalho do chat
      const headerAvatar = document.querySelector('.chat-header .dm-avatar, .chat-header .profile-avatar');
      const headerName = document.querySelector('.chat-header .dm-username');
      if (headerAvatar && headerName) {
        applyAvatar(headerAvatar, headerName.textContent.trim());
      }
      updateTimer = null;
    }, 50);
  }

  // ── 4. Solicita avatar ao servidor (se necessário) ──
  function requestAvatar(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (requested.has(key)) return;
    if (getAvatar(username)) return; // já tem

    if (window.socket && window.socket.connected) {
      requested.add(key);
      console.log('[AUTO-AVATAR] Solicitando avatar de', username);
      window.socket.emit('get-avatar', { username: username });
    }
  }

  // ── 5. Processa avatar recebido do servidor ──
  function receiveAvatar(username, base64) {
    if (!username || !base64) return;
    const key = username.toLowerCase();
    localStorage.setItem('avatar_' + key, base64);
    requested.delete(key);
    console.log('[AUTO-AVATAR] Avatar recebido para', username);
    refreshAllAvatars();
  }

  // ── 6. Socket listeners ──
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        receiveAvatar(data.username, data.avatar);
      }
    });
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        data.friends.forEach(f => {
          if (f.avatar) receiveAvatar(f.username, f.avatar);
        });
      }
    });
  }

  // ── 7. Polling: a cada 3 segundos, solicita avatares faltantes ──
  function pollMissingAvatars() {
    const dmView = document.getElementById('dm-view');
    if (!dmView || dmView.classList.contains('hidden')) return;
  ...
  // Inicia o polling
  setInterval(pollMissingAvatars, fallbackInterval);

  // ── 8. Integração com renderDmList ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      original.call(this);
      setTimeout(() => {
        document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
          ...
          if (name) {
            const username = name.textContent.trim();
            if (username) requestAvatar(username);
          }
        });
        refreshAllAvatars();
      }, 50);
  ...
  // ── 9. Observa a lista para novos itens ──
  ...
  // ── 10. Quando a view DM for aberta ──
  function onDmOpen() {
    setTimeout(() => {
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        const name = item.querySelector('.dm-conv-name');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    observeList();
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) onDmOpen();
    setTimeout(refreshAllAvatars, 200);
    setTimeout(refreshAllAvatars, 500);
  });

  // ── 12. Expõe funções para debug ──
  window.refreshAvatars = refreshAllAvatars;
  window.forceAvatarRefresh = refreshAllAvatars;

  console.log('[AUTO-AVATAR] Sistema ativo. Avatares serão solicitados automaticamente a cada 3 segundos.');
})();