...
// ============================================================
// SOLUÇÃO RADICAL: conecta botões da DM aos pickers existentes
// ============================================================
(function dmPickersRadical() {
  // Mapeia botão → picker
  const pickerMap = {
  ...
  // Função que garante que os pickers estejam no body
  function movePickersToBody() {
    Object.values(pickerMap).forEach(id => {
      const picker = document.getElementById(id);
      if (picker && picker.parentNode !== document.body) {
        document.body.appendChild(picker);
        console.log(`[DM] Pickers ${id} movido para o body`);
      }
    });
  }
  ...
  // Usa event delegation no documento para capturar cliques nos botões da DM
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('[id^="dm-btn-"]');
    if (!btn) return;

    // Verifica se é um dos botões de picker
  ...
  // Move os pickers para o body quando a página carregar
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    movePickersToBody();
  } else {
    document.addEventListener('DOMContentLoaded', movePickersToBody);
  }

  // Quando a view DM for aberta, move os pickers novamente (por segurança)
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(movePickersToBody, 100);
        setTimeout(movePickersToBody, 300);
      }
    };
  }

  console.log('[DM] Script radical carregado. Clique nos botões da DM para abrir pickers.');
})();