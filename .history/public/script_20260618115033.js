...
// ============================================================
// CORREÇÃO: AVATAR NÃO VOLTA PARA "M" (FORÇA IMAGEM)
// ============================================================
(function fixAvatarPersistence() {
  console.log('[PERSIST] Iniciando...');

  // Função que aplica avatar com verificação e persistência
  function applyAvatarPersistent(el, username) {
    if (!el || !username) return false;
    const key = username.toLowerCase();
    const img = localStorage.getItem('avatar_' + key);
    if (img && img.startsWith('data:image')) {
      ...
      el.textContent = '';
      console.log('[PERSIST] Avatar aplicado a', username);
      return true;
    } else {
      // Fallback: iniciais
      el.style.setProperty('background-image', 'none', 'important');
      el.style.setProperty('background', 'linear-gradient(135deg, #8b00ff, #ff00ff)', 'important');
      el.textContent = username.charAt(0).toUpperCase();
      console.log('[PERSIST] Fallback para', username);
      return false;
    }
  }

  // Sobrescreve a função global de aplicação de avatar (se existir)
  if (typeof window.applyAvatar === 'function') {
    window.applyAvatar = applyAvatarPersistent;
  }

  // Força a aplicação em todos os avatares existentes
  function forceAllAvatars() {
    document.querySelectorAll('.dm-avatar, .msg-avatar').forEach(el => {
      const parent = el.closest('.dm-conv-item, .message, .dm-message');
      if (!parent) return;
      const nameEl = parent.querySelector('.dm-conv-name, .msg-username, .username');
      if (!nameEl) return;
      const username = nameEl.textContent.trim();
      if (username) applyAvatarPersistent(el, username);
    });
  }

  // Observa mudanças no DOM e reaplica
  const observer = new MutationObserver(() => {
    // Quando o DOM mudar, reaplica avatares
    setTimeout(forceAllAvatars, 10);
  });
  observer.observe(document.body, { childList: true, subtree: true });

  // Polling agressivo a cada 500ms para manter a imagem
  setInterval(forceAllAvatars, 500);

  // Quando a view DM for aberta, força atualização
  const origShow = window.showLayout;
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(forceAllAvatars, 100);
        setTimeout(forceAllAvatars, 300);
        setTimeout(forceAllAvatars, 600);
      }
    };
  }

  // Também no clique em itens da lista
  document.addEventListener('click', function(e) {
    if (e.target.closest('.dm-conv-item')) {
      setTimeout(forceAllAvatars, 200);
    }
  });

  // Expõe função para forçar manualmente
  window.forceAvatars = forceAllAvatars;

  // Aplica imediatamente
  setTimeout(forceAllAvatars, 200);

  console.log('[PERSIST] Sistema ativo. Avatares serão mantidos.');
})();