...
// ============================================================
// PATCH: Status em tempo real no chat privado
// ============================================================
(function patchPrivateChatStatus() {
  // Mapeamento de status para cores e rótulos
  const STATUS_MAP = {
  ...
  // Função para atualizar o status no cabeçalho do chat privado
  function updatePrivateChatStatus(username) {
    const statusEl = document.getElementById('private-chat-status');
    if (!statusEl) return;

    const key = (username || '').toLowerCase();
    // Obtém o status do mapa global (definido em friends-fix-v6.js)
    let status = 'offline';
    if (window.userStatuses && window.userStatuses[key]) {
      status = window.userStatuses[key];
    } else if (window.onlineSet && window.onlineSet.has(key)) {
      status = 'online';
    }

    const info = STATUS_MAP[status] || STATUS_MAP.offline;
    statusEl.innerHTML = `● ${info.label}`;
    statusEl.style.color = info.color;
  }

  // Salva a função original para não quebrar
  const originalOpenPrivateChat = window.openPrivateChat;

  // Sobrescreve openPrivateChat para incluir o status dinâmico
  window.openPrivateChat = function(username) {
    // Chama a função original (que cria o modal e tudo)
    if (typeof originalOpenPrivateChat === 'function') {
      originalOpenPrivateChat(username);
    } else {
      // Fallback: se a original não existir, cria o chat manualmente (não deve acontecer)
      console.warn('[PATCH] openPrivateChat original não encontrada');
      return;
    }

    // Após o modal ser criado, atualiza o status
    setTimeout(() => {
      updatePrivateChatStatus(username);
    }, 100);
  };

  // Intercepta eventos de status para atualizar o chat se estiver aberto
  if (window.socket) {
    // Listener de status individual
    socket.on('friend:status', function(data) {
      if (!data || !data.username) return;
      // Se o chat privado estiver aberto para este usuário, atualiza
      const username = data.username;
      const chatModal = document.getElementById('private-chat-modal');
      if (chatModal) {
        // Verifica se o título do chat contém o nome (pode ser mais preciso)
        const titleEl = chatModal.querySelector('.pcb-title');
        if (titleEl && titleEl.textContent.trim() === username) {
          updatePrivateChatStatus(username);
        }
      }
    });

    // Listener de presença em massa
    socket.on('friends:presence', function(data) {
      if (!data) return;
  ...
  // Também atualiza quando o modal é aberto via clique no botão (fallback)
  document.addEventListener('click', function(e) {
    // Detecta clique em avatar ou nome que abre o chat (pode variar)
    const target = e.target.closest('[data-open-chat]') || e.target.closest('.dm-conv-item');
    if (target) {
      const username = target.dataset.username || target.textContent.trim();
      if (username) {
        setTimeout(() => updatePrivateChatStatus(username), 200);
      }
    }
  });

  console.log('[PATCH] Status em tempo real ativado para chat privado');
})();