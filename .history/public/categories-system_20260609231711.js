...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ CORREÇÃO DEFINITIVA: Quando clicar em "Adicionar categoria" no MODAL
  document.addEventListener('click', (e) => {
    if (e.target.textContent === 'Adicionar categoria' || e.target.closest('[onclick*="addCategory"]')) {
      e.preventDefault();
      e.stopPropagation();
      
      // Pegar o valor do input do modal
      const input = document.querySelector('input[placeholder="Ex: ROLEPLAY"]');
      if (!input) return;
      ...
      // ✅ CRIAR A CATEGORIA DIRETAMENTE
      createCategory(serverId, nomeCategoria);
      ...
      // ✅ Inicializar Drag & Drop novamente
