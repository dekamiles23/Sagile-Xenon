...
// ============================================================
// SISTEMA DE AVATAR – CARREGAMENTO E SALVAMENTO
// ============================================================

// 1. Salvar avatar no localStorage (quando o usuário faz upload)
function saveUserAvatar(base64Data) {
...
// 2. Carregar avatar do localStorage e aplicar em todos os elementos
function loadUserAvatar() {
  const saved = localStorage.getItem('user_avatar');
  
  // Se tiver uma imagem salva, usa ela
  if (saved && saved.startsWith('data:image')) {
    updateAllAvatars(saved);
    return true;
  }
  
  // Fallback: usa as iniciais do usuário
  const name = window.username || localStorage.getItem('zx_username') || 'Usuário';
  const initial = name.charAt(0).toUpperCase();
  updateAllAvatars(null, initial);
  return false;
}

// 3. Função que atualiza TODOS os elementos de avatar na página
function updateAllAvatars(imageData, fallbackInitial) {
  // Lista de todos os elementos que mostram o avatar
  ...
  avatarElements.forEach(el => {
    ...
    if (imageData) {
      ...
      el.style.background = ''; // remove fallback
      el.textContent = '';
      el.classList.add('has-image');
    } else {
      // Fallback: mostra as iniciais
      const initial = fallbackInitial || (window.username || 'U')[0].toUpperCase();
...
// 4. Força o carregamento do avatar ao iniciar
document.addEventListener('DOMContentLoaded', function() {
  // Aguarda o username estar definido
  const checkUser = setInterval(() => {
    if (window.username || localStorage.getItem('zx_username')) {
...
// 5. Atualiza o avatar quando o usuário faz login
// (chame esta função após definir window.username)
window.onUserLogin = function(username) {
  window.username = username;
  loadUserAvatar();
};

// 6. (Opcional) Se você tem um seletor de avatar com input file:
document.addEventListener('change', function(e) {
  const fileInput = e.target.closest('#avatar-upload-input');
...
console.log('[AVATAR] Sistema carregado com fallback para iniciais');