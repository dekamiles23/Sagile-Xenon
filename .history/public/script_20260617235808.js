...
// ============================================================
// PATCH FINAL: AVATAR NA LISTA DE DMs (COM LOGS E FALLBACK)
// ============================================================
(function dmListAvatarFinal() {
  console.log('[DM-AVATAR-FINAL] Iniciando...');

  // ── 1. Função para obter avatar com logs ──
  function getAvatar(username) {
    ...
    // Log para depuração
    console.log('[DM-AVATAR-FINAL] Buscando avatar para:', username);

    // Tenta cache em memória
    if (window._avatarCache && window._avatarCache[key]) {
      console.log('[DM-AVATAR-FINAL] Avatar encontrado no cache');
      return window._avatarCache[key];
    }

    // Tenta próprio usuário
    if (key === currentUser) {
      const saved = localStorage.getItem('zx_avatar');
      if (saved && saved.startsWith('data:image')) {
        console.log('[DM-AVATAR-FINAL] Avatar próprio carregado de zx_avatar');
        if (!window._avatarCache) window._avatarCache = {};
        window._avatarCache[key] = saved;
        return saved;
      }
    }

    // Tenta avatar do amigo no localStorage
    const stored = localStorage.getItem('avatar_' + key);
    if (stored && stored.startsWith('data:image')) {
      console.log('[DM-AVATAR-FINAL] Avatar do amigo carregado de avatar_' + key);
      if (!window._avatarCache) window._avatarCache = {};
      window._avatarCache[key] = stored;
      return stored;
    }

    // Tenta avatar de friends:data (se tiver sido guardado)
    if (window._friendAvatars && window._friendAvatars[key]) {
      console.log('[DM-AVATAR-FINAL] Avatar encontrado em _friendAvatars');
      return window._friendAvatars[key];
    }

    console.log('[DM-AVATAR-FINAL] Nenhum avatar encontrado para', username);
    return null;
  }
  ...
  function applyAvatarToElement(el, username) {
    ...
    if (img) {
      ...
      el.textContent = '';
      console.log('[DM-AVATAR-FINAL] Avatar aplicado a', username);
      return true;
    } else {
      el.style.backgroundImage = 'none';
      el.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
      el.textContent = username.charAt(0).toUpperCase();
      console.log('[DM-AVATAR-FINAL] Fallback (inicial) para', username);
      return false;
    }
  }

  // ── 3. Atualiza todos os avatares da lista ──
  function updateListAvatars() {
    const items = document.querySelectorAll('#dm-conversations-list .dm-conv-item');
    console.log('[DM-AVATAR-FINAL] Atualizando avatares de', items.length, 'itens');
    items.forEach(item => {
  ...
  // ── 4. Sobrescreve renderDmList para incluir avatares ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      console.log('[DM-AVATAR-FINAL] renderDmList chamado');
      original.call(this);
      // Após renderizar, aplica avatares
      setTimeout(updateListAvatars, 50);
      setTimeout(updateListAvatars, 200);
    };
  }

  // ── 5. Observer para aplicar avatares em novos itens ──
  function observeList() {
    ...
    console.log('[DM-AVATAR-FINAL] Observador da lista ativado');
  }

  // ── 6. Listener para receber avatares do servidor ──
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        const key = data.username.toLowerCase();
        if (!window._avatarCache) window._avatarCache = {};
        window._avatarCache[key] = data.avatar;
        localStorage.setItem('avatar_' + key, data.avatar);
        console.log('[DM-AVATAR-FINAL] Avatar recebido para', data.username);
        updateListAvatars();
        // Também atualiza mensagens se necessário
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        if (!window._friendAvatars) window._friendAvatars = {};
        data.friends.forEach(friend => {
          if (friend.avatar) {
            const key = friend.username.toLowerCase();
            window._friendAvatars[key] = friend.avatar;
            localStorage.setItem('avatar_' + key, friend.avatar);
          }
        });
        console.log('[DM-AVATAR-FINAL] Avatares carregados de friends:data');
        updateListAvatars();
      }
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    observeList();
    // Se a lista já existir, atualiza avatares
    setTimeout(updateListAvatars, 300);
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(updateListAvatars, 500);
    }
  });

  // ── 8. Fallback: polling a cada 3 segundos ──
  setInterval(function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      // Verifica se há itens sem avatar
      const items = document.querySelectorAll('#dm-conversations-list .dm-conv-item');
      let needsUpdate = false;
      items.forEach(item => {
        const avatarEl = item.querySelector('.dm-avatar');
        if (avatarEl && avatarEl.textContent && avatarEl.textContent.length === 1) {
          // Se tem apenas uma letra, provavelmente não tem imagem
          needsUpdate = true;
        }
      });
      if (needsUpdate) {
        console.log('[DM-AVATAR-FINAL] Polling: atualizando avatares');
        updateListAvatars();
      }
    }
  }, 3000);

  // ── 9. Expõe funções para debug ──
  window.forceAvatarUpdate = function() {
    updateListAvatars();
    console.log('[DM-AVATAR-FINAL] Atualização forçada');
  };
  ...
  window.setFriendAvatar = function(username, base64) {
    ...
    const key = username.toLowerCase();
    if (!window._avatarCache) window._avatarCache = {};
    window._avatarCache[key] = base64;
    localStorage.setItem('avatar_' + key, base64);
    updateListAvatars();
    console.log('[DM-AVATAR-FINAL] Avatar de', username, 'definido manualmente');
  };

  console.log('[DM-AVATAR-FINAL] Sistema ativo. Use forceAvatarUpdate() para forçar.');
})();