...
(function addAvatarsToDmMessages() {
  ...
  function getUserAvatar(username) {
    ...
    // Para amigos ou se não tiver imagem, retorna null (usará iniciais)
    return null;
  }
...
// ============================================================
// LISTA DE DMs COM AVATAR E CLIQUE (FINAL)
// ============================================================
(function dmListFinal() {
  console.log('[DM-LIST-FINAL] Iniciando...');

  // ── 1. Função para obter avatar diretamente do localStorage ──
  function getAvatarDirect(username) {
    ...
    // Amigo: chave avatar_username
    const stored = localStorage.getItem('avatar_' + key);
    if (stored && stored.startsWith('data:image')) {
      return stored;
    }

    // Se não tiver, retorna null (usa iniciais)
    return null;
  }

  // ── 2. Função para abrir o chat ──
  function openChatWith(username) {
    if (!username) return;
    const currentUser = (window.username || '').toLowerCase();
    if (username.toLowerCase() === currentUser) {
      console.log('[DM-LIST-FINAL] Não é possível abrir chat consigo mesmo');
      return;
    }
    console.log('[DM-LIST-FINAL] Abrindo chat com:', username);
    if (typeof window.openDmChat === 'function') {
      window.openDmChat(username);
  ...
  // ── 3. Função que cria um item da lista com avatar ──
  function createDmItem(username) {
    const item = document.createElement('div');
    item.className = 'dm-conv-item';
    item.dataset.username = username;
    item.style.cssText = `
    ...
    // Aplica avatar (se tiver) ou iniciais
    const avatarData = getAvatarDirect(username);
    if (avatarData) {
      avatarDiv.style.backgroundImage = 'url(' + avatarData + ')';
      avatarDiv.style.backgroundSize = 'cover';
      avatarDiv.style.backgroundPosition = 'center';
    ...
    // Evento de clique diretamente no item
  ...
  // ── 4. Sobrescreve renderDmList ──
  window.renderDmList = function() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) {
      console.warn('[DM-LIST-FINAL] Lista não encontrada');
      return;
    }
    ...
    // Cria itens
    unique.forEach(name => {
      const item = createDmItem(name);
      list.appendChild(item);
    });

    console.log('[DM-LIST-FINAL] Lista renderizada com', unique.size, 'itens');
  };

  // ── 5. Força a renderização imediata se a DM estiver aberta ──
  ...
  document.addEventListener('DOMContentLoaded', function() {
    forceRender();
    // Também renderiza quando a lista for modificada por outros scripts (fallback)
    const list = document.getElementById('dm-conversations-list');
    if (list) {
      const observer = new MutationObserver(() => {
        // Se a lista for modificada, re-renderiza (mas evita loop)
        if (list.children.length > 0) {
          // Verifica se o primeiro item já tem a estrutura correta
          const firstItem = list.querySelector('.dm-conv-item');
          if (firstItem && !firstItem.querySelector('.dm-avatar')) {
            // Se não tem avatar, recria
            forceRender();
          }
        }
      });
  ...
  // ── 7. Expõe função para forçar ──
  window.forceDmRender = forceRender;

  console.log('[DM-LIST-FINAL] Sistema ativo. Use forceDmRender() para forçar.');
})();

// ============================================================
// AVATAR AUTOMÁTICO - BUSCA DO SERVIDOR E CACHE
// ============================================================
(function autoAvatarSystem() {
  console.log('[AUTO-AVATAR] Iniciando...');

  // ── 1. Cache de solicitações (evita múltiplos pedidos) ──
  const requested = new Set();

  // ── 2. Função para obter avatar (prioriza localStorage) ──
  function getAvatarFromStorage(username) {
    if (!username) return null;
    const key = username.toLowerCase();
    const currentUser = (window.username || '').toLowerCase();
  ...
  // ── 3. Solicita avatar ao servidor (se não tiver em cache) ──
  function requestAvatarIfNeeded(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (requested.has(key)) return; // já foi pedido
    if (getAvatarFromStorage(username)) return; // já tem
  ...
  // ── 4. Aplica avatar a um elemento ──
  function applyAvatarToElement(el, username) {
    if (!el || !username) return false;
    const img = getAvatarFromStorage(username);
    if (img) {
  ...
  // ── 5. Atualiza avatares na lista e mensagens ──
  function updateAllAvatars() {
    ...
    document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
      ...
      if (avatarEl && nameEl) {
        ...
        if (username) applyAvatarToElement(avatarEl, username);
      }
    });
    // Mensagens
    document.querySelectorAll('.msg-avatar').forEach(el => {
      ...
      if (nameEl) {
        const username = nameEl.textContent.trim();
        if (username) applyAvatarToElement(el, username);
      }
    });
  }

  // ── 6. Sobrescreve renderDmList para solicitar avatares ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      original.call(this);
      // Após renderizar, solicita avatares para cada amigo
      setTimeout(() => {
        document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
          const nameEl = item.querySelector('.dm-conv-name');
          if (nameEl) {
            const username = nameEl.textContent.trim();
            if (username) requestAvatarIfNeeded(username);
          }
        });
        updateAllAvatars();
      }, 50);
  ...
  // ── 7. Listener para receber avatares do servidor ──
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        const key = data.username.toLowerCase();
        localStorage.setItem('avatar_' + key, data.avatar);
        console.log('[AUTO-AVATAR] Avatar recebido para', data.username);
        // Atualiza a UI
        updateAllAvatars();
        // Remove do cache de solicitações para permitir re-pedido se necessário
        requested.delete(key);
      }
    });

    // Se o servidor enviar avatares em friends:data, também salva
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        data.friends.forEach(friend => {
          if (friend.avatar) {
            const key = friend.username.toLowerCase();
            localStorage.setItem('avatar_' + key, friend.avatar);
            console.log('[AUTO-AVATAR] Avatar de', friend.username, 'recebido via friends:data');
          }
        });
        updateAllAvatars();
      }
  ...
  // ── 8. Observa a lista para novos itens ──
  ...
  // ── 11. Fallback: atualiza a cada 5 segundos (caso o servidor demore) ──
  setInterval(function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      // Se houver avatares faltando, solicita novamente
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        const nameEl = item.querySelector('.dm-conv-name');
        if (nameEl) {
          const username = nameEl.textContent.trim();
          if (username && !getAvatarFromStorage(username)) {
            requestAvatarIfNeeded(username);
          }
        }
      });
      updateAllAvatars();
  ...
  window.refreshAvatars = updateAllAvatars;
  window.requestAvatarForFriend = requestAvatarIfNeeded;

  console.log('[AUTO-AVATAR] Sistema ativo. Avatares serão carregados automaticamente do servidor.');
})();

// ============================================================
// CORREÇÃO: AVATAR INVISÍVEL (FORÇA FALLBACK)
// ============================================================
(function fixInvisibleAvatar() {
  console.log('[FIX-AVATAR] Corrigindo avatares invisíveis...');

  function forceFallback() {
    document.querySelectorAll('.dm-avatar, .msg-avatar').forEach(el => {
      // Se o elemento está vazio e não tem imagem
      const bg = el.style.backgroundImage || '';
      const hasImage = bg && bg !== 'none' && bg !== '';
      if (!hasImage && !el.textContent) {
        // Tenta obter o nome do amigo do elemento pai
        ...
        if (item) {
          ...
          if (parent) {
            const nameEl = parent.querySelector('.msg-username');
            if (nameEl) username = nameEl.textContent.trim();
          }
        }
        if (username) {
          ...
          el.style.backgroundImage = 'none';
          el.style.background = 'linear-gradient(135deg, #8b00ff, #ff00ff)';
          console.log('[FIX-AVATAR] Fallback aplicado para', username);
        } else {
          // Fallback genérico
          el.textContent = '?';
          el.style.backgroundImage = 'none';
  ...
  forceFallback();
  setTimeout(forceFallback, 300);
  setTimeout(forceFallback, 600);

  // Observa mudanças na lista
  const observer = new MutationObserver(() => {
    setTimeout(forceFallback, 50);
  });
  const list = document.getElementById('dm-conversations-list');
  if (list) observer.observe(list, { childList: true, subtree: true });

  // Também observa mensagens
  const messages = document.getElementById('dm-messages-area');
  if (messages) observer.observe(messages, { childList: true, subtree: true });

  // Fallback: polling a cada 2 segundos
  setInterval(forceFallback, 2000);

  // Expõe função para forçar
  window.fixAvatars = forceFallback;

  console.log('[FIX-AVATAR] Sistema ativo. Use fixAvatars() para forçar.');
})();