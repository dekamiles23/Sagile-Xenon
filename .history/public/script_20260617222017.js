...
// ============================================================
// COPIA BARRA DO SERVIDOR PARA DM (COM PICKERS FUNCIONAIS)
// ============================================================
(function copyInputBarFull() {
  function copyBar() {
    ...
    // ── PICKERS: Emoji, GIF, Sticker ──
    function setupPicker(btnId, pickerId, insertFn) {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn || !picker) return;

      // Fecha outros pickers e alterna este
      ...
      // Insere item no input
      if (insertFn && picker) {
        picker.addEventListener('click', function(e) {
          const target = e.target.closest('.emoji-item, .gif-item, .sticker-item');
          if (target) {
            const text = target.textContent || target.dataset.value || target.src || '';
            if (text) {
              insertFn(text);
    ...
    // Emoji: insere o caractere
    setupPicker('dm-btn-emoji', 'emoji-picker', function(text) {
      if (input) {
    ...
    // GIF: insere como texto [GIF: url]
    setupPicker('dm-btn-gif', 'gif-picker', function(text) {
      // text pode ser a URL da imagem do GIF
      if (input) {
        const gifText = text.startsWith('http') ? `[GIF] ${text}` : text;
        input.value += gifText + ' ';
        input.focus();
      }
    });

    // Sticker: insere como texto [Sticker]
    setupPicker('dm-btn-stickers', 'sticker-picker', function(text) {
      if (input) {
        input.value += text + ' ';
        input.focus();
      }
    ...
    // ── Anexar arquivo (já funcional) ──
    // ... (mantenha o código de anexo do script anterior)

    // ── Áudio (já funcional) ──
    // ... (mantenha o código de áudio do script anterior)

    // ── Mais opções (já funcional) ──
    // ... (mantenha o código de mais opções do script anterior)

    console.log('[COPY] Barra com pickers funcionais copiada');
  }
