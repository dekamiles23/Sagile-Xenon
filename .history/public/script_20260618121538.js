...
// ============================================================
// BOTÃO 📷 BASEADO NO ELEMENTO ENCONTRADO
// ============================================================
(function buttonByFoundElement() {
  console.log('[FOUND-BTN] Iniciando...');
  ...
  // 2. Função que adiciona o botão para um nome específico
  function addButtonForName(friendName) {
    // Procura o elemento que contém o nome exato
    const list = document.getElementById('dm-conversations-list');
    ...
    if (!nameElement) {
      console.warn('[FOUND-BTN] Nome não encontrado:', friendName);
      return false;
    }

    // O item da lista é o pai mais próximo que contém o nome
    let item = nameElement.parentElement;
    if (!item) return false;

    // Se o pai não for o item correto, tenta subir mais um nível
    if (item && !item.querySelector('.dm-conv-name')) {
      // Se o pai não tem o nome, talvez o item seja o avô
      const grandParent = item.parentElement;
      if (grandParent && grandParent.querySelector('.dm-conv-name')) {
        item = grandParent;
      }
    }
    ...
    console.log('[FOUND-BTN] ✅ Botão adicionado para', friendName);
    return true;
  }

  // 3. Escaneia a lista e adiciona para "Miles Deka"
  function scanAndAdd() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;

    // Encontra todos os nomes (classes conhecidas)
    ...
    // Fallback: se não encontrou pelos seletores, tenta pelo texto (Miles Deka)
    if (nameElements.length === 0) {
      addButtonForName('Miles Deka');
    }
  }

  // 4. Executa a cada 1.5 segundos (polling)
  setInterval(scanAndAdd, 1500);
  ...
  scanAndAdd();
  setTimeout(scanAndAdd, 300);
  setTimeout(scanAndAdd, 600);
  setTimeout(scanAndAdd, 1000);

  console.log('[FOUND-BTN] Sistema ativo. Procurando por "Miles Deka"...');
})();