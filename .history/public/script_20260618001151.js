...
function createDmItem(username) {
  console.log('[DM-LIST-FINAL] Criando item para:', username);
  const item = document.createElement('div');
  item.className = 'dm-conv-item';
  item.dataset.username = username;
  item.style.cssText = `
  ...
  const key = username.toLowerCase();
  let img = localStorage.getItem('avatar_' + key);
  if (!img) {
    // Tenta também com a chave original (sem lower) - fallback
    img = localStorage.getItem('avatar_' + username);
  }
  console.log('[DM-LIST-FINAL] Imagem encontrada?', img ? 'Sim' : 'Não');
  ...
  if (img && img.startsWith('data:image')) {
    ...
    avatarDiv.style.background = 'none';
    avatarDiv.textContent = '';
    console.log('[DM-LIST-FINAL] Avatar aplicado a', username);
  } else {
    avatarDiv.textContent = username.charAt(0).toUpperCase();
    console.log('[DM-LIST-FINAL] Fallback (inicial) para', username);
  }
