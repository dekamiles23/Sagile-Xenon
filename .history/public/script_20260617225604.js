...
// ============================================================
// CRIA A BARRA DO DM EM QUALQUER LUGAR (GARANTIDO)
// ============================================================
(function createDmBarGuaranteed() {
  function buildBar() {
    console.log('[DM-BAR] Tentando criar barra...');

    // 1. Procura onde colocar a barra
    let target = document.getElementById('dm-input-area');

    // Se não existir, procura dentro da view DM
    if (!target) {
      const dmView = document.getElementById('dm-view');
      if (dmView) {
        // Procura a área de mensagens ou o final da view
        const messagesArea = dmView.querySelector('#dm-messages-area, .messages-area');
        if (messagesArea) {
          // Insere após a área de mensagens
          ...
          console.log('[DM-BAR] Criado dm-input-area após a área de mensagens');
        } else {
          // Fallback: insere no final da view DM
          target = document.createElement('div');
          target.id = 'dm-input-area';
          target.style.cssText = 'padding: 12px 16px; border-top: 1px solid rgba(255,0,255,0.1); margin-top: auto;';
          dmView.appendChild(target);
          console.log('[DM-BAR] Criado dm-input-area no final da view DM');
        }
      } else {
        console.warn('[DM-BAR] View DM não encontrada');
        return;
      }
    }
    ...
    wrapper.innerHTML = `
      <button type="button" class="dm-btn" id="dm-btn-plus" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">➕</button>
      <button type="button" class="dm-btn" id="dm-btn-emoji" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">😀</button>
      <button type="button" class="dm-btn" id="dm-btn-gif" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎬</button>
      <button type="button" class="dm-btn" id="dm-btn-stickers" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎭</button>
      <button type="button" class="dm-btn" id="dm-btn-attach" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">📎</button>
      ...
      <button type="button" class="dm-btn" id="dm-btn-voice" style="background:transparent;border:none;color:#ccc;font-size:18px;cursor:pointer;padding:4px;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;">🎤</button>
    ...
    console.log('[DM-BAR] ✅ Barra criada com sucesso!');
    ...
    // ── Conecta os botões aos pickers (usando IDs fixos) ──
    function connectPicker(btnId, pickerId) {
      ...
      if (!picker) {
        console.warn(`[DM] Pickers ${pickerId} não encontrado. Criando...`);
        // Cria um picker placeholder se não existir
        const newPicker = document.createElement('div');
        newPicker.id = pickerId;
        newPicker.className = 'chat-picker';
        ...
        `;
        newPicker.innerHTML = `<div style="font-size:40px;">⚠️</div><div>Pickers ${pickerId} não encontrado</div><div style="font-size:12px;color:#666;">Clique em um item para inserir</div>`;
        document.body.appendChild(newPicker);
        console.log(`[DM] Pickers ${pickerId} criado placeholder`);
      }
      ...
      btn.addEventListener('click', function(e) {
        ...
        document.querySelectorAll('.chat-picker.active').forEach(p => {
          if (p !== pickerEl) p.classList.remove('active');
        });
        const isActive = pickerEl.classList.toggle('active');
        pickerEl.style.display = isActive ? 'flex' : 'none';
  ...
  // ── Executa quando a view DM ficar visível ──
  function waitForDM() {
    const dmView = document.getElementById('dm-view');
    if (!dmView) {
    ...
    const observer = new MutationObserver(() => {
      if (!dmView.classList.contains('hidden')) {
        console.log('[DM-BAR] DM visível, criando barra...');
    ...
    // Se a DM já estiver visível, cria a barra imediatamente
    if (!dmView.classList.contains('hidden')) {
