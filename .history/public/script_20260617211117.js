...
// ============================================================
// STATUS EM TEMPO REAL NO CHAT PRIVADO (VERSÃO ROBUSTA)
// ============================================================
(function privateChatStatus() {
  ...
  // Função para obter o status de um usuário
  ...
  // Função principal: atualiza o status no cabeçalho
  function updateChatStatus(username) {
    const modal = document.getElementById('private-chat-modal');
    if (!modal) {
      console.log('[STATUS] Modal não encontrado');
      return;
    }

    // Tenta encontrar o elemento de status com vários seletores
    let statusEl = modal.querySelector('.pcb-subtitle') ||
                   modal.querySelector('[style*="color: #00ff88"]') ||
                   modal.querySelector('[style*="font-size: 12px"]') ||
                   modal.querySelector('.private-chat-status');

    // Se não achou, cria um
    if (!statusEl) {
      console.log('[STATUS] Elemento de status não encontrado, criando...');
      // Procura onde colocar (geralmente ao lado do nome)
      const titleEl = modal.querySelector('.pcb-title');
      if (titleEl && titleEl.parentElement) {
        statusEl = document.createElement('div');
        statusEl.className = 'private-chat-status';
        statusEl.style.cssText = 'color: #747f8d; font-size: 12px; margin-top: 2px;';
        titleEl.parentElement.appendChild(statusEl);
      } else {
        console.warn('[STATUS] Não foi possível criar o elemento de status');
        return;
      }
    }
    ...
    console.log('[STATUS] Atualizado para', username, '→', status);
  }

  // Sobrescreve openPrivateChat para atualizar ao abrir
  ...
  if (origOpen) {
    ...
    console.warn('[STATUS] openPrivateChat não encontrada');
  }

  // Escuta eventos de status
  if (window.socket) {
    socket.on('friend:status', (data) => {
      console.log('[STATUS] Evento friend:status recebido:', data);
    ...
    socket.on('friends:presence', () => {
      console.log('[STATUS] Evento friends:presence recebido');
      const modal = document.getElementById('private-chat-modal');
      if (modal) {
        const titleEl = modal.querySelector('.pcb-title');
        if (titleEl) {
          updateChatStatus(titleEl.textContent.trim());
        }
      }
    });
  }

  // Se o chat já estiver aberto ao carregar a página, atualiza
  setTimeout(() => {
    const modal = document.getElementById('private-chat-modal');
    if (modal) {
      const titleEl = modal.querySelector('.pcb-title');
  ...
  console.log('[STATUS] Robusto ativado. Abra o console para ver logs.');
})();