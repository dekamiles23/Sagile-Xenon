...
// ✅ CORREÇÃO FINAL ABSOLUTA 100% FUNCIONAL
// CAPTURA TODO CLIQUE NA TELA, NÃO DEIXA NENHUMA OPÇÃO ESCAPAR
document.addEventListener('click', (e) => {
  
  // Verificar se o clique foi DENTRO do modal de criar comunidade
  if (!document.getElementById('community-modal').classList.contains('hidden')) {
    
    // Pegar o elemento que foi clicado
    let target = e.target;
    
    // Subir até encontrar o container da categoria
    for (let i = 0; i < 5; i++) {
      if (!target) break;
      
      // Verificar se este elemento é UMA DAS CAIXAS DE CATEGORIA
      if (target.clientWidth > 100 && target.clientHeight > 80 && target.querySelector('svg, img, span') && target.textContent.trim().length > 0 && target.textContent.trim().length < 15) {
        
        // ✅ ACHOU A CAIXA DE CATEGORIA!
        
        e.preventDefault();
        e.stopPropagation();
        
        // ✅ REMOVER ESTILO DE TODAS AS OUTRAS CAIXAS
        document.querySelectorAll('#community-modal div').forEach(b => {
          if (b.clientWidth > 100 && b.clientHeight > 80 && b.querySelector('svg, img, span') && b.textContent.trim().length > 0 && b.textContent.trim().length < 15) {
            b.style.background = 'transparent';
            b.style.border = '1px solid rgba(255, 0, 255, 0.3)';
            b.style.boxShadow = 'none';
            b.style.transform = 'scale(1)';
        ...
        // ✅ APLICAR ESTILO ROSA NA CAIXA CLICADA
        target.style.background = 'rgba(255, 0, 255, 0.25)';
        target.style.border = '2px solid #ff00ff';
        target.style.boxShadow = '0 0 12px rgba(255, 0, 255, 0.5)';
        target.style.transform = 'scale(1.02)';
        target.classList.add('selected');
        
        // ✅ Salvar categoria selecionada
        selectedCommunityCategory = target.textContent.trim().toLowerCase();
        
        console.log('✅ ✅ ✅ CATEGORIA SELECIONADA COM SUCESSO:', target.textContent.trim());
