...
// CACHE DE AVATARES DOS AMIGOS
// ============================================================
window.friendAvatarCache = (() => {
  try {
    return JSON.parse(localStorage.getItem('zx_friend_avatars') || '{}');
  } catch (e) {
    return {};
  }
})();

function setFriendAvatar(username, avatarUrl) {
  if (!username || !avatarUrl) return;
  const key = username.toLowerCase();
  window.friendAvatarCache[key] = avatarUrl;
  try {
    localStorage.setItem('zx_friend_avatars', JSON.stringify(window.friendAvatarCache));
  } catch (e) {}
}
...
function getFriendAvatar(username) {
  ...
  if (window.friendAvatarCache && window.friendAvatarCache[key]) {
    return window.friendAvatarCache[key];
  }
  // Fallback: tenta localStorage diretamente
  try {
    const stored = JSON.parse(localStorage.getItem('zx_friend_avatars') || '{}');
    if (stored[key]) {
      // Atualiza cache em memória
      window.friendAvatarCache[key] = stored[key];
      return stored[key];
    }
  } catch (e) {}
  return null;
...
// ======================================================
// Garante que renderDmList seja registrada apenas uma vez
// ======================================================
(function () {
    if (window.__renderDmListLocked) return;
    window.__renderDmListLocked = true;

    const originalRenderDmList = window.renderDmList;

    if (typeof originalRenderDmList !== "function") {
        console.warn("[DM] renderDmList não encontrada.");
        return;
    }

    window.renderDmList = function (...args) {
        return originalRenderDmList.apply(this, args);
    };

    Object.defineProperty(window, "renderDmList", {
        configurable: false,
        writable: false,
        value: window.renderDmList
    });

    console.log("[DM] renderDmList bloqueada. Outras definições serão ignoradas.");
})();

// ============================================================
// HANDLER PARA AVATAR DE AMIGOS EM TEMPO REAL
// ============================================================
...
socket.on('user:avatar:data', (data) => {
  ...
  // 1. Atualiza o cache em memória e localStorage
  if (typeof setFriendAvatar === 'function') {
    setFriendAvatar(username, avatar);
  } else {
    // fallback seguro
    if (!window.friendAvatarCache) window.friendAvatarCache = {};
    window.friendAvatarCache[username.toLowerCase()] = avatar;
    try {
      localStorage.setItem('zx_friend_avatars', JSON.stringify(window.friendAvatarCache));
    } catch (e) {}
  }

  // 2. Re-renderiza a lista de DM
  if (typeof renderDmList === 'function') {
    renderDmList();
  }

  // 3. Re-renderiza o modal de amigos (se estiver aberto)
  if (typeof renderFriendsModal === 'function') {
    renderFriendsModal();
  }

  // 4. Atualiza o cabeçalho da DM se o chat com este usuário estiver aberto
  const currentDmUser = window.currentDmUser || document.querySelector('#dm-chat-area')?.dataset.activeChat;
  if (currentDmUser && currentDmUser.toLowerCase() === username.toLowerCase()) {
    const avatarEl = document.querySelector('#dm-chat-area .dm-avatar');
    if (avatarEl) {
  ...
  // 5. Atualiza mensagens da DM (se houver)
  if (typeof renderDmMessages === 'function') {
    renderDmMessages();
  }

  // 6. Atualiza participantes do server-view (voz)
  if (typeof renderVoiceParticipants === 'function') {
    renderVoiceParticipants();
  }

  console.log(`[AVATAR] Atualizado avatar de ${username} em tempo real`);
});
  ...
  // 2. Atualiza a UI em todos os lugares que exibem o avatar
  // Atualiza a lista de DM
  if (typeof renderDmList === 'function') {
    renderDmList();
  }

  // Atualiza o modal de amigos (se estiver aberto)
  if (typeof renderFriendsModal === 'function') {
    renderFriendsModal();
  }

  // Atualiza o cabeçalho da DM, se o chat com este usuário estiver aberto
  const currentDmUser = window.currentDmUser || document.querySelector('#dm-chat-area')?.dataset.activeChat;
  if (currentDmUser && currentDmUser.toLowerCase() === username.toLowerCase()) {
    const avatarEl = document.querySelector('#dm-chat-area .dm-avatar');
    if (avatarEl) {
  ...
  // Atualiza a lista de mensagens da DM (se houver)
  if (typeof renderDmMessages === 'function') {
    renderDmMessages();
  }

  // Atualiza a lista de participantes do server-view (se aplicável)
  if (typeof renderVoiceParticipants === 'function') {
    renderVoiceParticipants();
  }
