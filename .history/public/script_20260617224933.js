...
// ============================================================
// RECRIA BARRA DO DM DO ZERO (com pickers globais ou criados)
// ============================================================
(function recreateDmBar() {
  function build() {
    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      console.warn('[DM-BAR] dm-input-area não encontrado. Tentando novamente...');
    ...
    wrapper.innerHTML = `
      <button type="button" class="input-action-btn" id="dm-btn-plus" title="Mais opções" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">➕</button>
      <button type="button" class="input-action-btn" id="dm-btn-emoji" title="Emojis" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">😀</button>
      <button type="button" class="input-action-btn" id="dm-btn-gif" title="GIFs" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎬</button>
      <button type="button" class="input-action-btn" id="dm-btn-stickers" title="Figurinhas" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎭</button>
      <button type="button" class="input-action-btn" id="dm-btn-attach" title="Anexar arquivo" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">📎</button>
      ...
      <button type="button" class="input-action-btn" id="dm-btn-voice" title="Gravar áudio" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎤</button>
      <button id="dm-send-btn" type="button" style="background:linear-gradient(135deg,#8b00ff,#ff00ff);border:none;border-radius:50%;width:44px;height:44px;color:#fff;font-size:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;">➤</button>
    `;
    ...
    // ── 3. GARANTE QUE OS PICKERS GLOBAIS EXISTAM E ESTEJAM NO BODY ──
    function ensurePicker(id, html) {
      ...
      if (!picker) {
        ...
        // Se existe mas está em outro lugar, move para o body
        document.body.appendChild(picker);
        console.log(`[DM-BAR] Pickers movido para o body: ${id}`);
      }
      return picker;
    }

    // ── Emoji picker ──
    const emojiPicker = ensurePicker('emoji-picker', `
    ...
    // ── GIF picker ──
    const gifPicker = ensurePicker('gif-picker', `
      ...
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;padding:12px;overflow-y:auto;flex:1;">
        ${['https://media.giphy.com/media/3o7aTskHEUdgCQAXde/giphy.gif','https://media.giphy.com/media/26tn33aiTi1jkl6H6/giphy.gif','https://media.giphy.com/media/3o6Zt481isNVuQI1l6/giphy.gif','https://media.giphy.com/media/l0HlNQ03J5JxX6lva/giphy.gif','https://media.giphy.com/media/3o7abKhOpu0N9H8s/giphy.gif','https://media.giphy.com/media/l0HlOBPvWqHj1D7J2/giphy.gif'].map(url => `<div class="gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:all 0.2s;aspect-ratio:16/9;background:rgba(255,0,255,0.05);"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
      </div>
    `);

    // ── Sticker picker ──
    const stickerPicker = ensurePicker('sticker-picker', `
      <div style="display:flex;border-bottom:1px solid rgba(255,0,255,0.15);background:rgba(0,0,0,0.3);padding:4px;flex-shrink:0;">
    ...
    // ── 4. CONECTA OS BOTÕES AOS PICKERS ──
    ...
    // Emoji
    connectPicker('dm-btn-emoji', 'emoji-picker', '.emoji-item', function(text) {
      if (input) {
    ...
    // GIF
    connectPicker('dm-btn-gif', 'gif-picker', '.gif-item img', function(url) {
      if (input) input.value += `[GIF] ${url} `;
    });

    // Sticker
    connectPicker('dm-btn-stickers', 'sticker-picker', '.sticker-item', function(text) {
    ...
    console.log('[DM-BAR] Barra recriada com pickers funcionais');
  }
  ...
  console.log('[DM-BAR] Script de recriação da barra carregado');
})();