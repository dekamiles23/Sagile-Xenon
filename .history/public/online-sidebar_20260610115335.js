...
setInterval(() => {
  ...
  if (countEl && container) {
    ...
    console.log('✅ Contador atualizado para:', realCount, 'Usuários no array:', onlineUsers.length);
  }
}, 500);

// ✅ CRIA A SETA AUTOMATICAMENTE SE NÃO EXISTIR
setInterval(() => {
  let btn = document.getElementById('toggle-online-sidebar');
  const sidebar = document.querySelector('.online-sidebar');
  
  if (sidebar) {
    // Se botão não existir, CRIA ELE DO ZERO COM TODO LAYOUT
    if (!btn) {
      ...
      // APLICA TODO O CSS DIRETO NO ELEMENTO
      btn.style.cssText = `
        ...
        background: rgba(15, 0, 25, 0.9) !important;
      ...
      btn.onmouseenter = function() {
        this.style.background = 'rgba(255, 0, 255, 0.2) !important';
        this.style.boxShadow = '-5px 0 25px rgba(0, 255, 255, 0.5) !important';
      };
      btn.onmouseleave = function() {
        this.style.background = 'rgba(15, 0, 25, 0.9) !important';
        this.style.boxShadow = '-5px 0 20px rgba(0, 255, 255, 0.3) !important';
      };
      
      sidebar.prepend(btn);
      console.log('✅ Botão da seta CRIADO automaticamente com layout completo');
    }
    
    // Move o botão para DENTRO da sidebar se não estiver
