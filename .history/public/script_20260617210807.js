...
// ============================================================
// FIX FINAL: DMs sem duplicatas e status em tempo real
// ============================================================
(function fixDmAndStatus() {
  // 1. CORRIGE A LISTA DE DMs (remove duplicatas e atualiza status)
  ...
  window.renderDmList = function() {
    ...
    // Coleta contatos únicos (amigos + histórico de mensagens)
    const friends = JSON.parse(localStorage.getItem('zx_friends') || '[]');
    const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
    const unique = new Map();
    ...
    // Renderiza cada contato com status
    unique.forEach((name) => {
      const key = name.toLowerCase();
      const status = (window.userStatuses && window.userStatuses[key]) || 
                     (window.onlineSet && window.onlineSet.has(key) ? 'online' : 'offline');
      const statusColors = { online: '#23d18b', idle: '#faa61a', dnd: '#ed4245', offline: '#747f8d' };
      ...
      item.innerHTML = `
        <div style="position:relative;width:32px;height:32px;">
          <div class="dm-avatar" style="
            ...
            font-size: 14px;
          ">${name[0].toUpperCase()}</div>
          <div style="position:absolute;bottom:0;right:0;width:10px;height:10px;border-radius:50%;background:${color};border:2px solid #12121a;"></div>
        </div>
        <div class="dm-conv-name" style="color: #eee; font-size: 14px; flex:1;">${name}</div>
      `;
      item.onclick = () => {
        if (typeof openDmChat === 'function') openDmChat(name);
      };
    ...
    console.log('[DM-FIX] Lista renderizada com', unique.size, 'contatos');
  };

  // 2. ATUALIZA O STATUS DO CABEÇALHO DO CHAT PRIVADO
  function updateChatStatus(username) {
    ...
    const status = (window.userStatuses && window.userStatuses[key]) || 
                   (window.onlineSet && window.onlineSet.has(key) ? 'online' : 'offline');
    const map = { online: { color: '#23d18b', label: 'Online' }, idle: { color: '#faa61a', label: 'Ausente' }, dnd: { color: '#ed4245', label: 'Não perturbe' }, offline: { color: '#747f8d', label: 'Offline' } };
    const info = map[status] || map.offline;
    statusEl.innerHTML = `● ${info.label}`;
    statusEl.style.color = info.color;
  }

  // 3. ESCUTA MUDANÇAS DE STATUS E ATUALIZA TUDO
  if (window.socket) {
    socket.on('friend:status', (data) => {
      if (data && data.username) {
        // Atualiza a lista (para mostrar o ponto de status)
        window.renderDmList();
        // Atualiza o cabeçalho se o chat estiver aberto para esse usuário
        const chatModal = document.getElementById('private-chat-modal');
        if (chatModal) {
          const titleEl = chatModal.querySelector('.pcb-title');
          if (titleEl && titleEl.textContent.trim() === data.username) {
    ...
    socket.on('friends:presence', () => {
      window.renderDmList();
      // Atualiza o cabeçalho do chat atual
      const chatModal = document.getElementById('private-chat-modal');
      if (chatModal) {
        const titleEl = chatModal.querySelector('.pcb-title');
        if (titleEl) updateChatStatus(titleEl.textContent.trim());
      }
    });
  }

  // 4. PATCH NO openPrivateChat para incluir o status
  const origOpen = window.openPrivateChat;
  window.openPrivateChat = function(username) {
    if (typeof origOpen === 'function') origOpen(username);
    setTimeout(() => updateChatStatus(username), 150);
  };
  ...
  console.log('[DM-FIX] Aplicado: lista sem duplicatas e status em tempo real');
})();