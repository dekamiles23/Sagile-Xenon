...
// ============================================================
// FIX: Avatar na DM – usando a chave zx_avatar
// ============================================================
(function fixDmAvatarFinal() {
  function loadDmAvatar() {
    ...
    // Procura o avatar na barra superior da DM
    let avatarEl = dmView.querySelector('#user-avatar-dm, #user-avatar, .profile-avatar');
    if (!avatarEl) {
      // Se não achar, tenta pelo id global (fallback)
      avatarEl = document.getElementById('user-avatar-dm') || document.getElementById('user-avatar');
    }
    if (!avatarEl) {
      console.warn('[DM-AVATAR] Elemento de avatar não encontrado');
      return;
    }

    // Carrega da chave correta
    ...
    if (saved && saved.startsWith('data:image')) {
      ...
      console.log('[DM-AVATAR] Avatar carregado de zx_avatar');
    } else {
      // Fallback: iniciais
      const name = window.username || localStorage.getItem('zx_username') || 'Usuário';
      ...
      console.log('[DM-AVATAR] Fallback (sem avatar)');
    }
  }

  // Executa quando a view DM for aberta
  ...
  console.log('[DM-AVATAR] Patch com chave zx_avatar aplicado');
})();

// ============================================================
// FIX DEFINITIVO: Avatar na barra superior da DM
// ============================================================
(function fixDmAvatarFinal() {
  // Função que aplica o avatar ao elemento correto
  function applyAvatarToDm() {
    ...
    if (saved && saved.startsWith('data:image')) {
      ...
      avatarEl.style.backgroundSize = 'cover';
      avatarEl.style.backgroundPosition = 'center';
      avatarEl.style.background = 'none'; // remove fallback
      avatarEl.textContent = '';
      avatarEl.classList.add('has-image');
  ...
  // 1. Observer para detectar quando o elemento aparecer no DOM
  ...
  // 2. Intercepta a abertura da view DM
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
      if (layoutId === 'dm-view') {
        // Tenta aplicar imediatamente e depois com delays
        applyAvatarToDm();
  ...
  // 4. Se a DM já estiver visível ao carregar a página
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      applyAvatarToDm();
  ...
  // 5. Força aplicação sempre que o elemento for modificado (ex: por outros scripts)
  const originalSetAttribute = Element.prototype.setAttribute;
  Element.prototype.setAttribute = function(name, value) {
    originalSetAttribute.call(this, name, value);
    if (this.id === 'user-avatar-dm' && name === 'class') {
      // Se a classe mudar, reaplica o avatar (para garantir)
      setTimeout(() => applyAvatarToDm(), 10);
    }
  };

  console.log('[DM-AVATAR] Correção definitiva ativada');
})();