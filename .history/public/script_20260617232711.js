...
// ============================================================
// STATUS EM TEMPO REAL NO CHAT DM (VERSÃO COM LOGS)
// ============================================================
(function dmStatusFinal() {
  console.log('[DM-STATUS] Iniciando...');

  const STATUS_MAP = {
    online:  { color: '#23d18b', label: 'Online' },
    idle:    { color: '#faa61a', label: 'Ausente' },
    dnd:     { color: '#ed4245', label: 'Não perturbe' },
    offline: { color: '#747f8d', label: 'Offline' }
  };

  function getStatus(username) {
    const key = (username || '').toLowerCase();
    // Tenta obter de userStatuses (mapa global)
    if (window.userStatuses && window.userStatuses[key]) {
      return window.userStatuses[key];
    }
    // Se estiver no onlineSet, considera online
    if (window.onlineSet && window.onlineSet.has(key)) {
      return 'online';
    }
    return 'offline';
  }
  ...
  function updateDmStatus() {
    // Procura o elemento de status
    const statusEl = document.querySelector('.dm-status') ||
                     document.querySelector('.chat-header .dm-status');
    const usernameEl = document.querySelector('.dm-username') ||
                       document.querySelector('.chat-header .dm-username');

    if (!statusEl) {
      console.warn('[DM-STATUS] Elemento .dm-status não encontrado');
      return;
    }
    if (!usernameEl) {
      console.warn('[DM-STATUS] Elemento .dm-username não encontrado');
      return;
    }

    const username = usernameEl.textContent.trim();
    if (!username) {
      console.warn('[DM-STATUS] Nome de usuário vazio');
      return;
    }
    ...
    statusEl.className = 'dm-status ' + status;

    console.log(`[DM-STATUS] ${username} → ${status} (${info.label})`);
  }

  // ── Observer para detectar mudanças no cabeçalho ──
  const headerObserver = new MutationObserver(() => {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      // Verifica se o cabeçalho mudou (ex: novo chat aberto)
      const usernameEl = document.querySelector('.dm-username');
  ...
  if (window.socket) {
    socket.on('friend:status', function(data) {
      if (data && data.username) {
        const usernameEl = document.querySelector('.dm-username');
        if (usernameEl && usernameEl.textContent.trim() === data.username) {
          console.log('[DM-STATUS] Status atualizado via friend:status:', data);
          updateDmStatus();
        }
      }
    });

    socket.on('friends:presence', function() {
      const dmView = document.getElementById('dm-view');
      if (dmView && !dmView.classList.contains('hidden')) {
        console.log('[DM-STATUS] Atualização via friends:presence');
        updateDmStatus();
      }
    });
  }

  // ── Quando a view DM for aberta ──
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(updateDmStatus, 100);
        setTimeout(updateDmStatus, 300);
        setTimeout(updateDmStatus, 600);
      }
    };
  }

  // ── Quando clicar em um item da lista de DMs ──
  document.addEventListener('click', function(e) {
    const target = e.target.closest('.dm-conv-item') ||
                   e.target.closest('#btn-dm-list') ||
                   e.target.closest('#btn-dm-list-sidebar') ||
                   e.target.closest('#btn-dm-list-dm');
    if (target) {
      setTimeout(updateDmStatus, 200);
      setTimeout(updateDmStatus, 500);
    }
  });
  ...
  setInterval(function() {
    ...
    if (dmView && !dmView.classList.contains('hidden')) {
      ...
      if (usernameEl && usernameEl.textContent.trim()) {
        const statusEl = document.querySelector('.dm-status');
        if (statusEl) {
          // Se o status atual for "offline" e o usuário estiver online, atualiza
          const username = usernameEl.textContent.trim();
          const status = getStatus(username);
          if (status !== 'offline') {
            updateDmStatus();
          } else {
            // Também atualiza para garantir consistência
            updateDmStatus();
          }
        }
      }
  ...
  // ── Executa imediatamente se a DM já estiver aberta ──
  document.addEventListener('DOMContentLoaded', function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      setTimeout(updateDmStatus, 300);
  ...
  // ── Expõe a função para debug no console ──
  window.updateDmStatus = updateDmStatus;

  console.log('[DM-STATUS] Sistema ativado. Use updateDmStatus() para forçar atualização.');
})();

// ============================================================
// CORRIGE CLIQUE NA LISTA DE DMs E FILTRA O PRÓPRIO USUÁRIO
// ============================================================
(function fixDmListClick() {
  console.log('[DM-LIST] Aplicando correção de clique...');

  // ── 1. ABRE O CHAT AO CLICAR EM UM ITEM DA LISTA ──
  document.addEventListener('click', function(e) {
    const item = e.target.closest('.dm-conv-item');
    if (!item) return;

    // Obtém o nome do usuário (pode estar em dataset.username ou no texto)
    ...
    // Evita abrir chat consigo mesmo (caso ainda apareça)
    const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
    if (username.toLowerCase() === currentUser) {
      console.log('[DM-LIST] Não é possível abrir chat consigo mesmo');
      return;
    }

    // Impede que o clique seja processado por outros listeners
    e.preventDefault();
    e.stopPropagation();

    console.log('[DM-LIST] Abrindo chat com:', username);

    // Usa a função global para abrir o chat
    if (typeof window.openDmChat === 'function') {
      window.openDmChat(username);
    } else if (typeof window.openPrivateChat === 'function') {
      window.openPrivateChat(username);
    } else {
      // Fallback: tenta simular um clique no botão de DM
      ...
      if (dmBtn) {
        dmBtn.click();
        // Após abrir a view DM, tenta abrir o chat
        setTimeout(() => {
          if (typeof window.openDmChat === 'function') {
            window.openDmChat(username);
          }
  ...
  // ── 2. GARANTE QUE O PRÓPRIO USUÁRIO SEJA FILTRADO DA LISTA ──
  function filterSelfFromDmList() {
    const currentUser = (window.username || localStorage.getItem('zx_username') || '').toLowerCase();
    ...
    items.forEach(item => {
      ...
      if (username === currentUser) {
        ...
        console.log('[DM-LIST] Removido próprio usuário da lista');
      }
    });
  }

  // ── 3. OBSERVA MUDANÇAS NA LISTA PARA FILTRAR NOVOS ITENS ──
  const listObserver = new MutationObserver(() => {
    filterSelfFromDmList();
  });

  // Inicia o observer quando a lista existir
  function startObserver() {
    const list = document.getElementById('dm-conversations-list');
    if (list) {
      listObserver.observe(list, { childList: true, subtree: true });
      filterSelfFromDmList();
      console.log('[DM-LIST] Observer ativado na lista de DMs');
    } else {
      setTimeout(startObserver, 300);
    }
  }

  // ── 4. EXECUTA QUANDO A VIEW DM FOR ABERTA ──
  function onDmViewOpen() {
    setTimeout(filterSelfFromDmList, 100);
    setTimeout(filterSelfFromDmList, 300);
  }
  ...
  // ── 5. TAMBÉM FILTRA QUANDO A LISTA FOR RENDERIZADA ──
  const origRenderDmList = window.renderDmList;
  if (origRenderDmList) {
    window.renderDmList = function() {
      origRenderDmList.call(this);
      filterSelfFromDmList();
    };
  }
  ...
  // Fallback: cliques nos botões de DM
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(onDmViewOpen, 200);
    }
  });

  console.log('[DM-LIST] Correção aplicada');
})();