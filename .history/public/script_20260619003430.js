// Cria o socket apenas se não existir
if (typeof window.socket === 'undefined') {
  window.socket = io('https://sagile-xenon.onrender.com', {
...
const socket = io('https://sagile-xenon.onrender.com', {
  transports: ['polling', 'websocket'],
  reconnection: true,
  reconnectionAttempts: Infinity,
  reconnectionDelay: 1000,
...
socket.on('connect', () => {
  ...
  // Re-emit login if user is logged in
  const username = window.username || window.currentUsername || sessionStorage.getItem('username') || localStorage.getItem('zx_username');
  if (username) {
...
socket.on('friends:data', (data) => {
  ...
  renderFriendsModal();
  // Atualizar lista DM agora que os avatares foram cacheados via setFriendAvatar acima.
  // Sem isso, a lista fica com iniciais porque renderDmList já rodou antes de friends:data.
  if (typeof renderDmList === 'function') renderDmList();
  refreshInicialAvatars();
...
async function addSuggested(serverId) {
