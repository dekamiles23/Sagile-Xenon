...
let pendingDmMessages = [];

// Substitua a função de envio de DM por esta:
function sendDirectMessage(toUsername, text) {
  ...
  if (window.socket && window.socket.connected) {
    ...
    // Opcional: adicionar localmente imediatamente
    addLocalDmMessage(msg);
  } else {
    // Enfileira
    pendingDmMessages.push(msg);
    console.log('[DM] Offline, mensagem enfileirada.');
  }
}

// Reenviar pendentes na reconexão
window.socket?.on('connect', () => {
  if (pendingDmMessages.length > 0) {
    console.log('[DM] Reenviando', pendingDmMessages.length, 'mensagens pendentes');
    pendingDmMessages.forEach(msg => {
      window.socket.emit('dm:message', msg);
      addLocalDmMessage(msg);
    });
    pendingDmMessages = [];
...
function sendDmMessage(text, toUsername) {
  ...
  // 🔥 SOLUÇÃO: envia o próprio nome do usuário como receiverId (fallback)
  // Isso garante que o servidor sempre tenha um valor para rotear
  const fallbackId = toUsername; // pode ser string, o servidor usa to

  // Tenta obter o ID real do mapa, mas se não tiver, usa o fallback
  const receiverId = (window.friendIds && window.friendIds.get(toUsername.toLowerCase())) || fallbackId;

  const msg = {
    from: window.username || 'Usuário',
    to: toUsername,
    fromId: window.myUserId || 'unknown',
    receiverId: receiverId, // NUNCA será null
    text: text,
    timestamp: Date.now()
  };
  ...
  if (window.socket && window.socket.connected) {
    ...
    // Enfileira para reenvio depois
    if (!window.dmQueue) window.dmQueue = [];
    window.dmQueue.push({ ...msg, pending: true });
    console.log('[DM] Off-line, enfileirado.');
  }

  const receiver = msg.receiverId || msg.to; // fallback
io.to('dm:user:' + receiver.toLowerCase()).emit('dm:message', msg);
