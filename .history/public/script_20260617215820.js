...
// ============================================================
// PATCH DEFINITIVO: Chat DM em tempo real (por título)
// ============================================================
(function fixDmRealtimeByTitle() {
  if (!window.socket) {
    console.warn('[DM-RT] Socket não disponível, aguardando...');
    setTimeout(fixDmRealtimeByTitle, 500);
    return;
  }
  ...
  socket.on('dm:message', function(msg) {
    ...
    if (!isForMe && !isFromMe) {
      console.log('[DM-RT] Mensagem não é para mim, ignorando');
      return;
    }

    // O interlocutor (a outra pessoa)
    const sender = isForMe ? msg.from : msg.to;

    // 1. Atualiza a lista lateral (reordena)
    updateDmList(sender);

    // 2. Verifica se a view DM está ativa
    const dmView = document.getElementById('dm-view');
    if (!dmView || dmView.classList.contains('hidden')) {
      console.log('[DM-RT] View DM não está ativa, apenas lista atualizada');
      return;
    }

    // 3. Verifica se o chat com este usuário está aberto
    //    Procuramos o nome no cabeçalho do chat
    const chatHeader = dmView.querySelector('.chat-header .dm-username') ||
                       dmView.querySelector('.chat-header .pcb-title') ||
                       dmView.querySelector('.dm-username');

    let openChatUser = null;
    if (chatHeader) {
      openChatUser = chatHeader.textContent.trim();
    }

    // Fallback: tenta encontrar pelo data-activeChat (se existir)
    const chatArea = document.getElementById('dm-chat-area');
    if (!openChatUser && chatArea && chatArea.dataset.activeChat) {
      openChatUser = chatArea.dataset.activeChat;
    }

    console.log('[DM-RT] Chat aberto com:', openChatUser);

    if (openChatUser && openChatUser.toLowerCase() === sender.toLowerCase()) {
      // O chat correto está aberto → adiciona a mensagem
      const messagesArea = document.getElementById('dm-messages-area');
      if (messagesArea) {
        // Evita duplicação (verifica a última mensagem)
        const lastMsg = messagesArea.lastElementChild;
        ...
        console.warn('[DM-RT] Área de mensagens #dm-messages-area não encontrada');
      }
    } else {
      console.log('[DM-RT] Chat com', sender, 'não está aberto (aberto com:', openChatUser, ')');
    }
  });

  // Função para reordenar a lista lateral
  function updateDmList(username) {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;
  ...
  // Força a atualização da lista inicial
  if (typeof renderDmList === 'function') {
    renderDmList();
  }

  console.log('[DM-RT] ✅ Patch por título aplicado. Aguardando mensagens...');
})();