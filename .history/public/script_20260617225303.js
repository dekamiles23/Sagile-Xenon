...
// ============================================================
// BARRA DO DM COM PICKERS GARANTIDOS (CRIA TUDO DO ZERO)
// ============================================================
(function createDmBarWithPickers() {
  function ensurePickers() {
    // Cria os pickers se não existirem
    const pickersConfig = [
      {
        id: 'emoji-picker',
        html: `
          <div style="display:flex;border-bottom:1px solid rgba(255,0,255,0.15);background:rgba(0,0,0,0.3);padding:4px;flex-shrink:0;">
            <button class="emoji-cat" data-cat="smileys" style="flex:1;padding:8px;background:transparent;border:none;color:#fff;cursor:pointer;font-size:16px;border-radius:6px;background:rgba(255,0,255,0.15);">😀</button>
            <button class="emoji-cat" data-cat="gestures" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">👋</button>
            <button class="emoji-cat" data-cat="objects" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">❤️</button>
            <button class="emoji-cat" data-cat="nature" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🐶</button>
            <button class="emoji-cat" data-cat="food" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🍔</button>
            <button class="emoji-cat" data-cat="flags" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:16px;border-radius:6px;">🏳️</button>
          </div>
          <div class="emoji-grid-container" style="display:grid;grid-template-columns:repeat(8,1fr);gap:4px;padding:12px;overflow-y:auto;flex:1;">
            ${['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','😗','😙','😚','☺️','🙂','🤗','🤩','🤔','🤨','😐','😑','😶','🙄','😏','😣','😥','😮','🤐','😯','😪','😫','😴','😌','😛','😜','😝','🤤','😒','😓','😔','😕','🙃','🤑','😲','☹️','🙁','😖','😞','😟','😤','😢','😭','😦','😧','😨','😩','🤯','😬','😰','😱','🥵','🥶','😳','🤪','😵','😡','😠','🤬','💀','☠️','💩','🤡','👹','👺','👻','👽','👾','🤖','😺','😸','😹','😻','😼','😽','🙀','😿','😾','❤️','🧡','💛','💚','💙','💜','🖤','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟']}
          </div>
        `
      },
      {
        ...
        html: `
          ...
          <div class="gif-grid-container" style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;padding:12px;overflow-y:auto;flex:1;">
      ...
      },
      {
        ...
        html: `
          <div style="display:flex;border-bottom:1px solid rgba(255,0,255,0.15);background:rgba(0,0,0,0.3);padding:4px;flex-shrink:0;">
            <button class="sticker-pack" data-pack="default" style="flex:1;padding:8px;background:transparent;border:none;color:#fff;cursor:pointer;font-size:13px;border-radius:6px;background:rgba(255,0,255,0.15);">Padrão</button>
            <button class="sticker-pack" data-pack="reactions" style="flex:1;padding:8px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:13px;border-radius:6px;">Reações</button>
          </div>
          <div class="sticker-grid-container" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;padding:12px;overflow-y:auto;flex:1;">
            ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️']}
          </div>
        `
      }
    ...
    pickersConfig.forEach(({ id, html }) => {
      ...
      if (!picker) {
        ...
        console.log(`[DM-PICKER] Criado: ${id}`);
      } else {
        // Se já existe, garante que está no body
        if (picker.parentNode !== document.body) {
          document.body.appendChild(picker);
        }
        // Atualiza o conteúdo para garantir os itens
        picker.innerHTML = html;
        picker.style.display = 'none';
        console.log(`[DM-PICKER] Atualizado: ${id}`);
      }
    });
  }
  ...
  function buildBar() {
    ...
    // Garante que os pickers existam
    ensurePickers();
    ...
    wrapper.innerHTML = `
      <button type="button" class="dm-btn" id="dm-btn-plus" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">➕</button>
      <button type="button" class="dm-btn" id="dm-btn-emoji" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">😀</button>
      <button type="button" class="dm-btn" id="dm-btn-gif" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎬</button>
      <button type="button" class="dm-btn" id="dm-btn-stickers" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎭</button>
      <button type="button" class="dm-btn" id="dm-btn-attach" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">📎</button>
      ...
      <button type="button" class="dm-btn" id="dm-btn-voice" style="background:transparent;border:none;color:#ccc;font-size:20px;cursor:pointer;padding:4px 8px;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">🎤</button>
    ...
    function connectPicker(btnId, pickerId, itemSelector, insertFn) {
      ...
      newBtn.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log(`[DM] Botão ${btnId} clicado, abrindo picker ${pickerId}`);
    ...
    // ── Anexo, áudio e mais opções (simplificados) ──
    ...
    if (attachBtn) {
      ...
      fileInput.addEventListener('change', function() {
        // ... lógica de anexo (pode ser adicionada depois)
        console.log('[DM] Arquivo(s) selecionado(s):', this.files);
        this.value = '';
      });
    }

    console.log('[DM-BAR] Barra construída com pickers funcionais');
  }
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-dm-list, #btn-dm-list-sidebar, #btn-dm-list-dm')) {
      setTimeout(buildBar, 200);
      setTimeout(buildBar, 500);
    }
  });

  console.log('[DM-BAR] Script de criação da barra carregado');
})();