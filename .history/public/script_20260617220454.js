...
// ============================================================
// POLLING SUAVE: Chat DM com cache para evitar piscar
// ============================================================
(function smoothDmPolling() {
  if (!window.socket) {
  ...
  let lastMessageTimestamp = {}; // Armazena o timestamp da última mensagem por usuário
  ...
  function pollDmMessages() {
    ...
    if (currentChatUser !== username) {
      ...
      // Cria um novo intervalo para este chat (2 segundos)
      pollingInterval = setInterval(() => {
        if (window.socket && window.socket.connected) {
          socket.emit('dm:history', { with: username });
        }
      }, 2000);
      console.log('[POLLING] Monitorando chat com', username, '(a cada 2s)');
    }
  ...
  // Escuta a resposta do servidor com o histórico (com cache)
  socket.on('dm:history', function(data) {
    ...
    // Obtém as mensagens existentes do localStorage
    const privateMessages = JSON.parse(localStorage.getItem('private_messages') || '{}');
    const existing = privateMessages[username] || [];

    // Filtra apenas mensagens realmente novas (por timestamp e texto)
    ...
    // Atualiza o localStorage (mantendo a ordem)
    privateMessages[username] = [...existing, ...newMessages];
    localStorage.setItem('private_messages', JSON.stringify(privateMessages));

    // Adiciona as novas mensagens ao DOM (sem recriar tudo)
    const messagesArea = document.getElementById('dm-messages-area');
    if (!messagesArea) return;

    // Adiciona uma a uma, sem recriar o container
    ...
    console.log('[POLLING] ✅ Adicionadas', newMessages.length, 'nova(s) mensagem(ns) sem piscar');
  });
  ...
  console.log('[POLLING] Suave ativado (verifica a cada 2s, sem piscar)');
})();