...
// ============================================================
// BARRA DE INPUT DO DM COM TODOS OS BOTÕES E PICKERS
// ============================================================
(function createDmInputBar() {
  function build() {
    console.log('[DM-BAR] Construindo barra...');

    const dmInputArea = document.getElementById('dm-input-area');
    if (!dmInputArea) {
      console.warn('[DM-BAR] dm-input-area não encontrado');
      return;
    }
    ...
    wrapper.style.cssText = 'display:flex;align-items:center;gap:6px;position:relative;width:100%;';

    wrapper.innerHTML = `
      <button type="button" class="input-action-btn" id="dm-btn-plus" title="Mais opções" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">+</button>
      <button type="button" class="input-action-btn" id="dm-btn-emoji" title="Emojis" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">😀</button>
      <button type="button" class="input-action-btn" id="dm-btn-gif" title="GIFs" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">🎬</button>
      <button type="button" class="input-action-btn" id="dm-btn-stickers" title="Figurinhas" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">🎭</button>
      <button type="button" class="input-action-btn" id="dm-btn-attach" title="Anexar arquivo" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">📎</button>
      <input id="dm-message-input" type="text" placeholder="Digite sua mensagem..." maxlength="500" autocomplete="off" style="flex:1;padding:10px 14px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,0,255,0.3);border-radius:24px;color:#fff;outline:none;font-size:14px;min-width:0;">
      <button type="button" class="input-action-btn" id="dm-btn-voice" title="Gravar áudio" style="background:transparent;border:none;color:#fff;font-size:20px;cursor:pointer;padding:8px;">🎤</button>
      <button id="dm-send-btn" type="button" style="background:linear-gradient(135deg,#8b00ff,#ff00ff);border:none;border-radius:50%;width:44px;height:44px;color:#fff;font-size:18px;cursor:pointer;flex-shrink:0;">➤</button>
    `;

    dmInputArea.innerHTML = '';
    dmInputArea.appendChild(wrapper);
    ...
    // ── Garantir que os pickers existam no body ──
    function ensurePicker(id, html) {
      ...
      if (!picker) {
        ...
        console.log(`[DM-BAR] Pickers criado: ${id}`);
      }
      return picker;
    }

    // ── Emoji picker ──
    const emojiPicker = ensurePicker('dm-emoji-picker', `
      <div class="emoji-categories" style="display:flex;border-bottom:1px solid rgba(255,0,255,0.2);background:rgba(0,0,0,0.3);">
        <button class="emoji-category-btn active" data-category="smileys" style="flex:1;padding:10px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:18px;">😀</button>
        <button class="emoji-category-btn" data-category="gestures" style="flex:1;padding:10px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:18px;">👋</button>
        <button class="emoji-category-btn" data-category="objects" style="flex:1;padding:10px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:18px;">❤️</button>
      </div>
      <div class="emoji-grid-container" style="overflow-y:auto;padding:12px;flex:1;">
    ...
    // ── GIF picker ──
    const gifPicker = ensurePicker('dm-gif-picker', `
      <div class="gif-search-bar" style="padding:12px;border-bottom:1px solid rgba(255,0,255,0.2);">
        <input type="text" class="gif-search-input" placeholder="Pesquisar GIFs..." style="width:100%;padding:10px 14px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,0,255,0.3);border-radius:8px;color:#fff;outline:none;font-size:14px;">
      </div>
      <div class="gif-grid-container" style="overflow-y:auto;padding:12px;flex:1;">
        <div class="gif-grid" style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
          ${['https://media.tenor.com/1a1f1f1f1f1f1f1f1f1f1f1f1f1f1f1f/tenor.gif','https://media.tenor.com/2a2f2f2f2f2f2f2f2f2f2f2f2f2f2f2f/tenor.gif','https://media.tenor.com/3a3f3f3f3f3f3f3f3f3f3f3f3f3f3f3f/tenor.gif','https://media.tenor.com/4a4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f/tenor.gif'].map(url => `<div class="gif-item" style="border-radius:8px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:all 0.2s;aspect-ratio:16/9;background:rgba(255,0,255,0.05);"><img src="${url}" style="width:100%;height:100%;object-fit:cover;"></div>`).join('')}
        </div>
      </div>
    `);

    // ── Sticker picker ──
    const stickerPicker = ensurePicker('dm-sticker-picker', `
      <div class="sticker-tabs" style="display:flex;border-bottom:1px solid rgba(255,0,255,0.2);background:rgba(0,0,0,0.3);overflow-x:auto;">
        <button class="sticker-pack-btn active" data-pack="default" style="padding:10px 14px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:13px;white-space:nowrap;">Padrão</button>
        <button class="sticker-pack-btn" data-pack="reactions" style="padding:10px 14px;background:transparent;border:none;color:#aaa;cursor:pointer;font-size:13px;white-space:nowrap;">Reações</button>
      </div>
      <div class="sticker-grid-container" style="overflow-y:auto;padding:12px;flex:1;">
        <div class="sticker-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;">
          ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️'].map(s => `<div class="sticker-item" style="width:70px;height:70px;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:10px;transition:all 0.2s;background:rgba(255,0,255,0.05);border:2px solid transparent;font-size:40px;">${s}</div>`).join('')}
        </div>
      </div>
    `);

    // ── Conectar pickers ──
    function connectPicker(btnId, pickerId, itemSelector, insertFn) {
      const btn = document.getElementById(btnId);
      const picker = document.getElementById(pickerId);
      if (!btn || !picker) {
        console.warn(`[DM-BAR] ${btnId} ou ${pickerId} não encontrado`);
        return;
      }
      ...
      newBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        document.querySelectorAll('.chat-picker.active').forEach(p => {
      ...
      picker.addEventListener('click', function(e) {
        ...
        if (!target) return;
        let value = '';
        if (itemSelector === '.gif-item img') {
          value = target.src || '';
        } else {
          value = target.textContent || target.dataset.value || '';
        }
        if (value) {
          insertFn(value);
      ...
      document.addEventListener('click', function(e) {
        if (!picker.contains(e.target) && e.target !== newBtn && !newBtn.contains(e.target)) {
          picker.classList.remove('active');
          picker.style.display = 'none';
        }
      });
    ...
    // ── Emoji ──
    connectPicker('dm-btn-emoji', 'dm-emoji-picker', '.emoji-item', function(text) {
      const input = document.getElementById('dm-message-input');
      if (input) {
    ...
    // ── GIF ──
    connectPicker('dm-btn-gif', 'dm-gif-picker', '.gif-item img', function(url) {
      const input = document.getElementById('dm-message-input');
      if (input) {
        input.value += `[GIF] ${url} `;
      }
    ...
    // ── Sticker ──
    connectPicker('dm-btn-stickers', 'dm-sticker-picker', '.sticker-item', function(text) {
      const input = document.getElementById('dm-message-input');
      if (input) {
        input.value += text + ' ';
      }
    ...
    // ── Anexar arquivo ──
    const attachBtn = document.getElementById('dm-btn-attach');
    if (attachBtn) {
        ...
        files.forEach(file => {
          ...
          reader.onload = function(e) {
            ...
            if (messagesArea) {
              ...
              div.innerHTML = `<div style="max-width:75%;background:linear-gradient(135deg,#8b00ff,#ff00ff);border-radius:18px 18px 4px 18px;padding:10px 14px;color:#fff;font-size:14px;">${msg.text}</div>`;
    ...
    // ── Gravar áudio ──
    const voiceBtn = document.getElementById('dm-btn-voice');
    if (voiceBtn) {
      ...
      voiceBtn.addEventListener('click', async function() {
        ...
        try {
          ...
          mediaRecorder.onstop = () => {
            ...
            reader.onload = function(e) {
              ...
              if (messagesArea) {
                ...
                div.className = 'message';
                div.style.cssText = 'display:flex;justify-content:flex-end;margin-bottom:6px;';
                div.innerHTML = `<div style="max-width:75%;background:linear-gradient(135deg,#8b00ff,#ff00ff);border-radius:18px 18px 4px 18px;padding:10px 14px;color:#fff;font-size:14px;">🎤 Áudio enviado</div>`;
                messagesArea.appendChild(div);
                messagesArea.scrollTop = messagesArea.scrollHeight;
              }
          ...
          console.error('[DM] Erro ao acessar microfone:', err);
    ...
    // ── Mais opções ──
    const plusBtn = document.getElementById('dm-btn-plus');
    if (plusBtn) {
      plusBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        let menu = document.getElementById('dm-plus-menu');
        if (menu) { menu.remove(); return; }
        menu = document.createElement('div');
        menu.id = 'dm-plus-menu';
        menu.style.cssText = `
          position: absolute;
          bottom: 100%;
          left: 0;
        ...
        `;
        menu.innerHTML = `
          <div class="plus-item" data-action="clear" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;transition:background 0.15s;">🗑️ Limpar histórico</div>
          <div class="plus-item" data-action="pin" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;transition:background 0.15s;">📌 Fixar conversa</div>
          <div class="plus-item" data-action="mute" style="padding:10px 12px;color:#fff;cursor:pointer;border-radius:6px;transition:background 0.15s;">🔇 Silenciar notificações</div>
        `;
        menu.querySelectorAll('.plus-item').forEach(item => {
          item.addEventListener('mouseenter', () => item.style.background = 'rgba(255,0,255,0.15)');
          ...
          item.addEventListener('click', function() {
            ...
            if (!activeUser) return;
            if (action === 'clear') {
              if (confirm(`Limpar histórico com ${activeUser}?`)) {
                const msgs = JSON.parse(localStorage.getItem('private_messages') || '{}');
                msgs[activeUser] = [];
                localStorage.setItem('private_messages', JSON.stringify(msgs));
                const area = document.getElementById('dm-messages-area');
                if (area) area.innerHTML = '';
              }
            } else if (action === 'pin') {
              alert(`📌 Conversa com ${activeUser} fixada (simulado)`);
            } else if (action === 'mute') {
              alert(`🔇 Notificações de ${activeUser} silenciadas (simulado)`);
            }
            menu.remove();
          });
        });
        plusBtn.parentElement.style.position = 'relative';
        plusBtn.parentElement.appendChild(menu);
        setTimeout(() => {
    ...
    console.log('[DM-BAR] Barra construída com sucesso!');
  }
  ...
  // ── Reconstrói quando a view DM for aberta ──
  ...
  console.log('[DM-BAR] Script carregado. Aguardando abertura do DM...');
})();

// ============================================================
// CORRIGE LAYOUT DOS PICKERS (emoji, GIF, sticker)
// ============================================================
(function fixPickersLayout() {
  // Adiciona estilos globais para os pickers
  const style = document.createElement('style');
  style.textContent = `
    /* Estilos gerais dos pickers */
    .chat-picker {
    ...
    .picker-header {
      display: flex !important;
      border-bottom: 1px solid rgba(255,0,255,0.15) !important;
  ...
  // ── Recria os pickers com layout corrigido ──
  function rebuildPickers() {
    ...
    if (emojiPicker) {
      emojiPicker.innerHTML = `
        <div class="picker-header">
          ...
          <button data-category="objects">❤️</button>
    ...
    // GIF picker
    const gifPicker = document.getElementById('dm-gif-picker');
    if (gifPicker) {
      gifPicker.innerHTML = `
        <div class="gif-search">
          <input type="text" class="gif-search-input" placeholder="Pesquisar GIFs..." autocomplete="off">
        </div>
        <div class="picker-grid gif-grid">
          ${['https://media.tenor.com/1a1f1f1f1f1f1f1f1f1f1f1f1f1f1f1f/tenor.gif','https://media.tenor.com/2a2f2f2f2f2f2f2f2f2f2f2f2f2f2f2f/tenor.gif','https://media.tenor.com/3a3f3f3f3f3f3f3f3f3f3f3f3f3f3f3f/tenor.gif','https://media.tenor.com/4a4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f/tenor.gif'].map(url => `<div class="gif-item"><img src="${url}"></div>`).join('')}
        </div>
      `;
      // Conecta busca (simples)
      const searchInput = gifPicker.querySelector('.gif-search-input');
      if (searchInput) {
        searchInput.addEventListener('input', function() {
          const term = this.value.toLowerCase();
          const items = gifPicker.querySelectorAll('.gif-item');
          items.forEach(item => {
            const img = item.querySelector('img');
            if (img) {
              const src = img.src.toLowerCase();
              item.style.display = src.includes(term) ? '' : 'none';
            }
          });
        });
      }
    ...
    // Sticker picker
    const stickerPicker = document.getElementById('dm-sticker-picker');
    if (stickerPicker) {
      stickerPicker.innerHTML = `
        <div class="picker-header">
          <button class="active" data-pack="default">Padrão</button>
          <button data-pack="reactions">Reações</button>
        </div>
        <div class="picker-grid sticker-grid">
          ${['👍','👎','👏','🙌','🤲','🤝','🙏','✌️','🤟','🤘','👌','🤌','🤙','💪','🦾','🦵','🦿','🦶','👣','👀','👁️','🧠','🫀','🫁','🩸','💉','💊','🧬','🔬','🔭','📡','💻','🖥️','🖨️','⌨️','🖱️','🖲️','💽','💾','💿','📀','🧮','🎮','🕹️','🎰','🎲','♟️','🎯','🎳','🎪','🎨','🎭','🎬','🎤','🎧','🎼','🎹','🥁','🎷','🎺','🎸','🪕','🎻','🪗','🎫','🎟️']}
        </div>
      `;
    }

    console.log('[PICKERS] Layout corrigido');
  }

  // Executa após a barra ser criada
  setTimeout(rebuildPickers, 100);
  setTimeout(rebuildPickers, 300);
  ...
  if (origShow) {
    window.showLayout = function(layoutId) {
      origShow.call(this, layoutId);
      if (layoutId === 'dm-view') {
        setTimeout(rebuildPickers, 200);
        setTimeout(rebuildPickers, 500);
      }
    };
  }

  console.log('[PICKERS] Script de correção de layout carregado');
})();