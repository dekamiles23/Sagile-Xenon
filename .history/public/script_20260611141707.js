...
function renderSuggestedCommunities() {
  ...
  if (suggestedCommunities.length === 0) {
    container.innerHTML = `
      <div class="suggested-empty">
        <div class="suggested-empty-icon">🌐</div>
        <span>Nenhuma comunidade ainda</span>
      </div>
    `;
    return;
  }

  container.innerHTML = suggestedCommunities.map(community => `
    <div class="suggested-community-card" onclick="openServer('${community.id}')">
      <div class="suggested-community-icon" ${community.icon ? `style="background-image:url('${community.icon}')"` : ''}>
        ${community.icon ? '' : community.name[0].toUpperCase()}
      </div>
      <div class="suggested-community-info">
        <div class="suggested-community-name">${escHtml(community.name)}</div>
        <div class="suggested-community-category">${escHtml(community.category || 'geral')}</div>
      </div>
    </div>
  `).join('');
}
