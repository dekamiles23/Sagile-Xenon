...
// ============================================================
// AVATAR NAS MENSAGENS DO DM (COM FOTO)
// ============================================================
(function fixDmAvatars() {
  console.log('[DM-AVATAR] Aplicando correção...');

  // Obtém avatar de um usuário (case-insensitive)
  function getUserAvatar(username) {
    if (!username) return null;
    const key = username.toLowerCase();
    const currentUser = (window.username || '').toLowerCase();

    // 1. Se for o próprio usuário, usa zx_avatar
    if (key === currentUser) {
      const saved = localStorage.getItem('zx_avatar');
      if (saved && saved.startsWith('data:image')) {
        return saved;
    ...
    // 2. Tenta do mapa global (para amigos)
    if (window.userAvatars && window.userAvatars[key]) {
      return window.userAvatars[key];
    }

    // 3. Tenta do localStorage (chave: avatar_username)
  ...
  function setUserAvatar(username, avatarData) {
    ...
    console.log('[DM-AVATAR] Avatar salvo para', username);
  }

  // ── SOBRESCREVE A FUNÇÃO DE RENDERIZAÇÃO DE MENSAGENS ──
  if (typeof window.renderPrivateMessages === 'function') {
    const originalRender = window.renderPrivateMessages;
    window.renderPrivateMessages = function(username) {
      console.log('[DM-AVATAR] Renderizando mensagens para', username);
      ...
      if (messages.length === 0) {
        messagesArea.innerHTML = `<div style="text-align:center;padding:60px 20px;color:#666;">Nenhuma mensagem ainda</div>`;
        return;
      }

      // Limpa e recria as mensagens com avatares
      messagesArea.innerHTML = '';
      messages.forEach(msg => {
        const isMe = msg.sender === 'me';
        const sender = isMe ? (window.username || 'Você') : username;
    ...
    console.log('[DM-AVATAR] renderPrivateMessages sobrescrita');
  }

  // ── TAMBÉM SOBRESCREVE addDmMessageToDom (se existir) ──
  if (typeof window.addDmMessageToDom === 'function') {
    const originalAdd = window.addDmMessageToDom;
    window.addDmMessageToDom = function(msg, isMe) {
      const messagesArea = document.getElementById('dm-messages-area');
      if (!messagesArea) return;
    ...
    console.log('[DM-AVATAR] addDmMessageToDom sobrescrita');
  }
  ...
  // ── EXECUTA QUANDO A DM FOR ABERTA ──
  const origShowLayout = window.showLayout;
  if (origShowLayout) {
    window.showLayout = function(layoutId) {
      origShowLayout.call(this, layoutId);
  ...
  console.log('[DM-AVATAR] Correção aplicada. Use getUserAvatar(username) para testar.');
})();