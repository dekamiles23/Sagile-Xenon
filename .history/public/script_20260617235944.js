...
// ============================================================
// AVATAR DEFINITIVO: BUSCA DO SERVIDOR E EXIBIÇÃO NA LISTA
// ============================================================
(function finalAvatarSystem() {
  console.log('[FINAL-AVATAR] Iniciando...');
  ...
  function updateAllAvatars() {
    ...
    console.log('[FINAL-AVATAR] Avatares atualizados');
  }

  // ── 3. Solicita avatar de um usuário ao servidor ──
  function requestAvatar(username) {
    if (!username) return;
    const key = username.toLowerCase();
    if (avatarCache[key] || localStorage.getItem('avatar_' + key)) return;
    if (window.socket && window.socket.connected) {
      console.log('[FINAL-AVATAR] Solicitando avatar de', username);
      window.socket.emit('get-avatar', { username: username });
    }
  }

  // ── 4. Listener de avatares do servidor ──
  if (window.socket) {
    window.socket.on('avatar:data', function(data) {
      if (data && data.username && data.avatar) {
        setAvatar(data.username, data.avatar);
        updateAllAvatars();
      }
    });
    ...
    window.socket.on('friends:data', function(data) {
      if (data && data.friends) {
        data.friends.forEach(friend => {
          if (friend.avatar) {
            setAvatar(friend.username, friend.avatar);
          }
        });
        updateAllAvatars();
      }
  ...
  // ── 5. Sobrescreve renderDmList ──
  if (typeof window.renderDmList === 'function') {
    const original = window.renderDmList;
    window.renderDmList = function() {
      original.call(this);
      setTimeout(() => {
        // Solicita avatar para cada amigo na lista
        document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
          const nameEl = item.querySelector('.dm-conv-name');
          if (nameEl) {
            const username = nameEl.textContent.trim();
  ...
  // ── 6. Observer para novos itens ──
  function observeList() {
    const list = document.getElementById('dm-conversations-list');
    if (!list) return;
    const observer = new MutationObserver(() => {
      setTimeout(updateAllAvatars, 50);
    ...
    console.log('[FINAL-AVATAR] Observador ativado');
  }

  // ── 7. Quando a view DM for aberta ──
  function onDmOpen() {
    setTimeout(() => {
      // Solicita avatar para todos os amigos da lista
      document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
        const nameEl = item.querySelector('.dm-conv-name');
        if (nameEl) {
          const username = nameEl.textContent.trim();
  ...
  // ── 9. Fallback: polling a cada 3s ──
  setInterval(function() {
    const dmView = document.getElementById('dm-view');
    if (dmView && !dmView.classList.contains('hidden')) {
      // Verifica se há avatares faltando
      ...
      if (needsUpdate) {
        console.log('[FINAL-AVATAR] Polling: atualizando avatares');
        // Re-solicita avatares
        document.querySelectorAll('#dm-conversations-list .dm-conv-item').forEach(item => {
          const nameEl = item.querySelector('.dm-conv-name');
          if (nameEl) {
            const username = nameEl.textContent.trim();
  ...
  window.forceAvatarUpdate = updateAllAvatars;
  window.setFriendAvatar = setAvatar;
  window.requestFriendAvatar = requestAvatar;

  console.log('[FINAL-AVATAR] Sistema ativo. Use forceAvatarUpdate() para forçar, setFriendAvatar(username, base64) para definir manualmente.');
})();